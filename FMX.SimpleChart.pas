...
type
  ...
  private
    ...
    procedure SetLineThickness(const AValue: Single);
    procedure SetMarkerRadius(const AValue: Single);
  protected
  ...
  published
    ...
    property Margins;
    property MarkerColor: TAlphaColor read FMarkerColor write FMarkerColor;
...
begin
  ...
  FLineColor := TAlphaColorRec.Blue;
  FLineThickness := 2.0;
  FMarkerColor := TAlphaColorRec.Blue;
  FMarkerRadius := 3.0;
...
procedure TSimpleChart.SetLineColor(const AValue: TAlphaColor);
begin
...
procedure TSimpleChart.SetLineThickness(const AValue: Single);
begin
  if SameValue(FLineThickness, AValue) then
    Exit;
  FLineThickness := Max(0.5, AValue);
  InvalidateChart;
end;

procedure TSimpleChart.SetMarkerRadius(const AValue: Single);
begin
  if SameValue(FMarkerRadius, AValue) then
    Exit;
  FMarkerRadius := Max(0.5, AValue);
  InvalidateChart;
end;
...
begin
  Result := TChartSeries.Create(ATitle, FLineColor);
  Result.Thickness := FLineThickness;
  Result.MarkerRadius := FMarkerRadius;
  Result.ShowMarkers := FShowMarkers;
  FSeries.Add(Result);

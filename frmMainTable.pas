  ...
  try
    ...
    for DevicePoint in Device.Points do
      if (DevicePoint <> nil) and
         Device.IsFlowInPoint(CurrentSpillage.QavgEtalon, DevicePoint) then
      begin
    ...
    CurrentSpillage.Name := MatchedPoint.Name;
    CurrentSpillage.DeviceTypeUUID := MatchedPoint.DeviceTypeUUID;

    ResultSpillage := Device.FindResultSpillageForPoint(
      MatchedPoint, CurrentSpillage);
    ...
    begin
      LogMKS('DBG SP 9204', 'GetErrorCellColor RESULT NOT MATCHED',
        Format('DevicePoint ID=%d Name=%s DeviceTypeUUID=%s | %s',
          [MatchedPoint.ID, MatchedPoint.Name, MatchedPoint.DeviceTypeUUID,
           DumpSpillage(CurrentSpillage)]));
      Exit;
    end;

    ActualError := ResultSpillage.Error;
    if SameValue(ActualError, -MaxDouble, 0.0) or
       SameValue(ActualError, MaxDouble, 0.0) then
    begin
      LogMKS('DBG SP 9205', 'GetErrorCellColor INVALID ACTUAL ERROR',
        Format('DevicePoint ID=%d Name=%s ActualError=%g | %s',
          [MatchedPoint.ID, MatchedPoint.Name, ActualError,
    ...
    begin
      LogMKS('DBG SP 9206', 'GetErrorCellColor INVALID ALLOWED ERROR',
        Format('DevicePoint ID=%d Name=%s AllowedError=%f | %s',
          [MatchedPoint.ID, MatchedPoint.Name, AllowedError,
           DumpSpillage(ResultSpillage)]));
      Exit;
    end;
    ...
    LogMKS('DBG SP 9207', 'GetErrorCellColor COLOR',
      Format('DevicePoint ID=%d Name=%s QavgEtalon=%f ActualError=%f AllowedError=%f Color=%x | %s',
        [MatchedPoint.ID, MatchedPoint.Name, CurrentSpillage.QavgEtalon,
         ActualError, AllowedError, AColor,
         DumpSpillage(ResultSpillage)]));
  finally

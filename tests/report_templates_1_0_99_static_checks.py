...
def test_version_and_project_unit():
    assert "APP_VERSION = '1.0.99'" in VERSION
...
def test_report_table_has_one_filter_and_dynamic_id():
    worksheet = SERVICE.split("function BuildWorksheetXml", 1)[1].split(
        "function BuildTableXml", 1
    )[0]
    table = SERVICE.split("function BuildTableXml", 1)[1].split(
        "function ResolveReportTableId", 1
    )[0]
    assert "<autoFilter" not in worksheet
    assert table.count("<autoFilter") == 1
    assert 'id="50000"' not in table
    assert "ATableId: Cardinal" in table
    assert 'id="%d" name="%s"' in table
    assert "BuildReportTableRange(ARowCount, AColumns.Count)" in table
    assert "BuildReportTableRange(ARows.Count, AColumns.Count)" in worksheet


def test_report_table_id_is_resolved_from_all_table_xml_files():
    resolver = SERVICE.split("function ResolveReportTableId", 1)[1].split(
        "procedure ValidateReportTableXml", 1
    )[0]
    assert "TDirectory.GetFiles(ATablesDirectory, '*.xml')" in resolver
    assert "LoadXMLData(ReadUtf8File(FileName))" in resolver
    assert "TDictionary<Cardinal, string>" in resolver
    assert "IdFiles.ContainsKey(IdValue)" in resolver
    assert "TStringComparer.OrdinalIgnoreCase" in resolver
    assert "RegisterName(NameValue, FileName)" in resolver
    assert "RegisterName(DisplayNameValue, FileName)" in resolver
    assert "ReportTableId := IdValue" in resolver
    assert "Exit(ReportTableId)" in resolver
    assert "MaxTableId = High(Cardinal)" in resolver
    assert "Result := MaxTableId + 1" in resolver


def test_report_table_range_and_columns_are_validated():
    assert "function BuildReportTableRange(ARowCount, AColumnCount: Integer): string;" in SERVICE
    range_helper = SERVICE.split("function BuildReportTableRange", 1)[1].split(
        "procedure ValidateReportTableColumns", 1
    )[0]
    assert "ARowCount < 0" in range_helper
    assert "AColumnCount > 16384" in range_helper
    assert "ARowCount + 1 > 1048576" in range_helper
    assert "procedure ValidateReportTableColumns(AColumns: TList<string>);" in SERVICE
    column_validator = SERVICE.split("procedure ValidateReportTableColumns", 1)[1].split(
        "function BuildWorksheetXml", 1
    )[0]
    assert "AColumns = nil" in column_validator
    assert "AColumns.Count = 0" in column_validator
    assert "Length(ColumnName) > 255" in column_validator
    assert "Ord(ColumnName[J]) < 32" in column_validator
    assert "TStringComparer.OrdinalIgnoreCase" in column_validator
    assert "XmlEscape(ColumnName) = ''" in column_validator


def test_generated_report_parts_are_dom_validated_before_writing():
    for declaration in (
        "procedure ValidateReportTableXml",
        "procedure ValidateReportWorksheetXml",
        "procedure ValidateReportWorksheetRelsXml",
    ):
        assert declaration in SERVICE
    inject = SERVICE.split("procedure InjectDataSheet", 1)[1].split(
        "class function TReportTemplateService.TemplatesPath", 1
    )[0]
    assert "ReportTableId := ResolveReportTableId(" in inject
    assert "ValidateReportTableColumns(Columns);" in inject
    assert "ExpectedTableRange := BuildReportTableRange(Rows.Count, Columns.Count);" in inject
    assert "WorksheetXml := BuildWorksheetXml(Rows, Columns);" in inject
    assert "TableXml := BuildTableXml(Rows.Count, Columns, ReportTableId);" in inject
    assert "ValidateReportWorksheetXml(WorksheetXml, ExpectedTableRange);" in inject
    assert "ValidateReportTableXml(TableXml, ExpectedTableRange," in inject
    assert "ValidateReportWorksheetRelsXml(WorksheetRelsXml);" in inject
    assert inject.index("ValidateReportWorksheetXml") < inject.index(
        "WriteUtf8File(SheetFile, WorksheetXml)"
    )
    assert inject.index("ValidateReportTableXml") < inject.index(
        "WriteUtf8File(TableFile, TableXml)"
    )
    assert "WriteUtf8File(SheetFile, BuildWorksheetXml" not in inject
    assert "WriteUtf8File(TableFile, BuildTableXml" not in inject


def test_report_part_validators_check_required_semantics():
    table_validator = SERVICE.split("procedure ValidateReportTableXml", 1)[1].split(
        "procedure ValidateReportWorksheetXml", 1
    )[0]
    for expected in (
        "CSpreadsheetNamespace", "AExpectedTableId", "DATA_TABLE_NAME",
        "AExpectedRange", "totalsRowShown", "AutoFilterCount <> 1",
        "TableColumnsCount <> 1", "TableColumnsNode.ChildNodes.Count",
        "tableColumn", "tableStyleInfo", "TStringComparer.OrdinalIgnoreCase",
    ):
        assert expected in table_validator
    worksheet_validator = SERVICE.split("procedure ValidateReportWorksheetXml", 1)[1].split(
        "procedure ValidateReportWorksheetRelsXml", 1
    )[0]
    for expected in (
        "dimension", "sheetData", "autoFilter", "AutoFilterCount <> 0",
        "tableParts", "tablePart", "r:id", "rId1",
    ):
        assert expected in worksheet_validator
    rels_validator = SERVICE.split("procedure ValidateReportWorksheetRelsXml", 1)[1].split(
        "procedure ValidateZipEntries", 1
    )[0]
    for expected in (
        "CPackageRelationshipsNamespace", "RelationshipCount <> 1",
        "CTableRelation", "../tables/flowServiceReportData.xml",
    ):
        assert expected in rels_validator
...
if __name__ == "__main__":
    ...
    test_report_table_has_one_filter_and_dynamic_id()
    test_report_table_id_is_resolved_from_all_table_xml_files()
    test_report_table_range_and_columns_are_validated()
    test_generated_report_parts_are_dom_validated_before_writing()
    test_report_part_validators_check_required_semantics()

...
def test_ui_is_moved_and_validated_before_notification_barrier_and_free():
    ...
    assert select < barrier < model_delete < tab_free < sync
    assert body.count("SelectWorkTable(NewActiveWorkTable)") == 1
...
def test_release_version():
    assert "APP_VERSION = '1.0.236'" in source("uAppVersion.pas")
...
def test_deleted_tab_is_freed_once_and_only_after_model_delete_succeeds():
    body = delete_body()
    model_delete = body.index("DeleteWorkTableByUUID(DeletedUUID)")
    failed = body.index("if not DeleteSucceeded then", model_delete)
    tab_guard = body.index("if Assigned(DeletedTab) and IsLiveWorkTableTab(DeletedTab) then", failed)
    parent_clear = body.index("DeletedTab.Parent := nil", tab_guard)
    tab_free = body.index("DeletedTab.Free", parent_clear)
    pointer_clear = body.index("DeletedTab := nil", tab_free)

    assert body.count("DeletedTab.Free") == 1
    assert model_delete < failed < tab_guard < parent_clear < tab_free < pointer_clear
    assert "SelectWorkTable(DeletedWorkTable)" in body[failed:tab_guard]
    assert "AttachWorkTableObservers(DeletedWorkTable)" in body[failed:tab_guard]
    assert "DeletedTab.Free" not in body[failed:tab_guard]

    tail = body[pointer_clear + len("DeletedTab := nil"):]
    for forbidden in ("DeletedTab.Parent", "DeletedTab.TagString", "DeletedTab.Text",
                      "Pointer(DeletedTab)", "IsLiveWorkTableTab(DeletedTab)"):
        assert forbidden not in tail
    assert "DeletedTabPointer := Pointer(DeletedTab)" in body[tab_guard:tab_free]
    assert "[DeletedUUID, DeletedTabPointer]" in body[tab_guard:tab_free]

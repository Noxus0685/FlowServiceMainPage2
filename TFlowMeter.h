...
class TFlowMeter : public TMeterDevice
{
  public:
    ...
    int Status = 0; // 0 - Ñîçäàí , 1 - Åñòü äàííûå, 1 - Åñòü äàííûå, íå ãîäåí,
		// 2 - Åñòü äàííûå, ãîäåí,
	...
	int SendStatus = 0; // 0 - íå îòïðàâëåí 1- îòïðàâëÿåòñÿ  2- îòïðàâëåí
    ...
    TDateTime DateTime; //äàòà è âðåìÿ èçìåðåíèÿ

    UnicodeString ModifidedDateTime;

    //
    // Êîýôôèöèåíò ïðåîáðàçîâàíèÿ
    double Kp;
    double FactoryKp;
    double FreqMax;
    //
    // Êàëèáðîâî÷íûå êîýôèöèåíòû
    double K[100];
    double Q[100];
    ...
    double Error;
    //Òåêóùèé ðàñõîä
    double Flow(void); // Ðàñõîä ñ ïîïðàâêàìè
	double GetRawFlow(void); // Ðàñõîä áåç ïîïðàâîê
    ...
    // Îáðàòíûå ôóíêöèè îáíîâëåíèÿ ñîñòîÿíèÿ
...
//    static System::UnicodeString fname;
	...
	eMeterFlowType MeterFlowCategory = WEIGHTS_TYPE;

	void SetMeterCategory(eMeterFlowType meterFlowType);
	void SetMeterCategory(UnicodeString meterFlowType);
    UnicodeString GetMeterCategory();
    ...
    //àðõèâ ïîñëåäíèõ 100 ñåêóíä èìïóëüñîâ
    uint16_t Impulses[100];
    uint8_t wr_imp; // èíäåêñ çàïèñè â àðõèâ
    uint8_t rd_imp; // èíäåêñ ÷òåíèÿ èç àðõèâà

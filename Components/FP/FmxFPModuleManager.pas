...
{ ===== Êëàññ TFmxModuleManager =====
  Íîâàÿ àðõèòåêòóðà: îòäåëüíûå ïîòîêè äëÿ êàæäîãî COM-ïîðòà.
  Êàæäûé ïîðò èìååò ñâîé TPortPollThread, êîòîðûé ñèíõðîííî îïðàøèâàåò ìîäóëè.
  SendCommand ðàáîòàåò ÷åðåç TTransferManager (ñèíõðîííî, áåç î÷åðåäè).
}
...
//  end;
  ...
  // ============================================================
    FStopping: Integer;
  // Êëàññ ïîòîêà äëÿ îïðîñà îäíîãî COM-ïîðòà (ñèíõðîííûé)
  // ============================================================
  ...
  TFmxModuleManager = class
  private
    // Ìåíåäæåð ïîðòîâ (äëÿ SendCommand)
    FTransferManager: TTransferManager;

    // Ïîòîêè äëÿ êàæäîãî ïîðòà
    FPortThreads: TDictionary<Integer, TPortPollThread>;
    FPortCurrentModule: TDictionary<Integer, Integer>; // ïîðò -> òåêóùèé èíäåêñ ìîäóëÿ
...
//    // Ïðîâåðÿåì ñîñòîÿíèå ïîðòà ÷åðåç ñïðàâî÷íèê
...
begin
  ...
  try
    ...
    // Ïðîâåðÿåì ñîñòîÿíèå ïîðòà ÷åðåç TransferManager
...
//  begin
//    PortStatus := psError;
//    PortState := 'Íåâåðíûé íîìåð ïîðòà';
//    Exit;
//  end;
//  {$IFDEF WINDOWS}
...
//    PortStatus := psOpen;
//    PortReady := True;
//    PortState := 'Ïîðò ' + PortName + ' îòêðûò';
//  end
//  else
//  begin
//    PortStatus := psError;
//    PortReady := False;
//    PortState := 'Îøèáêà îòêðûòèÿ ïîðòà ' + PortName;
//    ErrorConnection := True;
//  end;
//end;

// ============================================================
//  UpdateNextForPort - îïðîñ ìîäóëåé íà êîíêðåòíîì ïîðòó
// ============================================================
...
begin
  ...
  // Ïîëó÷àåì ñëåäóþùèé ìîäóëü äëÿ ýòîãî ïîðòà
...
// ============================================================
//  GetNextModuleForPort - ïîèñê ñëåäóþùåãî ìîäóëÿ íà ïîðòó
// ============================================================
...
begin
  ...
  // Ïîëó÷àåì òåêóùèé èíäåêñ äëÿ ýòîãî ïîðòà
  ...
  // Èùåì ñëåäóþùèé àêòèâíûé ìîäóëü íà ýòîì ïîðòó
...
begin
  ...
  try
    // Ñîáèðàåì óíèêàëüíûå íîìåðà ïîðòîâ èç àêòèâíûõ ìîäóëåé
    ...
    // Äëÿ êàæäîãî ïîðòà ñîçäà¸ì ïîòîê
    ...
    begin
      ...
      if Debug then
        DebugOutput('Çàïóùåí ïîòîê îïðîñà äëÿ COM' + IntToStr(PortNumber));
    end;
  finally
    PortsList.Free;
  end;
...
// ============================================================
//  SendCommand - ñèíõðîííàÿ îòïðàâêà êîìàíäû
// ============================================================
...
// ============================================================
//  ExecuteInCOMThread - çàãëóøêà (áîëüøå íå èñïîëüçóåòñÿ)
// ============================================================
...
begin
  ...
  // Çàâåðøàåì ïîòîêè îïðîñà ïîðòîâ
  ...
  begin
    try
      ...
      on E: Exception do
        if Debug then DebugOutput('Îøèáêà ïðè îñâîáîæäåíèè TCP êëèåíòà: ' + E.Message);
    end;
  end;
...
// ============================================================
//  SetActive - çàïóñê/îñòàíîâêà ïîòîêîâ
// ============================================================
...
// ============================================================
//  Ðàáîòà ñ COM-ïîðòàìè (èç îðèãèíàëüíîãî ôàéëà)
// ============================================================
...
//    begin
//      PortInfo.IsOpen := False;
//      PortState := 'Îøèáêà îòêðûòèÿ ïîðòà COM' + IntToStr(PortNumber);
...
//      AddComPortInfo(PortInfo);
//
//    PortState := 'Ïîðò COM' + IntToStr(PortNumber) + ' îòêðûò';
//
...
//
//          if Debug then
//            DebugOutput('Àâòîìàòè÷åñêè çàêðûò íåèñïîëüçóåìûé ïîðò COM' +
...
//    DebugOutput('Àâòîìàòè÷åñêè çàêðûòî ïîðòîâ: ' + IntToStr(ClosedCount));
//end;
...
begin
  ...
  begin
    try
      ...
      begin
        if Debug then
          DebugOutput('Ïîðò óñïåøíî çàêðûò');
        Exit;
      end
      else
      begin
        if Debug then
          DebugOutput('Ïîïûòêà ' + IntToStr(Attempts + 1) +
            ': íå óäàëîñü çàêðûòü ïîðò. Êîä îøèáêè: ' + IntToStr(GetLastError));
      end;
      {$ENDIF}
    except
      on E: Exception do
      begin
        if Debug then
          DebugOutput('Èñêëþ÷åíèå ïðè çàêðûòèè ïîðòà (ïîïûòêà ' +
  ...
  if Debug then
    DebugOutput('Íå óäàëîñü êîððåêòíî çàêðûòü ïîðò ïîñëå 3 ïîïûòîê. ' +
                'Äåñêðèïòîð: ' + IntToStr(PortHandle));
end;

// ============================================================
//  Ñòàòèñòèêà è âñïîìîãàòåëüíûå ìåòîäû
// ============================================================
...
begin
  // Çàãëóøêà - áîëüøå íå èñïîëüçóåòñÿ
end;
...
begin
  ...
  begin
    ...
    if not Value then
    begin
      // Çàêðûâàåì âñå ïîðòû ÷åðåç TransferManager
      if Assigned(FTransferManager) then
...
var
  LogDir: string;
begin
  // Çàêðûâàåì ôàéë åñëè îí áûë îòêðûò
  if LogFileIsOpened then
  begin
    try
      CloseFile(LogFile);
  ...
  begin
    ...
    FLogFileName := '';
    if Debug then
      DebugOutput('ËÎÃ îáìåíà îòêëþ÷åí');
  end
  else
  begin
    ...
    begin
      try
        ...
        on E: Exception do
        begin
          DebugOutput('Îøèáêà ñîçäàíèÿ äèðåêòîðèè äëÿ ëîãà: ' + E.Message);
          LogFileIsOpened := false;
          FLogFileName := '';
          Exit;
        end;
    ...
    try
      ...
      begin
        ...
        begin
          WriteLn(LogFile, '=== Ëîã îáìåíà äàííûìè ÷åðåç COM-ïîðò è Modbus TCP ===');
          WriteLn(LogFile, '=== Íà÷àëî: ' + DateTimeToStr(Now) + ' ===');
          WriteLn(LogFile, '');
        end;
        CloseFile(LogFile);
        if Debug then
          DebugOutput('ËÎÃ îáìåíà: ' + log_filename);
      end
      else
      begin
        DebugOutput('Îøèáêà îòêðûòèÿ ôàéëà ëîãà: ' + log_filename);
        FLogFileName := '';
      end;
    except
      on E: Exception do
      begin
        LogFileIsOpened := false;
        FLogFileName := '';
        DebugOutput('Îøèáêà ïðè ðàáîòå ñ ôàéëîì ëîãà: ' + E.Message);
      end;
    end;
  end;
end;
...
//    begin
//      if FComPortNumber > 0 then
//        Result := 'COM' + IntToStr(FComPortNumber) + ' (çàêðûò)'
//      else
//        Result := 'Ïîðòû íå íàñòðîåíû';
//    end;
//  finally
//    FComPortsLock.Leave;
//  end;
...
begin
  // Ïîëó÷àåì êîëè÷åñòâî îòêðûòûõ ïîðòîâ èç TransferManager
  if Assigned(FTransferManager) then
...
var
  PortInfo: TComPortInfo;
begin
  // Èíèöèàëèçèðóåì ïóñòîé ñòðóêòóðîé íà ñëó÷àé îøèáêè
...
// === ÍÎÂÛÉ ÌÅÒÎÄ ÄËß ÏÎËÓ×ÅÍÈß ÑÏÈÑÊÀ ÏÎÐÒÎÂ ===
...
// ============================================================
//  Ìåòîäû-çàãëóøêè äëÿ ñîâìåñòèìîñòè
// ============================================================
...
// ============================================================
//  Îáíîâëåíèå UpdateNext (ñòàðûé ìåòîä - çàãëóøêà)
// ============================================================

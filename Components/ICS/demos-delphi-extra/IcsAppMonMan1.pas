...
const
    ...
    ProgVersion = 'ICS Application Monitor - Remote Manager - V9.4 - 15th February 2025';
...
// not saved
  ...
  public
    ...
    function DisconnServer(SrvNr: Integer): Boolean;
...
begin
    ...
    with ServInfos[SrvNr] do begin
        ...
        // create WebSocket and it's events for updates for the arrays
            ...
            WebSocCli.WSPingSecs := 60;  // send keep-alive ping/pongs so we know if connection fails
            ConnectDT := 0;
            RetryTick := Trigger64Disabled;
            FrameTick := IcsGetTickCount64;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
// disconnect from Websocket from remote server
function TManForm.DisconnServer(SrvNr: Integer): Boolean;
var
    Ret: Integer;
    S: String;
begin
    ...
    with ServInfos[SrvNr] do begin
        try
            ...
            S :='HTTP Failed to Disconnect: ' + IcsGetExceptMess (ExceptObject) ;
            LastStatus := S;
            AddLog(S);
             WebsocCli := Nil;
        end;
...
begin
    ...
    // check if new connection attempt needed
        for I := 1 to TotServers do begin
            ...
            // no frames for over two minutes, assume Websocket connection has died
                if SrvState = StateConnected then begin
                    IdleSecs := IcsElapsedSecs64(FrameTick);
                    if (IdleSecs >= 130) then begin
                        LastStatus := 'No Traffic from Server for ' + IntToStr(IdleSecs) + ' seconds, Reconnecting';
                        AddLog(ServerName + ': ' + LastStatus);
                        DisconnServer(I);
                        RetryTick := IcsGetTrgSecs64(5); // try again quickly
                        LastStatus := 'Reconnect in 5 seconds';
                        AddLog(ServerName + ': ' + LastStatus);

...
Aug 11, 2022 V8.70 In IcsGetDirList don't resize array if set already.
...
const
    FtpSrvT_Unit       = 1000;
    CopyRight : String = ' FtpSrvT  (c) 1999-2024 F. Piette V10.0 ';
...
function DecodeMlsResp (Response: String; var Fname, FType, FAttr: String; var FSize: Integer; var FileUDT: TDateTime): boolean;
function TimeDateStr(dDateTime : TDateTime) : String;
function DateTimeToUTC(dtDT : TDateTime) : TDateTime;
function DecodeMlsResp64 (Response: String; var Fname, FType, FAttr: String; var FSize: Int64; var FileUDT: TDateTime): boolean;

{ V1.15 recursive directory listing and argument scanning }
function IcsGetDirList (const Path: string; SubDirs, Hidden: boolean; var LocFiles: TIcsFileRecs; var LocFileList: TList;
                                                            Obj: TObject = Nil; ProgressCallback: TMD5Progress = Nil): integer ;  { V7.08 }
function IcsBuildDirList (const LocDir, LocPartName: String; SubDirs, Hidden: boolean; Level, InitDLen: integer ;
                var TotFiles: integer; var LocFiles: TIcsFileRecs; Obj: TObject = Nil; ProgressCallback: TMD5Progress = Nil): boolean ;
...
function FtpFileMD5(const Filename: String; Obj: TObject = Nil; ProgressCallback : TMD5Progress = Nil; StartPos: Int64 = 0;
                                                                            EndPos: Int64 = 0; Mode: Word = DefaultMode): String;
function FtpFileCRC32B(const Filename: String; Obj: TObject = Nil; ProgressCallback : TCrcProgress = Nil; StartPos: Int64 = 0;
                                                                            EndPos: Int64 = 0; Mode: Word = DefaultMode): String;
            ...
            begin
             ...
             { found another directory, recursively call this function to process it }
                if savename and (((SearchRec.Attr and faDirectory) =
                    ...
                    if not IcsBuildDirList (LocDir + CurName + '\', LocPartName, SubDirs, Hidden, succ(Level),
                                                               InitDLen, TotFiles, LocFiles, Obj, ProgressCallback) then  { V7.08 }
                        exit;
...
begin
    if (Length(LocFiles) = 0) then  { V8.70 may be set already }
        SetLength(LocFiles, 1000);
    totfiles := 0 ;
    if not Assigned (LocFileList) then
        LocFileList := TList.Create ;
    LocFileList.Clear ;
    LocDir := ExtractFilePath (Path);
    LocPartName := ExtractFileName (Path);
    if LocPartName = '' then LocPartName := '*.*';
    flag := IcsBuildDirList (LocDir, LocPartName, SubDirs, Hidden,  0, 0, totfiles, LocFiles, Obj, ProgressCallback);  { V7.08 }

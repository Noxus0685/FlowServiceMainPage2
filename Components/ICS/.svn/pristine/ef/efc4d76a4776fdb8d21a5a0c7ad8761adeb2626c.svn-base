...
Dec xx, 2024 V10.0 Updated for Windows and Linux, planned for mobile platforms.
                   Shows NewMessaging on splash if so built.
...
{$ELSE}
    ...
    {$ENDIF}
{$ENDIF}
    sIcsLongProductName = 'Internet Component Suite V10.0'
...
{$ENDIF ICS_NewMessaging}   { V10 }
    ...
    sIcsDescription     = sIcsLongProductName + IcsCRLF +
                          //'Copyright (C) 1996-2024 by François PIETTE'+ IcsCRLF +
                          // Actually there's source included with different
                          // copyright, so either all or none should be mentioned
                          // here.
                          'https://www.overbyte.eu/' + IcsCRLF +
                          'Wiki: https://wiki.overbyte.eu/' + IcsCRLF +
                          'Support: https://en.delphipraxis.net/forum/37-ics-internet-component-suite/' + IcsCRLF +
                          'svn://svn.overbyte.be/icsv10' + IcsCRLF +
                          'https://svn.overbyte.be/svn/icsv10' + IcsCRLF +

...
Nov 21, 2024 V9.4  Fixed memork leak with multiple virtual PUT and POST documents,
                     thanks to Yves Vermeersch.
Mar 08, 2025 V9.5  INI file reads AttachmentTypes.
...
type
    THttpAppSrvDisplayEvent = procedure(Sender: TObject; const Msg : String) of object;

    TVirtualExceptionEvent = procedure (Sender: TObject; E: Exception; Method: THttpMethod; const Path: string) of object;  { V7.05 }
...
{$IFDEF ICS_NewMessaging}   { V10 }
        procedure WMFinish(const Value: TIcsSocketMsgRec);
{$ELSE}
        FMsg_WM_FINISH   : UINT;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
class procedure THttpAppSrv.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    HttpSrv: THttpAppSrv;
begin
    if Assigned(Value.Instance) and (Value.Instance is THttpAppSrv) then begin
        HttpSrv := THttpAppSrv(Value.Instance);
        try
            with Value do begin
                if Msg = WM_ICS_HTTPSRV_FINISH  then
                    HttpSrv.WMFinish(Value)
            end;
        except
            on E: Exception do
                HttpSrv.HandleBackGroundException(E, 'THttpAppSrv:DispatchMessage: ' + IntToStr(Value.Msg));
        end;
    end
    else
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure THttpAppSrv.WMFinish(const Value: TIcsSocketMsgRec);
var
    SObj : TUrlHandler;
    I    : Integer;
begin
    SObj := TUrlHandler(Value.LParam);
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
        THttpAppSrv.PostMessage(Self, WM_ICS_HTTPSRV_FINISH, 0, LPARAM(Self));
{$ELSE}

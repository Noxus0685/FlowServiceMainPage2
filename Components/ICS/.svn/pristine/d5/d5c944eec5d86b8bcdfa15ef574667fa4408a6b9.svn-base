...
Feb 24, 2025 V9.5  Version only.


Dec xx, 2024 V10.0 Removed old MacOS only Posix code and replaced with what is hopefully
                     more modern cross platform code initially targettng Linux and Windows,
                     but also MacOS, Android and iOS in the future, when the hardware
                     is available. DEFINE ICS_NewMessaging specifies use the new cross
                     platform code which uses modern Delphi language features so requires
                     Delphi 10.4 or later.
                   Added application friendly logging via unused OnDebugDisplay.event,
                     such as connection opened and closed.
                   Added WSocket_GetLastError.
                   Setting State and HSocket now protected by critical sections.
...
{$IFDEF ICS_NewMessaging}
{$IFDEF RTL_MESSAGING}
//  System.Messaging,      { V10 }
{$ELSE}
//  NX.Horizon,           { V10 }
{$ENDIF RTL_MESSAGING}
{$ENDIF ICS_NewMessaging}
...
{$ENDIF}
  ...
  OverbyteIcsTypes,      { most type and const definitions for ICS components }
  OverbyteIcsWinsock,
  OverbyteIcsUtils,
  OverbyteIcsAvlTrees,
...
{$IFDEF FMX}
  ...
  {$IFDEF ICS_NewMessaging}        { V10 }
      Ics.Fmx.OverbyteIcsMessMan,  { V10 }
  {$ELSE}
...
const
  WSocketVersion            = 1000;
  CopyRight    : String     = ' TWSocket (c) 1996-2026 Francois Piette V10.0 ';
...
{$ENDIF MSWINDOWS}
  ...
  private
    ...
    FFunc         : String;
  public
    constructor Create(const AMessage: String;  AErrorCode: Integer = 0;  const AErrorMessage: String = '';
      const AFriendlyMsg: String = ''; const AFunc: String = ''; const AIP: String = ''; const APort: String = ''; const AProto: String = '');
...
{$IFNDEF ICS_NewMessaging}   { V10 }
    ...
    { Pointer to a TIcsAsyncDnsLookup instance shared by all TWSocket instances which called one of the IcsAsyncXxxx methods from the same thread context}
    FAsyncLookupPtr     : Pointer;
{$ENDIF N ICS_NewMessaging} { V10 }
{$IFDEF ICS_NewMessaging}   { V10 }
    procedure CloseSocketInternal(ErrorCode : LongInt);
    procedure Do_FD_ACCEPT; virtual;
//    procedure ListenEventDetected;
    procedure AddSocketHandleToWaitList(SocketHandle: TSocket);
    procedure DelSocketHandleFromWaitList(SocketHandle: TSocket);
    procedure GetHostByNameDetected;
    procedure GetHostByAddrDetected;
    procedure CloseDelayedDetected;
    function  CheckValidHSocket : Boolean;
{$ENDIF ICS_NewMessaging}   { V10 }
  protected
    FHSocketX           : TSocket;        { V10.0 renamed, use Put/GetHSocket instead }
    FStateX             : TSocketState;   { V10.0 renamed, use Put/GetState instead }
{$IFDEF MSWINDOWS}
    FHSocketEvent       : TWSAEVENT;             { V10.0 }
{$ENDIF}
    FASocket            : TSocket;               { Accepted server socket }
...
{$ENDIF N ICS_NewMessaging}  { V10 }
    ...
    FMultiCast          : Boolean;
    { Multicast addresses consists of a range of addresses from 224.0.0.0 to  239.255.255.255. However, the multicast addresses from 224.0.0.0 to     }
    { 224.0.0.255 are reserved for multicast routing information; Application  programs should use multicast addresses outside this range.            }
  ...
  {xIFDEF MSWINDOWS}
    FConnectThreadId    : TThreadID; { V10.0 }
  {xENDIF}
    FRcvdFlag           : Boolean;
...
{$IFDEF ICS_NewMessaging}   { V10 }
    FEventPipe          : TPipeFd;
    FCloseInternalLock  : IReadWriteSync;
{$ENDIF ICS_NewMessaging}   { V10 }
...
{$ENDIF}
    function  GetHSocket : TSocket;                 { V10.0 setters for HSocketX and StateX }
    procedure SetHSocket(Value : TSocket);          { dup }
    procedure SetHSocketValue(Value : TSocket);
    procedure SetState(Value : TSocketState);
    function  GetState : TSocketState;
 {$IFDEF ICS_NewMessaging}   { V10 }
    procedure Do_FD_CONNECT; virtual;
    procedure Do_FD_CONNECT_Special(Error : Word); virtual;
    procedure Do_FD_CLOSE(ErrCode : WORD); virtual;
    procedure Do_FD_READ; virtual;
    procedure Do_FD_READ_Special(Error : Word); virtual;
    procedure Do_FD_WRITE; virtual;
    procedure ErrorEventDetected; virtual;
    procedure CloseSocketEventDetected; virtual;
{$IFDEF MSWINDOWS}
    procedure PipeCmdAddHandleToWaitList(AHandle: THandle; Handles: PHandle; var Count : Integer);
    procedure PipeCmdDelHandleFromWaitList(AHandle: THandle; Handles: PHandle; var Count : Integer);
    function  pipe(EventPipe : PPipeFd) : Integer;
    function __close(Hdl : THANDLE) : Integer;
    function __write(Hdl: THANDLE; Buf: Pointer; Cnt: size_t) : Integer;
    function __read(Hdl: THANDLE; Buf: Pointer; Cnt: size_t) : Integer;
{$ENDIF}
    procedure PipeCommand(Command : BYTE); overload;
    procedure PipeCommand(Command : array of const); overload;
{$ENDIF ICS_NewMessaging}   { V10 }
...
{$IFNDEF ICS_NewMessaging}   { V10 }
    ...
    { The next are wrappers around methods of TIcsAsyncDnsLookup. They mimic the MS native async DNS looup API that isn't available for IPv6.   }
    function IcsAsyncGetHostByName(AWnd: HWND; AMsgID: UINT; const ASocketFamily: TSocketFamily; const AName: String;  const AProtocol: Integer): THandle;
    function IcsAsyncGetHostByAddr(AWnd: HWND; AMsgID: UINT;  const ASocketFamily: TSocketFamily; const AAddr: String; const AProtocol: Integer): THandle;
{$ENDIF N ICS_NewMessaging}  { V10.0 }

    procedure   AbortComponent(E:Exception); override;                        { V8.68 added E to allow reporting }
    procedure   SocketError(sockfunc: String; LastError: Integer = 0; FriendlyMsg: String = ''); virtual;   { V8.36 added FriendlyMsg }
    ...
    procedure   SendText(const Str : UnicodeString; ACodePage : Cardinal); overload;  {$ENDIF}
    ...
    procedure   RaiseException(const Msg: String; AErrorCode: Integer; const AErrorMessage: String = ''; const AFriendlyMsg: String = '';
                          const AFunc: String = ''; const AIP: String = ''; const APort: String = ''; const AProto: String = ''); overload; virtual;
    ...
    function    DoRecv(var Buffer: TWSocketData; BufferSize: Integer; Flags: Integer) : Integer; virtual;
    function    DoRecvFrom(ASocket: TSocket; var Buffer: TWSocketData; BufferSize: Integer; Flags: Integer;
                                                                           var From: TSockAddr; var FromLen : Integer): Integer; virtual;
  public
...
{$IFDEF ICS_NewMessaging}   { V10 }
// moved Listener to WndControl
//    class procedure IcsSocketMessageListener({$IFDEF RTL_MESSAGING}const Sender: TObject;{$ENDIF}  const M: TIcsSocMsg);
//    class function PostMessage(const Sender: TObject; const Msg: Cardinal; const WParam: NativeUInt; const LParam: NativeUint): Boolean;
    class procedure DispatchMessage(const Value : TIcsSocketMsgRec); override;
    procedure CommonConnect; virtual;
    procedure ThreadExecuteProc; override;
    procedure ThreadTerminateProc; override;
    procedure XXXPipeWrite(Ch : BYTE);
{$ENDIF ICS_NewMessaging}   { V10 }
...
{$ENDIF N ICS_NewMessaging}   { V10 }
    ...
    procedure   Flush; virtual; deprecated 'Use events instead';    { V10.0 }
...
{$ENDIF}
    function    ReceiveFrom(Buffer: TWSocketData; BufferSize: Integer; var From: TSockAddr; var FromLen: Integer): Integer; virtual;
    function    ReceiveFrom6(Buffer: TWSocketData; BufferSize: Integer; var From: TSockAddrIn6; var FromLen: Integer) : Integer; virtual;
    function    ReceiveFromTB(var Data: TBytes; var From: TSockAddr;  var FromLen: Integer; MaxLen: Integer = -1): Integer; virtual;       { V8.70 }
    function    ReceiveFrom6TB(var Data: TBytes; var From: TSockAddrIn6;  var FromLen: Integer; MaxLen: Integer = -1): Integer; virtual;   { V8.70 }
    ...
    function    SendTo(Dest: TSockAddr; DestLen: Integer; Data: TWSocketData; Len: Integer): Integer; virtual;
    function    SendToTB(Dest: TSockAddr; DestLen: Integer; const Data: TBytes; Len: Integer = -1): Integer; virtual;     { V8.69 }
    function    SendTo6(Dest: TSockAddrIn6; DestLen: Integer;  Data: TWSocketData; Len: Integer) : Integer; virtual;
    function    SendToTB6(Dest: TSockAddrIn6; DestLen: Integer; const Data : TBytes; Len: Integer = -1): Integer; virtual;   { V8.69 }
...
//                                                    write SetIcsLogger;       { V5.21 }
    property IcsLogger;                                                       { V10 now in WndControl }
{$ENDIF}
    property PortNum : Integer                      read  FPortNum;
    property HSocket : TSocket                      read  GetHSocket         { V10.0 }
                                                    write SetHSocket;
    property State : TSocketState                   read  GetState           { V10.0 }
                                                    write SetState;
  ...
  THttpTunnelAuthType = (htatDetect, htatNone, htatBasic,{$IFNDEF NO_HTTP_TUNNEL_AUTHDIGEST}htatDigest,{$ENDIF}htatNtlm );
  THttpTunnelErrorEvent = procedure(Sender: TObject; ErrCode: Word; TunnelServerAuthTypes: THttpTunnelServerAuthTypes; const Msg: String) of object;
...
{$ENDIF N ICS_NewMessaging}  { V10 }
  ...
  protected
{$IFDEF ICS_NewMessaging}   { V10 }
      procedure WMHttpTunnelReconnect(const Value: TIcsSocketMsgRec);
{$ELSE}
      procedure AllocateMsgHandlers; override;
      procedure FreeMsgHandlers; override;
      function  MsgHandlersCount : Integer; override;
...
{$ENDIF ICS_NewMessaging}  { V10 }
      ...
      function  DoRecv(var Buffer : TWSocketData; BufferSize : Integer; Flags: Integer): Integer; override;
...
{$IFDEF ICS_NewMessaging}   { V10 }
      procedure   Do_FD_CONNECT_Special(Error : Word); override;
      procedure   Do_FD_READ_Special(Error : Word); override;
{$ELSE}
...
{$ENDIF ICS_NewMessaging}   { V10 }
  ...
  public
      constructor Create(AOwner : TComponent); override;
      procedure   SetSocks(const Host: string = ''; const Port: string = '';  const User: string = ''; const Pass: string = '');   { V8.66 }
      procedure   SetHTTPTunnel(const Host: string = ''; const Port: string = '';  const User: string = ''; const Pass: string = '');   { V8.66 }
      procedure   Connect; override;
      procedure   Listen; override;
  protected
...
type
  TSslVerifyPeerEvent = procedure (Sender: TObject; var Ok: Integer; Cert: TX509Base) of object;
  TSslHandshakeDoneEvent = procedure (Sender: TObject; ErrCode: Word; PeerCert: TX509Base;  var Disconnect: Boolean) of object;
 { V9.3 TCertVerMethod = CertVerOwnEvent }
  TSslCertVerifyEvent = procedure (Sender: TObject; CertChain: TX509List; var VerifyCode: Integer;
                                                                  var VerifyInfo: String;  var Disconnect: Boolean) of object;
  //Client-side session caching
  TSslCliGetSession = procedure(Sender: TObject; var SslSession: Pointer; var FreeSession: Boolean) of object;
  TSslCliNewSession = procedure(Sender: TObject; SslSession: Pointer; WasReused: Boolean; var IncRefCount: Boolean) of object;
  //Server-side session caching
  TSslSetSessionIDContext = procedure(Sender: TObject; var SessionIDContext: TSslSessionIdContext) of object;
  TSslSvrNewSession = procedure(Sender: TObject; SslSession: Pointer; SessId: Pointer; Idlen: Integer; var AddToInternalCache : Boolean) of object;
  TSslSvrGetSession = procedure(Sender: TObject; var SslSession: Pointer; SessId: Pointer; Idlen: Integer; var IncRefCount: Boolean) of object;
  TSslCliCertRequest = procedure(Sender: TObject;  var Cert : TX509Base) of object;
  TSslShutDownComplete = procedure(Sender: TObject;  Bidirectional: Boolean; ErrCode: Integer) of object;
  TSslServerNameEvent = procedure(Sender: TObject; var Ctx: TSslContext; var ErrCode: TTlsExtError) of object;
 { V8.40 handshake protocol message callback }
  TSslProtoMsgEvent = procedure (Sender: TObject; Info: String; Sending: integer; Version: integer;
                                                        ContentType: integer; Buffer: PAnsiChar; BuffSize: integer) of object;
 { V8.56 SSL ALPN protocol selectiopn }
  TSslAlpnSelect = procedure (Sender: TObject; ProtoList: TStrings; var SelProto: String; var ErrCode: TTlsExtError) of object;
...
{$IFDEF ICS_NewMessaging}   { V10 }
        FReadEventIgnore            : Boolean;
        FReadEventIgnored           : Boolean;
        procedure Do_FD_CONNECT; override;
        procedure Do_FD_READ; override;
        procedure Do_FD_WRITE; override;
        procedure Do_FD_CLOSE(ErrCode : WORD); override;
        procedure Do_FD_ACCEPT; override;
        procedure WMSslBiShutDown(const Value: TIcsSocketMsgRec);
        procedure WMSslASyncSelect(const Value: TIcsSocketMsgRec);
        procedure WMTriggerSslShutDownComplete(const Value: TIcsSocketMsgRec);
{$ELSE}
        procedure AllocateMsgHandlers; override;
        procedure FreeMsgHandlers; override;
        function  MsgHandlersCount : Integer; override;
...
{$ENDIF ICS_NewMessaging}   { V10 }
        ...
        procedure RaiseLastOpenSslError(EClass: ExceptClass; Dump: Boolean = FALSE; const CustomMsg: String  = ''); virtual;
        ...
        procedure TriggerSslVerifyPeer(var Ok: Integer; Cert: TX509Base); virtual;
...
{$ENDIF N ICS_NewMessaging}   { V10 }
        ...
        procedure   SslBiShutDownAsync;    { V10.0 was private }
...
{$ENDIF}
  ...
  private
      function my_WSocket_recv(s: TSocket; var Buf: TWSocketData; len, flags: Integer): Integer;
...
type
  TLineLimitEvent = procedure (Sender: TObject; RcvdLength: Integer; var ClearData: Boolean) of object;
...
{$IFDEF ICS_NewMessaging}   { V10 }
      procedure WMTriggerDataAvailable(const Value: TIcsSocketMsgRec);
{$ELSE}
      procedure   WndProc(var MsgRec: TMessage); override;
...
{$ENDIF ICS_NewMessaging}   { V10 }
      ...
      function    TriggerDataAvailable(ErrCode : Word) : Boolean; override;
      procedure   TriggerSessionClosed(Error : Word); override;
      procedure   TriggerLineLimitExceeded(Cnt: Integer;  var ClearData : Boolean); virtual;
...
{$IFNDEF ICS_NewMessaging}   { V10 }
  ...
  public
      procedure   ReadLine(Timeout : Integer; var Buffer : AnsiString); deprecated {$IFDEF COMPILER12_UP}'Do not use in new applications'{$ENDIF};
  end;
{$ENDIF N ICS_NewMessaging}   { V10 }
...
{$IFDEF BUILTIN_TIMEOUT}
  ...
  TCustomTimeoutWSocket = class(TCustomLineWSocket)                  { V10 was  TCustomSyncWSocket }
  private
...
function WSocket_getsockopt(s: TSocket; level, optname: Integer; optval: PAnsiChar; var optlen: socklen_t): Integer; overload;                { V8.70 }
function WSocket_getsockopt(s: TSocket; level, optname: Integer; optval: UnicodeString; var optlen: socklen_t): Integer; overload;            { V8.70 }
...
function WSocket_GetLastError: Integer;                             { V10.0 }
...
const
    ICS_MIN_TIMEOUT_SAMPLING_INTERVAL = 1000;     { V10.0 }
    ICS_DFT_TIMEOUT_SAMPLING_INTERVAL = 5000;     { V10.0 }
var
    IcsAsyncMinTimeoutSamplingInterval : Cardinal = ICS_MIN_TIMEOUT_SAMPLING_INTERVAL;
    IcsAsyncDftTimeoutSamplingInterval : Cardinal = ICS_DFT_TIMEOUT_SAMPLING_INTERVAL;
...
{$ENDIF}
    IPList        : TStrings;
    CritSecHSocket : TIcsCriticalSection;   { V10.0 }
    CritSecState   : TIcsCriticalSection;   { V10.0 }
...
type
  ...
  private
    ...
    function ExecAsync(AWnd: HWND; AMsgID: UINT; ASocketFamily: TSocketFamily; const AName: string; AReverse: Boolean; AProtocol: Integer): THandle;
  ...
  public
    constructor Create(const AMaxThreads: Integer; const AMinThreads: Integer = 0; const AThreadIdleTimeoutSec: Cardinal = 60);
    destructor Destroy; override;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function  ICS_IN6_IS_ADDR_LOOPBACK(const addr: PIn6Addr): Boolean;     { V9.4 }
begin
{$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.IN6_IS_ADDR_LOOPBACK(addr);
{$ELSE}
    Result := Posix.NetinetIn.IN6_IS_ADDR_LOOPBACK(addr^);
{$ENDIF}
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function ICS_IN6ADDR_ISANY(sa: PSockAddrIn6): Boolean;                                  { V10.0 }
begin
{$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.IN6ADDR_ISANY(PSockAddrIn6(sa));
{$ELSE}
    if sa <> nil then begin
        with sa^ do begin
            Result := (sin6_family = AF_INET6) and (PULONG(@sin6_addr.s6_addr[0])^ = 0) and (PULONG(@sin6_addr.s6_addr[4])^ = 0)
                                                and(PULONG(@sin6_addr.s6_addr[8])^ = 0) and (PULONG(@sin6_addr.s6_addr[12])^ = 0);
        end;
    end
    else
      Result := False;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function ICS_IN6_ADDR_EQUAL(const a: PIn6Addr; const b: PIn6Addr): Boolean;         { V10.0 }
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF MSWINDOWS}
function WSocket_Synchronized_WSAStartup(wVersionRequired: word; var WSData: TWSAData): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
    Result := OverbyteIcsWinsock.Ics_WSAStartup(wVersionRequired, WSData)
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_WSACleanup : Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
    Result := OverbyteIcsWinsock.Ics_WSACleanup;
end;
{$ENDIF}


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure WSocket_Synchronized_WSASetLastError(iError: Integer); {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    OverbyteIcsWinsock.Ics_WSASetLastError(iError);
  {$ENDIF}
  {$IFDEF POSIX}
    System.SetLastError(IError);
  {$ENDIF}
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_WSAGetLastError: Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_WSAGetLastError;
  {$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF MSWINDOWS}
function WSocket_Synchronized_WSACancelAsyncRequest(hAsyncTaskHandle: THandle): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
    Result := OverbyteIcsWinsock.Ics_WSACancelAsyncRequest(hAsyncTaskHandle);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_WSAAsyncGetHostByName(HWindow: HWND; wMsg: u_int;  name, buf: PAnsiChar;  buflen: Integer): THandle;
                                                                                              {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
    Result := OverbyteIcsWinsock.Ics_WSAAsyncGetHostByName(HWindow, wMsg, name, buf, buflen);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_WSAAsyncGetHostByAddr(HWindow: HWND; wMsg: u_int; addr: PAnsiChar;  len, Struct: Integer;
                                                    buf: PAnsiChar; buflen: Integer): THandle; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
    Result := OverbyteIcsWinsock.Ics_WSAAsyncGetHostByAddr(HWindow, wMsg, addr, len, struct, buf, buflen);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_WSAAsyncSelect(s: TSocket; HWindow: HWND; wMsg: u_int; lEvent: Integer): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
    Result := OverbyteIcsWinsock.Ics_WSAAsyncSelect(s, HWindow, wMsg, lEvent);
end;
{$ENDIF Windows }

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_getservbyname(name, proto: PAnsiChar): PServEnt; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_getservbyname(name, proto);
  {$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_getprotobyname(const Name: AnsiString): PProtoEnt; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_getprotobyname(PAnsiChar(Name));
  {$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_gethostbyname(name: PAnsiChar): PHostEnt; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_gethostbyname(name);
  {$ENDIF}
  {$IFDEF POSIX}
    Result := Posix.NetDB.gethostbyname(name);
  {$ENDIF}
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_gethostbyaddr(addr: Pointer; len, Struct: Integer): PHostEnt; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_gethostbyaddr(addr, len, Struct);
  {$ENDIF}
  {$IFDEF POSIX}
    Result := Posix.NetDB.gethostbyaddr(addr, len, Struct);
  {$ENDIF}
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_gethostname(name: PAnsiChar; len: Integer): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_gethostname(name, len);
  {$ENDIF}
  {$IFDEF POSIX}
    Result := Posix.Unistd.gethostname(name, len);
  {$ENDIF}
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_socket(af, Struct, protocol: Integer): TSocket;{$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_socket(af, Struct, protocol);
  {$ENDIF}
  {$IFDEF POSIX}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_shutdown(s: TSocket; how: Integer): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_shutdown(s, how);
  {$ENDIF}
  {$IFDEF POSIX}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_setsockopt(s: TSocket; level, optname: Integer; optval: PAnsiChar; optlen: Integer): Integer; overload;
                                                                                                  {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_setsockopt(s, level, optname, optval, optlen);
  {$ENDIF}
  {$IFDEF POSIX}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_setsockopt(s: TSocket; level, optname: Integer; var optval: TLinger; optlen: Integer): Integer; overload;
                                                                                                    {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_setsockopt(s, level, optname, @optval, optlen);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_setsockopt(s: TSocket; level, optname: Integer; var optval: ip_mreq; optlen: Integer): Integer; overload;
                                                                                                {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_setsockopt(s, level, optname, @optval, optlen);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_setsockopt(s: TSocket; level, optname: Integer; var optval: Integer; optlen: socklen_t): Integer; overload;
                                                                                                  {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_setsockopt(s, level, optname, @optval, optlen);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_setsockopt(s: TSocket; level, optname: Integer; var optval: TInAddr; optlen: socklen_t): Integer; overload;
                                                                                                  {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_setsockopt(s, level, optname, @optval, optlen);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_getsockopt(s: TSocket; level, optname: Integer; optval: PAnsiChar; var optlen: socklen_t): Integer;
                                                                                               {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_getsockopt(s, level, optname, optval, optlen);
  {$ENDIF}
  {$IFDEF POSIX}
    Result := Posix.SysSocket.getsockopt(s, level, optname, optval^, optlen);
  {$ENDIF}
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_sendto(s: TSocket; const Buf: TWSocketData; len, flags: Integer;  var addrto: TSockAddr;
                                                                    tolen: Integer): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_sendto(s, Buf^, len, flags, addrto, tolen);
  {$ENDIF}
  {$IFDEF POSIX}
    Result := Posix.SysSocket.sendto(s, Buf^, len, flags, Posix.SysSocket.psockaddr(@addrto)^, tolen);
  {$ENDIF}
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_send(s: TSocket; var Buf: TWSocketData; len, flags: Integer): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_send(s, Buf^, len, flags);
  {$ENDIF}
  {$IFDEF POSIX}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_ntohs(netshort: u_short): u_short; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_ntohs(netshort);
  {$ENDIF}
  {$IFDEF POSIX}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_ntohl(netlong: u_long): u_long; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_ntohl(netlong);
  {$ENDIF}
  {$IFDEF POSIX}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_listen(s: TSocket; backlog: Integer): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_listen(s, backlog);
  {$ENDIF}
  {$IFDEF POSIX}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_ioctlsocket(s: TSocket; cmd: Cardinal; var arg: u_long): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_ioctlsocket(s, cmd, arg);
  {$ENDIF}
  {$IFDEF POSIX}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF MSWINDOWS}
function WSocket_Synchronized_WSAIoctl(s: TSocket; IoControlCode: DWORD; InBuffer: Pointer; InBufferSize: DWORD; OutBuffer: Pointer;
                       OutBufferSize: DWORD;  var BytesReturned: DWORD; Overlapped: POverlapped; CompletionRoutine : FARPROC): Integer;
                                                                                                  {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
    Result := OverbyteIcsWinsock.Ics_WSAIoctl(s, IoControlCode, InBuffer, InBufferSize, OutBuffer, OutBufferSize, BytesReturned, Overlapped, CompletionRoutine);
end;
{$ENDIF}

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_inet_ntoa(inaddr: TInAddr): PAnsiChar; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_inet_ntoa(inaddr);
  {$ENDIF}
  {$IFDEF POSIX}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_inet_addr(const cp: AnsiString): u_long; overload; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_inet_addr(PAnsiChar(cp));
  {$ENDIF}
  {$IFDEF POSIX}
    Result := Posix.ArpaInet.inet_addr(PAnsiChar(cp));
  {$ENDIF}
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_inet_addr(const cp: UnicodeString): u_long; overload; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_htons(hostshort: u_short): u_short; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_htons(hostshort);
  {$ENDIF}
  {$IFDEF POSIX}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_htonl(hostlong: u_long): u_long; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_htonl(hostlong);
  {$ENDIF}
  {$IFDEF POSIX}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_getsockname(s: TSocket; var name: TSockAddr;  var namelen: Integer): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_getsockname(s, name, namelen);
  {$ENDIF}
  {$IFDEF POSIX}
  {$IFNDEF ANDROID}
    Result := Posix.SysSocket.getsockname(s, Posix.SysSocket.psockaddr(@name)^, Cardinal(namelen));
  {$ENDIF ANDROID}
  {$ENDIF POSIXF}
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_getpeername(s: TSocket; var name: TSockAddr;  var namelen : Integer): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_getpeername(s, name, namelen);
  {$ENDIF}
  {$IFDEF POSIX}
  {$IFNDEF ANDROID}
    Result := Posix.SysSocket.getpeername(s, Posix.SysSocket.psockaddr(@name)^, Cardinal(namelen));
  {$ENDIF ANDROID}
  {$ENDIF POSIXF}
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_connect(s: TSocket;  var name : TSockAddr; namelen: Integer): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_connect(s, name, namelen);
  {$ENDIF}
  {$IFDEF POSIX}
    Result := Posix.SysSocket.connect(s, Posix.SysSocket.psockaddr(@name)^, namelen);
  {$ENDIF}
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_closesocket(s: TSocket): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_closesocket(s);
  {$ENDIF}
  {$IFDEF POSIX}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_bind(s: TSocket;  var addr: TSockAddr;  namelen: Integer): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_bind(s, addr, namelen);
  {$ENDIF}
  {$IFDEF POSIX}
    Result := Posix.SysSocket.bind(s, Posix.SysSocket.psockaddr(@addr)^, namelen);
  {$ENDIF}
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_accept(s: TSocket; addr: PSockAddr;  addrlen: PInteger): TSocket; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_accept(s, addr, addrlen);
  {$ENDIF}
  {$IFDEF POSIX}
    Result := Posix.SysSocket.accept(s, Posix.SysSocket.psockaddr(addr)^, socklen_t(addrlen^));
  {$ENDIF}
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_recv(s: TSocket; var Buf: TWSocketData; len, flags: Integer): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_recv(s, Buf^, len, flags);
  {$ENDIF}
  {$IFDEF POSIX}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_recvfrom(s: TSocket;  var Buf: TWSocketData; len, flags: Integer;  var from: TSockAddr;
                                                                var fromlen: Integer): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    Result := OverbyteIcsWinsock.Ics_recvfrom(s, Buf^, len, flags, from, fromlen);
  {$ENDIF}
  {$IFDEF POSIX}
  {$IFNDEF ANDROID}
    Result := Posix.SysSocket.recvfrom(s, Buf^, len, flags, Posix.SysSocket.psockaddr(@from)^, Cardinal(fromlen));
  {$ENDIF ANDROID}
  {$ENDIF POSIXF}
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF MSWINDOWS}
function WSocket_Synchronized_GetAddrInfo(NodeName: PChar; ServName: PChar; Hints: PAddrInfo; var Addrinfo: PAddrInfo): Integer;
                                                                                              {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
    Result := OverbyteIcsWinsock.Ics_GetAddrInfo(NodeName, ServName, Hints, Addrinfo);
end;
{$ENDIF}
{$IFDEF POSIX}
function WSocket_Synchronized_GetAddrInfo(NodeName: PAnsiChar; ServName: PAnsiChar; Hints: PAddrInfo; var Addrinfo: PAddrInfo): Integer;
                                                                                                    {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
    Result := Posix.NetDB.getaddrinfo(NodeName, ServName, Hints^, Addrinfo);
end;
{$ENDIF}

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure WSocket_Synchronized_FreeAddrInfo(ai: PAddrInfo); {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
  {$IFDEF MSWINDOWS}
    OverbyteIcsWinsock.Ics_FreeAddrInfo(ai);
  {$ENDIF}
  {$IFDEF POSIX}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF MSWINDOWS}
function WSocket_Synchronized_GetNameInfo(addr: PSockAddr; namelen: Integer; host: PChar; hostlen : Cardinal; serv: PChar;
                                                  servlen: Cardinal; flags: Integer): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
    Result := OverbyteIcsWinsock.Ics_GetNameInfo(addr, namelen, host, hostlen, serv, servlen, flags);
end;
{$ENDIF}
{$IFDEF POSIX}
function WSocket_Synchronized_GetNameInfo(addr: PSockAddr; namelen: Integer; host: PAnsiChar; hostlen: Cardinal; serv: PAnsiChar;
                                                  servlen: Cardinal; flags: Integer): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
    Result := Posix.NetDB.getnameinfo(Posix.SysSocket.psockaddr(addr)^, namelen, host, hostlen, serv, servlen, flags);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF MSWINDOWS}
function WSocket_WSAAsyncGetHostByName(HWindow: HWND; wMsg: u_int; name, buf: PAnsiChar; buflen: Integer): THandle;
begin
{$IFNDEF NO_ADV_MT}
    SafeIncrementCount;
    try
{$ENDIF}
        Result := WSocket_Synchronized_WSAAsyncGetHostByName(HWindow, wMsg, name, buf, buflen);
{$IFNDEF NO_ADV_MT}
    finally
        SafeDecrementCount;
    end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_WSAAsyncGetHostByName(HWindow: HWND; wMsg: u_int; name, buf: UnicodeString; buflen: Integer): THandle; { V8.70 }
begin
    Result := WSocket_WSAAsyncGetHostByName(HWindow, wMsg, PAnsiChar(AnsiString(name)), PAnsiChar(AnsiString(buf)), buflen);
end;
...
{$IFNDEF NO_ADV_MT}
    ...
    try
{$ENDIF}
        Result := WSocket_Synchronized_WSAAsyncGetHostByAddr(HWindow, wMsg, addr, len, struct, buf, buflen);
{$IFNDEF NO_ADV_MT}
    finally
        SafeDecrementCount;
    end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_WSAAsyncGetHostByAddr(HWindow: HWND; wMsg: u_int; addr: UnicodeString; len, Struct: Integer;
                                                                                buf: UnicodeString; buflen: Integer): THandle;  { V8.70 }
begin
    Result := WSocket_WSAAsyncGetHostByAddr(HWindow, wMsg, PAnsiChar(AnsiString(addr)), len, Struct, PAnsiChar(AnsiString(buf)), buflen);
end;
...
{$IFNDEF NO_ADV_MT}
    ...
    try
{$ENDIF}
        Result := WSocket_Synchronized_WSAAsyncSelect(s, HWindow, wMsg, lEvent);
{$IFNDEF NO_ADV_MT}
    finally
        SafeDecrementCount;
    end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_WSAAsyncSelect(IEventSrc: IIcsEventSource; s: TSocket; HWindow: HWND; wMsg: u_int; lEvent: Integer): Integer;
begin
    Result := GAsyncSocketQueue.SynchronizedAsyncSelect(IEventSrc, s, HWindow, wMsg, LEvent);
end;
{$ENDIF POSIX}
...
begin
    Result := WSocket_getservbyname(PAnsiChar(AnsiString(name)), PAnsiChar(AnsiString(proto)));
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_setsockopt(s: TSocket; level, optname: Integer; optval: PAnsiChar; optlen: Integer): Integer;
begin
{$IFNDEF NO_ADV_MT}
    SafeIncrementCount;
    try
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_setsockopt(s: TSocket; level, optname: Integer; optval: UnicodeString; optlen: Integer): Integer;          { V8.70 }
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_setsockopt(s: TSocket; level, optname: Integer; var optval: TLinger; optlen: Integer): Integer;
begin
{$IFNDEF NO_ADV_MT}
    SafeIncrementCount;
    try
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_getsockopt(s: TSocket; level, optname: Integer; optval: PAnsiChar; var optlen: socklen_t): Integer;
begin
{$IFNDEF NO_ADV_MT}
    SafeIncrementCount;
    try
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_getsockopt(s: TSocket; level, optname: Integer; optval: UnicodeString;  var optlen: socklen_t): Integer;   { V8.70 }
begin
...
{$IFNDEF NO_ADV_MT}
    ...
    try
{$ENDIF}
        Result := WSocket_Synchronized_WSAIoctl(s, IoControlCode, InBuffer, InBufferSize, OutBuffer, OutBufferSize,
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF MSWINDOWS}
function WSocket_GetNameInfo(addr: PSockAddr; namelen: Integer; host: PChar; hostlen: Cardinal; serv: PChar; servlen: Cardinal;
...
constructor ESocketException.Create(  const AMessage: String; AErrorCode: Integer = 0; const AErrorMessage: String = '';  const
  AFriendlyMsg: String = '';  const AFunc: String = ''; const AIP: String = ''; const APort: String = ''; const AProto: String = '');
begin
    ...
    if FFriendlyMsg = '' then
      FFriendlyMsg := AMessage;
...
procedure TCustomWSocket.RaiseException(const Msg: String; AErrorCode: Integer; const AErrorMessage: String = ''; const
  AFriendlyMsg: String = '';  const AFunc: String = ''; const AIP: String = ''; const APort: String = ''; const AProto: String = '');
var
...
begin
    if Assigned(FOnError) then
        ...
        MyException := ESocketException.Create(MyMessage, AErrorCode, AErrorMessage, AFriendlyMsg, AFunc, AIP, APort, AProto);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TWSocketCounter.GetLastAliveTick : Int64;   { V8.71 Int64, assembler gone }
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure InitializeAddr(var AAddr: TSockAddrIn6; AIPVersion: TSocketFamily);{$IFDEF USE_INLINE} inline; {$ENDIF}
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function SizeOfAddr(const AAddr: TSockAddrIn6): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
begin
...
begin
    ...
    FLingerTimeout      := 0;
    SetHSocketValue(TSocket(INVALID_SOCKET));          { V10.0 }
{$IFDEF MSWINDOWS}
    FHSocketEvent       := INVALID_HANDLE_VALUE;       { V10.0 }
{$ENDIF}
    FSelectEvent        := 0;
    SetState(wsClosed);                                { V10.0 }
    bAllSent            := TRUE;
    FPaused             := FALSE;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TCustomWSocket.GetHSocket : TSocket;                        { V10.0 }
begin
    CritSecHSocket.Enter;
    try
        Result := FHSocketX;
    finally
        CritSecHSocket.Leave;
    end;
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.SetHSocket(Value : TSocket);                { V10.0 }
begin
    Dup(Value);
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.SetHSocketValue(Value : TSocket);           { V10.0 }
begin
    CritSecHSocket.Enter;
    try
        FHSocketX := Value;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TCustomWSocket.GetState : TSocketState;                     { V10.0 }
begin
    CritSecState.Enter;
    try
        Result := FStateX;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.SetState(Value : TSocketState);             { V10.0 }
begin
    CritSecState.Enter;
    try
        FStateX := Value;
...
begin
    ...
    if GetHSocket <> INVALID_SOCKET then
        WSocket_Synchronized_WSAASyncSelect(GetHSocket, Handle, FMsg_WM_ASYNCSELECT, FSelectEvent);
end;
...
begin
    if (IcsGetCurrentThreadID = FThreadID) and (GetHSocket <> INVALID_SOCKET) then
        WSocket_Synchronized_WSAASyncSelect(GetHSocket, Handle, 0, 0);
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loWsockInfo) then
        DebugLog(loWsockInfo, Format('TCustomWSocket.Create begin ThreadID=%d', [IcsGetCurrentThreadID]));
{$ENDIF}
    SetHSocketValue(INVALID_SOCKET);       { V10.0 }
{$IFDEF ICS_NewMessaging}   { V10 }
    FEventPipe.Clear;
{$ELSE}
    AllocateSocketHWnd;
{$ENDIF ICS_NewMessaging}   { V10 }
    ...
    FBufHandler.BufSize := 1460;              { Default buffer size }
...
{$IFDEF ICS_NewMessaging}   { V10 }
{$IFDEF MSWINDOWS}
    FEventPipe.Event    := TEvent.Create(nil, FALSE, FALSE, '');
{$ENDIF}
    if pipe(@FEventPipe) <> 0 then begin
    ...
    FCloseInternalLock := TMultiReadExclusiveWriteSynchronizer.Create;
    FThreadStarted := FALSE;
    GetThread;
    // TODO: Add a timeout to detect when thread doesn't start
    while not FThreadStarted do
        Sleep(0);

{$ELSE N ICS_NewMessaging}
    FConnectThreadId := 0; { V10.0 }
    EnterCriticalSection(GWSockCritSect);
    try
        Inc(WSocketGCount);
...
{$IFDEF ICS_NewMessaging}   { V10 }
        FThreadInternal.TerminateMe;
        while not FTerminated do
            Sleep(0);
{$IFDEF MSWINDOWS}
        FreeAndNil(FEventPipe.Event);
{$ENDIF}
        if FEventPipe.Read <> THANDLE(INVALID_FILE_HANDLE) then begin
            __close(FEventPipe.Read);
            FEventPipe.Read := THANDLE(INVALID_FILE_HANDLE);
        end;
        if FEventPipe.Write <> THANDLE(INVALID_FILE_HANDLE) then begin
            __close(FEventPipe.Write);
            FEventPipe.Write := THANDLE(INVALID_FILE_HANDLE);
        end;
        TIcsMessageManager.IcsDefaultManager.CleanPostQueue(Self);
        FreeAndNil(FThreadInternal);
        if HSocket <> INVALID_SOCKET then begin
            WSocket_closesocket(HSocket);
            SetHSocketValue(INVALID_SOCKET);
        end;
{$IFDEF MSWINDOWS}
        if FHSocketEvent <> WSA_INVALID_EVENT then begin
            Ics_WSACloseEvent(FHSocketEvent);
            FHSocketEvent := WSA_INVALID_EVENT;
        end;
{$ENDIF}
...
var
    iStatus : Integer;
    optlen  : socklen_t;   { V10.0 }
    tmp     : TSocket;
begin
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loWsockInfo) then
        DebugLog(loWsockInfo, Format('Dup of %d for HSocket %d', [NewHSocket, HSocket]));
{$ENDIF}
    ...
    if GetState <> wsClosed then begin
        Tmp := GetHSocket;
        SetHSocketValue(TSocket(INVALID_SOCKET));      { V10.0 }
        iStatus := WSocket_Synchronized_closesocket(Tmp);
    ...
    SetHSocketValue(NewHSocket);   { V10.0 }

    { Get winsock send buffer size }
    optlen  := SizeOf(FSocketSndBufSize);
    iStatus := WSocket_getsockopt(GetHSocket, SOL_SOCKET, SO_SNDBUF, PAnsiChar(@FSocketSndBufSize), optlen);
    ...
    iStatus := WSocket_getsockopt(GetHSocket, SOL_SOCKET, SO_RCVBUF, PAnsiChar(@FSocketRcvBufSize), optlen);
...
{$IFDEF ICS_NewMessaging}   { V10 }
{$IFDEF MSWINDOWS}
    FHSocketEvent := Ics_WSACreateEvent;
    if FHSocketEvent = WSA_INVALID_EVENT then begin
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loWsockInfo) then
        DebugLog(loWsockInfo, Format('HSocketEvent %d for HSocket %d  created', [FHSocketEvent, HSocket]));
{$ENDIF}

    iStatus := Ics_WSAEventSelect(GetHSocket, FHSocketEvent, FSelectEvent);
    if iStatus <> 0 then begin
        SocketError('WSAEventSelect');
        exit;
    end;
    AddSocketHandleToWaitList(FHSocketEvent);
{$ELSE}
    AddSocketHandleToWaitList(HSocket);
{$ENDIF}{$ENDIF ICS_NewMessaging}   { V10 }

{$IFNDEF ICS_NewMessaging}   { V10 }
    iStatus := WSocket_Synchronized_WSAASyncSelect(GetHSocket, Handle, FMsg_WM_ASYNCSELECT, FSelectEvent);
    if iStatus <> 0 then begin
...
begin
    ...
    if GetHSocket = INVALID_SOCKET then begin   { V10.0 }
        Result := 0;
        Exit;
    end;
    if WSocket_Synchronized_ioctlsocket(GetHSocket, FIONREAD, Temp) = SOCKET_ERROR then begin
        Result := -1;
...
begin
    CritSecState.Enter;           { V9.4 protect change during trigger }
    try
        if FStateX = NewState then
            Exit;
        OldState := FStateX;
        FStateX := NewState;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{ DoRecv is a simple wrapper around winsock recv function to make it a virtual function. }
function TCustomWSocket.DoRecv(var Buffer: TWSocketData; BufferSize: Integer; Flags: Integer): Integer;
begin
 { V9.4 When socket is closed, there can still be data in the buffer }
    if (GetState = wsClosed) or (GetHSocket = INVALID_SOCKET) then begin
        Result := 0;
        Exit;
    end;
    Result := WSocket_Synchronized_recv(GetHSocket, Buffer, BufferSize, Flags);
...
{$IFDEF ICS_NewMessaging}   { V10 }
        ...
        if HSocket <> INVALID_SOCKET then begin
            WSocket_Synchronized_ioctlsocket(HSocket, FIONREAD, Count);
            if Count > 0 then
                TCustomWSocket.PostMessage(Self, WM_ICS_WS_TRGR_DATA_AVAILABLE, 0, 0);
        end;
    end;
{$ENDIF ICS_NewMessaging}   { V10 }
        FLastError := WSocket_Synchronized_WSAGetLastError;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TCustomWSocket.DoRecvFrom(ASocket: TSocket; var Buffer: TWSocketData; BufferSize: Integer; Flags: Integer;
                                                                           var From: TSockAddr; var FromLen: Integer): Integer;
begin
    Result := WSocket_Synchronized_recvfrom(ASocket, Buffer, BufferSize, Flags, From, FromLen);
    FRcvdFlag := (Result >= BufferSize);
    if Result > 0 then begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TCustomWSocket.ReceiveFrom(Buffer: TWSocketData; BufferSize: Integer; var From: TSockAddr; var FromLen: Integer): Integer;
begin
    Result := DoRecvFrom(GetHSocket, Buffer, BufferSize, 0, From, FromLen);
    if Result < 0 then
        FLastError := WSocket_Synchronized_WSAGetLastError;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TCustomWSocket.ReceiveFrom6(Buffer: TWSocketData; BufferSize: Integer; var From: TSockAddrIn6; var FromLen: Integer): Integer;
begin
    Result := DoRecvFrom(GetHSocket, Buffer, BufferSize, 0, PSockAddrIn(@From)^, FromLen);
    if Result < 0 then
        FLastError := WSocket_Synchronized_WSAGetLastError;
end;
...
function TCustomWSocket.ReceiveFromTB(var Data: TBytes; var From: TSockAddr; var FromLen: Integer; MaxLen: Integer = -1): Integer;  { V8.70 }
var
    Buffer: TWSocketData;
begin
    Result := MaxLen;
    ...
    SetLength(Data, Result);
    Buffer := @Data[0];
    Result := DoRecvFrom(GetHSocket, Buffer, Result, 0, From, FromLen);
    if Result < 0 then begin
        FLastError := WSocket_Synchronized_WSAGetLastError;
...
function TCustomWSocket.ReceiveFrom6TB(var Data: TBytes; var From: TSockAddrIn6; var FromLen: Integer; MaxLen: Integer = -1): Integer;   { V8.70 }
var
    Buffer: TWSocketData;
begin
    Result := MaxLen;
    ...
    SetLength(Data, Result);
    Buffer := @Data[0];
    Result := DoRecvFrom(GetHSocket, Buffer, Result, 0, PSockAddrIn(@From)^, FromLen);
    if Result < 0 then begin
        FLastError := WSocket_Synchronized_WSAGetLastError;
        SetLength(Data, 0)                  { V8.71 fix range check error }
    end
...
function TCustomWSocket.SendTo(Dest: TSockAddr; DestLen: Integer; Data: TWSocketData;  Len: Integer) : Integer;
begin
    Result := WSocket_Synchronized_SendTo(GetHSocket, Data, Len, FSendFlags, TSockAddr(Dest), DestLen);
    if Result > 0 then begin
        FWriteCount := FWriteCount + Result;
...
{$IFNDEF ICS_NewMessaging}   { V10 }
        ...
        if bAllSent and (FType = SOCK_DGRAM) then
            PostMessage(Handle, FMsg_WM_ASYNCSELECT, WParam(GetHSocket), IcsMakeLong(FD_WRITE, 0));
{$ENDIF N ICS_NewMessaging}   { V10 }
    end
    else begin
...
function TCustomWSocket.SendTo6(Dest: TSockAddrIn6; DestLen: Integer; Data: TWSocketData; Len: Integer) : Integer;
begin
    Result := WSocket_Synchronized_SendTo(GetHSocket, Data, Len, FSendFlags, PSockAddrIn(@Dest)^, DestLen);
    if Result > 0 then begin
...
function TCustomWSocket.SendToTB(Dest: TSockAddr; DestLen: Integer; const Data: TBytes; Len: Integer = -1) : Integer;    { V8.69 }
var
    ActualLen: Integer;
begin
...
function TCustomWSocket.SendToTB6(Dest: TSockAddrIn6; DestLen: Integer; const Data: TBytes; Len: Integer = -1) : Integer; { V8.69 }
var
    ActualLen: Integer;
begin
    ActualLen := Length(Data);
...
begin
    if FType = SOCK_DGRAM then
        Result := WSocket_Synchronized_SendTo(GetHSocket, Data, Len, FSendFlags, PSockAddr(@Fsin)^, SizeOfAddr(Fsin))
    else
        Result := WSocket_Synchronized_Send(GetHSocket, Data, Len, FSendFlags);
    if Result > 0 then begin
        if Assigned(FCounter) then
            FCounter.FLastSendTick := IcsGetTickCount64;    { V8.71 }
{$IFDEF ICS_NewMessaging}   { V10 }
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loWsockDump) then
        DebugLog(loWsockInfo, 'PostMessage WM_ICS_WS_SEND_DATA');
{$ENDIF}
        TCustomWSocket.PostMessage(Self, WM_ICS_WS_SEND_DATA, Result, 0);
{$ELSE}
        TriggerSendData(Result);
{$ENDIF ICS_NewMessaging}   { V10 }
    end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
// TryToSend is called in the context of the worker thread for ICS_NewMessaging
...
begin
    FBufHandler.Lock;
    try
        if (GetHSocket = INVALID_SOCKET) or (FBufHandler.IsEmpty) then begin
            bAllSent := TRUE;
            Exit;
        end;
        ...
        while TRUE do begin
            ...
            if Count = SOCKET_ERROR then begin
                ...
                if (LastError = WSAECONNRESET) or (LastError = WSAENOTSOCK) or (LastError = WSAENOTCONN)  or
                          (LastError = WSAEINVAL) or (LastError = WSAECONNABORTED) or (LastError = WSAESHUTDOWN) then begin
                  {$IFNDEF NO_DEBUG_LOG}
                    if CheckLogOptions(loWsockErr) then
                        DebugLog(loWsockErr, Name +' Winsock Send failed - ' + GetWinsockErr(LastError));
                  {$ENDIF}
{$IFDEF ICS_NewMessaging}   { V10 }
                    CloseSocketInternal(0);
{$ELSE}
                    FCloseInvoked := TRUE;
                    Close;
                    TriggerSessionClosed(LastError);
{$ENDIF ICS_NewMessaging}   { V10 }
                end
                else if LastError = WSAEBADF then begin    { V10.0 }
...
begin
    if (GetState <> wsConnected) and (GetState <> wsSocksConnected) then begin
...
{$IFDEF ICS_NewMessaging}   { V10 }
    PipeCommand(PIPE_CMD_TRY_TO_SEND);
{$ELSE}
    TryToSend;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure GetAddrList(AddrInfo: PAddrInfo; SocketFamily: TSocketFamily; ToList: TStrings);     { V10.0 }
var
...
begin
    ...
    Idx := 0;
    while AddrInfoNext <> nil do begin
        if AddrInfoNext.ai_family = AF_INET then begin
            if SocketFamily = sfAnyIPv4 then begin
                ToList.Insert(IDX,
{$IFDEF MSWINDOWS}
...
{$ENDIF}
        ...
        else if AddrInfoNext.ai_family = AF_INET6 then begin
            if SocketFamily = sfAnyIPv6 then begin
                ToList.Insert(IDX, WSocketIPv6ToStr(PSockAddrIn6(AddrInfoNext.ai_addr)));
                Inc(IDX);
            end
            else
                ToList.Add(WSocketIPv6ToStr(PSockAddrIn6(AddrInfoNext.ai_addr)));
        end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFNDEF ICS_NewMessaging}   { V10 }
procedure TCustomWSocket.ASyncReceive(Error: Word; MySocketOptions: TWSocketOptions);
var
    bMore        : Boolean;
    lCount       : u_long;
...
begin
    ...
    while bMore do begin
        ...
        try
            ...
            else if WSocket_Synchronized_ioctlsocket(GetHSocket, FIONREAD, lCount) = SOCKET_ERROR then begin
                FLastError := WSocket_Synchronized_WSAGetLastError;
                bMore      := FALSE;
            end
...
begin
    if FConnectThreadId <> IcsGetCurrentThreadId() then begin  { V10.0 }
      while GetState = wsOpened do begin
        Sleep(5);
      end;
    end;

    if GetState <> wsConnected then begin
        if (IcsHiWord(msg.LParam) = 0) then { V9.1 don't set connected if it failed }
            ChangeState(wsConnected);
        TriggerSessionConnectedSpecial(IcsHiWord(msg.LParam));
        if (IcsHiWord(msg.LParam) <> 0) and (GetState <> wsClosed) then
            Close;
    end;
end;
...
begin
    ...
    if (GetState <> wsConnecting) and (GetHSocket <> INVALID_SOCKET) then begin
        ...
        if (GetState <> wsListening) then // We should not receive with listening sockets {AG}
...
{TriggerDisplay('AsyncSelect ' + IntToStr(msg.wParam) + ', ' + IntToStr(msg.LParamLo));}
    ...
    if msg.wParam <> WPARAM(GetHSocket) then
        Exit;
...
begin
    Result := 'error';
    if GetState in [wsConnected, wsBound, wsListening] then begin
        saddrlen := sizeof(saddr);
        if WSocket_Synchronized_GetSockName(GetHSocket, PSockAddr(@saddr)^, saddrlen) = 0 then begin
...
begin
    ...
    if GetState in [wsConnected, wsBound, wsListening] then begin
        saddrlen := sizeof(saddr);
        if WSocket_Synchronized_GetSockName(GetHSocket, PSockAddr(@saddr)^, saddrlen) = 0 then
        begin
...
begin
    ...
    { If the address is either a valid IPv4 or IPv6 address change current SocketFamily.  }
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.SetMinMaxIcsAsyncDnsLookupThreads(AMinThreads: Byte; AMaxThreads: Byte);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TCustomWSocket.IcsAsyncGetHostByName(AWnd: HWND; AMsgID: UINT; const ASocketFamily: TSocketFamily; const AName: String;
                                                                                                 const AProtocol: Integer): THandle;
begin
    if FAsyncLookupPtr = nil then
        RegisterIcsAsyncDnsLookup;
    Result := TIcsAsyncDnsLookup(FAsyncLookupPtr).ExecAsync(AWnd, AMsgID, ASocketFamily, AName, FALSE, AProtocol);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TCustomWSocket.IcsAsyncGetHostByAddr(AWnd: HWND; AMsgID: UINT; const ASocketFamily: TSocketFamily; const AAddr: String;
                                                                                                const AProtocol: Integer): THandle;
begin
    if FAsyncLookupPtr = nil then
        RegisterIcsAsyncDnsLookup;
    Result := TIcsAsyncDnsLookup(FAsyncLookupPtr).ExecAsync(AWnd, AMsgID, ASocketFamily, AAddr, TRUE, AProtocol);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TCustomWSocket.IcsCancelAsyncRequest(const ARequest: THandle): Integer;
begin
    if FAsyncLookupPtr <> nil then
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure WSocket_Synchronized_ResolveHost(const AHostName: string; var ASockAddrIn6: TSockAddrIn6; const AFamily: TSocketFamily;
                                                                                                const AProtocol: Integer); overload;
var
    Hints     : TAddrInfo;
    AddrInfo  : PAddrInfo;
...
begin
    ...
    begin
        ...
        begin
            ...
            begin
                PSockAddrIn(@ASockAddrIn6)^.sin_addr.S_addr := INADDR_BROADCAST;   { V10.0 }
                Exit;
            end;
        ...
        Exit;
          ...
          {$IFDEF POSIX}
            RetVal   := WSocket_Synchronized_GetAddrInfo(PAnsiChar(UnicodeToAnsi(AHostName, CP_UTF8)), nil, @Hints, AddrInfo);
          {$ELSE}
            RetVal   := WSocket_Synchronized_GetAddrInfo(PChar(AHostName), nil, @Hints, AddrInfo);
          {$ENDIF}
            if RetVal <> 0 then
                raise ESocketException.Create('Winsock Resolve Host: Cannot convert host address ''' + AHostName + ''' - ' + GetWinsockErr(RetVal));
            try
                PInfo:= nil;
                NextInfo := AddrInfo;
                ...
                begin
                    ...
                    raise ESocketException.Create('Winsock Resolve Host: Cannot convert host address ''' + AHostName + '''');
            finally
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function WSocket_Synchronized_ResolveName(const AName: string; const AReverse: Boolean; const AFamily: TSocketFamily;
                                                                        AResultList: TStrings; const AProtocol: Integer): Integer;
var
    Hints     : TAddrInfo;
    AddrInfo  : PAddrInfo;
        ...
        begin
            ...
            begin
                ...
                begin
                  ...
                  {$IFDEF MSWINDOWS}
                    RetVal := Ics_GetNameInfo(NextInfo^.ai_addr, NextInfo^.ai_addrlen, PChar(LHost), NI_MAXHOST, nil, 0, 0);
                  {$ELSE}
                    RetVal := GetNameInfo(NextInfo^.ai_addr^, NextInfo^.ai_addrlen, PAnsiChar(LHost), NI_MAXHOST, nil, 0, 0);
                  {$ENDIF}
                    if RetVal = 0 then
                    begin
                 { V8.64 if result has ACE xn--. convert it to Unicode, ignore errors }
                    ...
                    begin
                        ...
                        begin
                            ...
                            AResultList.Add(WSocketIPv4ToStr(PSockAddrIn(NextInfo.ai_addr)^.sin_addr.S_addr));
                    end
                    else begin
...
begin
    ...
    if WSocketIsDottedIP(InAddr) then begin
        ...
        if IPAddr = u_long(INADDR_NONE) then begin
            ...
            raise ESocketException.Create('Winsock Resolve Host: ''' + String(InAddr) + ''' Invalid IP address.');   { V5.26 }
        end;
    ...
    if Phe = nil then
        raise ESocketException.Create('Winsock Resolve Host: Cannot convert host address ''' +  String(InAddr) + ''' - ' +
...
begin
    ...
    if IsDigit(Port[1]) then
        ...
        if Pse = nil then
            raise ESocketException.Create( 'Winsock Resolve Port: Cannot convert port ''' + String(Port) + ''' - ' +
...
begin
    ...
    if IsDigit(sProto[1]) then
        ...
        if sProto = 'tcp' then
            ...
            if Ppe = nil then
                raise ESocketException.Create('Winsock Resolve Proto: Cannot convert protocol ''' + String(sProto) + ''' - ' +
                                                                   GetWinsockErr(WSocket_Synchronized_WSAGetLastError));    { V5.26 }
            Result := ppe^.p_proto;
        end;
    end;
...
begin
    Result := WSocket_Synchronized_GetSockName(GetHSocket, saddr, saddrlen);
end;
...
begin
    ...
    if GetState = wsConnected then begin
        saddrlen := sizeof(saddr);
        if WSocket_Synchronized_GetPeerName(GetHSocket,  PSockAddrIn(@saddr)^, saddrlen) = 0 then
        begin
            Result := WSocketSockAddrToStr(saddr);    { V8.71 }
        end
        else begin
...
begin
    if GetState = wsConnected then
        Result := WSocket_Synchronized_GetPeerName(GetHSocket, Name, NameLen)
    else
        Result := SOCKET_ERROR;
end;
...
{$IFDEF ICS_NewMessaging}   { V10 }
  ...
  {$ELSE}
    //    RetVal := IcsCancelAsyncRequest(FDnsLookupHandle);   // !!! pending
       RetVal := 0;
  {$ENDIF}
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.DnsLookup(const AHostName: String; const AProtocol: Integer);     // Asynchronous
var
    IPAddr   : TInAddr;
...
{$IFDEF ICS_NewMessaging}   { V10 }
    ...
    {$ELSE}
   //     IcsCancelAsyncRequest(FDnsLookupHandle);     // !!! pending
    {$ENDIF}
{$ELSE}
        if (FSocketFamily = sfIPv4) and (not HasOption(FComponentOptions, wsoIcsDnsLookup)) then   { V8.43, V8.70 }
            WSocket_Synchronized_WSACancelAsyncRequest(FDnsLookupHandle)
...
{$IFDEF ICS_NewMessaging}   { V10 }
    PipeCommand([AnsiChar(PIPE_CMD_GETHOSTBYNAME), AProtocol, FPunycodeHost]);
{$ELSE}
   ...
   { V8.43 new option for IPv4 DNS lookups to be done using thread, previously
    ...
    if (FSocketFamily = sfIPv4) and (not HasOption(FComponentOptions, wsoIcsDnsLookup)) then     { V8.70 }
        FDnsLookupHandle := WSocket_Synchronized_WSAAsyncGetHostByName(FWindowHandle, FMsg_WM_ASYNCGETHOSTBYNAME,
                                              PAnsiChar(AnsiString(FPunycodeHost)), @FDnsLookupBuffer, SizeOf(FDnsLookupBuffer))
    else
    if FDnsLookupHandle = 0 then begin
        Err := WSocket_Synchronized_WSAGetLastError;
        RaiseException(FPunycodeHost + ': can''t start DNS lookup - ' + GetWinsockErr(Err), Err);  { V5.26, V8.36 }
        TriggerDnsLookupDone(WSAEINVAL);   { V8.64 }
        Exit;
    end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.ReverseDnsLookup(const HostAddr: String; const AProtocol: Integer);     // Asynchonous
{$IFNDEF ICS_NewMessaging}   { V10 }
var
    lAddr  : u_long;
{$ENDIF N ICS_NewMessaging}   { V10 }
...
{$IFDEF ICS_NewMessaging}   { V10 }
    PipeCommand([AnsiChar(PIPE_CMD_GETHOSTBYADDR), AProtocol, HostAddr]);
{$ELSE}
    { Cancel any pending lookup }
    if FDnsLookupHandle <> 0 then begin
 ...
 { V8.44 new option for IPv4 DNS lookups to be done using thread, previously
     only IPv6 used thread.  This avoids windows limitation of one lookup at a time }
    if (FSocketFamily = sfIPv4) and (not HasOption(FComponentOptions, wsoIcsDnsLookup)) then    { V8.70 }
        FDnsLookupHandle := WSocket_Synchronized_WSAAsyncGetHostByAddr(FWindowHandle, FMsg_WM_ASYNCGETHOSTBYADDR,
                                                  PAnsiChar(@lAddr), 4, PF_INET, @FDnsLookupBuffer, SizeOf(FDnsLookupBuffer))
    else
        FDnsLookupHandle := IcsAsyncGetHostByAddr(FWindowHandle, FMsg_WM_ASYNCGETHOSTBYADDR, FSocketFamily,
                                                                                                IcsTrim(HostAddr), AProtocol);
    if FDnsLookupHandle = 0 then
        RaiseException(HostAddr + ': can''t start reverse DNS lookup - ' + GetWinsockErr(WSocket_Synchronized_WSAGetLastError));  { V5.26 }
{$ENDIF ICS_NewMessaging}   { V10 }
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.ReverseDnsLookupSync(const HostAddr: String; const AProtocol: Integer); {AG 03/03/06}
{$IFNDEF ICS_NewMessaging}   { V10 }
var
...
{$IFDEF ICS_NewMessaging}   { V10 }
    FDnsResult := String(WSocketResolveIp(AnsiString(HostAddr), FSocketFamily, AProtocol));
    if FDnsResult = '' then
        TriggerDnsLookupDone(WSAHOST_NOT_FOUND)
    else
        TriggerDnsLookupDone(0);
{$ELSE}
    { Cancel any pending lookup }
    ...
    if FSocketFamily = sfIPv4 then
    begin
        if (Length(HostAddr) >= SizeOf(szAddr)) then begin
            try
                RaiseException('Reverse DNS Lookup: Invalid host name.');
            finally   { V8.36 }
                TriggerDnsLookupDone(WSAEINVAL);
            end;
            Exit;
        ...
        lAddr := WSocket_Synchronized_ResolveName(HostAddr, TRUE, FSocketFamily, FDnsResultList, AProtocol);
        if lAddr <> 0 then
...
begin
    ...
    if WSocket_Synchronized_bind(GetHSocket, PSockAddrIn(@LocalSockName)^, SockNamelen) <> 0 then begin
        ...
        if (ErrorCode = WSAEADDRINUSE) or (ErrorCode = WSAEACCES)  then   { V8.36 more friendly message for common error }
            FriendlyMsg := 'Another client is using address ' +  LLocalAddr + ':' + IntToStr(FLocalPortNum) ;
        RaiseException('Bind socket failed - ' + GetWinsockErr(ErrorCode), ErrorCode, '', FriendlyMsg, 'Bind Socket',
                                                                              LLocalAddr, IntToStr(FLocalPortNum), FProtoStr);  { V8.36 }
        Exit;
    end;
    if WSocket_Synchronized_GetSockName(GetHSocket, PSockAddrIn(@SockName)^, SockNamelen) <> 0 then begin
        RaiseException('Winsock get socket name failed - ' + GetWinsockErr(WSocket_Synchronized_WSAGetLastError));  { V5.26 }
        Exit;
    end;
...
begin
    if FKeepAliveOnOff = wsKeepAliveOff then
        Exit;
    Assert(GetHSocket <> INVALID_SOCKET);
    if FKeepAliveOnOff = wsKeepAliveOnSystem then begin
        OptVal := 1;
        Status := WSocket_Synchronized_setsockopt(GetHSocket, SOL_SOCKET, SO_KEEPALIVE, @OptVal, SizeOf(OptVal));
  ...
  {$IFDEF MSWINDOWS}
    ...
    Status := WSocket_WSAIoctl(GetHSocket, SIO_KEEPALIVE_VALS, @KeepAliveIn,  SizeOf(KeepAliveIn), @KeepAliveOut,
                                                                                  SizeOf(KeepAliveOut), BytesReturned, nil, nil);
...
begin
    ...
    if GetHSocket = INVALID_SOCKET then begin
    ...
    iStatus := WSocket_Synchronized_setsockopt(GetHSocket, SOL_SOCKET, SO_LINGER, @li, SizeOf(li));
...
begin
    ...
    if HasOption(FComponentOptions, wsoTcpNoDelay) then
        ...
        optval := 0; { false }
    Result := WSocket_Synchronized_setsockopt(GetHSocket, IPPROTO_TCP, TCP_NODELAY, @optval, SizeOf(optval)) = 0;
    if not Result then
...
begin
    if GetHSocket <> INVALID_SOCKET then begin
        if FSelectEvent and FD_ADDRESS_LIST_CHANGE = 0 then begin
            ...
            Result := WSocket_Synchronized_WSAASyncSelect(GetHSocket, Handle, FMsg_WM_ASYNCSELECT, FSelectEvent) <> SOCKET_ERROR;
        end
        else
            Result := True;
        if Result then
            Result := (WSocket_WSAIoctl(GetHSocket, SIO_ADDRESS_LIST_CHANGE, nil, 0, nil, 0, LBytesRcvd, nil, nil) <>
                                                       SOCKET_ERROR) or (WSocket_Synchronized_WSAGetLastError = WSAEWOULDBLOCK);
    end
    else
        Result := True;
end;
...
begin
    if GetHSocket <> INVALID_SOCKET then begin
        if FSelectEvent and FD_ROUTING_INTERFACE_CHANGE = 0 then begin
            ...
            Result := True;
        if Result then
            Result := (WSocket_WSAIoctl(GetHSocket, SIO_ROUTING_INTERFACE_CHANGE, @Fsin, SizeOfAddr(Fsin), nil, 0, LBytesRcvd,
...
{$ENDIF N ICS_NewMessaging}   { V10 }
    ...
    ErrFlag  : boolean;  { V8.64 }
begin
    FConnectThreadId := IcsGetCurrentThreadId(); { V10.0 }
    FLastError := 0;         { V8.68 reset to avoid problems later }
    FLastErrorMsg := '';     { V10.0 }
    if ((GetHSocket <> INVALID_SOCKET) and (NOT (GetState in [wsClosed, wsDnsLookup]))) or FInternalDnsActive then begin    { V8.48 }
...
{$ELSE}
    ...
    { Open the socket }
    SetHSocketValue(WSocket_Synchronized_socket(FAddrFormat, FType, FProto));
    if GetHSocket = INVALID_SOCKET then begin
  ...
  {$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loWsockInfo) then
        DebugLog(loWsockInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' ' + 'Socket handle created handle=' + IntToStr(GetHSocket));
  {$ENDIF}
   ...
   { Get winsock send buffer size }
    ...
    if GetState <> wsOpened then begin  { 07/07/02 }
    ...
    if FType = SOCK_DGRAM then begin
        ...
        if FMultiCast then begin
            if FMultiCastIpTTL <> IP_DEFAULT_MULTICAST_TTL then begin
                ...
                if FAddrFormat = PF_INET then
                    iStatus := WSocket_Synchronized_SetSockOpt(GetHSocket, IPPROTO_IP, IP_MULTICAST_TTL, @optval, SizeOf(optval))
                else if FAddrFormat = PF_INET6 then
                    iStatus := WSocket_Synchronized_SetSockOpt(GetHSocket, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, @optval, SizeOf(optval))
                else begin
            ...
            if ((FAddrFormat = AF_INET) and (FLocalAddr <> ICS_ANY_HOST_V4)) or
                ...
                if FAddrFormat = PF_INET then
                    iStatus := WSocket_Synchronized_SetSockOpt(GetHSocket, IPPROTO_IP,  IP_MULTICAST_IF,
                                              @PSockAddr(@laddr)^.sin_addr.S_addr, SizeOf(PSockAddr(@laddr)^.sin_addr.S_addr))
                else if FAddrFormat = PF_INET6 then
                begin
                    OptVal := 0; // Default IPv6 interface, fix me!
                    iStatus := WSocket_Synchronized_SetSockOpt(GetHSocket, IPPROTO_IPV6, IPV6_MULTICAST_IF,
        ...
        begin
            OptVal  := 1;
            iStatus := WSocket_Synchronized_setsockopt(GetHSocket, SOL_SOCKET, SO_BROADCAST, @OptVal, SizeOf(OptVal));
            if iStatus <> 0 then begin
        ...
        optval  := -1;
        iStatus := WSocket_Synchronized_setsockopt(GetHSocket, SOL_SOCKET, SO_REUSEADDR, @optval, SizeOf(optval));
  ...
  {$ENDIF}
    ...
    iStatus := WSocket_Synchronized_WSAASyncSelect({$IFDEF POSIX}Self,{$ENDIF} GetHSocket, Handle, FMsg_WM_ASYNCSELECT, FSelectEvent);
    if iStatus <> 0 then begin
        SocketError('WSAAsyncSelect');
        Exit;
    end;
  ...
  {$ENDIF}
    ...
    if FType = SOCK_DGRAM then begin
      ...
      {$ENDIF}
        ...
        iStatus := WSocket_Synchronized_Connect(GetHSocket, PSockAddrIn(@Fsin)^, SizeOfAddr(Fsin));
        if iStatus = 0 then begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
procedure TCustomWSocket.CommonConnect;             { V10.0 }
var
    iStatus: Integer;
    optval: Integer;
    lAddr: TSockAddrIn6;
begin
    SetHSocketValue(WSocket_Synchronized_socket(FAddrFormat, FType{$IFDEF LINUX} or SOCK_NONBLOCK{$ENDIF}, FProto));
    if GetHSocket = INVALID_SOCKET then begin
        ChangeState(wsClosed);
        FLastError := WSocket_Synchronized_WSAGetLastError;
        FLastErrorMsg := 'socket failed';
        Exit;
    end;
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loWsockInfo) then
        DebugLog(loWsockInfo, Format('CommonConnect, HSocket %d created', [GetHSocket]));
{$ENDIF}
{$IFDEF MSWINDOWS}
    optval := 1;
    WSocket_Synchronized_ioctlsocket(HSocket, FIONBIO, u_long(optval));
{$ENDIF}
    ChangeState(wsOpened);

    if FType = Integer(SOCK_DGRAM) then begin
        BindSocket;
        if FMultiCast then begin
            if FMultiCastIpTTL <> IP_DEFAULT_MULTICAST_TTL then begin
                optval  := FMultiCastIpTTL; { set time-to-live for multicast }
            ...
            if ((FAddrFormat = AF_INET) and (FLocalAddr <> ICS_ANY_HOST_V4)) or ((FAddrFormat = AF_INET6) and
                                                                                     (FLocalAddr6 <> ICS_ANY_HOST_V6)) then begin
                if FAddrFormat = PF_INET then
                    PSockAddrIn(@laddr)^.sin_addr.S_addr := WSocket_Synchronized_ResolveHost(AnsiString(FLocalAddr)).S_addr
                else
                    WSocket_Synchronized_ResolveHost(FLocalAddr6, laddr, sfIPv6, FProto);

                if FAddrFormat = PF_INET then
                    iStatus := WSocket_Synchronized_SetSockOpt(GetHSocket, IPPROTO_IP, IP_MULTICAST_IF,
                                            @PSockAddrIn(@laddr)^.sin_addr.S_addr, SizeOf(PSockAddrIn(@laddr)^.sin_addr.S_addr))
                else if FAddrFormat = PF_INET6 then
                begin
                    OptVal := 0; // Default IPv6 interface, fix me!
                    iStatus := WSocket_Synchronized_SetSockOpt(GetHSocket, IPPROTO_IPV6, IPV6_MULTICAST_IF,
        ...
        if ((Fsin.sin6_family = AF_INET) and (u_long(PSockAddrIn(@FSin).sin_addr.S_addr) = u_long(INADDR_BROADCAST))) or
           ((Fsin.sin6_family = AF_INET6) and IN6_IS_ADDR_MULTICAST({$IFNDEF POSIX}@{$ENDIF}Fsin.sin6_addr)) then
        begin
            OptVal  := 1;
            iStatus := WSocket_Synchronized_setsockopt(GetHSocket, SOL_SOCKET, SO_BROADCAST, PAnsiChar(@OptVal), SizeOf(OptVal));
            if iStatus <> 0 then begin
                SocketError('setsockopt(SO_BROADCAST)');
                Exit;
            end;
        ...
        if (FLocalPortNum <> 0) or
           ((FAddrFormat = AF_INET)  and (FLocalAddr <> ICS_ANY_HOST_V4)) or ((FAddrFormat = AF_INET6) and
                                                                                         (FLocalAddr6 <> ICS_ANY_HOST_V6)) then
            BindSocket;

//        optval  := -1;
//        iStatus := Ics_setsockopt(GetHSocket, SOL_SOCKET, TCP_NODELAY, @optval, SizeOf(optval));

    end;

{$IFDEF MSWINDOWS}
    FSelectEvent := FD_CONNECT or FD_READ or FD_WRITE or FD_CLOSE;
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loWsockInfo) then
        DebugLog(loWsockInfo, Format('CommonConnect, HSocketEvent %d for HSocket %d  created', [FHSocketEvent, GetHSocket]));
{$ENDIF}
...
{$ENDIF}
    ...
    if FType = Integer(SOCK_DGRAM) then begin
        ...
        ChangeState(wsConnecting);
        iStatus := WSocket_Synchronized_Connect(GetHSocket, PSockAddr(@Fsin)^, sizeof(Fsin));
        if iStatus = 0 then begin
            ChangeState(wsConnecting);
        end
        else begin
...
begin
  ...
  { Remove any data from the internal output buffer (should already be empty !) }
    DeleteBufferedData;
    SetHSocketValue(WSocket_Synchronized_socket(FAddrFormat, FType, FProto));
    if GetHSocket = INVALID_SOCKET then begin
        SocketError('socket');
...
{$IFDEF MSWINDOWS}
    ...
    { V8.42 this is windows specific }
        optval  := -1;
        iStatus := WSocket_Synchronized_SetSockOpt(GetHSocket, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, @optval, SizeOf(optval));
        if iStatus <> 0 then begin
...
{$ENDIF}
    ...
    iStatus := WSocket_Synchronized_bind(GetHSocket, PSockAddr(@Fsin)^, SizeOfAddr(Fsin));
    if iStatus = 0 then
        ChangeState(wsBound)
    else begin
        try
            ...
            WSocket_Synchronized_closesocket(GetHSocket);
            SetHSocketValue(INVALID_SOCKET);
        end;
        Exit;
    end;
    ...
    SOCK_RAW :
        begin
            if HasOption(FComponentOptions, wsoSIO_RCVALL) then begin
                ...
                dwBufferOutLen := 0;
                iStatus := WSocket_Synchronized_WSAIoctl(GetHSocket, SIO_RCVALL, @dwBufferInLen,  SizeOf(dwBufferInLen),
...
{$ENDIF}
        ...
        begin
            if FMultiCast then begin
                if FAddrFormat = AF_INET then begin
                    ...
                    iStatus := WSocket_Synchronized_SetSockOpt(GetHSocket, IPPROTO_IP, IP_ADD_MEMBERSHIP, @mreq, SizeOf(mreq));
                    ...
                    iStatus := WSocket_Synchronized_SetSockOpt(GetHSocket, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, PAnsiChar(@mreqv6), SizeOf(mreqv6));
    ...
    SOCK_STREAM : { TCP }
        begin
            iStatus := WSocket_Synchronized_Listen(GetHSocket, FListenBacklog);
            if iStatus = 0 then
                ChangeState(wsListening)
            else begin
                SocketError('Listen');
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loWsockInfo) then
        DebugLog(loWsockInfo, Format('Listen, HSocketEvent %d for HSocket %d  created', [FHSocketEvent, GetHSocket]));
{$ENDIF}
    ...
    AddSocketHandleToWaitList(FHSocketEvent);
 {$ELSE}
    AddSocketHandleToWaitList(GetHSocket);
 {$ENDIF}
...
begin
{$IFDEF ICS_NewMessaging}   { V10 }
  {$IF DEFINED(POSIX) AND (NOT DEFINED(LINUX))}
    try
  {$ENDIF}
        if State <> wsListening then begin
            WSocket_Synchronized_WSASetLastError(WSAEINVAL);
        ...
        len := SizeOfAddr(Sin6);
        FASocket := WSocket_Synchronized_Accept(HSocket, PSockAddr(@Sin6), @len);
        Result := FASocket;
        ...
        if (FASocket = INVALID_SOCKET) then begin
          ...
          {$ENDIF}
          {$IFDEF POSIX}
            LastErr := WSocket_Synchronized_WSAGetLastError;
  ...
  {$ENDIF}
{$ELSE}
        if GetState <> wsListening then begin
            WSocket_Synchronized_WSASetLastError(WSAEINVAL);
            SocketError('not a listening socket');
        ...
        len := SizeOfAddr(sin6);
        FASocket := WSocket_Synchronized_Accept(GetHSocket, @sin6, @len);
        Result := FASocket;
        if (FASocket = INVALID_SOCKET) then begin
            SocketError('Accept');
            Exit;
...
begin
    FPaused := TRUE;
{$IFDEF ICS_NewMessaging}   { V10 }
    DelSocketHandleFromWaitList(HSocket);
{$ELSE}
    WSocket_Synchronized_WSAASyncSelect(GetHSocket, Handle, 0, 0);
{$ENDIF ICS_NewMessaging}   { V10 }
end;
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loWsockInfo) then
      DebugLog(loWsockInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' ' + 'TCustomWSocket.Shutdown ' +
                                                                                IntToStr(How) + ' handle=' + IntToStr(GetHSocket));
{$ENDIF}
    if GetHSocket <> INVALID_SOCKET then begin
        WSocket_Synchronized_Shutdown(GetHSocket, How);
    end;
end;
...
begin
{$IFDEF ICS_NewMessaging}   { V10 }
    PipeCommand(PIPE_CMD_CLOSE_DELAYED);
{$ELSE}
...
begin
{$IFDEF ICS_NewMessaging}   { V10 }
    if (HSocket = INVALID_SOCKET) or bAllSent then
        Exit;
    PipeCommand(PIPE_CMD_TRY_TO_SEND);
    while not ((HSocket = INVALID_SOCKET) or bAllSent) do
        Sleep(0);        //CheckSynchronize(0);
{$ELSE}
    while (GetHSocket <> INVALID_SOCKET) and     { No more socket   }
...
{$IFNDEF ICS_NewMessaging}   { V10 }
            ...
            WSocket_Synchronized_WSAASyncSelect(GetHSocket, Handle, 0, 0); { V8.05 }
{$ENDIF N ICS_NewMessaging}   { V10 }

        { Close the socket }
            iStatus := WSocket_Synchronized_closesocket(GetHSocket);
            if iStatus <> 0 then begin
                FLastError := WSocket_Synchronized_WSAGetLastError;
                if FLastError <> WSAEWOULDBLOCK then begin
                    SetHSocketValue(INVALID_SOCKET);
...
{$IFNDEF ICS_NewMessaging}   { V10 }
              { Next line is untested, however I think we have to reenable socket notification here.  (AG)  }
                WSocket_Synchronized_WSAASyncSelect(GetHSocket, Handle, FMsg_WM_ASYNCSELECT, FSelectEvent);    { V8.05 }
                MessagePump;
{$ENDIF N ICS_NewMessaging}   { V10 }
            end;
        until iStatus = 0;
    ...
    { Protect AssignDefaultValue because SessionClosed event handler may have destroyed the component. }
    try
        AssignDefaultValue;
    except
    end;
...
var
    ...
    Ch: AnsiChar;
begin
    while (GetHSocket <> INVALID_SOCKET) and (GetState <> wsClosed) do begin
        MessagePump;

        if WSocket_Synchronized_ioctlsocket(GetHSocket, FIONREAD, lCount) = SOCKET_ERROR then
            break;
        if lCount > 0 then
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
procedure TCustomWSocket.CloseSocketInternal(ErrorCode : LongInt);
var
    Tmp : TSocket;
begin
    FCloseInternalLock.BeginWrite;
    try
        if HSocket <> INVALID_SOCKET then begin
{$IFNDEF MSWINDOWS}
            DelSocketHandleFromWaitList(GetHSocket);
{$ENDIF}
            if ErrorCode <> WSAENOTSOCK then begin
                Tmp := HSocket;
                WSocket_Synchronized_closesocket(Tmp);
            end;
            SetHSocketValue(TSocket(INVALID_SOCKET));
        end;
{$IFDEF MSWINDOWS}
        if FHSocketEvent <> INVALID_HANDLE_VALUE then begin
            DelSocketHandleFromWaitList(FHSocketEvent);
{$IFNDEF NO_DEBUG_LOG}
            if CheckLogOptions(loWsockInfo) then
                DebugLog(loWsockInfo, Format('CloseSocketInternal, FHSocketEvent=%d ThreadID=%d', [FHSocketEvent, GetCurrentThreadID]));
{$ENDIF}
            Ics_WSACloseEvent(FHSocketEvent);
            FHSocketEvent := INVALID_HANDLE_VALUE;
        end;
{$ENDIF}
        ChangeState(wsClosed);
        if not FCloseInvoked then begin
            FCloseInvoked := TRUE;
            TCustomWSocket.PostMessage(Self, WM_ICS_WS_SESSION_CLOSED, ErrorCode, 0);
        end;
    finally
        FCloseInternalLock.EndWrite;
    end;
end;
{$ENDIF ICS_NewMessaging}   { V10 }
    ...
    if ASocketFamily = sfIPv4 then begin
        ...
        if Phe = nil then
          ...
          {$IFDEF MSWINDOWS}
            ...
            System.SysUtils.StrCopy(@Result[1], Phe^.h_name);
          {$ELSE}
            {$IFDEF DELPHI23_UP}           { V8.21 }
            SetLength(Result, StrLen(Phe^.h_name));
            System.SysUtils.StrCopy(@Result[1], Phe^.h_name);
            ...
            System.SysUtils.StrCopy(@Result[1], Phe^.hname);
            {$ENDIF}
          {$ENDIF}
        end;
    end
...
function  getifaddrs(var ifap: pifaddrs): Integer; cdecl; external libcmodulename name 'getifaddrs';
procedure freeifaddrs(ifap: pifaddrs); cdecl; external libcmodulename name 'freeifaddrs';

// https://stackoverflow.com/a/265978/189103
function LocalIPList( const ASocketFamily: TSocketFamily = DefaultSocketFamily; const AProtocol: Integer = IPPROTO_TCP): TStrings;
var
    ifAddrStruct  : Pifaddrs;
    ifa           : Pifaddrs;
    tmpAddrPtr    : Pointer;
    addressBuffer : array [0..255] of AnsiChar;
begin
{$IFNDEF NO_ADV_MT}
    CritSecIpList.Enter;
    try
{$ENDIF}
        ...
        Result := IPList;
        ifAddrStruct := nil;
        if getifaddrs(ifAddrStruct) = -1 then
             raise ESocketException.CreateFmt('getifaddrs failed. errno=%s', [errno]);
        ifa := ifAddrStruct;
        while ifa <> nil do begin
            if ifa.ifa_addr <> nil then begin
                if ifa.ifa_addr.sa_family = AF_INET then begin
                    // IPv4
                    if ASocketFamily = sfIPv4 then begin
                        tmpAddrPtr := @(PSockAddr_In(ifa.ifa_addr).sin_addr);
                        inet_ntop(AF_INET, tmpAddrPtr, addressBuffer, INET_ADDRSTRLEN);
                        if addressBuffer <> ICS_LOCAL_HOST_V4 then
                            IPList.Add(String(addressBuffer));
                    end;
                end
                else if ifa.ifa_addr.sa_family = AF_INET6 then begin
                    // IPv6
                    if ASocketFamily = sfIPv6 then begin
                        tmpAddrPtr := @(PSockAddr_In6(ifa.ifa_addr).sin6_addr);
                        inet_ntop(AF_INET6, tmpAddrPtr, addressBuffer, INET6_ADDRSTRLEN);
                        if addressBuffer <> ICS_LOCAL_HOST_V6 then
                            IPList.Add(String(addressBuffer));
                    end;
                end;
            end;
...
{$ENDIF}
      ...
      { V8.52 only use GetHostByName for Windows XP, 2003 and earlier }
        if (ASocketFamily = sfIPv4) and (Win32MajorVersion <= 5) then begin
            ...
            if RetVal <> 0 then
                raise ESocketException.Create('Winsock GetAddrInfo: Cannot convert host address ''' +
            ...
            try
                ...
                begin
                    ...
                    begin
                        ...
                        begin
                            IPList.Insert(IDX, WSocketIPv4ToStr(AddrInfoNext^.ai_addr^.sin_addr.S_addr));
                            Inc(IDX);
                        end
                        else
                            IPList.Add(WSocketIPv4ToStr(AddrInfoNext^.ai_addr^.sin_addr.S_addr));
                    end
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loWsockDump) then
        DebugLog(loWsockDump, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' TriggerDataAvailable (TCustomWSocket) handle=' +
                                                                        IntToStr(GetHSocket) +' ErrCode=' + IntToStr(Error));
{$ENDIF}
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loWsockDump) then
        DebugLog(loWsockDump, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' ' + 'TriggerDataSent handle=' + IntToStr(GetHSocket));
{$ENDIF}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.SocketError(sockfunc: String; LastError: Integer = 0; FriendlyMsg: String = '');   { V8.36 added FriendlyMsg }
var
    Error  : Integer;
    Line   : String;
begin
    ...
    if (Error = WSAECONNRESET) or (Error = WSAENOTCONN) then begin
        WSocket_Synchronized_closesocket(GetHSocket);
        SetHSocketValue(INVALID_SOCKET);
        if GetState <> wsClosed then
           TriggerSessionClosed(Error);
    ...
    FLastError := Error;
    RaiseException(Line, Error, WSocketErrorDesc(Error), FriendlyMsg, sockfunc, FAddrStr, FPortStr, FProtoStr);  { V8.42 }
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *} { V5.21 }
(*  V10.0 moved toWndControl
{$IFNDEF NO_DEBUG_LOG}
...
begin
    if (GetState <> wsClosed) or (FDnsLookupHandle <> 0) then begin
        ...
        if Value <> sfIPv4 then begin
            try
                if not IsIPv6APIAvailable then
                    raise ESocketException.Create('SetSocketFamily: New API requires winsock 2.2 ' +
...
begin
    ...
    iStatus := WSocket_setsockopt(GetHSocket, SOL_SOCKET, SO_RCVBUF, PAnsiChar(@BufSize), optlen);
    if iStatus = 0 then
...
begin
    optlen  := SizeOf(BufSize);
    iStatus := WSocket_setsockopt(GetHSocket, SOL_SOCKET, SO_SNDBUF, PAnsiChar(@BufSize), optlen);
    if iStatus = 0 then
...
begin
    ...
    else if GetState in [wsConnected, wsBound, wsListening] then
    begin
        Result := FSocketFamily; // Dummy
        saddrlen := sizeof(saddr);
        if WSocket_Synchronized_GetSockName(GetHSocket, PSockAddrIn(@saddr)^, saddrlen) = 0 then
        begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.AddSocketHandleToWaitList(SocketHandle: TSocket);
{$IFDEF LINUX}
var
    Event   : TEPoll_Event;
begin
    if SocketHandle = INVALID_SOCKET then
        Exit;
    if FEpollFd = 0 then
        raise ESocketException.Create('epoll handle closed');

    Event.Events  := EPOLLIN or EPOLLOUT or EPOLLERR or EPOLLET;
    Event.Data.fd := SocketHandle;
    if epoll_ctl(FEpollFd, EPOLL_CTL_ADD, SocketHandle, @Event) = -1 then
        raise ESocketException.CreateFmt('epoll_ctl ADD socket handle failed', [errno]);
end;
{$ELSE}
begin
    if SocketHandle = INVALID_SOCKET then
        Exit;
    PipeCommand([AnsiChar(PIPE_CMD_ADD_TO_WAITLIST), SocketHandle]);
end;
{$ENDIF}


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.DelSocketHandleFromWaitList(SocketHandle: TSocket);
{$IFDEF LINUX}
begin
    if FEpollFd = 0 then
        raise ESocketException.Create('epoll handle closed');
    if epoll_ctl(FEpollFd, EPOLL_CTL_DEL, SocketHandle, nil) = -1 then
        raise ESocketException.CreateFmt('epoll_ctl DEL FHSocket failed. errno=%d', [errno]);
end;
{$ELSE}
begin
    if SocketHandle = INVALID_SOCKET then
        Exit;
    PipeCommand([AnsiChar(PIPE_CMD_DEL_FROM_WAITLIST), SocketHandle]);
end;
{$ENDIF}
...
begin
    ErrorCode := 0;
    TCustomWSocket.PostMessage(Self, WM_ICS_WS_SESSION_AVAILABLE, ErrorCode, 0);
end;
...
function TCustomWSocket.pipe(EventPipe : PPipeFd) : Integer;
var
...
begin
    ...
    if CreatePipe(EventPipe.Read, EventPipe.Write, @saAttr, 0) then begin
        Result := 0;    // Success
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loWsockInfo) then
        DebugLog(loWsockInfo, Format('Pipe.Read=%d  Pipe.Write=%d  ThreadID=%d', [EventPipe.Read, EventPipe.Write,  GetCurrentThreadId]));
{$ENDIF}
    end
    else begin
        Result := -1;  // Failure
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loWsockInfo) then
        DebugLog(loWsockInfo, 'CreatePipe failed');
{$ENDIF}
    end;
end;
{$ENDIF}
...
{$IFDEF MSWINDOWS}
// Returns zero on success, -1 on error
function TCustomWSocket.__close(Hdl: THANDLE) : Integer;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF MSWINDOWS}
function TCustomWSocket.__write(Hdl: THANDLE; Buf: Pointer; Cnt: size_t) : Integer;
var
    BytesWritten : DWORD;
begin
    if WriteFile(Hdl, PChar(Buf)^, Cnt, BytesWritten, nil)  then
        Result := Integer(BytesWritten)   // Success
    else
        Result := -1;                     // Failure
end;
{$ENDIF}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF MSWINDOWS}
function TCustomWSocket.__read(Hdl: THANDLE; Buf: Pointer; Cnt: size_t) : Integer;
var
    BytesRead : DWORD;
begin
    if ReadFile(Hdl, PChar(Buf)^, Cnt, BytesRead, nil)  then
        Result := Integer(BytesRead)   // Success
    else
        Result := -1;                     // Failure
end;
{$ENDIF}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
(*
class procedure TCustomWSocket.IcsSocketMessageListener({$IFDEF RTL_MESSAGING}const Sender : TObject;{$ENDIF} const M: TIcsSocMsg);
var
{$IFDEF RTL_MESSAGING}
    Msg     : TIcsSocMsg;
{$ENDIF}
...
begin
{$IFDEF RTL_MESSAGING}
    if not (M is TIcsSocMsg) then
        Exit;
    // This try/except catch access violation because a queued messages has
    // Value.Instance refering to an already destroyed socket.
    // To avoid this, I should implement a list of all created sockets,
    // updating the list when a socket is destroyed. Then I could check here
    // the instance is in that list.
    try
        Msg := TIcsSocMsg(M);
        if not (Msg.Value.Instance is TIcsCustomWSocket) then
            Exit;
    except
        Exit;
    end;
...
{$ENDIF}
    // Ignore any message not related to TIcsCustomWSocket   - !! means no descendents are called
    if Assigned(Value.Instance) and (Value.Instance is TCustomWSocket) then
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
class procedure TCustomWSocket.DispatchMessage(const Value : TIcsSocketMsgRec);
var
    WSocket : TCustomWSocket;
begin
{$IFNDEF NO_DEBUG_LOG}
//    if CheckLogOptions(loWsockDump) then
//        DebugLog(loWsockInfo, 'TWSocket:DispatchMessage: Msg=' + IntToStr(Value.Msg));  // not allowed in class methods
{$ENDIF}
    IcsOutputDebugStr('TCustomWSocket:DispatchMessage: Msg=' + IntToStr(Value.Msg));   // TEMP !!!!!

    if Assigned(Value.Instance) and (Value.Instance is TCustomWSocket) then begin
        WSocket := TCustomWSocket(Value.Instance);
        try
            case Value.Msg of
                WM_ICS_WS_SESSION_CONNECTED :
                    WSocket.TriggerSessionConnected(Value.WParam);
                WM_ICS_WS_DATA_AVAILABLE :
                    WSocket.TriggerDataAvailable(Value.WParam);
                WM_ICS_WS_SESSION_CLOSED :
                    WSocket.TriggerSessionClosed(Value.WParam);
                WM_ICS_WS_SESSION_AVAILABLE :
                    WSocket.TriggerSessionAvailable(Value.WParam);
                WM_ICS_WS_SEND_DATA :
                    WSocket.TriggerSendData(Value.WParam);
                WM_ICS_WS_TRIGGER_ERROR :
                    WSocket.TriggerError;
                WM_ICS_WS_TRIGGER_EXCEPTION :
                    WSocket.TriggerException(ESocketException(Value.LParam));
                WM_ICS_WS_DNS_LOOKUP_DONE :
                    WSocket.TriggerDNSLookupDone(Word(Value.WParam));
            end;
        except
            on E:Exception do begin
                if Assigned(WSocket) then
                    WSocket.HandleBackGroundException(E, 'TWocket:DispatchMessage: ' + IntToStr(Value.Msg));
            end;
        end;
    end;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.PipeCommand(Command: BYTE);
begin
    if FEventPipe.Write <> THANDLE(INVALID_FILE_HANDLE) then
        __write(FEventPipe.Write, @Command, 1);
{$IFDEF MSWINDOWS}
    if Assigned(FEventPipe.Event) then
        FEventPipe.Event.SetEvent;
{$ENDIF}
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.PipeCommand(Command: array of const);
var
    I: Integer;
    Ch: BYTE;
    Len: Integer;
begin
    ...
    I := Low(Command);
    while I <= High(Command) do begin
        Ch := Command[I].VType;
        case Command[I].VType of
        vtChar:
            begin
                __write(FEventPipe.Write, @Command[I].VChar, SizeOf(AnsiChar));
            end;
        vtAnsiString:
            begin
                __write(FEventPipe.Write, Command[I].VPChar, System.SysUtils.StrLen(Command[I].VPChar));
            end;
        vtInteger:
            begin
                __write(FEventPipe.Write, @Command[I].VInteger, SizeOf(Integer));
            end;
        vtInt64:
            begin
                __write(FEventPipe.Write, Command[I].VInt64, SizeOf(Int64));
            end;
        vtUnicodeString:
            begin
                Len := Length(String(Command[I].VUnicodeString));
                // Write string length
                __write(FEventPipe.Write, @Len, SizeOf(Len));
                __write(FEventPipe.Write, Command[I].VUnicodeString, Len * SizeOf(Char));
            end;
        else begin
                Ch := 0;
                __write(FEventPipe.Write, @Ch, SizeOf(Ch));
            end;
        end;
        I := I + 1;
    end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
(* class function TCustomWSocket.PostMessage(const Sender: TObject; const Msg: Cardinal; const WParam: NativeUint; const LParam: NativeUint): Boolean;
begin
    Result := TIcsMessageManager.IcsDefaultManager.PostMessage(Sender, CreateIcsSocketMessage(Msg, Sender, WParam, LParam));
end;   *)
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.GetHostByNameDetected;
var
    AProtocol: Integer;
    ALen: Integer;
    AHostName: String;
    ...
    MsgItem: TIcsSocMsg;
begin
    __read(FEventPipe.Read, @AProtocol, SizeOf(AProtocol));
    __read(FEventPipe.Read, @ALen,      SizeOf(ALen));
    SetLength(AHostName, ALen);
    __read(FEventPipe.Read, PChar(AHostName), ALen * SizeOf(Char));

    FillChar(Hints, SizeOf(Hints), 0);
    if FSocketFamily = sfIPv6 then
        Hints.ai_family := AF_INET6
    else if FSocketFamily = sfIPv4 then
        Hints.ai_family := AF_INET;
    AddrInfo := nil;
    Hints.ai_protocol := AProtocol;
{$IFDEF MSWINDOWS}
    ErrCode := WSocket_GetAddrInfo(PChar(AHostName), nil, @Hints, AddrInfo);  { only 2003 and Vista and later }
{$ELSE}
    ErrCode := WSocket_GetAddrInfo(PAnsiChar(UnicodeToAnsi(AHostName, CP_UTF8)), nil, @Hints, AddrInfo);
{$ENDIF}
    if ErrCode = 0 then begin
        try
            GetAddrList(AddrInfo, FSocketFamily, FDnsResultList);
            if FDnsResultList.Count > 0 then
                FDnsResult := FDnsResultList[0];
        finally
            WSocket_FreeAddrInfo(AddrInfo);
    ...
    Thread.MySynchronize(
        procedure
        begin
            MsgItem := CreateIcsSocketMessage(WM_ICS_WS_DNS_LOOKUP_DONE, Self, NativeUInt(ErrCode), 0);
            TIcsMessageManager.IcsDefaultManager.SendMessage(nil, MsgItem);
        end);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.GetHostByAddrDetected;
var
    AProtocol     : Integer;
    ALen          : Integer;
    AHostName     : String;
...
begin
    ...
    FDnsResult := String(WSocketResolveIp(AnsiString(AHostName), FSocketFamily, AProtocol));
    if FDnsResult = '' then
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.Do_FD_CONNECT;
var
    OptVal, OptLen: socklen_t;
begin
    OptVal := 0;
    OptLen := SizeOf(OptVal);
    if WSocket_Synchronized_getsockopt(HSocket, SOL_SOCKET, SO_ERROR, PAnsiChar(@OptVal), OptLen) = -1 then begin
        FLastError := WSocket_Synchronized_WSAGetLastError;
        FLastErrorMsg := Format('getsockopt failed. errno=%d', [WSocket_Synchronized_WSAGetLastError]);
        CloseSocketInternal(FLastError);
        Exit;
    end;
    if OptVal <> 0 then begin
    // We come here for example when connection is refused by peer we will get ETIMEDOUT (110) error under Linux.
        FLastError    := OptVal;
        FLastErrorMsg := Format('Connect failed. errno=%d', [OptVal]);
   // We must trigger the event SessionConnected with the error code
        Do_FD_CONNECT_Special(FLastError);
        CloseSocketInternal(FLastError);   // PostMessage
        Exit;
    end;
    ChangeState(wsConnected);
    Do_FD_CONNECT_Special(0);   // PostMessage
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.Do_FD_CONNECT_Special(Error : Word);
begin
    TCustomWSocket.PostMessage(Self, WM_ICS_WS_SESSION_CONNECTED, Error, 0);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.Do_FD_CLOSE(ErrCode : WORD);
begin
    CloseSocketInternal(ErrCode);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.CloseDelayedDetected;
begin
    Thread.MySynchronize(
        procedure
        begin
...
begin
    ...
    if GetHSocket = TSocket(INVALID_SOCKET) then
        Exit;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.ErrorEventDetected;
begin
    // Check if socket already closed
    if GetHSocket = TSocket(INVALID_SOCKET) then
        Exit;
    // I have yet to find a way to trigger this!
    raise Exception.Create('TIcsCustomWSocket.ErrorEventDetected');
end;
...
begin
    ...
    if GetHSocket = TSocket(INVALID_SOCKET) then
        Exit;
    if FProto = IPPROTO_TCP then begin
        // To detect remote closing connection, try to read data without removing it from queue
        BufPtr := @Buffer;
        Count := WSocket_Synchronized_recv(GetHSocket, BufPtr, SizeOf(Buffer), MSG_PEEK);
...
{$IFNDEF NO_DEBUG_LOG}
            if CheckLogOptions(loWsockErr) then
                DebugLog(loWsockInfo, Format('ReadEventDetected, recv error %d', [Err]));
{$ENDIF}
            Exit;
        end;
{$IFNDEF NO_DEBUG_LOG}
        if CheckLogOptions(loWsockDump) then
            DebugLog(loWsockDump, Format('ReadEventDetected, recv count %d', [Count]));
{$ENDIF}
        if Count = 0 then begin
            Do_FD_CLOSE(0);
            Exit;
    ...
    Do_FD_READ_Special(0);   // PostMessage
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.Do_FD_READ_Special(Error : Word);
begin
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loWsockDump) then
        DebugLog(loWsockInfo, 'PostMessage WM_ICS_WS_DATA_AVAILABLE');
{$ENDIF}
    TCustomWSocket.PostMessage(Self, WM_ICS_WS_DATA_AVAILABLE, 0, 0);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.CloseSocketEventDetected;
begin
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loWsockInfo) then
        DebugLog(loWsockInfo,Format('CloseSocketEventDetected, HSocket not valid anymore. HSocket=%d', [GetHSocket]));
{$ENDIF}
    CloseSocketInternal(WSAENOTSOCK);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF MSWINDOWS}
procedure TCustomWSocket.PipeCmdDelHandleFromWaitList(AHandle: THandle; Handles: PHandle; var Count: Integer);
var
    I: Integer;
begin
    if AHandle = INVALID_HANDLE_VALUE then
        Exit;
    // Search backward if handle is in the list
    I := Count - 1;
    while I >= 0 do begin
        if PHandle(UIntPtr(Handles) + u_int(I) * SizeOf(THandle))^ = AHandle then
            break;
        Dec(I);
    end;
    if I >= 0 then begin
        // Found in the list at Ith position
        // Pack all handles
        if I = (Count - 1) then
            PHandle(UIntPtr(Handles) + u_int(I) * SizeOf(THandle))^ := 0
        else begin
            Inc(I);
            while I < Count do begin
                PHandle(UIntPtr(Handles) + u_int(I - 1) * SizeOf(THandle))^ := PHandle(UIntPtr(Handles) + u_int(I) * SizeOf(THandle))^;
                Inc(I);
            end;
        end;
        Dec(Count);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF MSWINDOWS}
procedure TCustomWSocket.PipeCmdAddHandleToWaitList(AHandle: THandle; Handles: PHandle; var Count: Integer);
var
    I: Integer;
begin
    // Search if handle already in the list
    I := Count - 1;
    while I >= 0 do begin
        if PHandle(UIntPtr(Handles) + u_int(I) * SizeOf(THandle))^ = AHandle then
            break;
    ...
    if I < 0 then begin   // Not yet in list
        // This should never occur since we have a thread per socket
        if Count >= WSA_MAXIMUM_WAIT_EVENTS then
            raise Exception.Create('Failure: too many handles for WSAWaitForMultipleObjects');
        PHandle(UIntPtr(Handles) + u_int(Count) * SizeOf(THandle))^ := AHandle;
        Inc(Count);
    end;
end;
{$ENDIF}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
// Check if HSocket is valid. Return TRUE if it is valid.
// Note that INVALID_SOCKET returns TRUE.
// The check is done by trying to receive a byte from the socket without removing it from the queue.
// The receiv will fail with WSAENOTSOCK when the handle is not more valid.
// Note: it is possible that the socket handle has already been reallocated to another socket!
function TCustomWSocket.CheckValidHSocket : Boolean;
var
    RcvCount: Integer;
    Buffer: BYTE;
    BufPtr: TWSocketData;
...
begin
    ...
    if GetHSocket = INVALID_SOCKET then
        Exit;
    // If the socket becomes invalid, it is likely it has been closed by a direct call to CloseSocket instead of a call of the Close method.
    BufPtr := @Buffer;
    RcvCount := WSocket_Synchronized_recv(GetHSocket, BufPtr, SizeOf(Buffer), MSG_PEEK);      // Don't remove!
    if RcvCount <> -1 then
        Exit;
    // An error
    Err := WSocket_Synchronized_WSAGetLastError;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF MSWINDOWS}
{ main thread processing and despatching events from the pipe and message queue }
procedure TCustomWSocket.ThreadExecuteProc;
var
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loWsockInfo) then
        DebugLog(loWsockInfo, Format('ThreadExecuteProc, WorkerThread begin ThreadID=%d', [IcsGetCurrentThreadID]));
{$ENDIF}
    FThreadStarted := TRUE;
    if not Assigned(FEventPipe.Event) then
        raise Exception.Create('FEventPipe.Event not created yet.');
    Count := 1;
    Handles[0] := FEventPipe.Event.Handle;
    while TRUE do begin
      // This should never occur since we have a thread per socket
        if Count > WSA_MAXIMUM_WAIT_EVENTS then
            raise Exception.Create('WSAWaitForMultipleEvents failure: too many handles');

     // We use a timeout to check for socket handle still being valid
        Status := Ics_WSAWaitForMultipleEvents(Count, @Handles[0], FALSE, 1000, FALSE);
        if Thread.Terminated then
            break;
        if Status = WSA_WAIT_TIMEOUT then begin
            // If the socket becomes invalid, it is likely it has been closed by a direct call to CloseSocket instead of a
            // call of the Close method.
            // Note: it is possible that the socket handle has already been reallocated to another socket!
            if not CheckValidHSocket then begin
                PipeCmdDelHandleFromWaitList(FHSocketEvent, @Handles, Count);
                CloseSocketEventDetected;
            end;
            Continue;
        end;
        if Status = WSA_WAIT_FAILED then begin
            ErrCode := GetLastError;
{$IFNDEF NO_DEBUG_LOG}
            if CheckLogOptions(loWsockInfo) then
                DebugLog(loWsockInfo, Format('ThreadExecuteProc, WSAWaitForMultipleEvents failed with error=%d', [ErrCode]));
{$ENDIF}
            if ErrCode = ERROR_INVALID_HANDLE then begin
                if GetHSocket = INVALID_SOCKET then begin
                    // Socket has been closed, Not an error
{$IFNDEF NO_DEBUG_LOG}
                    if CheckLogOptions(loWsockInfo) then
                         DebugLog(loWsockInfo, 'ThreadExecuteProc, WSAWaitForMultipleEvents failed socket closed');
{$ENDIF}
                    // Remove handles from wait list
                    while Count > 1 do
                        PipeCmdDelHandleFromWaitList(Handles[Count - 1], @Handles, Count);
                    continue; //break;
                end;
            end;
            break;
        end;
        if {$IF WSA_WAIT_EVENT_0 <> 0}(Status >= WSA_WAIT_EVENT_0) and{$ENDIF} (Status < (WSA_WAIT_EVENT_0 + DWORD(Count))) then begin
            Index := Status - WSA_WAIT_EVENT_0;
{$IFNDEF NO_DEBUG_LOG}
            if CheckLogOptions(loWsockDump) then
                DebugLog(loWsockInfo, Format('ThreadExecuteProc, WSAWaitForMultipleEvents signaled Index=%d Handle=%d ThreadID=%d',
                                                                                     [Index, Handles[Index], IcsGetCurrentThreadID]));
{$ENDIF}
            if Index = 0 then begin
                // Pipe signaled
                ByteRead := __read(FEventPipe.Read, @Ch, 1);
                if ByteRead > 0 then begin
                    ...
                    PIPE_CMD_ADD_TO_WAITLIST :
                        begin
                            ByteRead := __read(FEventPipe.Read, @AHandle, SizeOf(AHandle));
                            if ByteRead <> SizeOf(AHandle) then
                                 raise Exception.Create('Failure: __read returned incorrect ByteRead');
{$IFNDEF NO_DEBUG_LOG}
                            if CheckLogOptions(loWsockInfo) then
                                DebugLog(loWsockInfo, Format('ThreadExecuteProc, Add handle %d to WaitList ThreadID=%d',
                                                                                             [AHandle, IcsGetCurrentThreadID]));
{$ENDIF}
                            if Count >= WSA_MAXIMUM_WAIT_EVENTS then
                                raise Exception.Create('WSAWaitForMultipleEvents failure: too many handles');
                            PipeCmdAddHandleToWaitList(AHandle, @Handles, Count);
                        end;
                    PIPE_CMD_DEL_FROM_WAITLIST :
                        begin
                            ByteRead := __read(FEventPipe.Read, @AHandle, SizeOf(AHandle));
                            if ByteRead <> SizeOf(AHandle) then
                                 raise Exception.Create('Failure: __read returned incorrect ByteRead');
{$IFNDEF NO_DEBUG_LOG}
                            if CheckLogOptions(loWsockInfo) then
                                DebugLog(loWsockInfo, Format('ThreadExecuteProc, Del handle %d from WaitList ThreadID=%D',
                                                                                             [AHandle, IcsGetCurrentThreadID]));
{$ENDIF}
                            PipeCmdDelHandleFromWaitList(AHandle, @Handles, Count);
                        end;
                    PIPE_CMD_TRY_TO_SEND :
                        begin
                            TryToSend;
                            if bAllSent then
                                TriggerDataSent(0);
                        end;
                    PIPE_CMD_SENDMESSAGE :
                        begin
                            Thread.MySynchronize(
                                procedure
                                begin
                                    MsgItem := CreateIcsSocketMessage(WM_ICS_TEST, nil, 12, 34);
                                    TIcsMessageManager.IcsDefaultManager.SendMessage(nil, MsgItem);
                                end);
                        end;
                    PIPE_CMD_GETHOSTBYNAME :
                        GetHostByNameDetected;
                    PIPE_CMD_GETHOSTBYADDR :
                        GetHostByAddrDetected;
                    PIPE_CMD_CLOSE_DELAYED :
                        CloseDelayedDetected;
                    end;
                end;
            end
            else begin
                if Handles[Index] = FHSocketEvent then begin
                    Ics_WSAResetEvent(FHSocketEvent);
                    if HSocket = INVALID_SOCKET then begin
                        // Socket has been closed
                        continue;
                    end;
                    if Ics_WSAEnumNetworkEvents(HSocket, FHSocketEvent, NetworkEvents) = SOCKET_ERROR then begin
                        if Ics_WSAGetLastError = WSAENOTSOCK then begin
                            // Socket has been closed
                            continue;
                        end;
                        SocketError('Ics_WSAEnumNetworkEvents');
                        break;
                    end;
{$IFNDEF NO_DEBUG_LOG}
                    if CheckLogOptions(loWsockDump) then begin
                        if (NetworkEvents.lNetworkEvents and FD_ACCEPT) <> 0 then
                            DebugLog(loWsockDump, Format('FD_ACCEPT Socket=%d  State=%s', [HSocket, GetEnumName(TypeInfo(TSocketState), Ord(State))]));
                        if (NetworkEvents.lNetworkEvents and FD_CONNECT) <> 0 then
                            DebugLog(loWsockDump, Format('FD_CONNECT Socket=%d  State=%s', [HSocket, GetEnumName(TypeInfo(TSocketState), Ord(State))]));
                        if (NetworkEvents.lNetworkEvents and FD_READ) <> 0 then
                            DebugLog(loWsockDump, Format('FD_READ Socket=%d  State=%s', [HSocket, GetEnumName(TypeInfo(TSocketState), Ord(State))]));
                        if (NetworkEvents.lNetworkEvents and FD_WRITE) <> 0 then
                            DebugLog(loWsockDump, Format('FD_WRITE Socket=%d  State=%s', [HSocket, GetEnumName(TypeInfo(TSocketState), Ord(State))]));
                        if (NetworkEvents.lNetworkEvents and FD_OOB) <> 0 then
                            DebugLog(loWsockDump, Format('FD_OOB Socket=%d  State=%s', [HSocket, GetEnumName(TypeInfo(TSocketState), Ord(State))]));
                        if (NetworkEvents.lNetworkEvents and FD_CLOSE) <> 0 then
                            DebugLog(loWsockDump, Format('FD_CLOSE Socket=%d  State=%s', [HSocket, GetEnumName(TypeInfo(TSocketState), Ord(State))]));
                        if (NetworkEvents.lNetworkEvents and FD_ROUTING_INTERFACE_CHANGE) <> 0 then
                            DebugLog(loWsockDump, Format('FD_ROUTING_INTERFACE_CHANGE Socket=%d  State=%s', [HSocket, GetEnumName(TypeInfo(TSocketState), Ord(State))]));
                        if (NetworkEvents.lNetworkEvents and FD_ADDRESS_LIST_CHANGE) <> 0 then
                            DebugLog(loWsockDump, Format('FD_ADDRESS_LIST_CHANGE Socket=%d  State=%s', [HSocket, GetEnumName(TypeInfo(TSocketState), Ord(State))]));
                        if (NetworkEvents.lNetworkEvents and FD_QOS) <> 0 then
                            DebugLog(loWsockDump, Format('FD_QOS Socket=%d  State=%s', [HSocket, GetEnumName(TypeInfo(TSocketState), Ord(State))]));
                        if (NetworkEvents.lNetworkEvents and FD_GROUP_QOS) <> 0 then
                            DebugLog(loWsockDump, Format('FD_GROUP_QOS Socket=%d  State=%s', [HSocket, GetEnumName(TypeInfo(TSocketState), Ord(State))]));
                    end;
{$ENDIF}
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loWsockInfo) then
        DebugLog(loWsockInfo, Format('ThreadExecuteProc, WorkerThread end ThreadID=%d', [IcsGetCurrentThreadID]));
{$ENDIF}
end;
{$ENDIF}
...
var
    Event: TEPoll_Event;
    Events: array [0..MAX_EVENTS - 1] of TEPoll_Event;
    FdEvents: TPoll_t;
    nfds: Integer;
    Ch: BYTE;
    I: Integer;
...
begin
    ...
    try
        ...
        Event.Data.fd := FEventPipe.Read;
        if epoll_ctl(FEpollFd, EPOLL_CTL_ADD, FEventPipe.Read, @Event) = -1 then
            raise ESocketException.Create('ThreadExecuteProc, epoll_ctl  FEventPipe.Read failed');
        while TRUE do begin
            FillChar(Events, SizeOf(Events), 0);
            nfds := epoll_wait(FEpollFd, @Events, MAX_EVENTS, -1);
            if Thread.Terminated then
                break;
            if nfds < 0 then begin
{$IFNDEF NO_DEBUG_LOG}
                if CheckLogOptions(loWsockInfo) then
                DebugLog(loWsockInfo, Format('ThreadExecuteProc, epoll_wait returned nfds=%d  errno=%d', [nfds, errno]));
{$ENDIF}
...
{$IFNDEF NO_DEBUG_LOG}
            if CheckLogOptions(loWsockInfo) then
                DebugLog(loWsockInfo, Format('epoll_wait returned nfds=%d', [nfds]));
{$ENDIF}
            I := 0;
            while I < nfds do begin
                Fd := Events[I].Data.fd;
                FdEvents := Events[I].Events;
                if Fd = FEventPipe.Read then begin
                    __read(FEventPipe.Read, @Ch, 1);
                    case Ch of
                    PIPE_CMD_TRY_TO_SEND :
                        TryToSend;
                    PIPE_CMD_SENDMESSAGE :
                ...
                else if Fd = GetHSocket then begin
                    if (FdEvents and (EPOLLIN or EPOLLRDNORM)) <> 0 then begin
                        if State = wsListening then
                            Do_FD_ACCEPT
                        else
                            Do_FD_READ;
                    end;
                    if (FdEvents and (EPOLLOUT or EPOLLWRBAND)) <> 0 then begin
                        if State = wsConnecting then
                            Do_FD_CONNECT;
                        Do_FD_WRITE;
                    end;
                    if (FdEvents and EPOLLERR) <> 0 then begin
                        ErrorEventDetected;
                    end;
                    if (FdEvents and EPOLLHUP) <> 0 then begin
                        // Connection closed, handled elsewhere
                    end;
                    if (FdEvents and EPOLLNVAL) <> 0 then begin
                        ErrorEventDetected;
                    end;
                    if (FdEvents and EPOLLRDHUP) <> 0 then begin
                        ErrorEventDetected;
                    end;
                end;
                Inc(I);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.ThreadTerminateProc;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.XXXPipeWrite(Ch : BYTE);
begin
    PipeCommand(Ch);
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
(*
procedure TIcsAsync2DnsLookupThread.Execute;
{$IFDEF LINUX}
begin
    raise Exception.Create('TIcsAsync2DnsLookupThread.Execute not implemented yet.');
end;
{$ELSE}
var
    Request : TIcsAsync2DnsLookupRequest;
    LRes : WORD;
{$IFDEF POSIX}
    LMsgPump : TMessagePump;
{$ENDIF}
begin
{$IFDEF DEBUG}
...
begin
    ...
    if (newValue <> '4')  and (newValue <> '5') and (newValue <> '4A') and (newValue <> '4a') then begin
...
begin
    ...
    if FSocksState <> socksData then
        DataAvailableError(socksGeneralFailure, WSocketErrorMsgFromErrorCode(socksGeneralFailure));
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
procedure TCustomSocksWSocket.Do_FD_READ_Special(Error : Word);
var
    Len     : Integer;
    I       : Integer;
    ErrCode : Word;
...
begin
   if FSocksState = socksData then begin
        inherited Do_FD_READ_Special(Error);
        Exit;
    end;
    ...
    if FSocksState = socksNegociateMethods then begin
        ...
        if FSocksLevel[1] = '4' then begin { We should never comes here }
            ...
            if FRcvBuf[1] = $00 then begin { No authentication required }
                if FSocksAuthNumber <> #$00 then { We asked for authentification, so complains... }
                    TriggerSocksAuthState(socksAuthNotRequired);
            end
            else if FRcvBuf[1] = $02 then begin { Usercode/Password authentication required }
                SocksDoAuthenticate;
                Exit;
            end
            else begin
...
//        Result := TRUE;
        if FSocksLevel[1] = '4' then begin   { We want at most 8 characters }
            ...
            FSocksState := socksData;
            TriggerSessionConnectedSpecial(0);
            inherited Do_FD_READ_Special(0);
        end
        else begin { SOCKS5 }
            Len := Receive(@FRcvBuf[FRcvCnt], Sizeof(FRcvBuf) - FRcvCnt - 1);
            if Len < 0 then
                Exit;
            FRcvCnt := FRcvCnt + Len;
            if FRcvCnt >= 1 then begin  { First byte is version, we expect version 5 }
                if FRcvBuf[0] <> $05 then begin
                    DataAvailableError(socksVersionError, 'socks version error');
                    Exit;
                end;
            ...
            if FRcvBuf[3] = $01 then begin      { IP V4 address }
                InAddr.S_addr := FRcvBuf[4] or (FRcvBuf[5] shl 8) or (FRcvBuf[6] shl 16) or (FRcvBuf[7] shl 24);
                FBoundAddr := WSocket_Synchronized_inet_ntoa(InAddr);
                I := 4 + 4;
            end
            else if FRcvBuf[3] = $03 then begin  { Domain name }
           ...
           { V8.56 IPv6 support from Max Terentiev }
            ...
            else begin { Unsupported address type }
                DataAvailableError(socksUnknownAddressType, 'address type not supported');
                Exit;
            end;

            FBoundPort  := IcsIntToStrA(WSocket_Synchronized_ntohs(FRcvBuf[I] or (FRcvBuf[I + 1] shl 8)));
            I           := I + 2;
            FSocksState := socksData;
            Do_FD_CONNECT_Special(0);     // PostMessage
        ...
        if FRcvCnt < 2 then
            Exit;
        { First byte is version of the subnegotiation proto (rfc1929),  we expect version 1. However i.e. WinGate returns socks      }
        { version 5 here if user credentials are wrong, so don't trigger a version error but socksAuthenticationFailed. AG 15/02/11 }
        if FRcvBuf[0] <> $01 then begin
            DataAvailableError(socksAuthenticationFailed, 'socks authentication failed'); {AG 15/02/11}
            Exit;
        end;
        if FRcvCnt = 2 then begin  { Second byte is status }
            if FRcvBuf[1] <> $00 then begin
    ...
    else begin  { We should never comes here ! }
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomSocksWSocket.Do_FD_CONNECT_Special(Error : Word);
var
    Buf : array [0..2] of AnsiChar;
begin
    if FSocksState = socksNegociateMethods then begin
        ...
        if Error <> 0 then begin
            FSocksState := socksData;
            inherited Do_FD_CONNECT_Special(Error);
            Exit;
        end;
        if FSocksLevel[1] = '4' then
            SocksDoConnect
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomSocksWSocket.DataAvailableError(ErrCode: Integer; Msg: String);
begin
    TriggerSocksError(ErrCode, Msg);
    FSocksState := socksData;
    TriggerSessionConnectedSpecial(ErrCode);
    InternalClose(TRUE, ErrCode);
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TCustomSocksWSocket.DoRecv(var Buffer: TWSocketData; BufferSize: Integer; Flags: Integer): Integer;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
class procedure TCustomLineWSocket.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    WSocket : TCustomLineWSocket;
begin

    IcsOutputDebugStr('TCustomLineWSocket:DispatchMessage: Msg=' + IntToStr(Value.Msg));   // TEMP !!!!!

    if Assigned(Value.Instance) and (Value.Instance is TCustomLineWSocket) then begin
        WSocket := Value.Instance as TCustomLineWSocket;
        try
            if Value.Msg = WM_ICS_WS_TRGR_DATA_AVAILABLE then begin
                WSocket.WMTriggerDataAvailable(Value);
             end;
        except
            on E:Exception do begin
                if Assigned(WSocket) then
                    WSocket.HandleBackGroundException(E, 'TWocket:DispatchMessage: WMTriggerDataAvailable');
            end;
        end;
    end
    else
        inherited DispatchMessage(Value);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomLineWSocket.WMTriggerDataAvailable(const Value: TIcsSocketMsgRec);
var
    Count : Integer;
begin
    Count := 0;
    while FRcvdCnt > 0 do begin
        ...
        FLineFound := FALSE;
        TriggerDataAvailable(0);
        if (FRcvdCnt <= 0) or (FLineMode and (Count > 3) and (not FLineFound)) then
            Break;
    end;
end;
{$ENDIF ICS_NewMessaging}   { V10 }
...
function TCustomLineWSocket.SendLine(const Str: UnicodeString; ACodePage: Cardinal): Integer;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TCustomLineWSocket.DoRecv(var Buffer: TWSocketData; BufferSize: Integer; Flags: Integer): Integer;
begin
        ...
        while I < (FRcvdCnt + Len) do begin
            if PAnsiChar(FRcvdPtr)[I] = #8 then begin   { BACKSPACE character }
                ...
                if not Edited then begin
                    { Not edited yet, so we allocate a buffer to store edited data and we remember we edited data.      }
                    Edited := TRUE;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomLineWSocket.TriggerLineLimitExceeded(Cnt: Integer; var ClearData: Boolean);
begin
...
{  if (not FLineMode) or (Length(FLineEnd) = 0) then begin }
    if (not FLineMode) or (Length(FLineEnd) = 0) or (FSocksState <> socksData) or (FHttpTunnelState <> htsData) then begin
    ...
    while FLineMode do begin
        ...
        while I < (FRcvdCnt - Length(FLineEnd) + 1) do begin
            if PAnsiChar(FRcvdPtr)[I] = AnsiChar(FLineEnd[1]) then begin
                Found := StrLComp(PAnsiChar(@(PAnsiChar(FRcvdPtr)[I])), PAnsiChar(FLineEnd), Length(FLineEnd)) = 0;
                if Found then
        ...
        FLineFound        := TRUE;
        { We received a complete line. We need to signal it to application, the application may not have a large buffer so we may need       }
        { several events to read the entire line. In the meanwhile, the application may turn line mode off.                              }
        ...
        if FLineLength > 0 then begin
            { Line mode was turned off in the middle of a line read. We preserve unread line and other received data.       }
        ...
        SearchFrom := 0;
        { It is possible the user has turned line mode to off. If data is still available in the receive buffer, we will deliver it.      }
 ...
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFNDEF ICS_NewMessaging}   { V10 }
procedure TCustomSyncWSocket.InternalDataAvailable(Sender: TObject; Error: Word);
var
    Len : Integer;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{ DEPRECATED: DO NOT USE Synchronize procedure for a new application. Instead, use pure event-driven design. }
function TCustomSyncWSocket.Synchronize(Proc: TWSocketSyncNextProc; var DoneFlag: Boolean) : Integer;
begin
    DoneFlag := FALSE;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{ DEPRECATED: DO NOT USE ReadLine procedure for a new application. Instead, use pure event-driven design using OnDataAvailable event.   }
 { timeout is seconds if positive, milli-seconds if negative }
procedure TCustomSyncWSocket.ReadLine(Timeout: Integer;  var Buffer: AnsiString);
var
...
begin
    ...
    { Positive timeout means seconds. Negative means milli-seconds, Null means 60 seconds. }
    if TimeOut = 0 then
        FTimeOut := 60000
    ...
    try
        ...
        if Status = WSA_WSOCKET_TIMEOUT then begin
             { Sender didn't send line end within allowed time. Get all data available so far.  }
        ...
        { Should I raise an exception to tell the application that some error occured ?  }
    finally
...
begin
    inherited;
{$IFDEF ICS_NewMessaging}   { V10 }
    FTimeoutSampling := IcsAsyncDftTimeoutSamplingInterval;
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomTimeoutWSocket.TimeoutHandleTimer(Sender: TObject);
begin
    if (FTimeoutConnect > 0) and (GetState <> wsConnected) then begin
...
{$IFDEF ICS_NewMessaging}   { V10 }
        FTimeoutTimer := TIcsTimer.Create(Self);
        FTimeoutTimer.SynchronizeEvent := TRUE;
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomTimeoutWSocket.TriggerSessionConnectedSpecial(Error: Word);
begin
...
begin
    if Assigned(FBandwidthTimer) and (IcsGetCurrentThreadID = FThreadID) then begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomThrottledWSocket.SetBandwidthKeepThreadAlive(const Value: Boolean);
begin
...
{$IFDEF ICS_NewMessaging}   { V10 }
            ...
            FBandwidthTimer.SynchronizeEvent := TRUE;
{$ELSE}
...
{$ENDIF ICS_NewMessaging}   { V10 }
    ...
    {$IFNDEF NO_DEBUG_LOG}
        if CheckLogOptions(loWsockInfo) then
            DebugLog(loWsockInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' Bandwidth ON handle=' + IntToStr(GetHSocket));
    {$ENDIF}
    end
    else begin
        if Assigned(FBandwidthTimer) then begin
            ...
            {$IFNDEF NO_DEBUG_LOG}
                if CheckLogOptions(loWsockInfo) then
                    DebugLog(loWsockInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' Bandwidth OFF handle=' + IntToStr(GetHSocket));
            {$ENDIF}
            end;
        end;
    end;
...
begin
    if Value <> FBandwidthLimit then begin
        ...
        if Assigned(FBandwidthTimer) then
            SetBandwidthControl;  { only if done this already, to change it }
    end;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TCustomThrottledWSocket.RealSend(var Data: TWSocketData; Len: Integer): Integer;
begin
    if not FBandwidthEnabled then
        Result := inherited RealSend(Data, Len)
    else begin
        { Try to adjust amount of data actually passed to winsock }
        if (Len > 0) and (FBandwidthCount < FBandwidthMaxCount) and (FBandwidthCount + Cardinal(Len) > FBandwidthMaxCount) then
        ...
        if (Result > 0) then begin
            ...
            if (FBandwidthCount > FBandwidthMaxCount) and (not FBandwidthPaused) then begin
                FBandwidthPaused := TRUE;
                Pause;
           {$IFNDEF NO_DEBUG_LOG}
                if CheckLogOptions(loWsockInfo) then
                    DebugLog(loWsockInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' Bandwidth Paused on send handle=' + IntToStr(GetHSocket));
           {$ENDIF}
           end;
       end;
    end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TCustomThrottledWSocket.Receive(Buffer: TWSocketData; BufferSize: Integer): Integer;
begin
    { The Receive throttle does not work if FD_CLOSE message has been received  yet since handler Do_FD_CLOSE removes option wsoNoReceiveLoop.  }
    if (not FBandwidthEnabled) or not HasOption(FComponentOptions, wsoNoReceiveLoop) then    { V8.70 }
        ...
        if (Result > 0) then begin
            ...
            {$IFNDEF NO_DEBUG_LOG}
                if CheckLogOptions(loWsockInfo) then
                    DebugLog(loWsockInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' Bandwidth Paused on receive handle=' + IntToStr(GetHSocket));
            {$ENDIF}
            end;
        end;
    end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomThrottledWSocket.BandwidthHandleTimer(Sender: TObject);
begin
    if FBandwidthPaused then begin
        ...
        {$IFNDEF NO_DEBUG_LOG}
            if CheckLogOptions(loWsockInfo) then
                DebugLog(loWsockInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' Bandwidth Resume ' + IntToStr(GetHSocket));
        {$ENDIF}
            Resume;
        end;
    end
...
begin
    { Turn on the throttle early, inherited TriggerSessionConnectedSpecial might already process the first data chunk. }
    if (Error = 0) then
        SetBandwidthControl;
    inherited TriggerSessionConnectedSpecial(Error);
...
{$ENDIF}
             ...
             {   if Ok <> 0 then
                ...
                if Obj.GetHSocket = INVALID_SOCKET then
{$IFDEF ICS_NewMessaging}   { V10 }
                    Obj.PostMessage(Obj, WM_ICS_WSSSL_RESET , 0, 0);
{$ELSE}
            ...
            try
                if Assigned(Obj.FOnSslCliCertRequest) then begin
                    ...
                    try
                        ...
                        if (Cert <> nil) and (Cert.X509 <> nil) and (Cert.PrivateKey <> nil) then begin
      ...
      { V8.55 client now handled here as well }
            ...
            {$IFNDEF NO_DEBUG_LOG}
                    if Obj.CheckLogOptions(loSslInfo) then
                        Obj.DebugLog(loSslInfo, IntToHex(INT_PTR(Obj), SizeOf(Pointer) * 2) + ' CliNewSessionCB [' +
        ...
        { all this stuff is debug logging, not necessary for normal operation }
            if Obj.CheckLogOptions(loSslErr) or
               Obj.CheckLogOptions(loSslInfo) then begin
                ...
                if ((Where and SSL_CB_LOOP) <> 0) then begin
                    if Obj.CheckLogOptions(loSslInfo) then    { V8.55 was SslDevel, really Errs }
                        Obj.DebugLog(loSslInfo, Pre + Str +  String(SSL_state_string_long(ssl)));
                end
                    ...
                    if (Where and SSL_CB_READ) <> 0 then
                        ...
                        Str := 'write ';
                    Obj.DebugLog(loSslInfo, Pre + 'SSL3 alert ' + Str +  String(SSL_alert_type_string_long(ret)) + ' ' +
             ...
             //         if Obj.CheckLogOptions(loSslErr) then   { V8.59 was Devel, Err and Info }
                            Obj.DebugLog(loSslErr, Pre + Str + 'failed in ' +  String(SSL_state_string_long(ssl)));
                    end
                    else if Ret < 0 then begin
                        Err := SSL_get_error(ssl, Ret);
                        if NOT ((Err = SSL_ERROR_WANT_READ) or  (Err = SSL_ERROR_WANT_WRITE)) then begin
                            Obj.DebugLog(loSslErr, Pre + Str + 'error ' + IntToStr (Err) + ' in ' + String(SSL_state_string_long(ssl)));
                        end;
                    end;
                end
                else begin
                    if Obj.CheckLogOptions(loSslInfo) then     { V8.59 really meant Info }
                        Obj.DebugLog(loSslDevel, Pre + Str + 'where=' + IntToHex(where, 8) + ', state=' + String(SSL_state_string_long(ssl))); { V8.53 added state }
                end;
            end;
{$ENDIF}
         { OpenSSL InfoCallback is when state changes }
           ...
           { V8.53 TLSv1.3 does not have renegotiation so skip these checks, V8.66 no longer supporting renegotiation }
            end
          { V8.55 TLSv1.3 comes here too often due to tickets, FHandshakeEventDone is  used to make sure the event is only called once }
...
{$IFNDEF NO_DEBUG_LOG}
                if Obj.CheckLogOptions(loSslErr) or Obj.CheckLogOptions(loSslInfo) then begin
                    ...
                    if Obj.CheckLogOptions(loSslInfo) or (Err <> X509_V_OK) then
                       Obj.DebugLog(loSslErr, Pre + 'SSL_CB_HANDSHAKE_DONE, Error ' + IcsX509VerifyErrorToStr (Err));   { V8.14 real literal, V8.39 better function }
                end;
{$ENDIF}
            end
        finally
        ...
        try
         { V8.64 if result has ACE xn--. convert it to Unicode, ignore errors }
            ...
            begin
                ...
                begin
                    { Clear the options inherited from current Ctx. Not sure whether it is required, shouldn't hurt. }
            ...
            Ws.FSsl_In_CB := FALSE;
            if Ws.GetHSocket = INVALID_SOCKET then
{$IFDEF ICS_NewMessaging}   { V10 }
                    Ws.PostMessage(Ws, WM_ICS_WSSSL_RESET , 0, 0);
{$ELSE}
...
{ client sends us a list of protocols it supports, and we can select one or ignore them all, ie spdy/1, http/1.1, h2 (http/2). pop3, acme-tls/1, etc }
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{ V8.64 new callback in OpenSSL 1.1.1 that partially replaces ServerNameCallback and ALPN callback and has more info }
    ...
    try
        if (Ws.FSslServerName <> '') then begin
            ...
            { Do not switch context if not initialized }
                ...
                Ws.FSsl_In_CB := FALSE;
                if Ws.GetHSocket = INVALID_SOCKET then begin
{$IFDEF ICS_NewMessaging}   { V10 }
                    Ws.PostMessage(Ws, WM_ICS_WSSSL_RESET , 0, 0);
{$ELSE}
                    PostMessage(Ws.FWindowHandle, Ws.FMsg_WM_RESET_SSL, 0, 0);
...
{$IFNDEF NO_DEBUG_LOG}
    ...
    Inc(TraceCount);
        DebugLog(loSslDevel, Format('%s BIO_ctrl_pending(%s) = %d   [%d]',  [IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2),
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then begin  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
        Inc(TraceCount);
        DebugLog(loSslDevel, Format('%s BIO_read(%s, 0x%x, %d) = %d   [%d]', [IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2),
                                                                     GetMyBioName(b), INT_PTR(Buf), Len, Result, TraceCount]));
    end
    else if CheckLogOptions(loSslDump) then begin
        Inc(TraceCount);
        DebugLog(loSslDump, Format('%s BIO_read(%s, 0x%x, %d) = %d   [%d] Data:%s', [IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2),
                                             GetMyBioName(b), INT_PTR(Buf), Len, Result, TraceCount, DataToString(Buf, Result)]));
    end;
{$ENDIF}
end;
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then begin  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
        Inc(TraceCount);
        DebugLog(loSslDevel, Format('%s BIO_ctrl(%s, %s, %s, 0x%x) = %d   [%d]', [IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2),
                             GetMyBioName(bp), CmdName, LArgName, INT_PTR(PArg), Result, TraceCount]));
    end;
{$ENDIF}
end;
...
{$IFNDEF NO_DEBUG_LOG}
    ...
    Inc(TraceCount);
        DebugLog(loSslDevel, Format('%s BIO_ctrl_get_write_guarantee(%s) = %d   [%d]',  [IntToHex(INT_PTR(Self),
                                                            SizeOf(Pointer) * 2), GetMyBioName(b), Result, TraceCount]));
    end;
{$ENDIF}
end;
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then begin  { V5.21 }
        Inc(TraceCount);
        DebugLog(loSslDevel, Format('%s BIO_ctrl_get_read_request(%s) = %d   [%d]', [IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2),
                                                                                             GetMyBioName(b), Result, TraceCount]));
    end;
{$ENDIF}
end;
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then begin  { V5.21 }
        Inc(TraceCount);
        DebugLog(loSslDevel, Format('%s BIO_write(%s, 0x%x, %d) = %d   [%d]', [IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2),
                                                                      GetMyBioName(b), INT_PTR(Buf),  Len, Result, TraceCount]));
    end
    else if CheckLogOptions(loSslDump) then begin
        Inc(TraceCount);
        DebugLog(loSslDump, Format('%s BIO_write(%s, 0x%x, %d) = %d   [%d] Data:%s', [IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2),
                                               GetMyBioName(b), INT_PTR(Buf), Len, Result, TraceCount, DataToString(Buf, Result)]));
    end;
{$ENDIF}
end;
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslErr) or CheckLogOptions(loSslInfo) then begin     { V8.55 }
        Inc(TraceCount);
        DebugLog(loSslErr, Format('%s HandleSslError handle=%d  [%d] %s', [IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2),
                                                                                 GetHSocket, TraceCount, FSslHandshakeRespMsg]));
    end
    else
        ERR_clear_error;
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TCustomSslWSocket.my_WSocket_recv(s: TSocket; var Buf: TWSocketData; len, flags: Integer): Integer;
begin
    Result := WSocket_recv(s, Buf, Len, Flags);
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then begin  { V5.21 }
        Inc(TraceCount);
        DebugLog(loSslDevel, Format('%s Winsock recv( %d, 0x%x, %d, %d) = %d   [%d]', [IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2),
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then begin  { V5.21 }
        Inc(TraceCount);
        DebugLog(loSslDevel, Format('%s my_RealSend (0x%x, %d, %d) = %d   [%d]', [IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2),
                                                                                GetHSocket, INT_PTR(Buf), Len, Result, TraceCount]));
    end;
{$ENDIF}
end;
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then begin  { V5.21 }
        Inc(TraceCount);
        DebugLog(loSslDevel, Format('%s BIO_should_retry(%s) = %d   [%d]', [IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2),
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
        DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' Do_FD_ACCEPT handle=' + IntToStr(GetHSocket));
{$ENDIF}
...
var
    SslStOk : Boolean;
begin
    if (not FSslEnable) or (FSocksState <> socksData) or (FHttpTunnelState <> htsData) or (not Assigned(FSsl)) then begin
...
{$IFNDEF NO_DEBUG_LOG}
    ...
    if CheckLogOptions(loSslInfo) then  { V5.21 }
        DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' TCustomSslWSocket.Do_FD_CLOSE error #' +
          IntToStr(msg.LParamHi) + ', handle=' + IntToStr(GetHSocket) + ', SslIntShutDown=' + IntToStr(FSslIntShutDown) +
          ', SocketState=' + SocketStateNames[GetState] + ', DataPending=' + IntToStr(Ord(SocketDataPending)) +
...
{$ENDIF}
    if (GetState = wsConnecting) or (GetHSocket = INVALID_SOCKET) then
        Exit;
...
{$IFNDEF NO_DEBUG_LOG}
        if CheckLogOptions(loSslInfo) then
            DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) +
                     ' TCustomSslWSocket.Do_FD_CLOSE CloseCalled handle=' + IntToStr(GetHSocket) +    { V9.4 }
...
{$ENDIF}
    ...
    if FNetworkError = 0 then begin
        { The connection was closed , we need to read as much as we can as well as process data pending in the SslBio }
        if (SslStOk and (my_BIO_ctrl_pending(FSslbio) > 0)) and (FSslIntShutDown < 2) then begin  { V9.4 not if stutdown started }
          ...
          {$IFDEF POSIX}
            { We'll receive a FD_READ message in Windows only! in POSIX we have to trigger a read event explicitly }
    ...
    if (not SslStOk) and (not (csDestroying in ComponentState)) then begin         // AG 03/03/06
        ...
        if (GetState = wsConnected) and (FSslIntShutDown < 2) and (msg.LParamHi = 0) then begin
...
{$ENDIF}
    if (GetHSocket <> INVALID_SOCKET) and (not FCloseInvoked) and (not (csDestroying in ComponentState)) then begin   // AG 03/03/06
        FCloseInvoked := TRUE;
...
begin
    ...
    if not (Event in FPendingSslEvents) then begin
        case Event of
            sslFdRead  :  Result := PostMessage(Handle, FMsg_WM_SSL_ASYNCSELECT, WParam(GetHSocket), IcsMakeLong(FD_READ, ErrCode));  { V8.08 }
            sslFdWrite :  Result := PostMessage(Handle, FMsg_WM_SSL_ASYNCSELECT, WParam(GetHSocket), IcsMakeLong(FD_WRITE, ErrCode)); { V8.08 }
            sslFdClose :  Result := PostMessage(Handle, FMsg_WM_SSL_ASYNCSELECT, WParam(GetHSocket), IcsMakeLong(FD_CLOSE, ErrCode)); { V8.08 }
        end;
        if Result then
...
{$IFNDEF NO_DEBUG_LOG}
    if Result and CheckLogOptions(loSslDevel) then begin
        ...
        DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' TriggerEvent handle=' + S + IntToStr(GetHSocket));
    end;
{$ENDIF}
end;
...
var
    ...
    Buffer     : array of AnsiChar;  { V8.27 size now configurable }
    ...
    Dummy      : Byte;
begin
    if (not FSslEnable) or (FSocksState <> socksData) or  (FHttpTunnelState <> htsData) then begin { V8.42 don't stop reading for non-SSL }
 ...
 { stop read windows events until we've processed this block }
    WSocket_Synchronized_WSAASyncSelect({$IFDEF POSIX}Self,{$ENDIF} GetHSocket, Handle, FMsg_WM_ASYNCSELECT, FD_WRITE or FD_CLOSE or FD_CONNECT);
    try
      {$IFNDEF NO_DEBUG_LOG}
        if CheckLogOptions(loSslDevel) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
            DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' TCustomSslWSocket.Do_FD_READ handle=' + IntToStr(GetHSocket));
      {$ENDIF}

        if (FNetworkError > 0) then
            Exit;

        if (SSL_get_state(FSsl) = TLS_ST_OK){ V8.27 } and (FSslBioWritePendingBytes < 0) and (my_BIO_ctrl_pending(FSslbio) > 0) then begin
          {$IFNDEF NO_DEBUG_LOG}
            if CheckLogOptions(loSslDevel) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
                DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' TriggerDataAvailable (Do_FD_READ_1) handle=' + IntToStr(GetHSocket));
          {$ENDIF}
            TriggerDataAvailable(0);
            Exit;
        end;
        ...
        // Receive data
        PBuf := @Buffer[0];
        NumRead := my_WSocket_recv(GetHSocket, PBuf, Len, 0);
                      ...
                      {$IFNDEF NO_DEBUG_LOG}
                        if CheckLogOptions(loSslErr) then  { V5.21 }
                            DebugLog(loSslErr, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) +
                              ' NetworkError #' + IntToStr(FNetworkError) + ' TCustomSslWSocket.Do_FD_READ handle=' + IntToStr(GetHSocket));  { V8.70 added more }
                      {$ENDIF}
                        Exit;
                    end
                    else begin
        ...
        if Numread = 0 then begin
            ...
            if (nError > WSABASEERR) and (nError <> WSAEWOULDBLOCK) and (nError <> WSAENOTCONN) and (nError <> WSAECONNABORTED) then begin   { V8.70 ignore Aborted error }
              ...
              {$IFNDEF NO_DEBUG_LOG}
                if CheckLogOptions(loSslErr) then  { V5.21 }
                    DebugLog(loSslErr, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' NetworkError #' + IntToStr(FNetworkError) +
                                                                     ' TCustomSslWSocket.Do_FD_READ_2 handle=' + IntToStr(GetHSocket));  { V8.70 added more }
              {$ENDIF}
                Exit;
            end;
        end;

        if (SSL_get_state(FSsl) = TLS_ST_OK) and (FSslBioWritePendingBytes < 0) and (my_BIO_ctrl_pending(FSslbio) > 0) then begin    { V8.27, V8.66 }
          {$IFNDEF NO_DEBUG_LOG}
            if CheckLogOptions(loSslDevel) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
                DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' TriggerDataAvailable (Do_FD_READ_2) handle=' + IntToStr(GetHSocket));
          {$ENDIF}
            TriggerDataAvailable(0);
        end;
       ...
       { V8.22 moved here from Do_SSL_FD_READ }
        WSocket_Synchronized_WSAASyncSelect({$IFDEF POSIX}Self,{$ENDIF} GetHSocket, Handle, FMsg_WM_ASYNCSELECT, FD_READ or FD_WRITE or FD_CLOSE or FD_CONNECT);
    end;
end;
...
var
    ...
    Err        : Cardinal;
begin
    if (not FSslEnable) or (FSocksState <> socksData) or (FHttpTunnelState <> htsData) then begin
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
        DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' TCustomSslWSocket.Do_FD_WRITE handle=' + IntToStr(GetHSocket));
{$ENDIF}
    if (FNetworkError > 0) then
        Exit;
...
{$IFNDEF NO_DEBUG_LOG}
        if CheckLogOptions(loSslDevel) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
            DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' INVALID_SOCKET');
{$ENDIF}
        Exit;
    end
...
{$IFNDEF NO_DEBUG_LOG}
        if CheckLogOptions(loSslDevel) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
            DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' * Not bAllSent handle=' + IntToStr(GetHSocket));
{$ENDIF}
        FMayTriggerFD_Write := TRUE;
    ...
    if Len > BuffSize then
        Len := BuffSize;            { V8.27 sanity check }
    ...
    while (NumRead > 0) do begin
        ...
        if (NumSent = SOCKET_ERROR) or (NumSent < NumRead) then begin
            if NumSent = SOCKET_ERROR then begin
                ...
                if (Err > WSABASEERR) and (Err <> WSAEWOULDBLOCK) and (Err <> WSAENOTCONN) then begin
                    FNetworkError := Err;
                    FLastError    := Err; //XX
                    TriggerEvent(sslFdClose, 0);
{$IFNDEF NO_DEBUG_LOG}
                    if CheckLogOptions(loSslErr) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
                        DebugLog(loSslErr, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' Winsock Error ' +
                                                        WSocketErrorDesc(FNetworkError) + ' handle=' + IntToStr(GetHSocket));
{$ENDIF}
                    Exit;
                end
                else
...
var
    Count : u_long;
begin
    FLastError := WSocket_Synchronized_ioctlsocket(GetHSocket, FIONREAD, Count);
    Result := Count > 0;
    if FLastError = SOCKET_ERROR then begin
        FLastError := WSocket_WSAGetLastError;
        if (FLastError > WSABASEERR) and (FLastError <> WSAEWOULDBLOCK) and (FLastError <> WSAENOTCONN) then
        else
            FLastError := 0;
    end;
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then
        DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' Socket data pending: ' + IntToStr(Count) + ' Err: ' +
                                                                              IntToStr(FLastError) + ' handle=' + IntToStr(GetHSocket));
{$ENDIF}
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TCustomSslWSocket.DoRecv(var Buffer: TWSocketData; BufferSize: Integer; Flags: Integer): Integer;
var
    Numread : Integer;
begin
    if (not FSslEnable) or (FSocksState <> socksData) or (FHttpTunnelState <> htsData) then begin
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
        DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' TCustomSslWSocket.DoRecv handle=' + IntToStr(GetHSocket));
{$ENDIF}
    if FNetworkError > 0 then begin
        if (my_BIO_ctrl_pending(FSslbio) > 0) and (FSslIntShutDown = 0) then begin
    ...
    if (SSL_get_state(FSsl) <> TLS_ST_OK) or (my_BIO_ctrl_pending(FSslbio) = 0) then begin { V8.27, V8.66 }
        if GetState = wsclosed then begin
            Result := 0;
            Exit;
        end;
...
{$ENDIF ICS_NewMessaging}   { V10 }
        ...
        else if GetState <> wsconnected then begin
            WSocket_WSASetLastError(FNetworkError);
            FLastError := FNetworkError; //XX
            Result := SOCKET_ERROR;
            Exit;
    ...
    if Numread = 0 then begin
        if (FSslVersNum >= SSL3_VERSION) and (SSL_get_error(FSsl, Numread) = SSL_ERROR_ZERO_RETURN) then begin
            { SSL closure alert received }
            if FSslState < sslInShutdown then begin
                FSslState := sslInShutdown;   { V7.80 }
                if (not FSslBiShutDownFlag) then begin // May be set later in the message handler if a SSL bi-shutdown message is pending
...
{$IFNDEF NO_DEBUG_LOG}
            if CheckLogOptions(loSslErr) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
                DebugLog(loSslErr, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' NetworkError #' + IntToStr(FNetworkError) +
                                                     ' TCustomSslWSocket.DoRecv handle=' + IntToStr(GetHSocket));  { V8.70 added more }
{$ENDIF}
            Exit;
        end
        else begin
...
begin
    ...
    if (not FSslEnable) or (FSocksState <> socksData) or  (FHttpTunnelState <> htsData) then
        Result := inherited GetRcvdCount
    else
...
{$IFNDEF NO_DEBUG_LOG}
        if CheckLogOptions(loSslInfo) then
            DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' SocketCloseCalled handle=' + IntToStr(GetHSocket));
{$ENDIF}
    end;

    if FSslEnable and Assigned(FSsl) and (SSL_get_state(FSsl) = TLS_ST_OK) and (my_BIO_ctrl_pending(FSslbio) > 0) then  { V8.27, V8.66 }
        TriggerEvents
    else begin
        inherited Close;
    end;
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslInfo) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
        DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' TCustomSslWSocket.ShutDown ' + IntToStr(How) + ' handle=' + IntToStr(GetHSocket));
{$ENDIF}
    FShutDownHow       := How;
    FSslBiShutDownFlag := FALSE;
...
{   Manifest constants for Shutdown
    ...
    try
        if (FNetworkError = 0) and ((FSslIntShutDown = 0) or (not bAllSent) {or (not bSslAllSent)}) then
            Exit;

        { A bi-directional SSL shutdown w/o socket  close. We need to receive peer's shutdown notification before
          the SSL can be killed and communication may continue in plaintext data.                 }
        if FSslBiShutDownFlag and (FNetworkError = 0) then begin
            NumRead := SSL_get_shutdown(FSsl);
            if (NumRead and SSL_RECEIVED_SHUTDOWN = 0) or (NumRead and SSL_SENT_SHUTDOWN = 0) then
                Exit;
        end;
...
{$IFNDEF NO_DEBUG_LOG}
        if CheckLogOptions(loSslInfo) then
            DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' SslShutdownCompleted *'+ IntToStr(Ord(Result)) + '* handle=' + IntToStr(GetHSocket));
{$ENDIF}
    end;
end;
...
{   Manifest constants for Shutdown
    ...
    if (GetHSocket = INVALID_SOCKET) or (not FSslEnable) or (not Assigned(FSsl) or (FSslState = sslShutdownComplete)) then begin
        Exit;
    end;
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslInfo) then
        DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' SslInternalShutdown handle=' + IntToStr(GetHSocket));
{$ENDIF}
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslInfo) then
        DebugLog(loSslInfo, Format('%s TriggerSslShutDownComplete(%d) %d', [IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2), ErrCode, GetHSocket]));
{$ENDIF}
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslInfo) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
        DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' Dup accepting accepted socket = ' + IntToStr(NewHSocket));
{$ENDIF}
...
begin
    ...
    if Ws.CheckLogOptions(loSslInfo) then begin
        Ws.DebugLog(loSslInfo, Format('ProtoMsg: %s, DataLen: %d, Data= %s', [info, len, IcsBufferToHex (buf[0], len)]));
    end;
...
procedure TlsExtension_cb(SSL: PSSL; client_server: Integer; type_: Integer; data: PAnsiChar; len: Integer; arg: Pointer); cdecl;
var
    ExtName : String;
    CS : String;
    Ws : TCustomSslWSocket;
begin
    ...
    if client_server = 0 then
        ...
        CS := 'server';
    if Ws.CheckLogOptions(loSslInfo) then
         Ws.DebugLog(loSslInfo, Format('TLSExtCB> TLS %s extension "%s" (id=%d), len=%d', [CS, ExtName, Type_, len]));
end;
{$ENDIF}
...
{ Supported=True returns only ciphers acceptable according to protocol and settings, otherwise it's complete list available
  Remote=True for server client connections gets list sent by remote client, otherwise it's the list the client or servers
  supports according to protocol settings }
...
begin
    ...
    if (NOT Assigned(FSsl)) then begin
        if NOT Assigned(FSslContext.SslCtxPtr) then
            Exit;
    ...
    if Supported and (NOT Remote) then
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslInfo) then  { V5.21 }
        DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' StartSslHandshake handle=' + IntToStr(GetHSocket));
{$ENDIF}
...
{$IFNDEF NO_DEBUG_LOG}
            if CheckLogOptions(loSslErr) then  { V8.57 was loSslInfo }
                DebugLog(loSslErr, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) +' Fatal error StartSslHandshake handle=' +
                                                                        IntToStr(GetHSocket) + ' ' + E.Classname + ' ' + E.Message);
{$ENDIF}
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslInfo) then  { V5.21 }
        DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' AcceptSslHandshake handle=' + IntToStr(GetHSocket));
{$ENDIF}
    if not FSslEnable then
        Exit;
    if not Assigned(FSslContext) then
...
{$IFNDEF NO_DEBUG_LOG}
            if CheckLogOptions(loSslErr) then  { V8.57 was loSslInfo }
                DebugLog(loSslErr, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' Fatal error AcceptSslHandshake handle=' +
                                                                       IntToStr(GetHSocket) + ' ' + E.Classname + ': ' + E.Message);
{$ENDIF}
            if NOT FHandshakeEventDone then
                TriggerSslHandshakeDone(1);  { V8.55 handshake failed during negotiation }
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslInfo) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
        DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' DupConnected');
{$ENDIF}
end;
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslInfo) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
        DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' SslInternalClose handle=' + IntToStr(GetHSocket));
{$ENDIF}
    if GetHSocket = INVALID_SOCKET then begin
        if GetState <> wsClosed then begin
            ChangeState(wsClosed);
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslInfo) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
        DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' Listening');
{$ENDIF}
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomSslWSocket.TriggerSslVerifyPeer(var Ok: Integer; Cert: TX509Base);
begin
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslInfo) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslInfo) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
        DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' InitSSLConnection handle=' + IntToStr(GetHSocket));
{$ENDIF}
    FSslCertChain.Clear;
...
{$ENDIF}
                ...
                if Assigned(FOnSslSetSessionIDContext) then begin
                    ...
                    {$IFNDEF NO_DEBUG_LOG}
                            if CheckLogOptions(loSslErr) then { V5.21 }
                                DebugLog(loSslErr, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) +
                                                      ' ssl_set_session_id_context failed handle=' + IntToStr(GetHSocket));
                        end
                        else begin
                            if CheckLogOptions(loSslInfo) then
                                DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) +
                                                  ' SessionIDContext: ' + SessionIDContext + ' handle=' + IntToStr(GetHSocket));
                    {$ENDIF}
                        end;
                    end;
                end;
              ...
              { sets and starts server handshake, eventually }
...
{$ENDIF}
            ...
            if Count <= 0 then begin
                if not my_BIO_should_retry(FSslbio) then begin
                    { Usually happens with an invalid context option set, V8.55 such as no ciphers available, tell someone!! }
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then begin  { V5.21 }
        Inc(TraceCount);
        DebugLog(loSslDevel, Format('%s PutDataInSslBuffer handle= %s len %d  [%d] ', [IntToHex(INT_PTR(Self),
                                                                SizeOf(Pointer) * 2), IntToStr(GetHSocket), Len, TraceCount]));
    end
    else if CheckLogOptions(loSslDump) then begin  { V5.21 }
        Inc(TraceCount);
        DebugLog(loSslDump, Format('%s PutDataInSslBuffer handle=%s [%d] Data:%s', [IntToHex(INT_PTR(Self),
                                          SizeOf(Pointer) * 2), IntToStr(GetHSocket), TraceCount, DataToString(Data, Len)]));
    end;
{$ENDIF}
    if (Len <= 0) or (Data = nil) then
        Exit;
...
begin
{$IFDEF ICS_NewMessaging}   { V10 }
     TCustomSslWSocket.PostMessage(Self, WM_ICS_WSSSL_RESET , 0, 0);
{$ELSE}
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslInfo) then  { V5.21 }    { V8.70 even if no FSsl }
        DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' ResetSslSession starting handle=' + IntToStr(GetHSocket));
{$ENDIF}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomSslWSocket.RaiseLastOpenSslError(EClass: ExceptClass; Dump: Boolean = FALSE; const CustomMsg: String  = '');
begin
    FLastSslError := ERR_peek_error;
...
{$IFNDEF NO_DEBUG_LOG}
        if CheckLogOptions(loSslDevel) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
            DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' TryToSend handle=' + IntToStr(GetHSocket));
{$ENDIF}
        inherited TryToSend;
        Exit;
    end;
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
        DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' SslTryToSend handle=' + IntToStr(GetHSocket));
{$ENDIF}

{$IFNDEF ICS_NewMessaging}   { V10 }
  if FThreadId <> IcsGetCurrentThreadId then begin                                            { TIN }
...
{$ENDIF N ICS_NewMessaging}   { V10 }
    ...
    FSslBufList.Lock;
    try
        if (GetHSocket = INVALID_SOCKET) or (FSslBufList.IsEmpty) then begin
            bSslAllSent := TRUE;
            Exit;
        end;
        ...
        while TRUE do begin
            ...
            if (GetHSocket = INVALID_SOCKET) or                { No more socket  }
...
{$ENDIF ICS_NewMessaging}   { V10 }
            ...
            { We don't trigger any error so far, just ignoring user data to be sent. We could close at once with WSAESHUTDOWN ?     }
{$IFDEF ICS_NewMessaging}   { V10 }
                   // pending !!!
{$ELSE}
                TriggerEvent(sslFdClose, 0);
...
{$ENDIF ICS_NewMessaging}   { V10 }
            ...
            if (not Assigned(FSsl)) or (SSL_get_state(FSsl) <> TLS_ST_OK) or (SSL_renegotiate_pending(FSsl) = 1) then begin     { V8.27, V8.66 }
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
class procedure TCustomSslWSocket.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    WSocket : TCustomSslWSocket;
begin
    if Assigned(Value.Instance) and (Value.Instance is TCustomSslWSocket) then begin
        WSocket := Value.Instance as TCustomSslWSocket;
        try
            if Value.Msg = WM_ICS_WSSSL_ASYNCSELECT then
                WSocket.WMSslASyncSelect(Value)
            else if Value.Msg = WM_ICS_WS_TRGR_DATASENT then
                WSocket.TriggerDataSent(0)
            else if Value.Msg = WM_ICS_WSSSL_RESET then
                WSocket.ResetSsl
            else if Value.Msg = WM_ICS_WSSSL_BI_SHUTDOWN then
                WSocket.WMSslBiShutDown(Value)
            else if Value.Msg = WM_ICS_WSSSSL_SHUTDOWN_DONE then
                WSocket.WMTriggerSslShutDownComplete(Value);
        except
            on E:Exception do begin
                if Assigned(WSocket) then
                  WSocket.HandleBackGroundException(E, 'TSslWocket:DispatchMessage: ' + IntToStr(Value.Msg));
            end;
        end;
    end
    else
...
begin
    Result := FALSE;
    if (not FSslEnable) or FPaused then
        Exit;
    { Returns TRUE if a message was posted successfully and the socket isn't paused }
    if not (Event in FPendingSslEvents) then begin
        case Event of
            sslFdRead:   Result := TCustomSslWSocket.PostMessage(Self, WM_ICS_WSSSL_ASYNCSELECT, WParam(HSocket), IcsMakeLong(FD_READ, ErrCode));
            sslFdWrite:  Result := TCustomSslWSocket.PostMessage(Self, WM_ICS_WSSSL_ASYNCSELECT, WParam(HSocket), IcsMakeLong(FD_WRITE, ErrCode));
            sslFdClose:  Result := TCustomSslWSocket.PostMessage(Self, WM_ICS_WSSSL_ASYNCSELECT, WParam(HSocket), IcsMakeLong(FD_CLOSE, ErrCode));
        end;
        if Result then
            FPendingSslEvents := FPendingSslEvents + [Event];
    end;
{$IFNDEF NO_DEBUG_LOG}
    if Result and CheckLogOptions(loSslDevel) then begin
        ...
        DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' TriggerEvent handle=' + S + IntToStr(HSocket));
    end;
{$ENDIF}
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomSslWSocket.WMSslBiShutDown(const Value: TIcsSocketMsgRec);
begin
    if FSslEnable and (FSslIntShutDown = 0) then begin
        FSslBiShutDownFlag := (Value.LParam = 0);
        InternalShutdown(Value.WParam);
    end;
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomSslWSocket.WMSslASyncSelect(const Value: TIcsSocketMsgRec);
begin
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomSslWSocket.WMTriggerSslShutDownComplete(const Value: TIcsSocketMsgRec);
begin
    TriggerSslShutDownComplete(Value.LParam);
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomSslWSocket.Do_FD_CLOSE(ErrCode: Word);
var
    SslStOk : Boolean;
begin
    if (not FSslEnable) or (FSocksState <> socksData)  or (not Assigned(FSsl)) then begin
        inherited Do_FD_CLOSE(ErrCode);
        Exit;
    end;
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslInfo) then
        DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' TIcsCustomSslWSocket.SessionClosedEventDetected error #' +
                                                                                    IntToStr(ErrCode) + ' handle=' + IntToStr(GetHSocket));
{$ENDIF}
    if (State = wsConnecting) or (HSocket = INVALID_SOCKET) then
        Exit;
...
{$IFNDEF NO_DEBUG_LOG}
        if CheckLogOptions(loSslInfo) then
            DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' *CloseCalled handle=' + IntToStr(HSocket) +  ', State=' +
              String(SSL_state_string_long(Fssl)) +  ' (' + GetEnumName(TypeInfo(OSSL_HANDSHAKE_STATE), Ord(SSL_get_state(FSsl))) +
                                                                                               '), Err=' + OpenSslErrMsg(FLastSslError));
{$ENDIF}
    end;
    if FNetworkError = 0 then begin { The connection was closed , we need to read as much as we can as well as process data pending in the SslBio }
    ...
    if FNetworkError > 0 then begin
        if (ErrCode = 0) then
            ErrCode := FNetworkError;
    end;

    if (not SslStOk) and (not (csDestroying in ComponentState)) then begin
        if NOT FHandshakeEventDone then
            TriggerSslHandshakeDone(1);  // error !!!
        FHandshakeEventDone := TRUE;
        if (State = wsConnected) and (FSslIntShutDown < 2) and (ErrCode = 0) then begin
            inherited ShutDown(1);
        end;
    end;
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslInfo) then
        DebugLog(loSslInfo, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' FCloseInvoked=' + IntToStr(Ord(FCloseInvoked)) + ' handle=' +
                                                            IntToStr(HSocket) + ', State=' + String(SSL_state_string_long(Fssl))); { V8.27 }
{$ENDIF}
    if (HSocket <> INVALID_SOCKET) and (not FCloseInvoked) and (not (csDestroying in ComponentState)) then begin
        FCloseInvoked := TRUE;
    ...
    if State <> wsClosed then begin
        inherited InternalClose(FALSE, ErrCode);  // close the socket
    end;
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomSslWSocket.Do_FD_CONNECT;
begin
    FCloseCalled := FALSE;
    FSslIntShutDown := 0;
    inherited Do_FD_CONNECT;
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomSslWSocket.Do_FD_ACCEPT;
begin
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then
        DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' Do_FD_ACCEPT handle=' + IntToStr(HSocket));
{$ENDIF}
    inherited Do_FD_ACCEPT;
end;
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then begin
        Str := IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' TriggerEvents handle=' + IntToStr(HSocket) + ' State=' +
                                                                                          String(SSL_state_string_long(FSsl));  { V8.27  }

        DebugLog(loSslDevel, Str + ' // MayFD_Read=' + BoolToStr(FMayTriggerFD_Read, True) + ' MayDoRecv=' +
           BoolToStr(FMayTriggerDoRecv, True) + ' MayFD_Write=' + BoolToStr(FMayTriggerFD_Write, True) + ' MaySslTryToSend=' +
           BoolToStr(FMayTriggerSslTryToSend, True) + ' bSslAllSent=' + BoolToStr(bSslAllSent, True) + ' bAllSent=' + BoolToStr(bAllSent, True));
    end;
{$ENDIF}
    ...
    if (my_BIO_ctrl_pending(FNbio) > 0) then begin
        if FMayTriggerFD_Write then begin
            ...
            (my_BIO_ctrl_get_write_guarantee(FSslbio) > 0) and
             FMayTriggerFD_Write then begin
        if TriggerEvent(sslFdWrite, 0) then
            FMayTriggerFD_Write := FALSE;
    end
    ...
    if ((FCloseCalled and (FSslIntShutDown = 0)) or
       ((FSslIntShutDown = 2) and not FSslBiShutdownFlag)) and  (State = TLS_ST_OK) and (my_BIO_ctrl_pending(FSslbio) = 0) then    { V8.27 }
        TriggerEvent(sslFdClose, 0);
end;  *)


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomSslWSocket.Do_FD_READ;
var
    Len: Integer; // How much to receive
    ...
    Dummy: Byte;
begin
    if (not FSslEnable) or (FSocksState <> socksData) then begin  { V8.42 don't stop reading for non-SSL }
        inherited Do_FD_READ;
        Exit;
    end;

 // Prevent re-entrancy for SSL
    if FReadEventIgnore then begin
        FReadEventIgnored := TRUE;  // Remember we ignored one event
        Exit;
    end;
    FReadEventIgnore := TRUE;
    try
        BuffSize := (GSSL_BUFFER_SIZE * 2)-1;  { V8.27 size now configurable }
        SetLength(Buffer, BuffSize);
      ...
      {$IFNDEF NO_DEBUG_LOG}
        if CheckLogOptions(loSslDevel) then
            DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' TIcsCustomSslWSocket.Do_FD_READ handle=' + IntToStr(HSocket));
      {$ENDIF}

        if (FNetworkError > 0) then
            Exit;

        if (SSL_get_state(FSsl) = TLS_ST_OK) and (FSslBioWritePendingBytes < 0) and (my_BIO_ctrl_pending(FSslbio) > 0) then begin
          {$IFNDEF NO_DEBUG_LOG}
            if CheckLogOptions(loSslDevel) then
                DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' TriggerDataAvailable (Do_FD_READ_1) handle=' + IntToStr(HSocket));
          {$ENDIF}
            TriggerDataAvailable(0);
            Exit;
        end;
     ...
     // Receive data
        PBuf := @Buffer[0];
        NumRead := my_WSocket_recv(HSocket, PBuf, Len, 0);
        if (NumRead > 0) then begin
         // Store it in the network input bio and process data
            my_BIO_write(FNBio, PBuf, NumRead);     { V8.27 was @Buffer }
            my_BIO_ctrl(FNBio, BIO_CTRL_FLUSH, 0, nil);
         ...
         // We may not call BIO_read if a write operation is pending !!
            if (FSslBioWritePendingBytes < 0) then begin
                ...
                else if (FSslVersNum >= SSL3_VERSION) then begin
                    if SSL_get_Error(FSSL, Res) = SSL_ERROR_ZERO_RETURN then begin
                    { SSL closure alert received }
                        if FSslState < sslInShutdown then begin
                            FSslState := sslInShutdown;
        ...
        if Numread = 0 then begin
            ...
            nError := WSocket_WSAGetLastError;
            if (nError > WSABASEERR) and (nError <> WSAEWOULDBLOCK) and (nError <> WSAENOTCONN) then begin
                FNetworkError := nError;
                FLastError    := FNetworkError;
                TriggerEvent(sslFdClose, 0);
              {$IFNDEF NO_DEBUG_LOG}
        ...
        if (SSL_get_state(FSsl) = TLS_ST_OK) and (FSslBioWritePendingBytes < 0) and (my_BIO_ctrl_pending(FSslbio) > 0) then begin
          {$IFNDEF NO_DEBUG_LOG}
            if CheckLogOptions(loSslDevel) then
                DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' TriggerDataAvailable (Do_FD_READ_2) handle=' + IntToStr(HSocket));
          {$ENDIF}
            TriggerDataAvailable(0);
        end;
        ...
        FReadEventIgnore  := FALSE;
        if FReadEventIgnored then begin
            FReadEventIgnored := FALSE;
            // Should I execute Do_FD_READ when one was ignored???
        end;
    end;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomSslWSocket.Do_FD_WRITE;
var
    Len: Integer;    // How much to send
    Buffer: array of AnsiChar;  { V8.27 size now configurable }
    BuffSize: integer;
...
begin
    if (not FSslEnable) or (FSocksState <> socksData) then begin
        inherited Do_FD_WRITE;
        Exit;
    end;

{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then
        DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' TIcsCustomSslWSocket.Do_FD_WRITE handle=' + IntToStr(HSocket));
{$ENDIF}
    if (FNetworkError > 0) then
        Exit;
...
{$IFNDEF NO_DEBUG_LOG}
        if CheckLogOptions(loSslDevel) then
            DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' * Not bAllSent handle=' + IntToStr(HSocket));
{$ENDIF}
        FMayTriggerFD_Write := TRUE;
        { AG 01/10/07 - Outcommented next line in order to avoid too many  }
        { socket errors WSAEWOULDBLOCK, this is experimental and needs to  }
...
{$IFNDEF NO_DEBUG_LOG}
                    if CheckLogOptions(loSslErr) then
                        DebugLog(loSslErr, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' Winsock Error ' +
                                                          WSocketErrorDesc(FNetworkError) + ' handle=' + IntToStr(HSocket));
{$ENDIF}
                    Exit;
                end
                else
    ...
    if (SSL_get_state(FSsl) = TLS_ST_OK) then begin { V8.27, V8.66 }
        if not bSslAllSent then
            TryToSend
        else if bAllSent and (my_BIO_ctrl_pending(FNBio)= 0) and FSendPending then begin
            FSendPending := FALSE;
            TriggerDataSent(0);
        end;
    end;
...
begin
{$IFDEF ICS_NewMessaging}   { V10 }
    TCustomSslWSocket.PostMessage(Self, WM_ICS_WSSSL_BI_SHUTDOWN, 0, 0);
{$ELSE}
...
begin
{$IFDEF ICS_NewMessaging}   { V10 }
     TCustomSslWSocket.PostMessage(Self, WM_ICS_WSSSL_BI_SHUTDOWN, How, 0);
{$ELSE}
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then begin { V5.21 }
        if __DataSocket = Self then
            DebugLog(loSslDevel, IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' SslAsyncSelect DataSocket ' +
                                         IntToStr(msg.wParam) +  ', ' + IntToStr(msg.LParamLo) + WinsockMsgToString(Msg))
        else
            DebugLog(loSslDevel,  IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) + ' SslAsyncSelect ' + IntToStr(msg.wParam) +
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslDevel) then begin
        Str := IntToHex(INT_PTR(Self), SizeOf(Pointer) * 2) +' TriggerEvents handle=' + IntToStr(GetHSocket) + ' State=' +
                                                                               String(SSL_state_string_long(FSsl));  { V8.27  }
        DebugLog(loSslDevel, Str + ' // MayFD_Read='      + BoolToStr(FMayTriggerFD_Read, True) + ' MayDoRecv=' +
            BoolToStr(FMayTriggerDoRecv, True) + ' MayFD_Write=' + BoolToStr(FMayTriggerFD_Write, True) + ' MaySslTryToSend=' +
                BoolToStr(FMayTriggerSslTryToSend, True) + ' bSslAllSent=' + BoolToStr(bSslAllSent, True) +
...
procedure TCustomSslWSocket.SetAcceptableHostsList(const SemiColonSeparatedList: String);
var
    Host : String;
    Buf  : String;
    I    : Integer;
...
begin
    ...
    if FSslState = sslEstablished then begin
        FSslSupportsSecureRenegotiation := SSL_get_secure_renegotiation_support(FSsl) = 1;
...
{$ELSE}
        if ICS_OPENSSL_VERSION_NUMBER >=  OSSL_VER_3200 then
            FSslGroupName := String(SSL_get0_group_name(FSsl));
{$ENDIF YuOpenSSL}  { V10.0 }

        if FSslContext.SslVerifyPeer then begin
        { Get the peer cert from OSSL. Note that servers always send their  certificates, clients on server request only.
          This gets the peer cert also when a session was reused.  }
     ...
     { V8.14 set with success or failure message once handshake completes }
       ...
       { V8.64 always show real error, add state if possible }
        ...
        if Fssl <>  Nil then
            FSslHandshakeRespMsg := FSslHandshakeRespMsg + ', State: ' + String(SSL_state_string_long(Fssl));   { V8.54 }
        if (ErrCode = 1) then
            FSslHandshakeRespMsg := FSslHandshakeRespMsg + ', connection closed unexpectedly';
    end;
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loSslInfo) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
        DebugLog(loSslInfo, Format('%s SslHandshakeDone(%d) Handle=%d. %s, ' + 'VerifyResult=%s', [IntToHex(INT_PTR(Self),
                   SizeOf(Pointer) * 2), ErrCode, GetHSocket, FSslHandshakeRespMsg, IcsX509VerifyErrorToStr(FSslVerifyResult)])); { V8.66 }
{$ENDIF}
    if Assigned(FSslPeerCert) then begin                                { V8.65 }
        ...
        if Assigned(PeerX) then begin
            ...
            X509_free(PeerX);
            { If we have a chain with the peer certificate (new session copy some values.   }
            if RefCert <> nil then
            begin
            ...
            { We don't have a chain with peer certificate (reused session )  get the session verify result from OSSL and assign it. This     }
            { verify result is the original one which is i.e. not changed if we set "Ok := 1;" in OnSslVerifyPeer event.                     }
            else begin
...
{$IFNDEF NO_DEBUG_LOG}
        if CheckLogOptions(loSslDevel) then begin { V5.21 }
            Inc(TraceCount);
            DebugLog(loSslDump, Format('%s PutDataInSendBuffer handle=%s  len %d [%d]',  [IntToHex(INT_PTR(Self),
                                                           SizeOf(Pointer) * 2),  IntToStr(GetHSocket), Len, TraceCount]));
        end
        else if CheckLogOptions(loSslDump) then begin { V5.21 }
            Inc(TraceCount);
            DebugLog(loSslDump, Format('%s PutDataInSendBuffer handle=%s [%d] Data:%s', [IntToHex(INT_PTR(Self),
                                           SizeOf(Pointer) * 2), IntToStr(GetHSocket), TraceCount, DataToString(Data, Len)]));
        end;
{$ENDIF}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
class procedure TCustomHttpTunnelWSocket.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    WSocket: TCustomHttpTunnelWSocket;
begin
    if Assigned(Value.Instance) and (Value.Instance is TCustomHttpTunnelWSocket) then begin
        WSocket := Value.Instance as TCustomHttpTunnelWSocket;
        try
            if Value.Msg = WM_ICS_WS_TUNNEL_RECONNECT then
                WSocket.WMHttpTunnelReconnect(Value);
        except
            on E:Exception do begin
                if Assigned(WSocket) then
                    WSocket.HandleBackGroundException(E, 'THttpTunnelWocket:DispatchMessage: WMHttpTunnelReconnect');
            end;
        end;
    end
    else
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomHttpTunnelWSocket.WMHttpTunnelReconnect(const Value: TIcsSocketMsgRec);
begin
    if FHttpTunnelReconnectRequest <> htrrNone then
        Connect
    else begin // May not happen
        FHttpTunnelLastResponse := 'THttpTunnelWSocket - Fatal: Internal error in WMHttpTunnelReconnect';
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TCustomHttpTunnelWSocket.DoRecv(var Buffer: TWSocketData; BufferSize, Flags: Integer): Integer;
begin
...
{$IFNDEF ICS_NewMessaging}   { V10 }
            if (GetState = wsConnected) then
                PostMessage(Handle, FMsg_WM_ASYNCSELECT,  WPARAM(GetHSocket), LPARAM(FD_READ));
{$ENDIF N ICS_NewMessaging}   { V10 }
        end;
    end
    else
...
begin
    ...
    Result := NtlmGetMessage3(LDomain, Hostname, LUser, FHttpTunnelPassword, NtlmInfo, CP_ACP, FHttpTunnelLmCompatLevel); { V7.86 }
end;
...
begin
    ...
    LAuthHdr := IcsCrLf + sHttpProxyAuthorization + 'Basic ' + IcsBase64Encode(FHttpTunnelUsercode + ':' +  FHttpTunnelPassword);  { V10.0 }
    SendStr(sHttpProxyConnect + ' ' + FAddrStr + ':' + IntToStr(FPortNum) + IcsSpace + sHttpProto + LAuthHdr + IcsCrLf + IcsCrLf);
end;
...
begin
    ...
    LAuthHdr := IcsCrLf + sHttpProxyAuthorization + 'Digest ' +  AuthDigestGenerateRequest(FHttpTunnelUsercode, FHttpTunnelPassword,
                                                        sHttpProxyConnect, Uri, FHttpTunnelAuthDigestHash, FHttpTunnelAuthDigestInfo);
    SendStr(sHttpProxyConnect + ' ' + FAddrStr + ':' + IntToStr(FPortNum) + IcsSpace + sHttpProto + LAuthHdr + IcsCrLf + IcsCrLf);
end;
{$ENDIF}
...
begin
    ...
    FHttpTunnelState := htsWaitResp1;
    LAuthHdr := IcsCrLf + sHttpProxyAuthorization + 'NTLM ' +  NtlmGetMessage1('', '', FHttpTunnelLmCompatLevel); { V7.86 }
    if not HasOption(FComponentOptions, wsoNoHttp10Tunnel) then                 { V8.70 }
        { Make some HTTP/1.0 proxies happy, i.e MSP 2.0 }
        LAuthHdr := LAuthHdr + IcsCrLf + sHttpProxyKeepAlive;
    SendStr(sHttpProxyConnect + ' ' + FAddrStr + ':' + IntToStr(FPortNum) + IcsSpace + sHttpProto + LAuthHdr + IcsCrLf + IcsCrLf);
end;
...
begin
    HttpTunnelClear;
    FHttpTunnelState := htsWaitResp2;
    LAuthHdr := IcsCrLf + sHttpProxyAuthorization + 'NTLM ' + HttpTunnelGetNtlmMessage3;
    SendStr(sHttpProxyConnect + ' ' + FAddrStr + ':' + IntToStr(FPortNum) + IcsSpace + sHttpProto + LAuthHdr + IcsCrLf + IcsCrLf);
end;
...
begin
    ...
    SendStr(sHttpProxyConnect + ' ' + FAddrStr + ':' + IntToStr(FPortNum) + IcsSpace + sHttpProto + IcsCrLf +
                                                                                        sHttpProxyKeepAlive + IcsCrLf + IcsCrLf);
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TCustomHttpTunnelWSocket.HttpTunnelProcessHdrLine(Data: PAnsiChar; Cnt: Integer): Boolean;
var
    I : Integer;
    LStatusCode : Integer;
    LStr : AnsiString;
...
{$ENDIF}
    ...
    begin
        ...
        while (I < Cnt) and (Data[I] = #$20) do
            Inc(I);
        Inc(Data, I);
        FHttpTunnelChunked := (Cnt >= I + 7) and (StrLIComp(Data, PAnsiChar('chunked'), 7) = 0);
    end
        ...
        while (I < Cnt) and (Data[I] = #$20) do
            Inc(I);
        while (I < Cnt) and (Data[I] in ['0'..'9']) do begin
            FHttpTunnelContentLength := FHttpTunnelContentLength * 10 + Byte(Data[I]) - Byte('0');
            Inc(I);
        end;
    end
    else if (FHttpTunnelStatusCode = 407) then begin
        ...
        if (Cnt >= 24) and (StrLIComp(Data, PAnsiChar('Proxy-Authenticate:'), 19) = 0) then begin
            I := 19;
            while (I < Cnt) and (Data[I] = #$20) do
                Inc(I);
            Inc(Data, I);
            ...
            {$IFNDEF NO_HTTP_TUNNEL_AUTHDIGEST}
                ...
                FHttpTunnelAuthDigestValid := AuthDigestValidateResponse(FHttpTunnelAuthDigestInfo);
            {$ENDIF}
...
begin
    if FHttpTunnelState = htsData then
        ...
        LRcvd := Receive(@FHttpTunnelBuf[FHttpTunnelRcvdCnt], FHttpTunnelBufSize - FHttpTunnelRcvdCnt);
        if LRcvd <= 0 then
            Exit;
        ...
        while I < FHttpTunnelRcvdCnt do begin
            ...
            begin
                ...
                begin
                    if not HttpTunnelProcessHdrLine(@FHttpTunnelBuf[LBufIdx], (I - 1) - LBufIdx) then
                        Exit;
                end
                else begin
            ...
            if (LBufIdx > 0) and (FHttpTunnelRcvdCnt >= LBufIdx) then begin
                ...
                if FHttpTunnelRcvdCnt > 0 then begin
                    Move(FHttpTunnelBuf[LBufIdx], FHttpTunnelBuf[0],
                         FHttpTunnelRcvdCnt);
                    { We've buffered data, we MUST ensure it's read! Otherwise the application might wait for it infinitely }
{$IFNDEF ICS_NewMessaging}   { V10 }
                    if (FHttpTunnelState = htsData) and (GetState = wsConnected) then
                        PostMessage(Handle, FMsg_WM_ASYNCSELECT, WPARAM(GetHSocket), LPARAM(FD_READ));
{$ENDIF N ICS_NewMessaging}   { V10 }
                end;
            end;
            ...
            if FHttpTunnelChunked then begin
                ...
                while (LBufIdx < FHttpTunnelRcvdCnt) and (FHttpTunnelChunkState <> htcsDone) do begin
                    if FHttpTunnelChunkState = htcsGetSize then begin
                        while (LBufIdx < FHttpTunnelRcvdCnt) do begin
                            ...
                            FHttpTunnelChunkSize := FHttpTunnelChunkSize * 16 + XDigit(AnsiChar(FHttpTunnelBuf[LBufIdx]));
                            Inc(LBufIdx);
                        end;
                    end;
                ...
                if LBufIdx > 0 then begin
                    Dec(FHttpTunnelRcvdCnt, LBufIdx);
                    if FHttpTunnelRcvdCnt > 0 then begin
                        Move(FHttpTunnelBuf[LBufIdx], FHttpTunnelBuf[0], FHttpTunnelRcvdCnt);
                        { We've buffered data, we MUST ensure it is read!  Otherwise the application might wait for it infinitely }
{$IFNDEF ICS_NewMessaging}   { V10 }
                        if (FHttpTunnelChunkState = htcsDone) and (GetState = wsConnected) then
                            PostMessage(Handle, FMsg_WM_ASYNCSELECT, WPARAM(GetHSocket), LPARAM(FD_READ));
{$ENDIF N ICS_NewMessaging}   { V10 }
                    end;
                end;
               ...
               { Skip body data with Content-Length header }
                ...
                if FHttpTunnelContentLength = 0 then begin
                    ...
                    { Otherwise the application might wait for it infinitely }
{$IFNDEF ICS_NewMessaging}   { V10 }
                    if (FHttpTunnelRcvdCnt > 0) and (GetState = wsConnected) then
                        PostMessage(Handle, FMsg_WM_ASYNCSELECT, WPARAM(GetHSocket), LPARAM(FD_READ));
{$ENDIF N ICS_NewMessaging}   { V10 }
                end;
            end;
        ...
        if FHttpTunnelRcvdCnt = FHttpTunnelBufSize then begin
          { No CRLF found in entire buffer (default size 1024 bytes) }
            FHttpTunnelLastResponse := 'Received header line too long. Increase HttpTunnelBufferSize.';
            TriggerHttpTunnelError(ICS_HTTP_TUNNEL_PROTERR);
        end;
    end
    else begin
        Result := FALSE; // Should never happen
        FHttpTunnelLastResponse := 'THttpTunnelWSocket - Fatal: Invalid state in TriggerDataAvailable';
        TriggerHttpTunnelError(FHttpTunnelStatusCode);
    end;
end;
...
begin
    if FHttpTunnelState <> htsData then begin
        FHttpTunnelLastResponse := AnsiString(WSocketErrorMsgFromErrorCode(ICS_HTTP_TUNNEL_GENERR));
        TriggerHttpTunnelError(ICS_HTTP_TUNNEL_GENERR);
    end
    else if FHttpTunnelReconnectRequest <> htrrNone then begin
        { Start ansynchronous reconnect by posting a custom message. This message is processed in method WMHttpTunnelReconnect. }
{$IFDEF ICS_NewMessaging}   { V10 }
       TCustomHttpTunnelWSocket.PostMessage(Self, WM_ICS_WS_TUNNEL_RECONNECT, 0, 0)
{$ELSE}
...
begin
    ...
    if Assigned(FOnHttpTunnelError) then
        FOnHttpTunnelError(Self, ErrCode, FHttpTunnelServerAuthTypes, String(FHttpTunnelLastResponse));
    FHttpTunnelState := htsData;
    FHttpTunnelReconnectRequest := htrrNone;
        ...
        if ErrCode <> 0 then begin
            ...
            if FHttpTunnelReconnectRequest <> htrrNone then begin
                ...
                if (FHttpTunnelUsercode <> '') then begin
                    if (FHttpTunnelAuthType <> htatDetect) and (FHttpTunnelCurAuthType <> FHttpTunnelAuthType) then
                        FHttpTunnelCurAuthType := FHttpTunnelAuthType;
                end
                else begin
...
{$IFNDEF NO_ADV_MT}
    ...
    try
{$ENDIF}
        Result := WSocket_Synchronized_ResolveName(AName, AReverse, AFamily, AResultList, AProtocol);
{$IFNDEF NO_ADV_MT}
    finally
        SafeDecrementCount;
    end;
    ...
    begin
        case FEvent.WaitFor(FDnsLookup.FThreadIdleTimeoutMsec) of
            ...
            wrTimeout :
                begin
                    if FDnsLookup.FThreadsLock.TryEnter then
                    try { If we cannot enter the critsec next timeout might be successful. EnterCriticalSection may not be called   }
                        { since it might lead to a deadlock when FDnsLookup is freed.   }
                        if (not FBusy) and (not Terminated) and
                           (FDnsLookup.FThreads.Count > FDnsLookup.FMinThreads) then
                        begin
                            FDnsLookup.FThreads.Delete(FDnsLookup.FThreads.IndexOf(Self));
        ...
        try
            if not Request.FCanceled then
            try
                LRes := WSocket_ResolveName(Request.FLookupName, Request.FReverse, Request.FSocketFamily, FDnsResultList, Request.FProtocol);
                if (not Terminated) and (not Request.FCanceled) and
                    IsWindow(Request.FWndHandle) then
                begin
                    { Ensure CancelAsyncRequest() returns correct result }
                    ...
                    Request.FResultList := FDnsResultList;
                    SendMessage(Request.FWndHandle, Request.FMsgID, WPARAM(Request), IcsMakeLong(0, LRes));
                end;
                { If you see an AV on SendMessage() or somewhere else while debugging and stepping thru this code it's most likely a  }
                { debugger bug. If you think it's not, let me know (Arno).  }
            except
            {$IFDEF DEBUG}
                on E: Exception do
...
{ TIcsAsyncDnsLookup }
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TIcsAsyncDnsLookup.ExecAsync(AWnd: HWND; AMsgID: UINT; ASocketFamily: TSocketFamily; const AName: string;
                                                                        AReverse: Boolean; AProtocol: Integer): THandle;
var
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TIcsAsyncDnsLookup.GetNextRequest(AThread: TIcsAsyncDnsLookupThread): TIcsAsyncDnsLookupRequest;
var
    I : Integer;
begin
    LockQueue;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TIcsAsyncDnsLookup.CancelAsyncRequest(AReq: THandle): Integer;
var
    I   : Integer;
    Req : TIcsAsyncDnsLookupRequest;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
constructor TIcsAsyncDnsLookup.Create(const AMaxThreads: Integer; const AMinThreads: Integer = 0;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TThreadStoreTree.Notification(Data: Pointer; Action: TIcsAvlTreeNotification);
begin
...
begin
    ...
    Result := 'Server Name: ' + CliHello.ServerName + ', ' +  'ALPN: ' + CliHello.AlpnList + ', Versions: ' + S;
...
{$ENDIF}
    ...
    CritSecHSocket := TIcsCriticalSection.Create;
    CritSecState   := TIcsCriticalSection.Create;
{$IFNDEF ICS_NewMessaging}   { V10 }
...
finalization
    ...
    FreeAndNil(CritSecHSocket);
    FreeAndNil(CritSecState);
{$IFNDEF NO_ADV_MT}
    CritSecIpList.Free;
{$ENDIF}
{$IFDEF ICS_NewMessaging}   { V10 }
    TIcsMessageManager.FreeIcsDefaultManager;
{$ELSE}

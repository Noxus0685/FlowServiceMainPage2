...
Mar 02, 2025 V9.5  Added RespAttachment (boolean) and RespFileName, parsed from
...
const
    HttpCliVersion       = 1000;
    CopyRight : String   = ' THttpCli (c) 1997-2025 F. Piette V10.0 ';

    DefaultProxyPort     = '80';


{ V9.3 most types moved to OverbyteIcsTypes }
...
{$ENDIF}
    ...
    THttpBeforeAuthEvent = procedure(Sender: TObject; AuthType: THttpAuthType; ProxyAuth: Boolean; const AuthHdr: String;
                                                                                                   var Allow: Boolean) of object;
    TOnCommand = procedure (Sender: TObject;  var S: String) of object;
    TDocDataEvent = procedure (Sender: TObject; Buffer: Pointer; Len: Integer) of object;
    TCookieRcvdEvent = procedure (Sender: TObject; const Data: String; var Accept: Boolean) of object;
    THttpRequestDone = procedure (Sender: TObject; RqType: THttpRequest; ErrCode : Word) of object;
    TBeforeHeaderSendEvent = procedure (Sender: TObject; const Method: String; Headers: TStrings) of object;
    TLocationChangeExceeded = procedure (Sender: TObject; const RelocationCount: Integer; var AllowMoreRelocations: Boolean) of object;
    TSelectDnsEvent = procedure(Sender: TObject; DnsList: TStrings; var NewDns: String) of object;      { V8.57 }
...
{$IFDEF ICS_NewMessaging}   { V10 }
        procedure WMHttpRequestDone(const Value: TIcsSocketMsgRec);
        procedure WMHttpSetReady(const Value: TIcsSocketMsgRec);
        procedure WMHttpLogin(const Value: TIcsSocketMsgRec);
{$ELSE}
...
{$IFDEF UseDigestAuthentication}
        ...
        function  GetDigestAuthorizationHeader(const HttpMethod: String; ProxyAuth : Boolean): String;
...
{$ENDIF}
        ...
        function GetBasicAuthorizationHeader(const HttpMethod: String; ProxyAuth: Boolean): String;
        ...
        procedure TriggerCookie(const Data: String; var bAccept: Boolean); virtual;
        procedure TriggerSessionConnected; virtual;
        procedure TriggerSessionClosed; virtual;
        procedure TriggerBeforeHeaderSend(const Method: String; Headers: TStrings); virtual;
...
{$IFDEF USE_SSL}
        ...
        procedure TransferSslHandshakeDone(Sender: TObject; ErrCode: Word; PeerCert: TX509Base; var Disconnect: Boolean); virtual; { V8.61 }
{$ENDIF}
    public
...
function EncodeLine(Encoding: THttpEncoding; SrcData: PAnsiChar; Size: Integer): AnsiString;
function EncodeStr(Encoding: THttpEncoding; const Value: RawByteString): RawByteString; overload;
function EncodeStr(Encoding: THttpEncoding; const Value: UnicodeString; ACodePage: LongWord = CP_ACP ) : UnicodeString; overload;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
class procedure THttpCli.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    HttpCli: THttpCli;
begin
    if Assigned(Value.Instance) and (Value.Instance is THttpCli) then begin
        HttpCli := THttpCli(Value.Instance);
        try
            with Value do begin
                if Msg = WM_ICS_HTTP_REQUEST_DONE then
                    HttpCli.WMHttpRequestDone(Value)
                else if Msg = WM_ICS_HTTP_SET_READY then
                    HttpCli.WMHttpSetReady(Value)
                else if Msg = WM_ICS_HTTP_LOGIN then
                    HttpCli.WMHttpLogin(Value)
            end;
        except
            on E: Exception do
                HttpCli.HandleBackGroundException(E, 'THttpCli:DispatchMessage: ' + IntToStr(Value.Msg));
        end;
    end
    else
...
{$ENDIF ICS_NewMessaging}   { V10 }
    ...
    FAccept := 'image/gif, image/x-xbitmap, image/jpeg, image/pjpeg, */*';
...
{$IFDEF UseContentCoding}
    FContentCodingHnd := THttpContCodHandler.Create(@FRcvdStream, TriggerDocData);
    GetOptions;
{$ENDIF}
    CreateSocket;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure THttpCli.DoSocksError(Sender: TObject; ErrCode: Integer; Msg: String);
begin
...
begin
{$IFDEF ICS_NewMessaging}   { V10 }
    TWSocket.PostMessage(Self, WM_ICS_HTTP_SET_READY, 0, 0);
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure THttpCli.TriggerBeforeHeaderSend(const Method: String; Headers: TStrings);
begin
...
begin
    { if you place this code in GetHeaderLineNext, not each time will be called ...  }
...
{$IFNDEF NO_DEBUG_LOG}                                                  { V1.91 }
    if CheckLogOptions(loProtSpecDump) then begin
        DebugLog(loProtSpecDump, Format('PrepareNTLMAuth begin, FStatusCode = %d FProxyAuthNTLMState=%d FAuthNTLMState=%d',
                                                              [FStatusCode, Ord(FProxyAuthNTLMState), Ord(FAuthNTLMState)]));
    end;
{$ENDIF}
    ...
    if WAI < 0 then Exit;
    if (httpoNoNTLMAuth in FOptions) and (((FStatusCode = 401) and (FServerAuth = httpAuthNtlm)) or
                                                                 ((FStatusCode = 407) and (FProxyAuth = httpAuthNtlm))) then
          Exit;
    if (FStatusCode = 401) and (FProxyAuthBasicState = basicNone)       { V8.69 was 407, got broken in V8.61 }
{$IFDEF UseDigestAuthentication}
...
{$ENDIF}
    ...
    else if (FStatusCode = 401) and (FProxyAuthBasicState = basicNone) and
{$IFDEF UseDigestAuthentication}
...
begin
    ...
    if WAI < 0 then Exit;
    if (httpoNoBasicAuth in FOptions) and (((FStatusCode = 401) and (FServerAuth = httpAuthBasic)) or
...
begin
{$IFDEF ICS_NewMessaging}   { V10 }
    TWSocket.PostMessage(Self, WM_ICS_HTTP_REQUEST_DONE, 0, 0);
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
procedure THttpCli.WMHttpRequestDone(const Value: TIcsSocketMsgRec);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure THttpCli.WMHttpSetReady(const Value: TIcsSocketMsgRec);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure THttpCli.WMHttpLogin(const Value: TIcsSocketMsgRec);
begin
    Login;
end;
...
{$ENDIF}
    ...
    try
        ...
        { SocketFamily in case a host name is either a valid IPv6 or IPv4 address.  }
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loProtSpecInfo) then
        DebugLog(loProtSpecInfo, 'Socket DNS Lookup Done - ' + FCtrlSocket.DnsResultList.CommaText);  { V8.60 }
{$ENDIF}
...
begin
    ...
    // IfMatch need one or more quoted Etags, ie "<etag_value>", "<etag_value>"
        ...
        if not FProxyConnected then begin
            { We did not call SetReady while in relocation, adjust ProxyAuthNtlmState if we got disconnected from proxy }
          {$IFDEF UseNTLMAuthentication}
            if FProxyAuthNtlmState = ntlmDone then
                FProxyAuthNtlmState := ntlmMsg1;
          {$ENDIF}
...
{$ENDIF}
    ...
    {$ENDIF}
        ...
        if (FContentRangeBegin <> '') or (FContentRangeEnd <> '') then begin
...
//      trailer         = *(entity-header CRLF)
        ...
        while (N > 0) and (FChunkState <> httpChunkDone) do begin
            if FChunkState = httpChunkGetSize then begin
                while N > 0 do begin
                    if not IsXDigit(AnsiChar(FReceiveBuffer[P])) then begin
                    ...
                    FChunkLength := FChunkLength * 16 + XDigit(AnsiChar(FReceiveBuffer[P]));
                    Inc(P);
                    Dec(N);
                end;
            end;
...
begin
    ...
    { Some server send HTML document without header ! I don't know if it is  legal, but it exists (AltaVista Discovery does that). }
    if (FHeaderLineCount =  1) and (FLastResponse    <> '') and (FLastResponse[1] =  '<') and
             (SameText(Copy(FLastResponse, 1, 6), '<HTML>') or SameText(Copy(FLastResponse, 1, 9), '<!DOCTYPE')) then begin
...
{$ENDIF}
        ...
        if ((FContentLength = -1) and (FTransferEncoding <> 'chunked') and ((FStatusCode < 200) or (FStatusCode = 204) or (FStatusCode
                 = 301) or (FStatusCode = 302) or (FStatusCode = 304))) or (FContentLength = 0)  or (FRequestType = httpHEAD) then begin
            if FCloseReq then begin
                if FLocationFlag then
                    StartRelocation
                else begin
                    if FRequestType = httpHEAD then begin { Added 23/07/04 }
                        { With HEAD command, we don't expect a document but some server send one !   }
                ...
                if FLocationFlag then     { V8.67 do before checking HEAD, or get stuck here }
                    ...
                    if FRequestType = httpHEAD then begin
                        { With HEAD command, we don't expect a document but some server send one ! }
                        FReceiveLen := 0;      { Cancel received data   }
                        StateChange(httpWaitingBody);
                        FNext := nil;
    ...
    if (FHeaderLineCount = 1) then begin
        if (Copy(FLastResponse, 1, 8) = 'HTTP/1.0') or (Copy(FLastResponse, 1, 8) = 'HTTP/1.1') then begin
       ...
       { V8.64 Location is A-Label (Punycode ASCII with ACE xn--), leave it alone
          ...
          { NOTE we don't set FLocation if we are actually relocating somewhere, only for data  }
            else if FFollowRelocation and (FStatusCode >= 301) and (FStatusCode <= 308) then begin  { V8.60 not for 201 }
        ...
        else if Field = 'content-range' then begin
            tmpInt := Pos('-', Data) + 1;
            FContentRangeBegin := Copy(Data, 7, tmpInt-8);
            FContentRangeEnd   := Copy(Data, tmpInt, Pos('/', Data) - tmpInt);
        end
        ...
        { rawbite 31.08.2004 Connection controll }
            ...
            if (Length(Data) > 8) then
                ...
                FRespRetryDT := Now + (atoi(Data) div SecsPerDay);  { convert seconds to real time }
        end
...
{$ENDIF}
    ...
    { if protocol version is 1.0 then we suppose that the connection must be  closed. If server response will contain a
        Connection: keep-alive header we will set it to False. }
    { If protocol version is 1.1 then we suppose that the connection is kept alive. If server response will contain a
        Connection: close  we will set it to True. }
    if FRequestVer = '1.0' then
...
begin
    ...
    if (Rq in [httpPOST, httpPUT, httpPATCH]) and (not Assigned(FSendStream)) then
...
{$ENDIF}
    ...
    if (FProxy <> '') and (FProxyAuth <> httpAuthNone) and (FProxyUsername <> '') and (FProxyPassword <> '') then begin
        { If it is still connected there is no need to restart the authentication on the proxy }
        if (FCtrlSocket.State = wsConnected) and (FHostName = FCurrentHost) and (FPort = FCurrentPort)  and
...
{$ENDIF}
    ...
    if (FServerAuth <> httpAuthNone) and (FCurrUsername <> '') and (FCurrPassword <> '') then begin
{$IFDEF UseNTLMAuthentication}
...
{$ENDIF}
    ...
    if FCtrlSocket.State = wsConnected then begin
        if (FHostName = FCurrentHost) and (FPort = FCurrentPort)  and (FProtocol = FCurrentProtocol) then begin
...
{$ENDIF ICS_NewMessaging}   { V10 }
      ...
      { V8.71 JK see if user wants to abandon this request }
        ...
        if (FState <> httpReady) and ((IdleSec >= TimeOutSec) or AbortRequired) then begin   { V8.71 }
            bFlag := (FState = httpDnsLookup);
            StateChange(httpAborting);
            if bFlag then
            try
            ...
            if AbortRequired then begin          { V8.71 JK }
                if AbortMsg = '' then
...
{$IFDEF ICS_NewMessaging}   { V10 }
    TWSocket.PostMessage(Self, WM_ICS_HTTP_LOGIN, 0, 0);
{$ELSE}
...
{  V1.90 25 Nov 2005 - restrict number of relocations to avoid continuous loops }
    ...
    if FLocationChangeCurCount > FLocationChangeMaxCount then begin
        ...
        if Assigned (FOnLocationChangeExceeded) then
            FOnLocationChangeExceeded(Self, FLocationChangeCurCount, AllowMoreRelocations) ;
...
{ Conditional compile will compile this fix by default. It's there because I don't want to delete the original code before confirming everything is OK.  }
    if FLocationFlag then
        LocationSessionClosed(Self, 0)
    else begin
...
{$IFDEF USE_SSL}
    ...
    {$ENDIF}
        ...
        if ( {Bjornar - Start}
           (IcsTbytesStarts(FReceiveBuffer, 'HTTP/1.0 200') or IcsTbytesStarts(FReceiveBuffer,'HTTP/1.1 200') or
           ...
           and not
           ((IcsTbytesStarts(FReceiveBuffer, 'HTTP/1.1 200 OK') or IcsTbytesStarts(FReceiveBuffer, 'HTTP/1.0 200 OK')) and
           IcsTbytesContains(FReceiveBuffer, 'Content-Length:') and  not IcsTbytesContains(FReceiveBuffer, 'Content-Length: 0'))
           ) then {Bjornar - End}
        begin
    ...
    if FState = httpWaitingBody then begin
        if FReceiveLen > 0 then begin
            if FRequestType = httpHEAD then begin   { 23/07/04 }
                { We are processing a HEAD command. We don't expect a document but some server send one anyway.
                   We just throw it away and abort the connection }
                FReceiveLen := 0;
                FCtrlSocket.Abort;
                Exit;
            end;
//          FBodyData    := FReceiveBuffer;
    ...
    while FReceiveLen > 0 do begin
        ...
        if I >= FReceiveLen then                                       { V9.4 fixed overflowing buffer, > to >= }
            break;
        if I = 0 then
            SetLength(FLastResponse, 0)
        else begin
            if (I > 0) and (Ord(FReceiveBuffer[I - 1]) = 13) then
                SetLength(FLastResponse, I - 1)
            else
                SetLength(FLastResponse, I);
            if Length(FLastResponse) > 0 then
                IcsMoveTBytesToString(FReceiveBuffer, 0, FLastResponse, 1, Length(FLastResponse));
        end;
...
{$ENDIF}
        ...
        if FReceiveLen > 0 then begin
            IcsMoveTBytes(FReceiveBuffer, I + 1, 0, FReceiveLen);
        end
        ...
        if FState in [httpWaitingHeader, httpWaitingBody] then begin
            ...
            { We are receiving data outside of any request. It's a server push.  }
...
{$ENDIF}
    ...
    if (FCurrentHost = FHostName) and (FCurrentPort = FPort) and (FCurrentProtocol = FProtocol) and (not FCloseReq) then begin      { SAE 01/06/04 }
        Inc (FLocationChangeCurCount) ;
        if FLocationChangeCurCount > FLocationChangeMaxCount then begin
            AllowMoreRelocations := false;
            if Assigned (FOnLocationChangeExceeded) then
        ...
        { No need to disconnect, Trigger the location changed event  27/04/2003 }
        TriggerLocationChange;  { V8.61 }
        if FLocation = '' then begin    { V9.1 cancel relocation if new location cleared in event }
            FLocationFlag := False;
            SetReady;
        ...
        FDocName  := FPath;
        AdjustDocName;
        { When relocation occurs doing a POST, new relocated page has to be GET.  01/05/03  }
        { angus V8.07  unless a 307 or 308 POST which must not revert to GET }
        ...
        if ((SavedStatus=303) and (FRequestType <> httpHEAD)) or ((FRequestType = httpPOST) and ((SavedStatus=301) or (SavedStatus=302))) then
            FRequestType  := httpGET;
        { Must clear what we already received }
        CleanupRcvdStream; {11/11/04}
        CleanupSendStream;
...
{$ENDIF}
        ...
        { if request version is 1.0 we must tell the server that we want to keep the connection or NTLM will not work }
        if (FRequestVer = '1.0') or (FResponseVer = '1.0') or (FResponseVer = '') then
            FCurrConnection := 'Keep-alive';
        ...
        FDoAuthor.Clear;
        FAuthNTLMState := ntlmNone;
        { We comes here when NTLM has failed so we trigger the end request   }
{$IFDEF ICS_NewMessaging}   { V10 }
        TWSocket.PostMessage(Self, WM_ICS_HTTP_REQUEST_DONE, 0, 0);
{$ELSE}
        PostMessage(Handle, FMsg_WM_HTTP_REQUEST_DONE, 0, 0);
...
{$ENDIF}
        ...
        FDoAuthor.Clear;
        FAuthBasicState := basicNone;
        { We comes here when Basic has failed so we trigger the end request  }
{$IFDEF ICS_NewMessaging}   { V10 }
        TWSocket.PostMessage(Self, WM_ICS_HTTP_REQUEST_DONE, 0, 0);
{$ELSE}
        PostMessage(Handle, FMsg_WM_HTTP_REQUEST_DONE, 0, 0);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function THttpCli.GetBasicAuthorizationHeader(const HttpMethod: String; ProxyAuth: Boolean): String;
begin
    if ProxyAuth then
...
begin
    ...
    {$IFDEF UseNTLMAuthentication}
       (FAuthNTLMState = ntlmNone) and
    {$ENDIF}
       (FAuthBasicState = basicNone) and (FCurrUserName <> '') and (FCurrPassword <> '') then begin
     { V8.69 used parsed headers from WWWAuthInfos instead of enumerating FDoAuthor.Strings }
     { also supports both MD5 and SHA-256 hash algorithms }
        ...
        {$IFDEF UseNTLMAuthentication}
            (FProxyAuthNTLMState = ntlmNone) and
        {$ENDIF}
            (FProxyAuthBasicState = basicNone) and (FProxyUsername <> '') and (FProxyPassword <> '') then begin
     { V8.69 used parsed headers from WWWAuthInfos instead of enumerating FDoAuthor.Strings }
     { also supports both MD5 and SHA-256 hash algorithms }
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function THttpCli.GetDigestAuthorizationHeader(const HttpMethod: String; ProxyAuth: Boolean): String;
var
    Proto, User, Pass, Host, Port, Uri : String;
begin
    { It's probably faster to use a field FAuthDigestUri which is set at some other places when ParseUrl is called anyway }
    ...
    if ProxyAuth then begin
        ...
        Result := 'Proxy-Authorization: Digest ' +
           AuthDigestGenerateRequest(FProxyUserName, FProxyPassword, HttpMethod, Uri, FAuthDigestEntityHash, FAuthDigestProxyInfo);
    end
    else begin
        FAuthDigestInfo.Nc := 1;
        Result := 'Authorization: Digest ' +
                 AuthDigestGenerateRequest(FCurrUserName, FCurrPassword, HttpMethod, Uri, FAuthDigestEntityHash, FAuthDigestInfo);
    end;
end;
...
begin
    ...
    {$ENDIF}
        ...
        FDoAuthor.Clear;
        FProxyAuthDigestState := digestNone;
        { We come here when Digest has failed so we trigger the end request       }
{$IFDEF ICS_NewMessaging}   { V10 }
        TWSocket.PostMessage(Self, WM_ICS_HTTP_REQUEST_DONE, 0, 0);
{$ELSE}
        PostMessage(Handle, FMsg_WM_HTTP_REQUEST_DONE, 0, 0);
{$ENDIF ICS_NewMessaging}   { V10 }
        ...
        raise EHttpException.Create('Unexpected ProxyAuthDigestState', httperrInvalidAuthState);
end;
{$ENDIF UseDigestAuthentication}

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF USE_SSL}
procedure THttpCli.TransferSslHandshakeDone(Sender: TObject; ErrCode: Word; PeerCert: TX509Base; var Disconnect: Boolean);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure THttpCli.SslHandshakeDone(Sender: TObject; ErrCode: Word; PeerCert: TX509Base; var Disconnect: Boolean);
...
begin
    ...
    if (ErrCode <> 0) or Disconnect then begin
        ...
        if FReasonPhrase = '' then begin  { V8.12 may have set better reason in event }
         { V8.55 we always disconnect failure... }
...
begin
    if FRequestVer <> Ver then begin
        if (Ver = '1.0') or (Ver = '1.1') then
            ...
            raise EHttpException.Create('Insupported HTTP version', httperrVersion);
    end;
end;
...
begin
    ...
    while Pos1 < Size do begin
        if Offset > 0 then begin
            Result[Pos2] := AnsiChar(ord(Result[Pos2]) or ((ord(SrcData[Pos1]) and ($3f shl Offset)) shr Offset));
            Offset := Offset - 6;
            Inc(Pos2);
            Result[Pos2] := #0;
        end
            ...
            Offset := Abs(Offset);
            Result[Pos2] := AnsiChar(ord(Result[Pos2]) or ((ord(SrcData[Pos1]) and ($3f shr Offset)) shl Offset));
            Offset := 8 - Offset;
            Inc(Pos1);
        end
        else begin
            Result[Pos2] := AnsiChar(ord(Result[Pos2]) or ((ord(SrcData[Pos1]) and $3f)));
            Inc(Pos2);
            Inc(Pos1);
            Result[Pos2] := #0;
            Offset    := 2;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function EncodeStr(Encoding: THttpEncoding; const Value: RawByteString): RawByteString;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function EncodeStr(Encoding: THttpEncoding; const Value: UnicodeString; ACodePage: LongWord = CP_ACP ): UnicodeString;
var
    AStr : AnsiString;
begin
...
begin
    { it is very common not to send domain and workstation strings on the first message }
    if ForProxy then begin
        if assigned (FAuthNtlmSession) then                  // V8.61
            Result := 'Proxy-Authorization: NTLM ' + FAuthNtlmSession.GetUserCredentials    // V8.61
        else
        Result := 'Proxy-Authorization: NTLM ' + NtlmGetMessage1('', '', FLmCompatLevel)  { V7.25 }
    end
    else
        Result := 'Authorization: NTLM ' + NtlmGetMessage1('', '', FLmCompatLevel); { V7.25 }
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function THttpCli.GetNTLMMessage3(const HttpMethod: String; const ForProxy: Boolean): String;
var
    Hostname : String;
    ...
    if ForProxy then begin
        if assigned (FAuthNtlmSession) then                   // V8.61
            ...
            Result := 'Proxy-Authorization: NTLM ' +
                  NtlmGetMessage3(LDomain, Hostname, LUser, FProxyPassword, FProxyNTLMMsg2Info, CP_ACP, FLmCompatLevel);
        end
    end
    else begin
        NtlmParseUserCode(FCurrUsername, LDomain, LUser);
        Result := 'Authorization: NTLM ' +
                      NtlmGetMessage3(LDomain,  Hostname, LUser, FCurrPassword, FNTLMMsg2Info, CP_ACP, FLmCompatLevel);
    end;
end;
{$ENDIF}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TSslHttpCli.TransferSslCliCertRequest(Sender: TObject; var Cert: TX509Base);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TSslHttpCli.TransferSslVerifyPeer(Sender: TObject; var Ok: Integer; Cert: TX509Base);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TSslHttpCli.TransferSslCliGetSession(Sender: TObject; var SslSession: Pointer; var FreeSession: Boolean);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TSslHttpCli.TransferSslCliNewSession(Sender: TObject; SslSession: Pointer; WasReused: Boolean; var IncRefCount: Boolean);
begin
...
procedure TSslHttpCli.SetAcceptableHostsList(const SemiColonSeparatedList : String);
var
    Host : String;
    Buf  : String;
    I    : Integer;

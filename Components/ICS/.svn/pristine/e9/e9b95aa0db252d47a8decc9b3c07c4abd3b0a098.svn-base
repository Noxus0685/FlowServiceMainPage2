...
Description:  ICS HTTPS REST functions demo, TSslHttpRest.
Creation:     Apr 2018
Updated:      Apr 2025
Version:      V9.5
...
Apr 3, 2025 V9.5   Check log is open before trying to flush it.
                   Save attachment as file, if Content-Disposition: attachment response header
                     found or if 'Prompt to Save Non-Textual Content' is ticked.
                   Added WebSocket event triggered when all frames have been sent, for
                     flow control when sending lots of large data.
                   Added OAuth2 Allow Blank Secret tick box which allows blank client secret
                     for public clients using corporate accounts with a tenant GUID.
                   OAuth2 now sets SocketFamily if need IPV4 only or something.
...
type
  THttpRestForm = class(TForm)
    ...
    SaveNonTextual: TCheckBox;       { V9.5 }
    OAuthOptNoSecret: TCheckBox;     { V9.5 }
 ...
 // properties not saved
    ...
    procedure SslWebSocketCli1WSFramesDone(Sender: TObject);
  private
...
begin
    ...
    // form positions
       ...
       with IniFile do begin
          ...
          if ReadString (SectionData, 'SaveNonTextual_Checked', 'False') = 'True' then SaveNonTextual.Checked := true else SaveNonTextual.Checked := false ;  { V9.5 }
          if ReadString (SectionData, 'OAuthOptNoSecret_Checked', 'False') = 'True' then OAuthOptNoSecret.Checked := true else OAuthOptNoSecret.Checked := false ;   { V9.5 }
  end;
        IniFile.Free;
    end;
 ...
 { V8.65 build OAuth2 Account Title list, V9.5 CertCenter gone }
    OAuthAccTitle.Items.Clear;
    SetLength(OAuthUris, 6);
    OAuthUris[0] :=  OAuthUriNone;
    OAuthUris[1] :=  OAuthUriGoogle;
    OAuthUris[2] :=  OAuthUriMSRest;
    OAuthUris[3] :=  OAuthUriMSSmtp;
    OAuthUris[4] :=  OAuthUriTwitterOA2;
    OAuthUris[5] :=  OAuthUriSipgate;
    for I := 0 to 5 do
...
begin
    ...
    with IniFile do begin
      ...
      if SaveNonTextual.Checked then temp := 'True' else temp := 'False' ; WriteString (SectionData, 'SaveNonTextual_Checked', temp) ; { V9.5 }
      if OAuthOptNoSecret.Checked then temp := 'True' else temp := 'False' ; WriteString (SectionData, 'OAuthOptNoSecret_Checked', temp) ;  { V9.5 }
    end;
    IniFile.UpdateFile;
    IniFile.Free;
end;
...
procedure THttpRestForm.FlushLog;    { V9.5 }
begin
    try
        if (DirLogs.Text = '') then Exit ;
        if NOT Assigned(FIcsBuffLogStream) then Exit; // sanity check
        FIcsBuffLogStream.FlushFile;
    except
    end;
end;
...
var
    ...
    Async: Boolean;
    FName: String;
    Proto, User, Pass, Host, Port, Dirs, Section, Query: String;
begin
    Async := (ReqMode.ItemIndex = 1);
    doStartReq.Enabled := True;
 ...
 {   if (Pos('text/', HttpRest1.ContentType) > 0) or (Pos('xml', HttpRest1.ContentType) > 0) or
     ...
     if (NOT HttpRest1.RespAttachment) and IcsMimeIsTextual(HttpRest1.RespMimeType) then    { V9.3 simplify code }
        AddLog (String(HttpRest1.ResponseRaw))
     else begin
     { V9.5 save attachment as file, if Content-Disposition: attachment response header found, or requested }
        if HttpRest1.RespAttachment or SaveNonTextual.Checked then begin
            FName := HttpRest1.RespFileName;  // from response header
            if FName = '' then begin          // or try to find it from URL
                IcsParseExURL (HttpRest1.URL, Proto, User, Pass, Host, Port, Dirs, FName, Section, Query);
            end;
            AddLog('Response is File Attachment: ' + FName + ' (Size: ' + IntToKbyte(HttpRest1.ResponseSize) + ')');
            SaveDialog1.Title := 'Save Downloaded File As';
            SaveDialog1.FileName := FName ;
            SaveDialog1.InitialDir := DirLogs.Text;
            if SaveDialog1.Execute then begin
                try
                    if FileExists(SaveDialog1.FileName) then
                        IcsDeleteFile(SaveDialog1.FileName, True);
                    (HttpRest1.ResponseStream as TMemoryStream).SaveToFile(SaveDialog1.FileName);
                    if FileExists(SaveDialog1.FileName) then
                        AddLog('Saved Response OK as: ' + SaveDialog1.FileName)
                     else
                        AddLog('Failed to Saved Response as: ' + SaveDialog1.FileName);
                except
                    on E:Exception do
                        AddLog('Failed to Save File: ' + SaveDialog1.FileName + ' - ' + E.Message);
                end;
            end
            else
                AddLog('Cancelled Save File');
        end
        else
...
begin
    ...
    if OAuthOptNoSecret.Checked then begin             { V9.5 }
        RestOAuth1.OAOptions := RestOAuth1.OAOptions  + [OAopAuthNoSecret];
    end;
    ...
    RestOAuth1.HttpRest.SocketFamily := TSocketFamily(IpSockFamily.ItemIndex);   { V9.5 }
end;
...
procedure THttpRestForm.SslWebSocketCli1WSFramesDone(Sender: TObject);     { V9.5 }
begin
    if (SslWebSocketCli1.DebugLevel >= DebugParams) then
        AddLog('Websocket Frame Queue Empty');
end;

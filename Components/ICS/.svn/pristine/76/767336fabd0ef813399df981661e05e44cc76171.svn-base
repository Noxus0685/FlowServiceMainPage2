...
Legal issues: Copyright (C) 2004-2024 by François PIETTE
...
Dec xx, 2024 V10.0 Updated for DEFINE ICS_NewMessaging that implements new cross platform
                   ...
                   Added TWSocketData and PSockChar, moved Posix.Types here.
                   Added WM_ICSxx and PIPExx literals.
...
const
  OverbyteIcsTypesVersion = 1000;
  CopyRight : String      = ' OverbyteIcsTypes (c) 2004-2025 F. Piette V10.0 ';
...
{ * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{ V10.0 following moved from OverbyteIcsMessMan }
{ * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
    ...
    // TIcsSocketMsgRec is used to send messages within the component.
    // Those messages are NOT related to Windows own messaging system but
    // rather use Delphi messaging system (TMessageManager).
    TIcsSocketMsgRec = record
        Msg      : Cardinal;
        Instance : TObject;
        case Integer of
        0: (
...
{$IFDEF RTL_MESSAGING}
    // TIcsSocketMessage is an object encapsulating a message
    TIcsSocMsg = class(TMessage<TIcsSocketMsgRec>)
    public
        constructor CreateEx(Msg: Cardinal; Instance: TObject; WParam: NativeUInt; LParam: NativeUInt);
    end;
{$ELSE}
    TIcsSocMsg = type TIcsSocketMsgRec;     { was TIcsSocketMessage }
//    TMessage          = TIcsSocketMessage;  { don't conflict with Windows Messages or allow use in Posix }
{$ENDIF}
...
{ * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{ V10 following moved from OverbyteIcsWSocketBuf }
{ * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
  TWSocketData = type Pointer;

{$IFDEF POSIX}
{ * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{ V10 moved from Ics.Posix.Types  }
{ * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
  DWORD                           = UInt32;
  UINT                            = UInt32;
  INT_PTR                         = NativeInt;
...
const
  ...
  FIONREAD     = IOC_OUT or ((Longint(SizeOf(Longint)) and IOCPARM_MASK) shl 16) or (Longint(Byte('f')) shl 8) or 127; { get # bytes to read }
  {$EXTERNALSYM FIONREAD}
  FIONBIO      = IOC_IN or ((Longint(SizeOf(Longint)) and IOCPARM_MASK) shl 16) or (Longint(Byte('f')) shl 8) or 126; { set/clear non-blocking i/o }
  {$EXTERNALSYM FIONBIO}
  FIOASYNC     = IOC_IN or ((Longint(SizeOf(Longint)) and IOCPARM_MASK) shl 16) or (Longint(Byte('f')) shl 8) or 125; { set/clear async i/o }
...
type
  ...
  TWSAEvent                     = WSAEVENT;           { V9.4 }
  PWSAEvent                     = LPWSAEVENT;         { V9.4 }
...
// V10 commands sent to the worker thread using an anonymous pipe
    PIPE_CMD_NOOP              = 0;
    PIPE_CMD_TERMINATE         = 1;
    PIPE_CMD_TRY_TO_SEND       = 2;
    PIPE_CMD_SENDMESSAGE       = 3;
{$IFDEF MSWINDOWS}
    PIPE_CMD_ADD_TO_WAITLIST   = 4;
    PIPE_CMD_DEL_FROM_WAITLIST = 5;
{$ENDIF}
    PIPE_CMD_GETHOSTBYNAME     = 6;
    PIPE_CMD_GETHOSTBYADDR     = 7;
    PIPE_CMD_CLOSE_DELAYED     = 8;

// V10 Messages sent to worker thread. Those are NOT Windows messages event on Windows platform!
// send using TIcsCustomWSocket.PostMessage(), receveed in DispatchMessage(); override;
    WM_ICS_WS_SESSION_CONNECTED   = 15000;
    WM_ICS_WS_DATA_AVAILABLE      = 15001;
    WM_ICS_WS_SESSION_CLOSED      = 15002;
    WM_ICS_WS_SESSION_AVAILABLE   = 15003;
    WM_ICS_WS_SEND_DATA           = 15004;
    WM_ICS_TEST                   = 15005;
    WM_ICS_WS_TRIGGER_ERROR       = 15006;
    WM_ICS_WS_TRIGGER_EXCEPTION   = 15007;
    WM_ICS_WS_DNS_LOOKUP_DONE     = 15008;
    WM_ICS_WS_TRGR_DATA_AVAILABLE = 15009;
    WM_ICS_WS_TUNNEL_RECONNECT    = 15010;
    WM_ICS_WS_TRGR_DATASENT       = 15011;
    WM_ICS_WSSSL_ASYNCSELECT      = 15101;
    WM_ICS_WSSSL_RESET            = 15102;
    WM_ICS_WSSSL_BI_SHUTDOWN      = 15103;
    WM_ICS_WSSSSL_SHUTDOWN_DONE   = 15104;
    WM_ICS_HTTP_REQUEST_DONE      = 15201;
    WM_ICS_HTTP_SET_READY         = 15202;
    WM_ICS_HTTP_LOGIN             = 15203;
    WM_ICS_HTTPSRV_FINISH         = 15221;
    WM_ICS_HTTPSRV_DONE           = 15222;
    WM_ICS_HTTPD_START_MULTI      = 15251;
    WM_ICS_SRV_CLIENT_CLOSED      = 15301;
    WM_ICS_SRVTHREAD_START_CNX    = 15351;     // Threaded Server
    WM_ICS_SRVTHREAD_TERMINATED   = 15352;
    WM_ICS_SRVTHREAD_EXCEPTION    = 15353;
    WM_ICS_FTP_REQUEST_DONE       = 15401;
    WM_ICS_FTP_SENDDATA           = 15402;
    WM_ICS_FTP_CLOSEDOWN          = 15403;
    WM_ICS_FTPSRV_CLOSE_REQUEST   = 15421;
    WM_ICS_FTPSRV_ABORT_TRANSFER  = 15422;
    WM_ICS_FTPSRV_CLOSE_DATA      = 15423;
    WM_ICS_FTPSRV_START_SEND      = 15424;
    WM_ICS_FTPD_START_MULTI       = 15451;
    WM_ICS_FTPD_PART_DONE         = 15452;
    WM_ICS_NNTP_REQUEST_DONE      = 15501;
    WM_ICS_POP3_REQUEST_DONE      = 15511;
    WM_ICS_SMTP_REQUEST_DONE      = 15521;
    WM_ICS_SMTP_DATA_NEXT         = 15522;
    WM_ICS_SMTP_QUIT_DELAYED      = 15523;
    WM_ICS_SNMP_LISTENING         = 15531;
    WM_ICS_SNMP_GETNEXT           = 15532;
    WM_ICS_SMTPSRV_CLI_LOOKUP_DONE= 15541;
    WM_ICS_SMTPSRV_CLI_CLOSE      = 15542;
    WM_ICS_PROXY_TARGET_CONNECTED = 15551;
    WM_ICS_IPADDRCHANGE           = 15561;
    WM_ICS_THREAD_ADD_CLIENT      = 15601;  // Threaded Server
    WM_ICS_THREAD_REMOVE_CLIENT   = 15602;
    WM_ICS_THREAD_TERMINATE       = 15603;
    WM_ICS_THREAD_DISCONNECT_ALL  = 15604;
    WM_ICS_THREAD_EXCEPTION_TEST  = 15605;
...
type
  ...
  TSocketState       = (wsInvalidState, wsOpened, wsBound, wsConnecting, wsSocksConnected, wsConnected,
                        wsAccepting,  wsListening, wsClosed, wsDnsLookup);  { V8.48 added DnsLookup }
  TSocketSendFlags   = (wsSendNormal, wsSendUrgent);
  TSocketLingerOnOff = (wsLingerOff, wsLingerOn, wsLingerNoSet);
  TSocketKeepAliveOnOff = (wsKeepAliveOff, wsKeepAliveOnCustom, wsKeepAliveOnSystem);
...
type
    ...
    TWSockettProgEvent = procedure (Sender: TObject; LogOption: TLogOption; const Msg: string) of object;   { V9.4 }
  { alised to THttpRestProgEvent }

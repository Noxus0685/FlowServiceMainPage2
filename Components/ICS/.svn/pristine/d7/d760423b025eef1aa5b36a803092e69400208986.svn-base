...
April 2023 - pending use FDnsQuery.PTRLookupSync to simpify DNS lookup.
April 2023 - pending use IcsHosts for multiple listeners
...
const
    SmtpCliVersion     = 1000;
    CopyRight : String = ' SMTP Server (c) 1997-2024 Francois Piette V10.0 ';
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
                TSmtpSrvCli.PostMessage(Self, WM_ICS_SMTPSRV_CLI_LOOKUP_DONE, 0, lParam(Self.FServer.Client[i]))
{$ELSE}
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
                TSmtpSrvCli.PostMessage(Self, WM_ICS_SMTPSRV_CLI_LOOKUP_DONE, 0, lParam(Client))
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
class procedure TSmtpSrvCli.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    Reason: string;
    Action: TSmtpmailAction;
    SmtpSrv:TSmtpSrvCli;
begin
    if Assigned(Value.Instance) and (Value.Instance is TSmtpSrvCli) then begin
        SmtpSrv := TSmtpSrvCli(Value.Instance);
        try
            if Value.Msg = WM_ICS_SMTPSRV_CLI_LOOKUP_DONE then begin
                with TSmtpSrvCli (Value.LParam) do begin
                    if FContext = mcConnecting then begin
                  // Setup connection
                        FDnsQuery.Free;
                        FDnsQuery := nil;
                    ...
                    // Inform "caller"
                        ...
                        begin
                            FSmtpServer.FOnConnect (FSmtpServer, SmtpSrv, FClientIpAddr, Action, Reason);
                        end;
                        if Action = wsmtpOK then
                        begin
                        ...
                        begin
                            FSmtpServer.SendActionFailed (SmtpSrv, Action, Reason);
                          // Reject connection
                            TSmtpSrvCli (Value.LParam).Close;
                        end;
                    end;
                end ;
            end
            else if Value.Msg = WM_ICS_SMTPSRV_CLI_CLOSE then
            begin
                TSmtpSrvCli (Value.LParam).Close;
            end
       except
            on E: Exception do
                SmtpSrv.HandleBackGroundException(E, 'TSmtpSrvCli:DispatchMessage: ' + IntToStr(Value.Msg));
        end;
    end
    else
    ...
    begin
        ...
        begin
            ...
            begin
              ...
              // No rDNS available.
{$IFDEF ICS_NewMessaging}   { V10.0 }
                    TSmtpSrvCli.PostMessage(Self, WM_ICS_SMTPSRV_CLI_LOOKUP_DONE, 0, lParam(Self))
{$ELSE}
...
//******************************************************************//
procedure TSslSmtpServer.TlsSslHandshakeDone(Sender: TObject; ErrCode: Word; PeerCert: TX509Base; var Disconnect: Boolean);
var
    Client: TSmtpSrvCli;
begin
    Client := Sender as TSmtpSrvCli;
    ...
    begin
{$IFDEF ICS_NewMessaging}   { V10.0 }
        TSmtpSrvCli.PostMessage(Self, WM_ICS_SMTPSRV_CLI_CLOSE, 0, lParam(Self));
{$ELSE}

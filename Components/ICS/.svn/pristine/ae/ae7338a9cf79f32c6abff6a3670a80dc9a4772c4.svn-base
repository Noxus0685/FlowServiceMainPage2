...
const
    CopyRight    : String     = ' IcsIpHlpApi  (c) 2024 V10.0 ';
...
//------------------------------------------------------------------------------
{$IFDEF ICS_NewMessaging}   { V10 }
class procedure TIcsIpChanges.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    IpChanges: TIcsIpChanges;
    ...
    Ret: Integer;
begin
    if Assigned(Value.Instance) and (Value.Instance is TIcsIpChanges) then begin
        IpChanges := TIcsIpChanges(Value.Instance);
        try
            with Value do begin
                if Msg = WM_ICS_IPADDRCHANGE then begin
                    NotificationType := TMibNoticationType(Value.WParam);
                    PartRow := PMibUnicastIpAddressRow(Value.LParam);
                    ...
                    IpChanges.FIpChangesEvent(IpAddrInfo, NotificationType);
                end;
            end;
        except
            on E: Exception do
                IpChanges.HandleBackGroundException(E, 'TIcsIpChanges:DispatchMessage: ' + IntToStr(Value.Msg));
        end;
    end
    else
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
    TIcsIpChanges.PostMessage(CallerContext, WM_ICS_IPADDRCHANGE, WPARAM(NotificationType), LPARAM(Row));
{$ELSE}

...
Jan 22, 2024 V9.1  Moved TSslContext, TSslBaseComponent, TX509Base and TX509List to OverbyteIcsSslBase
                   ...
                   TIcsEventQueue now supports Android and Linux.  
...
const
  WSocketVersion            = 901;
  CopyRight    : String     = ' TWSocket (c) 1996-2024 Francois Piette V9.1 ';
...
type
  ...
  TIcsIPv4Address    = {$IFDEF POSIX} Cardinal {$ELSE} Integer {$ENDIF};
...
function  WSocketStrToMappedIPv4(const IPv4Str: string; APortNum: Word;
  AScopeIDValue: Cardinal; out Success: Boolean): TSockAddrIn6;
{$ENDIF}
...
function WSocket_setsockopt(s: TSocket; level, optname: Integer; optval: UnicodeString;
                            optlen: Integer): Integer; overload;                    { V8.70 }
...
function WSocket_getsockopt(s: TSocket; level, optname: Integer; optval: PAnsiChar;
                            var optlen: Integer): Integer; overload;                { V8.70 }
...
function  WSocket_GetNameInfo(addr: PSockAddr; namelen: Integer; host: PChar;
                              hostlen: Cardinal; serv: PChar; servlen: Cardinal;
...
function  WSocket_GetNameInfo(addr: PSockAddr; namelen: Integer; host: PAnsiChar;
                              hostlen: Cardinal; serv: PAnsiChar; servlen: Cardinal;
                              flags: Integer): Integer;
{$ENDIF}
    ...
    if sa <> nil then begin
        with sa^ do begin
            Result := (sin6_family = AF_INET6) and
                      (PCardinal(@sin6_addr.s6_addr[0])^ = 0) and
                      (PCardinal(@sin6_addr.s6_addr[4])^ = 0) and
                      (PCardinal(@sin6_addr.s6_addr[8])^ = 0) and
...
function WSocketStrToIPv6(
    const S     : string;
    out Success : Boolean;
    out ScopeID : Cardinal): TIcsIPv6Address;
const
    Colon       = ':';
    Percent     = '%';
var
...
begin
  ...
  {$ENDIF}
  {$IFDEF POSIX}
    Result := getsockname(s, Posix.SysSocket.psockaddr(@name)^, {$IFDEF ANDROID32} namelen {$ELSE}Cardinal(namelen){$ENDIF});
  {$ENDIF POSIX}
end;
...
begin
  ...
  {$ENDIF}
  {$IFDEF POSIX}
    Result := getpeername(s, Posix.SysSocket.psockaddr(@name)^, {$IFDEF ANDROID32} namelen {$ELSE}Cardinal(namelen){$ENDIF});
  {$ENDIF POSIX}
end;
...
begin
  ...
  {$ENDIF}
  {$IFDEF POSIX}
     Result := recvfrom(s, Buf^, len, flags, Posix.SysSocket.psockaddr(@from)^, {$IFDEF ANDROID32} fromlen {$ELSE}Cardinal(fromlen){$ENDIF});
 {$ENDIF POSIX}
end;
...
begin
    ...
    begin
        ...
        begin
            ...
            begin
                PSockAddrIn(@ASockAddrIn6)^.sin_addr.S_addr := {$IFDEF POSIX}Cardinal{$ELSE}Integer{$ENDIF}(INADDR_BROADCAST);   { V8.65 }
                Exit;
            end;
...
var
  ...
  LoParam   : Word;
  HiParam   : Word;
  CurEvt    : {$IF DEFINED(MACOS) OR DEFINED(IOS)}TKEvent{$ELSEIF DEFINED(LINUX) OR DEFINED(ANDROID)}TEPoll_Event{$ENDIF};
  nChanges  : Integer;
  LLastErr  : Integer;
  LEvtMask  : Cardinal;
...
begin
  ...
  if FInitialized then
    Exit;
  FQueue := {$IF DEFINED(MACOS) OR DEFINED(IOS)} KQueue {$ELSEIF DEFINED(LINUX) OR DEFINED(ANDROID)} epoll_create(0) {$ENDIF};
  if FQueue = INVALID_FILE_HANDLE then

...
Legal issues: Copyright (C) 2005-2024 by François PIETTE
...
Dec xx, 2024 V10.0 Updated for DEFINE ICS_NewMessaging that implements new cross platform
   ...
   NEW WARNING, will build with ICS_NewMessaging, but not won't work yet !!!!


   WARNING - the multi threaded server sample has not been tested for several years....
...
const
    WSocketThrdServerVersion = 1000;
    CopyRight : String       = ' TWSocketThrdServer (c) 2005-2024 F. Piette V10.0 ';
...
{$IFDEF ICS_NewMessaging}   { V10 }
  {$MESSAGE 'TODO TWSocketThrdServer for ICS_NewMessaging'}
{$ELSE}
...
begin
    for I := 0 to FThreadList.Count - 1 do begin
        ...
        if not PostThreadMessage(AThread.ThreadID,  WM_THREAD_TERMINATE, 0, 0) then
            raise Exception.Create('PostThreadMessage ' + SysErrorMessage(GetLastError));
    end;
end;
...
begin
    ...
    if not PostThreadMessage(Result.ThreadID,  WM_THREAD_ADD_CLIENT, WPARAM(Client), 0) then  { V7.02 }
        raise Exception.Create('PostThreadMessage ' + SysErrorMessage(GetLastError));
    H[0] := Result.FEventArray[cteiClientAttached];
    { Wait until thread has attached client socket to his own context. }
    WaitRes := WaitForMultipleObjects(2, @H, FALSE, Infinite);
    if WaitRes = WAIT_FAILED then
        raise Exception.Create('Wait client ThreadAttach ' + SysErrorMessage(GetLastError))
...
{$IFNDEF ICS_NewMessaging}  { V10 }
        ...
        repeat
            WaitRes := MsgWaitForMultipleObjects(0, Dummy, FALSE, 500, QS_POSTMESSAGE or QS_SENDMESSAGE);
            if WaitRes = WAIT_FAILED then
                raise Exception.Create('Wait for threads failed ' + SysErrorMessage(GetLastError))
...
begin
    for I := 0 to FThreadList.Count - 1 do begin
{$IFNDEF ICS_NewMessaging}  { V10 }
        PostThreadMessage(TThread(FThreadList[I]).ThreadID, WM_THREAD_DISCONNECT_ALL, 0, 0);
{$ENDIF N ICS_NewMessaging} { V10 }
    end;
end;
...
begin
    ...
    if Assigned(AThread) then begin
        H[1] := AThread.Handle;
{$IFNDEF ICS_NewMessaging}  { V10 }
        if not PostThreadMessage(AThread.ThreadID, WM_THREAD_REMOVE_CLIENT, WPARAM(Client), 0) then    { V7.02 }
            raise Exception.Create('PostThreadMessage ' + SysErrorMessage(GetLastError));
{$ENDIF N ICS_NewMessaging} { V10 }
        ...
        WaitRes := WaitForMultipleObjects(2, @H, FALSE, Infinite);
        if WaitRes = WAIT_FAILED then
            raise Exception.Create('Wait client ThreadDetach ' + SysErrorMessage(GetLastError));
        {else if WaitRes = WAIT_OBJECT_0 + 1 then ...
        ...
        if Msg.hwnd = 0 then begin
            ...
            WM_THREAD_ADD_CLIENT :
                begin
                    ...
                    WaitRes := WaitForSingleObject(FEventArray[cteiThreadUnlock], INFINITE);
                    if WaitRes = WAIT_FAILED then
                        raise Exception.Create('Wait failed ' + SysErrorMessage(GetLastError));
...
{$IFNDEF ICS_NewMessaging}  { V10 }
                    PostMessage(FServer.Handle, FServer.FMsg_WM_THREAD_EXCEPTION, WPARAM(Self), LPARAM(ErrMsg));  { V7.02 }
{$ENDIF N ICS_NewMessaging} { V10 }
                end;
            end;
...
begin
    try
        if Assigned(FOnThreadException) then
            FOnThreadException(Self, TObject(Msg.WParam) as TWsClientThread, StrPas(PChar(Msg.LParam)));
    finally

...
Mar 13, 2025 V9.5 Added SSL_CTX_set_keylog_callback, allows Wireshark to use TLS keys.
...
const
    IcsSSLEAYVersion   = 1000;
    CopyRight : String = ' IcsSSLEAY (c) 2003-2025 F. Piette V10.0 ';
...
{       Look for a dynamic symbol exported from this program.
        ...
        if (Addr <> nil)  and (dlerror = nil)  and (dladdr(UIntPtr(Addr), Info) <> 0) and (Info.dli_fname <> nil)
                                                                            and (Info.dli_fbase = ExeBaseAddress) then begin
            Result := System.SysUtils.StrLen(Info.dli_fname);   { V10.0 }

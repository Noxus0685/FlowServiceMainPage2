...
const
    FtpServerVersion         = 1000;
    CopyRight : String       = ' TFtpServer (c) 1998-2024 F. Piette V10.0 ';
...
{$IFDEF USE_SSL}
    TFtpSslType  = (ftpAuthSsl, ftpAuthTls, ftpAuthTlsP, ftpAuthTlsC, ftpImplicitSsl);
    TFtpSslTypes = set of TFtpSslType;
    TCurFtpSslType  = (curftpSslNone, curftpAuthSsl, curftpAuthTls, curftpAuthTlsP, curftpAuthTlsC, curftpImplicitSsl);
{$ENDIF}
    ...
    TListType        = (ListTypeName, ListTypeUnix, ListTypeFacts);    { angus V1.54 same as Server }
type
    EFtpCtrlSocketException = class(Exception);
    TFtpCtrlState = (ftpcInvalid, ftpcWaitingUserCode, ftpcWaitingPassword, ftpcReady, ftpcWaitingAnswer, ftpcFailedAuth);  { angus V7.06 }
...
{$ENDIF}
    ...
    TFtpSrvAuthenticateEvent  =  procedure (Sender: TObject; Client: TFtpCtrlSocket; UserName: TFtpString; Password: TFtpString;
                                                                                            var Authenticated : Boolean) of object;
    TFtpSrvOtpMethodEvent  =  procedure (Sender: TObject; Client: TFtpCtrlSocket; UserName: TFtpString;
...
{$IFDEF ICS_NewMessaging}   { V10 }
        procedure WMFtpSrvCloseRequest(const Value : TIcsSocketMsgRec); virtual;
        procedure WMFtpSrvAbortTransfer(const Value : TIcsSocketMsgRec); virtual;
        procedure WMFtpSrvCloseData(const Value : TIcsSocketMsgRec); virtual;
        procedure WMFtpSrvStartSend(const Value : TIcsSocketMsgRec); virtual;
{$ELSE}
...
{$IFNDEF ICS_NewMessaging}   { V10 }
        ...
        procedure FreeMsgHandlers; override;
{$ENDIF N ICS_NewMessaging}   { V10 }
        function  ValidateHosts(Stop1stErr: Boolean=True; NoExceptions: Boolean=False): String; virtual; { V8.63 }
        function  RecheckSslCerts(var CertsInfo: String; Stop1stErr: Boolean=True; NoExceptions: Boolean=False): Boolean; { V8.63 }
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
class procedure TFtpServer.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    FtpSrv: TFtpServer;
begin
    if Assigned(Value.Instance) and (Value.Instance is TFtpServer) then begin
        FtpSrv := TFtpServer(Value.Instance);
        try
            with Value do begin
            if  Msg =  WM_ICS_FTPSRV_CLOSE_REQUEST  then
                FtpSrv.WMFtpSrvCloseRequest(Value)
            else if Msg =  WM_ICS_FTPSRV_ABORT_TRANSFER then
                FtpSrv.WMFtpSrvAbortTransfer(Value)
            else if Msg =  WM_ICS_FTPSRV_CLOSE_DATA then
                FtpSrv.WMFtpSrvCloseData(Value)
            else if Msg =  WM_ICS_FTPSRV_START_SEND then
                FtpSrv.WMFtpSrvStartSend(Value)
            end;
        except
            on E: Exception do
                FtpSrv.HandleBackGroundException(E, 'TFtpSrv:DispatchMessage: ' + IntToStr(Value.Msg));
        end;
    end
    else
...
{$IFDEF ICS_NewMessaging}   { V10 }
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServer.WMFtpSrvCloseRequest(const Value: TIcsSocketMsgRec);
var
    Client : TFtpCtrlSocket;
begin
    Client := TFtpCtrlSocket(Value.LParam);
    try
        if FSocketServer.IsClient(Client) then begin
            { Check if client.ID is still the same as when message where posted }
            if WPARAM(Client.ID) = Value.WParam then begin
            ...
            TriggerDisplay (Client, 'Warning, Client Close Request When Not Found');
    except
        on E:Exception do
            TriggerDisplay (Client, 'Warning, Client Close Request Exception');
    end;
end;

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServer.WMFtpSrvAbortTransfer(const Value: TIcsSocketMsgRec);
var
    Client : TFtpCtrlSocket;
    Data   : TWSocket;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServer.WMFtpSrvCloseData(const Value: TIcsSocketMsgRec);
var
    Client : TFtpCtrlSocket;
    Data   : TWSocket;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServer.WMFtpSrvStartSend(const Value: TIcsSocketMsgRec);
var
    Client      : TFtpCtrlSocket;
begin
    Client := TObject(Value.LParam) as TFtpCtrlSocket;
...
begin
  { delete temporary ZLIB file if not from cache }
    try
        ...
        if (Client.ZStreamState > ftpzStateNone) and Client.ZCompFileDelete then begin
            try
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServer.ServerClientConnect(Sender: TObject; Client: TWSocketClient; Error: Word);    { angus V7.00 }
var
    MyClient: TFtpCtrlSocket;   { renamed to avoid conflict with TWSocketClient angus V7.00 }
begin
    if Error <> 0 then
        raise FtpServerException.Create('Session Available Error - ' +  GetWinsockErr(Error));
    ...
    if Self is TSslFtpServer then begin     {  V1.48 }
       ...
       { V8.64 see if LOGIN only allowed after SSL/TLS negotiated }
        ...
        if TSslFtpserver(Self).GetIcsHosts <> Nil then begin
             if (TSslFtpserver(Self).GetIcsHosts.Count > 0) and (MyClient.FIcsHostIdx >= 0) then begin       { V8.64 }
...
{$IFNDEF NO_DEBUG_LOG}                                       { V1.46 }
    if CheckLogOptions(loProtSpecDump) then
        DebugLog(loProtSpecDump,  IntToHex(INT_PTR(MyClient), SizeOf(Pointer) * 2) + ' Client Connect Error - ' +
                                                                        GetWinsockErr(Error) + ' ' + IntToStr(MyClient.HSocket));
{$ENDIF}
...
begin
    ...
    try
        ...
        if (Client.CurrentCodePage <> CP_UTF8) and (ftpAutoDetectCodepage in Client.Options) and (CharsetDetect(RawParams) = cdrUtf8) then
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
        TFtpServer.PostMessage(Self, WM_ICS_FTPSRV_CLOSE_REQUEST, WPARAM(Client.ID), LPARAM(Client));
{$ELSE}
        PostMessage(Handle, FMsg_WM_FTPSRV_CLOSE_REQUEST, WPARAM(Client.ID), LPARAM(Client));
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServer.ServerClientDisconnect(Sender: TObject; Client: TWSocketClient; Error: Word); { angus V7.00 }
var
...
{$ENDIF}
  ...
  { angus V1.54 - check if user account is set-up for authentication using a
    ...
    if Client.OtpMethod = OtpKeyNone then
        ...
        Challenge := OtpCreateChallenge(Client.OtpMethod, Client.OtpSequence, Client.OtpSeed);
     ...
     {  angus V1.54 - if authenticating using a one time password, we need to get
       ...
       Client.OtpSequence should be saved in the user account details }
        if Client.OtpMethod > OtpKeyNone then begin
            ...
            Authenticated := OtpTestPassword(Client.PassWord, UserPassword, Client.OtpMethod, Client.OtpSequence, Client.OtpSeed);
        end;
...
begin
    ...
    Answer := Format(msgPWDSuccess, [FormatResponsePath(Client, Client.Directory)]);
end;
...
begin
    ...
    Answer := Format(msgPWDSuccess, [FormatResponsePath(Client, Client.Directory)]); { AG V1.52 }
end;
...
{ angus V1.54 report performance }
    ...
    if (Client.ZStreamState = ftpZStateSaveDecom) and (Client.ZFileStream.Size > 0) and Assigned(Client.DataStream) and
...
begin
    ...
    try
        ...
        if (not Client.HasOpenedFile) and (Length(Client.FilePath) > 0) and (not Assigned(Client.DataStream)) then begin
            ...
            if (Client.CurCmdType = ftpcAPPE) and (NewPos > -1) then begin
                ...
                try
                    Client.DataStream := OpenFileStream(Client.FilePath, Client.FileModeWrite); { angus V1.57 }
                finally
            ...
            if Client.DataStream.Position <> Client.RestartPos then begin
                ...
                Client.AbortingTransfer := TRUE;
{$IFDEF ICS_NewMessaging}   { V10.0 }
                TFtpServer.PostMessage(Self, WM_ICS_FTPSRV_ABORT_TRANSFER, WPARAM(Client.ID), LPARAM(Client));
{$ELSE}
                PostMessage(Handle, FMsg_WM_FTPSRV_ABORT_TRANSFER, WPARAM(Client.ID), LPARAM(Client));
{$ENDIF ICS_NewMessaging}   { V10.0 }
                Exit;
            end;
...
begin
  { angus V8.50 check for nasty that allowed access to higher level directories than root }
    ...
    { angus V7.08 check if a virtual directory is being used, assume allowed if non-blank }
        ...
        {$IFDEF MSWINDOWS}
          Result := Pos(AnsiLowerCase(ExcludeTrailingPathDelimiter(Client.HomeDir)), AnsiLowerCase(Path)) = 1
        {$ELSE}
    ...
    if IsUNC(FileName) then
        ...
        if (Length(FileName) > 1) and (FileName[2] = ':') then begin
            ...
            Path  := Copy(FileName, 3, Length(FileName));
                ...
                Path  := Copy(Client.HomeDir, Length(Drive) + 1, Length(Client.HomeDir)) + Copy(FileName, 2, Length(FileName));
        end
        else begin
        ...
        if (not Client.HasOpenedFile) and (Length(Client.FilePath) > 0) and (not Assigned(Client.DataStream)) then begin
            ...
            try
                if not FileExists(Client.FilePath) then begin
                    Answer := Format(msgRetrNotExists, [FormatResponsePath(Client, Client.FilePath)]);
                    Exit;
                end;
                Client.DataStream := OpenFileStream(Client.FilePath, Client.FileModeRead); { angus V1.57 }
                Client.DataStream.Position := Client.RestartPos;
            finally
     ...
     { angus V1.54 see if compressing the file with zlib }
        if (Client.CurrTransMode = ftpTransModeZDeflate) then begin
            ...
            if Client.FilePath <> '' then begin { directory listings don't have file name }
             ...
             { check sufficient space on work volume for compressed file }
                ...
                if (Client.DataStream.Size + 100000) > FreeSpace then begin   { don't fill volume!! }
                    TriggerDisplay(Client, 'Insufficient space on ' + FZlibWorkDir + ', need ' + IntToKByte
                                                             (Client.DataStream.Size) + ', free ' + IntToKByte (FreeSpace));
                ...
                if Done then begin
                    ...
                    Client.ZCompInfo := ' compressed size ' + IntToKbyte (Client.ZFileStream.Size) + 'bytes, uncompressed size ' +
         ...
         {angus V8.04 don't use thread if no real compression needed unless more than one meg }
                if (Client.ZCurLevel = Z_NO_COMPRESSION) and (Client.DataStream.Size < 1000000) then begin
                    TriggerDisplay(Client, 'Skipped thread to compress upload file, no compression');
                    try
                        ZlibCompressStreamEx(Client.DataStream, Client.ZFileStream, Client.ZCurLevel, zsZLib, false, Self, UpdateThreadOnProgress);
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
        TFtpServer.PostMessage(Self, WM_ICS_FTPSRV_START_SEND, 0, LPARAM(Client));
{$ELSE}
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
                TFtpServer.PostMessage(Self, WM_ICS_FTPSRV_CLOSE_DATA, WPARAM(Client.ID), LPARAM(Client));
{$ELSE}
                PostMessage(Handle, FMsg_WM_FTPSRV_CLOSE_DATA, WPARAM(Client.ID), LPARAM(Client));
{$ENDIF ICS_NewMessaging}   { V10.0 }
            end;
        end;
    except
        ...
        NOTE3: -R or -SUBDIRS recursive listings have a file name with path and leading /, ie
         ...
         { angus V1.54 see if returning listing on control socket instead of data socket }
            if Client.CurCmdType in [ftpcSiteIndex, ftpcSiteCmlsd, ftpcXCMLSD] then begin   { angus 7.01 }
                ...
                if Client.CurCmdType = ftpcSiteIndex then
                     Answer := Format (msgIndexFollows, [Params]) + IcsCRLF + Answer + msgIndexDone;
                if Client.CurCmdType in [ftpcSiteCmlsd, ftpcXCMLSD] then    { angus 7.01 }
                     Answer := msgMlstFollows + IcsCRLF + Answer + msgMlstFollowDone;
                CloseFileStreams(Client);
            end
            else
            begin
...
{        Client.DirListPath := Client.Directory + '*.*'   V7.08 }
    ...
    begin
        Buf := FormatResponsePath(Client, Client.DirListPath) + ' Permission denied' + IcsCRLF;
 ...
 { angus 1.54 see if using a thread to list directory }
    ...
    try
     { angus 1.54 moved all listing code to FtpSrvC }
        ...
        if TotalFiles = -1 then
            ...
            TriggerDisplay(Client, 'Completed directory listing for: ' + Client.DirListPath + ', Total Files: ' + IntToStr (TotalFiles));
    finally
        TriggerLeaveSecurityContext(Client);              { AG V1.52 }
    end;
    ...
    if Client.DataStream.Size = 0 then begin
        if TotalFiles = -1 then
            Buf := 'Listing failed' + IcsCRLF  { V7.08 }
        else
            Buf := FormatResponsePath(Client, Client.DirListPath) + ' not found' + IcsCRLF; { AG V1.52 }
...
begin
    ...
    try
        ...
        if Client.RestartPos < 0 then begin        { 20020916 }
            ...
            if (ftpsModeZNoResume in Options) and (Client.CurrTransMode = ftpTransModeZDeflate) then   { angus V1.55 }
...
begin
    ...
    if Allowed then begin
        Answer := Format(msgRntoOk, [FormatResponsePath(Client, Client.FromFileName), FormatResponsePath(Client, Client.ToFileName)]);
...
{$ENDIF}
    if (CurrentPort >= FPasvPortRangeStart) and (CurrentPort <= (FPasvPortRangeStart + FPasvPortRangeSize)) then begin
        PBoolean(IncPtr(FPasvPortTable, SizeOf(Boolean) * (CurrentPort - FPasvPortRangeStart)))^ := FALSE;
    end;
...
{$IFDEF USE_SSL}
        TriggerDisplay (Client, '! Server IP: ' + Client.CServerAddr + ', Passive Port: ' + Client.DataSocket.Port +
                                                           ', External IP: ' + FPasvIpAddr + ', Remote IP: ' + Client.CPeerAddr);
{$ENDIF}

        if Client.sin.sin_addr.s_addr = WSocket_htonl($7F000001) then
            Answer := Format(msgPasvLocal, [IcsHiByte(DataPort), IcsLoByte(DataPort)])
        else begin
...
{$ENDIF}
            ...
            if not SetPasvIp then
                Answer := Format(msgPasvRemote, [ord(IPAddr.S_un_b.s_b1), ord(IPAddr.S_un_b.s_b2), ord(IPAddr.S_un_b.s_b3),
                                                            ord(IPAddr.S_un_b.s_b4), IcsHiByte(DataPort), IcsLoByte(DataPort)])
            else begin
                PASVAddr.S_addr := WSocket_inet_addr(AnsiString(APasvIp));
                if (PASVAddr.S_addr = u_long(INADDR_NONE)) or (PASVAddr.S_addr = 0) then { angus v1.53 0.0.0.0 not allowed }
                    raise Exception.Create('Invalid PASV IP Address')
                else
                    Answer := Format(msgPasvRemote, [ord(PASVAddr.S_un_b.s_b1), ord(PASVAddr.S_un_b.s_b2), ord(PASVAddr.S_un_b.s_b3),
                                                       ord(PASVAddr.S_un_b.s_b4), IcsHiByte(DataPort), IcsLoByte(DataPort)]);
            end;
        end;
...
begin
    ...
    try
        ...
        while (J <= Length(Params)) and (((Params[J] >= '0') and (Params[J] <= '9')) or (Params[J] = '.')) do
           Inc(J);
        if (J >= 15) and (J <= 19) then begin  { found date and time so we are setting it, not getting it }
            ...
            while (J <= Length(Params)) and ((Params[J] = '+') or (Params[J] = '-') or ((Params[J] >= '0') and (Params[J] <= '9'))) do
                Inc(J);
        ...
        if not FExists then
            ...
            if Length(FileTime) <> 0 then
                ...
                Answer := Format(msgMdtmFailed, ['UTC File time retrieval failed']) ;
        end;
    except
        on E:Exception do begin
   ...
   { angus V1.54 }
       ...
       { check sufficient space on work volume for compressed files }
            ...
            if FZlibMinSpace > FreeSpace then begin   { don't fill volume!! }
                if FreeSpace = -1 then
                    TriggerDisplay(Client, 'Error, working directory volume not available ' + FZlibWorkDir + ' - ' + GetWindowsErr (GetLastError))
                else
                    TriggerDisplay(Client, 'Insufficient space on ' + FZlibWorkDir + ', need ' + IntToKByte(FZlibMinSpace) + ', free ' + IntToKByte(FreeSpace));
    ...
    try
        ...
        try
            ...
            try
                ...
                if (UniqueName = '') or (FileExists(BuildFilePath(Client, Client.Directory, UniqueName))) then begin
                    UniqueName := ExtractFilename(CreateUniqueFile(Client.Directory, 'FTP', ''));
...
begin
    ...
    try
        ...
        Answer := msgFeatFollows + IcsCRLF +
                  ' HOST'+ IcsCRLF +             { angus V7.01 }
                  ' SIZE'+ IcsCRLF +
                  ' EPRT'+ IcsCRLF +             { V9.4 }
                  ' EPSV'+ IcsCRLF +             { V9.4 }
                  ' REST STREAM'+ IcsCRLF +      { angus V1.39 (been supported for years) }
                  ' MDTM'+ IcsCRLF +
                  ' MDTM YYYYMMDDHHMMSS[+-TZ] filename'+ IcsCRLF +       { angus V1.38 }
                  ' MLST size*;type*;perm*;create*;modify*;'+ IcsCRLF +  { angus V1.38 }
                  ' MFMT'+ IcsCRLF +                                     { angus V1.39 }
                  ' MD5'+ IcsCRLF +                                      { angus V1.39 }
                  ' XCRC "filename" start end'+ IcsCRLF +                { angus V1.54 }
                  ' XMD5 "filename" start end'+ IcsCRLF +                { angus V1.54 }
                  ' CLNT'+ IcsCRLF +                                     { angus V1.54 }
                  ' SITE INDEX;ZONE';                                   { angus V1.54 }
        if Assigned (FOnSiteMsg) then
            Answer := Answer + ';MSG';         { angus V1.54 }
        if Assigned (FOnSiteExec) then
            Answer := Answer + ';EXEC';       { angus V1.54 }
        if Assigned (FOnSitePaswd) then
            Answer := Answer + ';PSWD';      { angus V1.54 }
        if ftpsSiteXmlsd in FOptions then
             Answer := Answer + ';CMLSD;DMLSD'; { angus V1.54 }
        Answer := Answer + IcsCRLF;
        if Assigned (FOnCombine) then
               Answer := Answer + ' COMB'+ IcsCRLF; { angus V1.54 }
        if ftpModeZCompress in Client.Options then
               Answer := Answer + ' MODE Z'+ IcsCRLF;
        if ftpsSiteXmlsd in FOptions then
               Answer := Answer + ' XCMLSD' + IcsCRLF +
                                  ' XDMLSD' + IcsCRLF;        { angus V7.01 }
        if ftpsEnableUtf8 in FOptions then
               Answer := Answer + ' UTF8' + IcsCRLF +
                                  ' LANG ' + FLanguage + IcsCRLF +
                                  ' OPTS MODE;UTF8;' + IcsCRLF; { angus V7.01 }
    {$IFDEF USE_SSL}
        if Self is TSslFtpServer then begin     {  V1.48 }
            if Client.FtpSslTypes <> [] then begin             { V1.47 }
                ...
                Answer := Answer + ' PROT C;P;' + IcsCRLF +
                                   ' PBSZ'      + IcsCRLF;
            end;
        end;
    {$ENDIF}
...
begin
    ...
    if NewValue < OldValue then begin
        ...
        while I < OldValue do begin
            if TablePtr^ then
                raise Exception.Create('Unable to change PasvPortRangeSize when port is in use.');
            Inc(I);
            Inc(TablePtr);
        end;
    end;
...
begin
    ...
    while I < FPasvPortRangeSize do begin
        if TablePtr^ then
            raise Exception.Create('Unable to change PasvPortRangeStart when port is in use.');
        Inc(I);
        Inc(TablePtr);
    end;
...
begin
    ...
    try
        if FindFirst(FileName, faArchive + faDirectory, F) = 0 then
            Answer := msgMlstFollows + Params + IcsCRLF +
                      ' ' + FormatFactsDirEntry(F, F.Name) + IcsCRLF +
                      msgMlstFollowDone
        else
...
begin
    ...
    try
        ...
        if Md5Sum = '' then begin
            ...
            if (FMd5UseThreadFileSize = 0) or  (FileSize < FMd5UseThreadFileSize) then begin
                Md5Sum := FtpFileMD5(FileName, Client, FileMD5OnProgress, Client.HashStartPos, Client.HashEndPos, Client.FileModeRead); { angus V1.57 V7.07 }
    ...
    try
      { get file name and optional start and end arguments }
        ...
        if Crc32b = '' then begin
            ...
            { a blocking function.                                          }
            if (FMd5UseThreadFileSize = 0) or (FileSize < FMd5UseThreadFileSize) then begin
                Crc32b := FtpFileCRC32B(FileName, Client, FileMD5OnProgress, Client.HashStartPos, Client.HashEndPos, Client.FileModeRead); { angus V1.57 V7.07 }
...
begin
    ...
    try
        ...
        begin
            ...
            { Check proto }
            if not(((Proto = 1) and (Client.CurrentSocketFamily = sfIPv4)) or ((Proto = 2) and (Client.CurrentSocketFamily = sfIPv6))) then
            begin
            ...
            else if ((Proto = 1) and (AFamily <> sfIPv4)) or ((Proto = 2) and (AFamily <> sfIPv6)) then
            begin
                Answer := msgSyntaxParam;
                Exit;
            end;
    ...
    if IsClient(AThread.Client) and (AThread.Client.ID = AThread.ClientID) then begin
     ...
     { angus V1.54 }
             ...
             if AThread.OutData = '' then begin
                ...
                AThread.Client.AnswerDelayed := FALSE;
{$IFDEF ICS_NewMessaging}   { V10.0 }
                TFtpServer.PostMessage(Self, WM_ICS_FTPSRV_START_SEND, 0, LPARAM(AThread.Client));
{$ELSE}
...
begin
    ...
    begin
        ...
        try
            ...
            L1 :=  IcsMbToWc(SrcCodePage, 0, Buf, BytesRead, nil, 0);
            GetMem(BufW, L1 * SizeOf(WideChar));
            try
                IcsMbToWc(SrcCodePage, 0, Buf, BytesRead, BufW, L1);
                L2 := IcsWcToMb(CP_ACP, 0, BufW, L1, nil, 0, nil, nil);
                if L2 <> Len then
                    ReallocMem(Buf, L2);
                L1 := IcsWcToMb(CP_ACP, 0, BufW, L1, Buf, L2, nil, nil);
                SetLength(Str, L1);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF COMPILER12_UP}
procedure TFtpCtrlSocket.DataStreamReadString(var Str: UnicodeString; Len: TFtpBigInt; SrcCodePage: LongWord); { AG 7.02 }
var
...
begin
    if SrcCodePage <> 1200 {CP_UTF16} then begin
        if Len <= SizeOf(SBuf) then begin
            ...
            Len := IcsMbToWc(SrcCodePage, 0, @SBuf, eLen, nil, 0);
            SetLength(Str, Len);
            IcsMbToWc(SrcCodePage, 0, @SBuf, eLen, Pointer(Str), Len);
        end
        else begin
            GetMem(HBuf, Len);
            try
                eLen := DataStream.Read(HBuf^, Len);
                Len := IcsMbToWc(SrcCodePage, 0, HBuf, eLen, nil, 0);
                SetLength(Str, Len);
                IcsMbToWc(SrcCodePage, 0, HBuf, eLen, Pointer(Str), Len);
            finally
                FreeMem(HBuf);
            end;
        end;
    ...
    if Length(newDrive) = 0 then begin        { AG V1.52 }
        if (ftpCdUpHome in Options) then begin
            if (Length(newValue) > 0) and (newValue[1] = '\') then begin
                ...
                newPath  := Copy(FHomeDir, Length(newDrive) + 1, Length(FHomeDir)) + Copy(newValue, 2, Length(newValue))
            end
            else begin
...
{$ENDIF}
  ...
  {$IFDEF MSWINDOWS}
    if F.FindData.nFileSizeHigh = 0 then
        ...
        SizeStr := IntToStr(F.FindData.nFileSizeLow + (Int64(F.FindData.nFileSizeHigh) shl 32));
  {$ENDIF}
  {$IFDEF POSIX}
  ...
  {$ENDIF}
    ...
    Result := Attr + '   1 ftp      ftp  ' + Format('%11s %s %2.2d %5s ', [SizeStr, StrMonth[Month], Day, TimeStr]) + FileName + IcsCRLF;
end;
...
{$WARNINGS OFF}
  ...
  {$IFDEF MSWINDOWS}
    result := result + 'create=' + FileTimeToStr (F.FindData.ftCreationTime) + ';modify=' +
                                                    FileTimeToStr (F.FindData.ftLastWriteTime) + '; ' + FileName;    { note space before filename is delimiter }
  {$ENDIF}
 ...
 { angus 1.54 build sorted recursive directory }
    ...
    try
     { fill LocFiles dynamic array with SearchRecs, sorted by LocFileList }
        TotFiles := IcsGetDirList (DirListPath, DirListSubDir, DirListHidden, LocFiles, LocFileList, Self, UpdateThreadOnProgress) ;  { V7.08 }
          ...
          {  TMemoryStream (Stream).SetCapacity (TotFiles * 128);  }
            for I := 0 to Pred (TotFiles) do begin
                ...
                if DirListSubDir then   { add path before file name }
                    Buf := BackSlashesToSlashes(FileRecX^.FrSubDirs) + FileRecX^.FrSearchRec.Name
                else
            ...
            { build single line according to listing style }
                if DirListType = ListTypeUnix then
                    ...
                    Buf := FormatFactsDirEntry(FileRecX^.FrSearchRec, Buf) + IcsCRLF
                else
                    Buf := Buf + IcsCRLF;
    ...
    try
        with Client.ProcessingThread do begin
            ...
            if (Keyword = 'MD5') or (Keyword = 'XMD5')  then    { angus V1.54 }
                ...
                OutData := FtpFileCRC32B(InData, Self, UpdateThreadOnProgress, Client.HashStartPos, Client.HashEndPos,
                                                                                                        Client.FileModeRead) { angus V1.57, V7.08 }
                ...
                try
                    ...
                    try
                        try
                            ...
                            if TotalFiles = -1 then
                                ...
                                AuxData := 'Completed directory listing for: ' + Client.DirListPath + ', Total Files: ' + IntToStr (TotalFiles);
                        except
                            on E:Exception do begin     { V8.71 handle exception }
                    ...
                    if Client.DataStream.Size = 0 then begin   { V7.08 }
                        if TotalFiles = -1 then
                            Buf := 'Listing failed' + IcsCRLF
                        else
                            Buf := Client.FtpServer.FormatResponsePath(Client, Client.DirListPath) + ' not found' + IcsCRLF;
...
begin
    ...
    if Value <> FFtpSslTypes then begin
        if (ftpImplicitSsl in Value) and ((ftpAuthSsl in Value) or (ftpAuthTls in Value) or (ftpAuthTlsP in Value) or (ftpAuthTlsC in Value)) then begin
            FFtpSslTypes := [];
            raise Exception.Create('Option ftpImplicitSsl cannot be combined with explicit SSL types.');
         end
         else
            FFtpSslTypes := Value;
    end;
...
begin
    ...
    if (Params = 'TLS')   and (ftpAuthTls  in FFtpSslTypes) then
        TmpCurFtpSslType := curftpAuthTls
    else if (Params = 'SSL')   and (ftpAuthSsl  in FFtpSslTypes) then
        TmpCurFtpSslType := curftpAuthSsl
    else if (Params = 'TLS-C') and (ftpAuthTlsC in FFtpSslTypes) then
        TmpCurFtpSslType := curftpAuthTlsC
    else if (Params = 'TLS-P') and (ftpAuthTlsP in FFtpSslTypes) then
        TmpCurFtpSslType := curftpAuthTlsP;
    ...
    try
     ...
     { this will be ignored if Server Name sent in SSL HELO to select another IcsHost }
                        ...
                        if Assigned(SslCtx) then begin
                            ...
                            {$IFNDEF NO_DEBUG_LOG}
                                if CheckLogOptions(loSslInfo) then
                                    DebugLog(loSslInfo, 'HOST "' + Client.Host + '" for IcsHost #' + IntToStr(J));
                            {$ENDIF}
                                break;
                            end;
                        end;
        ...
        if (Client.SslState = sslNone) then begin
            ...
            PlainAnswer := PlainAnswer + IcsCRLF;
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
            TFtpServer.PostMessage(Self, WM_ICS_FTPSRV_CLOSE_DATA, WPARAM(Client.ID), LPARAM(Client));
{$ELSE}
            PostMessage(FHandle, FMsg_WM_FTPSRV_Close_Data, WPARAM(Client.ID), LPARAM(Client));
{$ENDIF ICS_NewMessaging}   { V10.0 }
            Disconnect := FALSE;
        end;
    end;
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
                TFtpServer.PostMessage(Self, WM_ICS_FTPSRV_ABORT_TRANSFER, WPARAM(Client.ID), LPARAM(Client));
{$ELSE}
                PostMessage(FHandle, FMsg_WM_FTPSRV_ABORT_TRANSFER, WPARAM(Client.ID), LPARAM(Client));
{$ENDIF ICS_NewMessaging}   { V10.0 }
            end;
        end;
    end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TSslFtpServer.TriggerRetrSessionConnected(Client: TFtpCtrlSocket; Data: TWSocket; AError: Word);
begin
...
begin
  { Implicit SSL cannot be combined with explicit SSL }
    if Value <> FFtpSslTypes then begin
        if (ftpImplicitSsl in Value) and ((ftpAuthSsl in Value) or (ftpAuthTls in Value) or  (ftpAuthTlsP in Value) or
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TSslFtpServer.ValidateHosts(Stop1stErr: Boolean=True; NoExceptions: Boolean=False): String; { V8.63 }
var
    J: Integer;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TSslFtpServer.RecheckSslCerts(var CertsInfo: String; Stop1stErr: Boolean=True; NoExceptions: Boolean=False): Boolean;  { V8.63 }
begin
    Result := False;
    if Assigned(FSocketServer) then begin
        Result := TSslWSocketServer(FSocketServer).RecheckSslCerts(CertsInfo, Stop1stErr, NoExceptions);
    end;
end;

...
const
    THttpServerVersion = 1000;
    CopyRight : String = ' TIcsHttpProxy (c) 2025 F. Piette V10.0 ';
    DefServerHeader : string = 'Server: ICS-Proxy-V10.0';
...
//    procedure TargetDnsLookupDone(Sender: TObject; Error: Word);
    ...
    procedure TargetHandshakeDone(Sender: TObject; ErrCode: Word; PeerCert: TX509Base; var Disconnect : Boolean);
    procedure TargetDataAvailable(Sender: TObject; Error: Word);
    procedure TargetCliNewSession(Sender: TObject; SslSession: Pointer; WasReused: Boolean; var IncRefCount : Boolean);
    procedure TargetCliGetSession(Sender: TObject; var SslSession: Pointer; var FreeSession : Boolean);
...
{$ENDIF} // AUTO_X509_CERTS
  ...
  protected
   { Protected declarations }
    ...
    procedure ServerSvrNewSession(Sender: TObject; SslSession, SessId: Pointer; Idlen: Integer; var AddToInternalCache: Boolean);
    procedure ServerSvrGetSession(Sender: TObject; var SslSession: Pointer; SessId: Pointer; Idlen: Integer; var IncRefCount: Boolean);
    ...
    procedure ServerAlpnSelect(Sender: TObject; ProtoList: TStrings; var SelProto : String; var ErrCode: TTlsExtError);  { V8.57 }
{$IFDEF ICS_NewMessaging}   { V10 }
    procedure WMTargetConnected(const Value : TIcsSocketMsgRec);
{$ELSE}
...
begin
    Self.FLogDescr := FHostTag + ' - ' + FProxySource.IcsHosts[IcsHostIdx].Descr + ' (' + IntToStr (FCliId) + ') ';
    if (FProxySource.DebugLevel >= DebugConn) then
        LogSrcEvent('Host #' + IntToStr(FIcsHostIdx) + ' ' + FSrcHost + ', Listener ' + CServerAddr + ':' + CServerPort);
...
begin
    ...
    with FProxySource.ProxyTargets[FPxyTargetIdx] do begin
        if (FProxySource.DebugLevel >= DebugConn) then
            LogSrcEvent('Target #' + IntToStr(FPxyTargetIdx) + ' - ' + TarHost + ':' + IntToStr(TarPort));
       ...
       { V4.49 zero target means copy source port }
         if TarPort > 0 then begin
            ...
            if (FProxySource.DebugLevel >= DebugConn) then
                LogSrcEvent('Copied source port for target - ' + TarHost + ':' + CServerPort);
        end;
...
//    if FTarConnecting then Exit;
    if (FTarHost = '') or (FTarPort = '') then begin
        LogSrcEvent('Failed to Target #' + IntToStr(FPxyTargetIdx) + ' - Host and/or Port Blank');
        Result := False;
        exit;
    end;
    FTarClosedFlag := False;
  ...
  { localhost fails if real local address used }
    ...
    if (FProxySource.DebugLevel >= DebugConn) then begin
        if FPxyTargetIdx >= 0 then
            LogSrcEvent('Connecting to Target #' + IntToStr(FPxyTargetIdx) + ' - ' + FTarHost + ':' + FTarPort)
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
            TProxyClient.PostMessage(Self, WM_ICS_PROXY_TARGET_CONNECTED, 0, LPARAM(Self))
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TProxyClient.TargetHandshakeDone(Sender: TObject; ErrCode: Word; PeerCert: TX509Base; var Disconnect : Boolean);
var
    CertChain: TX509List;
{$IFDEF MSCRYPT_Any}
...
{$ENDIF MSCRYPT_Any}
            ...
            info := info + IntToStr(CertChain.Count) + ' SSL certificates in the verify chain:' +
                                                         IcsCRLF + CertChain.AllCertInfo (true, true) ;
            LogTarEvent(info);
        end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TProxyClient.TargetCliNewSession(Sender: TObject; SslSession: Pointer; WasReused: Boolean; var IncRefCount : Boolean);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
class procedure TIcsProxy.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    IcsProxy: TIcsProxy;
begin
    if Assigned(Value.Instance) and (Value.Instance is TIcsProxy) then begin
        IcsProxy := TIcsProxy(Value.Instance);
        try
            with Value do begin
                if Msg = WM_ICS_PROXY_TARGET_CONNECTED then
                    IcsProxy.WMTargetConnected(Value)
            end;
        except
            on E: Exception do
                IcsProxy.HandleBackGroundException(E, 'TIcsProxy:DispatchMessage: ' + IntToStr(Value.Msg));
        end;
    end
    else
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsProxy.WMTargetConnected(const Value: TIcsSocketMsgRec);
var
    MyClient: TProxyClient;
    Resp: String;
begin
    MyClient := TProxyClient(Value.LParam);
...
begin
    if Assigned (FonProxyProg) then
        FonProxyProg(Self, LogOption, Msg) ;
end ;
...
begin
    if Assigned (FonProxyProg) then
        FonProxyProg(Self, loProtSpecInfo, Msg) ;
end ;
...
begin
    if Assigned (FonProxyProg) then
        FonProxyProg(Self, loProtSpecErr, Msg) ;
end ;
...
begin
    if Assigned(FSourceServer) then
        FSourceServer.SetIcsHosts(Value);
end;
...
{$ENDIF} // AUTO_X509_CERTS
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TIcsProxy.ValidateHosts(Stop1stErr: Boolean=True; NoExceptions: Boolean=False): String;
var
    I, J, K: Integer;
    HTag: String;
begin
    ...
    if Assigned(FSourceServer) then begin
     ...
     { each IcsHost must have at least one matching Target HostTag }
        for I := 0 to IcsHosts.Count - 1 do begin
            ...
            if (J < 0) then begin
                Result := Result + 'Host ' + IntToStr(I) + ' ' + HTag + ', no matching proxy target found';
          ...
          { check source tag is not a duplicate }
            if I > 0 then begin
                for K := 0 to I - 1 do begin
                    ...
                    if HTag = IcsHosts[K].HostTag then begin
                        Result := Result + 'Host ' + IntToStr(I) + ' ' + HTag + ' is a duplicate';
                        if Stop1stErr then raise ESocketException.Create(Result);
                        continue;
                    end;
                end;
            ...
            if Assigned(FSslSessCache) then begin
                IcsHosts[I].SslCtx.SslSessionCacheModes := [sslSESS_CACHE_CLIENT, sslSESS_CACHE_NO_INTERNAL_LOOKUP, sslSESS_CACHE_NO_INTERNAL_STORE];
            end;
        end;
    end;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TIcsProxy.RecheckSslCerts(var CertsInfo: String; Stop1stErr: Boolean=True; NoExceptions: Boolean=False): Boolean;
begin
    Result := False;
    if Assigned(FSourceServer) then begin
        Result := TSslWSocketServer(FSourceServer).RecheckSslCerts(CertsInfo, Stop1stErr, NoExceptions);
    end;
end;
    ...
    if IcsHosts.Count > 0 then begin
        for I := 0 to IcsHosts.Count - 1 do begin
            with IcsHosts[I] do begin
                ...
                if ((BindIdxNone = MLIndx) or (BindIdx2None = MLIndx) or (BindIdxSsl = MLIndx) or (BindIdx2Ssl = MLIndx)) and
...
{$IFNDEF NO_DEBUG_LOG}
 ...
 {$ENDIF}
   FTarSslCtx.SslSessionCacheModes := [];
    if Assigned(FSslSessCache) then begin
        FTarSslCtx.SslSessionCacheModes := [sslSESS_CACHE_CLIENT, sslSESS_CACHE_NO_INTERNAL_LOOKUP, sslSESS_CACHE_NO_INTERNAL_STORE];
    end;
...
{$ENDIF}
     ...
     { V8.49 returns list of exceptions, if any }
        ...
        if FSourceServer.ListenAllOK then
            LogProgEvent('Proxy server started listening OK' + IcsCRLF + FSourceServer.ListenStates)
        else
            LogProgEvent('Proxy server failed to start listening' + IcsCRLF + FSourceServer.ListenStates);
    end;
...
begin
    ...
    { create target socket, sets up event handlers and buffers, and counters  }
        ...
        MyClient.FLogDescr := IcsHosts[MyClient.IcsHostIdx].Descr + ' (' + IntToStr (MyClient.CliId) + ') ';
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsProxy.ServerAlpnSelect(Sender: TObject; ProtoList: TStrings; var SelProto : String; var ErrCode: TTlsExtError);  { V8.57 }
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsProxy.ServerSetSessionIDContext(Sender : TObject; var SessionIDContext : TSslSessionIdContext);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsProxy.ServerSvrNewSession(Sender: TObject; SslSession, SessId: Pointer; Idlen: Integer; var AddToInternalCache: Boolean);
var
    LookupKey : string;
    MyClient: TProxyClient;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsProxy.ServerSvrGetSession(Sender: TObject; var SslSession: Pointer; SessId: Pointer; Idlen: Integer; var IncRefCount: Boolean);
var
    LookupKey : string;
begin
    if NOT Assigned(FSslSessCache) then Exit;
...
procedure TIcsProxy.ServerHandshakeDone(Sender: TObject; ErrCode: Word; PeerCert: TX509Base; var Disconnect : Boolean);
var
    MyClient: TProxyClient;
begin
    try
  ...
  { periodically flush SSL host list so certificates are rechecked }
    ...
    if (Duration > FlushSslCacheMins) then begin
        ...
        if FTarSslCertList.Count > 0 then begin
            LogProgEvent('Clearing target SSL certificate cache, total ' + IntToStr(FTarSslCertList.Count));
...
begin
  ...
  { find ProxyTarget may change later if checking path in HTTP header }
    ...
    { V8.49 tell the user something before closing the connectionn }
        if FTarHttp then
            TargetErrorResponse('502 Bad Gateway', 'The gateway can not be found');
        FClosingFlag := True;
        Self.Close;
        exit;
    end;
...
begin
    try
     ...
     { see if we looked up a DNS address, that might be silly }
        ...
        { V8.49 tell the user something before closing the connectionn }
            if FTarHttp then TargetErrorResponse('502 Bad Gateway', 'The gateway can not connect to the requested server: ' + FTarHost);
            if Assigned(FProxySource.FOnTarDisconnect) then
                FProxySource.FOnTarDisconnect(FProxySource, Self, Error);
            Self.Close;
            Exit;
...
begin
  ...
  { send header and body, V8.65 allow to remove passwords before logging }
    ...
    if (FRequestMethod <> httpMethodHead) and (FHtmlRespBodyLen > 0) then begin
        LogTarEvent('Sending single-block body content, length ' + IntToStr(FHtmlRespBodyLen));
...
begin
  ...
  { application event may generate a body to send, but blank contenttype }
    ...
    if Assigned((FProxySource as TIcsHttpProxy).onHttpWellKnown) then begin
        ...
        if BodyStr <> '' then
            ContentType := '';  // no content-type for challenge
    end;
  ...
  { create page and send it, connection closed on return }
    if BodyStr = '' then
        TargetErrorResponse('404 Not Found', 'The requested URL ' + TextToHtmlText(FRequestPath) + ' was not found on this server.')
    else
...
begin
    if FProxySource.IcsHosts[IcsHostIdx].WebRedirectURL = '' then  begin
     ...
     LogTarEvent('Handling redirection to: ' + FProxySource.IcsHosts[IcsHostIdx].WebRedirectURL);
  ...
  // generally, must close connection after relocation, since port may change
     SourceCreatePage(RespStatus, 'text/html', 'Location: ' + FProxySource.IcsHosts[IcsHostIdx].WebRedirectURL, '');
 end;
...
//    FTarConnecting := False;
    ...
    if (FProxySource.DebugLevel >= DebugConn) then begin
        if (Error = 0) or (Error = 10053) then
            LogTarEvent('Remote closed, Data sent ' + IntToStr(FTarSocket.WriteCount) + ', Data recvd ' + IntToStr(FTarSocket.ReadCount))
        else begin
            LogTarEvent('Remote lost (' + WSocketErrorDesc(Error) + '), Data sent ' + IntToStr(FTarSocket.WriteCount) +
                                                                                 ', Data recvd ' + IntToStr(FTarSocket.ReadCount));
        end;
    end;
    ...
    if (OldBody <> ArgStr) then begin
     ...
     { V8.65 see if logging new data and allow to modify it }
      ...
      { now move string back into TBytes buffer with correct codepage }
        FHtmlReqBodyLen := IcsMoveStringToTBytes(BodyStr, FHtmlReqBody, Length(BodyStr), ACodePage, false);
...
begin
  ...
  { V8.50 find codepage, keep Bom so we can restore it later }
    ...
    if (FProxySource.DebugLevel >= DebugHttpHdr) then
        LogTarEvent('Checking for body content URLs, Codepage ' + CodePageToMimeCharsetString(ACodepage) + ', Bom size ' + IntToStr(BOMSize));
...
//    FTarRespModified := True;  // TEMP testing !!!!!!!!!
    if FTarRespModified then begin
      { now move string back into TBytes buffer with correct codepage, keep Bom if it had one }
        FHtmlRespBodyLen := IcsMoveStringToTBytes(BodyStr, FHtmlRespBody, Length(BodyStr), ACodePage, (BOMSize <> 0));
        FRespContentLength := FHtmlRespBodyLen;
    ...
    try  // except
        if (Pos('deflate', FHttpInfo [FHttpCurrResp].ReqAcceptEnc) > 0) then begin
          ...
          ZStreamType := zSGZip;
            ...
            LogTarEvent('Ignored compression, unknown encoding: ' + FHttpInfo [FHttpCurrResp].ReqAcceptEnc);
            exit;
        end;
      ...
      { PENDING - zlib unit needs to be able to read and write buffers!!! }
        ...
        try
            ...
            FTarRespModified := True;
            if (FProxySource.DebugLevel >= DebugHttpHdr) then
                LogTarEvent('Compressed body from ' + IntToStr(InStream.Size) + ' to ' + IntToStr(FRespContentLength)) ;
        finally
            InStream.Destroy;
            OutStream.Destroy;
        end;
...
begin
    ...
    try  // except
        ...
        try
            ...
            FTarRespModified := True;
            if (FProxySource.DebugLevel >= DebugHttpHdr) then
                LogTarEvent('Decompressed body from ' + IntToStr(InStream.Size) + ' to ' + IntToStr(FRespContentLength)) ;
        finally
            InStream.Destroy;
            OutStream.Destroy;
        end;
 ...
 { process one line in header at a time }
  ...
  { beware Transfer-Encoding: gzip only relates to chunks }
    if (FRespContentEncoding <> '') then begin
        if (Pos('deflate', FRespContentEncoding) > 0) or (Pos('gzip', FRespContentEncoding) > 0) or
     ...
     { send request header to remote target, once connected }
      ...
      { V8.65 allow modify data before it's logged to remove passwords }
            if FTarReqModified and (FProxySource.DebugLevel >= DebugHttpHdr) then begin
                ...
                if LogHeader <> '' then
                    LogTarEvent('Modified Request Header: #' + IntToStr(FHttpTotReqs) + IcsCRLF + LogHeader);
            end;
            TargetHdrReqXmit;
        ...
        if FPxyReqState = PxyHdrFind then begin
          ...
          { search for blank line in receive buffer which means we have complete request header }
            ...
            if (HdrLen <= 0) then begin
                if (FProxySource.DebugLevel >= DebugAll) then
                    LogTarEvent('Waiting for more source data');
                Exit;
            end ;
          ...
          { increment request pipeline and keep it, client may send us several requests
            ...
            if FHttpCurrReq < 1 then begin
                ...
                if FHttpInfo[FHttpCurrReq].HttpReqState <> httpStReqStart then begin
                    ...
                    if FHttpCurrReq > MaxPipelineReqs then
                        FHttpCurrReq := 1;
                end;
            end;
         ...
         { tell user what we got }
            if (FProxySource.DebugLevel >= DebugHttpHdr) then begin
             { V8.65 allow modify data before it's logged to remove passwords }
                ...
                if LogHeader <> '' then
                    LogTarEvent('Original Request Header: #' + IntToStr(FHttpTotReqs) + IcsCRLF + LogHeader);
            end
            else if (FProxySource.DebugLevel >= DebugConn) then
            ...
            if FTarConditional AND (FLastReqPath <> FRequestPath) then begin
              ...
              { forward proxy, get target from CONNECT method or HOST header  }
               ...
               { V8.49 ignore request type }
                    ...
                    if FPxyTargetIdx < 0 then begin  { should have been checked earlier }
                        if (FProxySource.DebugLevel >= DebugConn) then
                           LogSrcEvent('Host #' + IntToStr(IcsHostIdx) + ' -' +  HostTag + ', no matching proxy target found for Path: ' + FRequestPath);
                        FClosingFlag := True;
                        Close;
                        exit;
                    end;
         ...
         { if this a new targert }
            if (FTarSocket.State <> wsConnected) or (FTarHost <> FTarCurHost) or (FTarPort <> FTarSocket.Port) then begin
              ...
              { start connection to target server, request won't be sent until later once target connects }
                ...
                if NOT TargetConnect then begin
                    if FTarHttp then
                        TargetErrorResponse('502 Bad Gateway', 'The gateway can not connect to the requested server: ' + FTarHost);
                    FClosingFlag := True;
                    Close;
                    Exit;
                end;
          ...
          { sanity text }
            if (FTarHost = '') then begin
                LogSrcEvent('Host #' + IntToStr(IcsHostIdx) + ' -' + HostTag + ', failed to find new target host, closing');
                FClosingFlag := True;
                Close;
                exit;
            end;
    ...
    { POST content according to length in header }
        if FPxyReqState = PxyLenEnd then begin
          ...
          // keep POST content in buffer and remove from receive buffer
                ...
                if (FProxySource.DebugLevel >= DebugAll) then
                    LogTarEvent('Building POST body, Length ' + IntToStr(FRequestContentLength) + ', Added ' + IntToStr(NewLen) +
                                             ', Remaining ' + IntToStr(FTarReqLenRemain) + ', Left Over ' + IntToStr(FSrcWaitTot)) ;
    ...
    begin
        ...
        { V8.65 allow modify data before it's logged to remove passwords }
            ...
            if Assigned((FProxySource as TIcsHttpProxy).OnLogHeader) then
                (FProxySource as TIcsHttpProxy).OnLogHeader(Self, LogHeader, false, false);
            if LogHeader <> '' then
                LogTarEvent('Modified Response Header: #' + IntToStr(FHttpTotResps) + IcsCRLF + LogHeader);
        end;
        SourceHdrRespXmit;
...
begin
    LoopCounter := 0;
    if (FProxySource.DebugLevel >= DebugAll) then
        LogTarEvent('Forwarding data to source');
    while TRUE do begin
        inc (LoopCounter);
        if (LoopCounter > 100) then Exit;  // sanity check
        if (Self.State <> wsConnected) then Exit;
      ...
      { waiting for headers to arrive }
        if FPxyRespState = PxyHdrFind then begin
          ...
          { search for blank line in receive buffer which means we have complete request header }
            ...
            if (HdrLen <= 0) then begin
                if (FProxySource.DebugLevel >= DebugAll) then
                    LogTarEvent('Waiting for more target data');
                Exit;
            end ;
            HdrLen := HdrLen + 4; // add blank line length
          ...
          { update pipeline responses }
            ...
            inc (FHttpCurrResp);
            if FHttpCurrResp > MaxPipelineReqs then
                FHttpCurrResp := 1;
            if FHttpWaiting > 0 then
                dec (FHttpWaiting) ;
          ...
          { tell user what we got }
            if (FProxySource.DebugLevel >= DebugHttpHdr) then begin
             { V8.65 allow modify data before it's logged to remove passwords }
                ...
                LogTarEvent('Response: #' + IntToStr(FHttpTotResps) + ', for: ' +  FHttpInfo [FHttpCurrResp].ReqStartLine +
                                                    ', Status: ' + FRespStatusLine + ', Length: ' + IntToStr(FRespContentLength));
         ...
         { note Transfer-Encoding: chunked and Content-Encoding: gzip is common, uncompress after unchunk }
            if (FRespTransferEncoding <> '') then begin
                FChunkGzip := false;
                if (Pos('deflate', FRespTransferEncoding) > 0) or (Pos('gzip', FRespTransferEncoding) > 0) or
                ...
                if FRespContentLength > 0 then begin
                 { ignore actual content with HEAD request }
                    if FHttpInfo [FHttpCurrResp].HttpReqMethod <> httpMethodHead then begin
                        ...
                        if (FProxySource.DebugLevel >= DebugHttpHdr) then
                                LogTarEvent('Expected body content length ' + IntToStr(FRespContentLength));
                    end
                    else
                    ...
                    if FRespContentLenSet or
                        (FRespStatusCode = 204) or (FRespStatusCode = 205) or (FRespStatusCode = 304) or
                                                                                             (FRespStatusCode = 400) then begin
            ...
            if (FRespConnection <> '') then begin
                if NOT FRespKAFlag then begin
                  ...
                  { change header to keep-alive, don't close local client yet  }
                    if FRespKAFlag and ((FProxySource as TIcsHttpProxy).FHttpIgnoreClose) and FUpdateHttp then begin
              ...
              { get chunk size, short line with single word hex, ie 5ee5 }
                if (FChunkState = PxyChunkGetSize) and (FTarWaitTot > 2) then begin
                    ...
                    if (NewLen < 0) or (NewLen > 20000000) then begin  // 20MB snaity check
                        LogTarEvent('!! Warning, invalid chunk encoding length: ' + S + ' (' + IntToStr(NewLen) + ')');
                        if (FProxySource.DebugLevel >= DebugAll) then begin
                  ...
                  { zero size is end of response, but may be followed by trailer header fields, wait for blank line }
                        ...
                        if (FProxySource.DebugLevel >= DebugAll) then
                            LogTarEvent('End of chunked data');
                    end
                  ...
                  { got chunk length, get ready to read it }
                      ...
                      { increase buffer to take chunk, it was initially 1MB }
                        ...
                        if (FProxySource.DebugLevel >= DebugChunks) then
                            LogTarEvent('Extracting chunk, size=' + IntToStr(FChunkRcvd) + ', new content-len=' + IntToStr(FHtmlRespBodyLen + FChunkRcvd)) ;
                    end;
                end;
            ...
            if FTarRespTooLarge then begin
             // send response header now, only once, then partial body
                if (FHttpInfo [FHttpCurrResp].HttpReqState = httpStRespHdrWait) then begin
                    SendRespHdr;
                    if (FProxySource.DebugLevel >= DebugHttpHdr) then
                        LogTarEvent('Sending multi-block body content, length ' + IntToStr(FRespContentLength));
                end;
            ...
            { got whole body content or target closed, send it to target }
             ...
             { keep body content in buffer and remove from receive buffer }
                ...
                if FTarWaitTot > 0 then
                    IcsMoveTBytes(FTarBuffer, NewLen, 0, FTarWaitTot);  // remove from receive buffer
                if (FProxySource.DebugLevel >= DebugAll) then
                    LogTarEvent('Building content body, Length ' + IntToStr(FRespContentLength) + ', Added ' + IntToStr(NewLen) +
                                           ', Remaining ' + IntToStr(FTarRespLenRemain) + ', Left Over ' + IntToStr(FTarWaitTot)) ;
...
begin
  ...
  { allow up to 100 hosts }
    for J := 1 to 100 do begin
        ...
        if S = '' then continue;
        if NOT IcsCheckTrueFalse(MyIniFile.ReadString (section, 'HostEnabled', 'False')) then
            continue;
        ProxyTargets.Add;
        Result := Result + 1;
...
begin
    ...
    with IcsHttpProxy do begin
        ...
        if V < 0 then
            V := Ord(sslCliSecDefault); // sanity check
        ...
        if V < 0 then
            V := Ord(CertVerBundle); // sanity check

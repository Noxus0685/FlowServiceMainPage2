...
const
    TIcsLoggerVersion   = 1000;
    CopyRight : String  = ' IcsLogger (c) 2005-2024 by François PIETTE V10.0 ';
...
*)
    ...
    TIcsLogEvent = procedure (Sender: TObject; LogOption: TLogOption;  const Msg : String) of object;
        ...
        if loAddStamp in FLogOptions then begin
            if loDestEvent in FLogOptions then
                if Assigned(FOnIcsLogEvent) then
                    FOnIcsLogEvent(Sender, LogOption, AddTimeStamp + FTimeStampSeparator + Msg); {V6.03}
            if loDestOutDebug in FLogOptions then
                IcsOutputDebugStr(AddTimeStamp + FTimeStampSeparator + Msg); {V6.03}
            if loDestFile in FLogOptions then
                WriteToLogFile(AddTimeStamp + FTimeStampSeparator +  Msg + #13#10); {V6.03}
        end
        else begin
            if loDestEvent in FLogOptions then
                if Assigned(FOnIcsLogEvent) then

...
const
    NntpCliVersion     = 1000;
    CopyRight : String = ' TNntpCli (c) 1997-2024 F. Piette V10.0';
{$IFDEF VER80}
...
{$IFDEF ICS_NewMessaging}   { V10 }
        procedure WMNntpRequestDone(const Value : TIcsSocketMsgRec); virtual;
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
procedure TNntpCli.WMNntpRequestDone(const Value: TIcsSocketMsgRec);
begin
    TriggerRequestDone(TNntpRequest(Value.WParam), Value.LParam);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
class procedure TNntpCli.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    NntpCli: TNntpCli;
begin
    if Assigned(Value.Instance) and (Value.Instance is TNntpCli) then begin
        NntpCli := TNntpCli(Value.Instance);
        try
            with Value do begin
                if Msg = WM_ICS_SMTP_REQUEST_DONE then
                    NntpCli.WMNntpRequestDone(Value)
            end;
        except
            on E: Exception do
                NntpCli.HandleBackGroundException(E, 'TNnCli:DispatchMessage: ' + IntToStr(Value.Msg));
        end;
    end
    else
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TNntpCli.AbortComponent(E:Exception);    { V8.68 added E to allow reporting }
begin
    try
        Abort;
    except
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
        TNntpCli.PostMessage(Self, WM_ICS_NNTP_REQUEST_DONE, WORD(FRequestType), ErrCode);
{$ELSE}
...
var
    ...
    RandPart : String;
begin
    TickPart := '----=_NextPart_000_' + IntToHex(LongInt(IcsGetTickCount), 8);     { V8.71 }

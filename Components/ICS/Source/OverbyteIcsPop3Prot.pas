...
Jan 26, 2023 V8.71 If getting an OAuth2 token fails with an error, raise exception
...
const
    Pop3CliVersion     = 1000;
    CopyRight : String = ' POP3 component (c) 1997-2024 F. Piette V10.0 ';
...
type
    ...
    TPop3ProtocolState  = (pop3Disconnected,  pop3WaitingUser,  pop3WaitingPass,   pop3Transaction);
    ...
    TPop3RequestDone = procedure(Sender: TObject; RqType: TPop3Request; Error: Word) of object;
...
{$IFDEF ICS_NewMessaging}   { V10 }
        procedure   WMPop3RequestDone(const Value: TIcsSocketMsgRec); virtual;
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
procedure TCustomPop3Cli.WMPop3RequestDone(const Value : TIcsSocketMsgRec);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomPop3Cli.AbortComponent(E:Exception);    { V8.68 added E to allow reporting }
begin
    try
        Abort;
        FLastResponse  := '-ERR Abort on Exception: ' + AnsiString(E.Message);    { V8.68 }
    except
    end;
    inherited;
end;
...
begin
    if Error <> 0 then begin
        FLastResponse  := '-ERR ' + AnsiString(WSocketErrorDesc(Error)) + ' (Winsock error #' + IcsIntToStrA(Error) + ')';
        FStatusCode    := 500;
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
            TCustomPop3Cli.PostMessage(Self, WM_ICS_POP3_REQUEST_DONE, 0, Error);
{$ELSE}
    ...
    if FAuthType in [popAuthXOAuth2, popAuthOAuthBearer] then begin
        if (FOAuthToken = '') or (FTokenExpireDT < Now) then begin
            ...
            if FOALastErrCode > 0 then
                raise Pop3Exception.Create(FOALastError);                       { V8.71 }
            if (FOAuthToken = '') then
...
begin
    FFctPrv := pop3FctRetr;
    StartTransaction('RETR',   IcsIntToStrA(FMsgNum), pop3Retr, pop3Transaction, RetrDone);
end;
...
begin
    ...
    FFctPrv := pop3FctTop;
    StartTransaction('TOP', IcsIntToStrA(FMsgNum) + ' ' + IcsIntToStrA(FMsgLines),  pop3Top, pop3Transaction, RetrDone);
end;
...
begin
    FFctPrv := pop3FctDele;
    StartTransaction('DELE', IcsIntToStrA(FMsgNum), pop3Dele, pop3Transaction, nil);
end;
...
{$ENDIF}
  ...
  {$ENDIF}
    while TRUE do begin
        ...
        if Terminated or ((FTimeout > 0) and IcsTestTrgTick64(FTimeStop)) then begin

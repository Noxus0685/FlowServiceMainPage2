...
Legal issues: Copyright (C) 2007-2023 by François PIETTE
...
{$IFDEF ICS_NewMessaging}   { V10 }
        procedure WMPartDone(const Value : TIcsSocketMsgRec);
{$ELSE}
...
{$ENDIF ICS_NewMessaging}   { V10 }
        procedure WMStartMulti;    { V10.0 common version, no params }
...
begin
    if FPartCount < 1 then
        raise ERangeError.Create('PartCount (' + IntToStr(FPartCount) + ') must be positive and not nul');
    FAbortFlag     := FALSE;
    FPauseFlag     := FALSE;
    FTimeoutFlag   := FALSE;
    FContentLength := -1;
    FTotalCount    := 0;
    FPrevCount     := 0;
    FPrevTick      := IcsGetTickCount64;    { V8.71 }
    FStartTime     := Now;
    FElapsedTime   := 0;
    TriggerShowStats;
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
        TMultipartFtpDownloader.PostMessage(Self, WM_ICS_FTPD_START_MULTI,  0, 0);
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
procedure TMultipartFtpDownloader.WMPartDone(const Value: TIcsSocketMsgRec);
var
    FtpCli : TMyFtpCli;
begin
    FtpCli := TObject(Value.LParam) as TMyFtpCli;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
class procedure TMultipartFtpDownloader.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    FtpCli: TMultipartFtpDownloader;
begin
    if Assigned(Value.Instance) and (Value.Instance is TMultipartFtpDownloader) then begin
        FtpCli := TMultipartFtpDownloader(Value.Instance);
        try
            with Value do begin
                if Msg = WM_ICS_FTPD_START_MULTI  then
                    FtpCli.WMStartMulti()
                else if Msg = WM_ICS_FTPD_PART_DONE then
                    FtpCli.WMPartDone(Value)
                end;
        except
            on E: Exception do
                FtpCli.HandleBackGroundException(E, 'TMultipartFtpDownloader:DispatchMessage: ' + IntToStr(Value.Msg));
        end;
    end
    else
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TMultipartFtpDownloader.AbortComponent(E:Exception);    { V8.68 added E to allow reporting }
begin
    try
        Abort;
    except
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TMultipartFtpDownloader.WMStartMulti;
var
    Chunk  : TFtpBigInt;
    Offset : TFtpBigInt;
    I      : Integer;
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
        TMultipartFtpDownloader.PostMessage(Self, WM_ICS_FTPD_PART_DONE,  0, LParam(FtpCli));
{$ELSE}

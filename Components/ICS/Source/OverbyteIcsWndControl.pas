...
Nov 27, 2021 V8.68 BgException now passes exception to AbortComponent virtual handler
                     and ExceptAbortProc method so higher level components can
                     report errors better, such as out of memory without needing
                     an BgException handler.  Note requires similar change to all
                     units that override AbortComponent or use ExceptAbortProc.
Aug 10, 2023 V8.71 Made IcsFinalHandleException public.
                   Using Int64 ticks.
                   Increased G_WH_MAX_MSG to 200 since IDE ran out of messages.
...
Dec xx, 2024 V10.0 Updated for DEFINE ICS_NewMessaging that implements new cross platform
                   ...
                   Removed old MacOS only code, MacOS not currently supported.
                   Moved IcsLogger functions here so logging can be used easily, this also
                     allowed a lot of duplicate code to be removed from several units
                     that descent from TIcsWndControl, ie Http, Ftp, Smpt, Pop3 clients.
...
const
  TIcsWndControlVersion  = 1000;
  CopyRight : String     = ' TIcsWndControl (c) 2002-2024 F. Piette V10.0 ';

  IcsWndControlWindowClassName = 'IcsWndControlWindowClass';

type
  TIcsBgExceptionEvent = procedure (Sender: TObject; E: Exception;  var CanClose : Boolean) of object;
  TIcsFinalBgExceptionHandling = (fehNone, fehAppHandleException, fehShowException);
{$IFNDEF ICS_NewMessaging}   { V10 }
  TIcsMessageEvent =   procedure (Sender: TObject; var MsgRec: TMessage;  var Handled : Boolean) of object;
{$ENDIF N ICS_NewMessaging} { V10.0 }
  ...
  TIcsExceptAbortProc = procedure(E:Exception) of object;                     { V8.68 added E to allow reporting }

{ V10  new cross platform thread based messaging  }
{$IFDEF ICS_NewMessaging}
  TIcsWndControlThread  = class;      { V10 }
  TIcsTimerThread       = class;      { V10 }

    TIcsTimerThread = class(TThread)
    protected
        FParentCtrl       : TComponent;
        FInterval         : Cardinal;
        FIntervalLock     : IReadWriteSync;
        FEnabled          : Boolean;
        FEnabledLock      : IReadWriteSync;
        FOnTimer          : TNotifyEvent;
        FOnTimerLock      : IReadWriteSync;
        FEvent            : TEvent;
        FSynchronizeEvent : Boolean;
    ...
    public
        constructor Create(AInterval: Cardinal; CreateSuspended: Boolean; AParentCtrl: TComponent); overload;
        destructor Destroy; override;
        procedure Execute; override;
        procedure TerminateMe;
        property Enabled          : Boolean      read  GetEnabled
                                                 write SetEnabled;
        property Interval         : Cardinal     read  GetInterval
                                                 write SetInterval;
        property ThreadDone       : Boolean      read  FThreadDone;
        property ThreadStarted    : Boolean      read  FThreadStarted;
        property SynchronizeEvent : Boolean      read  FSynchronizeEvent
                                                 write FSynchronizeEvent;
        property ParentCtrl       : TComponent   read  FParentCtrl
                                                 write FParentCtrl;
        property OnTimer          : TNotifyEvent read  GetOnTimer
                                                 write SetOnTimer;
    end;

    TIcsTimer = class(TComponent)                   { V10 cross platform version }
    protected
        FIcsWndControl : TIcsWndControl;
        FTimerThread   : TIcsTimerThread;
        FOnTimer       : TNotifyEvent;
        function  GetEnabled: Boolean;
        function  GetInterval: Cardinal;
        procedure SetEnabled(const Value: Boolean);
        ...
        procedure SetOnTimer(const Value: TNotifyEvent);
        function  GetOnTimer: TNotifyEvent;
        function  GetSynchronizeEvent: Boolean;
        procedure SetSynchronizeEvent(const Value: Boolean);
    public
    ...
    published
        ...
        property Interval         : Cardinal     read  GetInterval
                                                 write SetInterval;
        property SynchronizeEvent : Boolean      read  GetSynchronizeEvent
                                                 write SetSynchronizeEvent;
        property OnTimer          : TNotifyEvent read  GetOnTimer
                                                 write SetOnTimer;
    end;
{$ENDIF ICS_NewMessaging}

{ V10 Windows old messaging only version }
{$IFNDEF ICS_NewMessaging}
...
{$ENDIF N ICS_NewMessaging}

{$IFDEF ICS_NewMessaging}

    TIcsWndControl = class(TComponent)  { V10 version for NewMesssaging }
    private
    protected
        FEPollFd            : Integer;
        FThreadInternal     : TIcsWndControlThread;
        FSubscription       : TIcsSocMsgSubscription;
        FTerminated         : Boolean;
        FThreadStarted      : Boolean;
        FMultiThreaded      : Boolean;
        FExceptAbortProc    : TIcsExceptAbortProc;
{$IFNDEF NO_DEBUG_LOG}
        FIcsLogger          : TIcsLogger;
{$ENDIF}
        FOnBgException : TIcsBgExceptionEvent;
        class var   FInstances: Integer;
        procedure   AbortComponent(E: Exception); virtual;
        procedure   SetOnBgException(const Value: TIcsBgExceptionEvent); virtual;
        procedure   HandleBackGroundException(E : Exception; const Source: String = 'Unknown'); virtual;  { V8.62 added source }
        procedure   TriggerBgException(E: Exception; var CanClose : Boolean); virtual;
...
{$IFNDEF NO_DEBUG_LOG}
        ...
        procedure   DebugLog(LogOption : TLogOption; const Msg : String); virtual;
        function    CheckLogOptions(const LogOption: TLogOption): Boolean; virtual;
{$ENDIF}
    public
        ...
        class procedure DispatchMessage(const Value : TIcsSocketMsgRec); virtual;
        function    GetThread : TIcsWndControlThread;
        function    HasThread : Boolean;
        procedure   ThreadExecuteProc; virtual; { abstract;  }
        procedure   ThreadTerminateProc; virtual; { abstract; }
        property Thread : TIcsWndControlThread          read  GetThread;
...
{$ENDIF}
        ...
        property OnBgException   : TIcsBgExceptionEvent read  FOnBgException
                                                        write SetOnBgException;
        property ExceptAbortProc : TIcsExceptAbortProc  read  FExceptAbortProc
                                                        write FExceptAbortProc;
    end;

    TIcsWndControlThread = class(TThread)
    protected
        FParentCtrl : TIcsWndControl;
        procedure Terminate;
    public
        destructor Destroy; override;
        procedure Execute; override;
        procedure MySynchronize(AMethod: TThreadProcedure);
        procedure TerminateMe;
        property ParentCtrl : TIcsWndControl read  FParentCtrl
                                             write FParentCtrl;
        property Terminated;
    end;
...
{$IFNDEF ICS_NewMessaging}
  ...
  TIcsWndControl = class(TComponent)                 { Windows only version V9.4 removed Posix stuff }
  private
...
{$ENDIF}
    ...
    procedure   AbortComponent(E:Exception); virtual;  { V8.68 added E to allow reporting }
{$IFNDEF NO_DEBUG_LOG}
    procedure   SetIcsLogger(const Value : TIcsLogger); virtual;
    procedure   DebugLog(LogOption : TLogOption; const Msg : String); virtual;
    function    CheckLogOptions(const LogOption: TLogOption): Boolean; virtual;
...
{$ENDIF}
  ...
  public
    constructor Create;
    destructor  Destroy; override;
    function    GetWndHandler(HandlerCount : UINT; ThreadID : TThreadID): TIcsWndHandler;
...
procedure SetIcsThreadLocalFinalBgExceptionHandling( const Value: TIcsFinalBgExceptionHandling);
function GetIcsThreadLocalFinalBgExceptionHandling: TIcsFinalBgExceptionHandling;
procedure IcsFinalHandleException(Sender: TObject);   { V8.71 JK }
...
{$IFNDEF ICS_NewMessaging}   { V10 }
  GUIDOffSet   : Integer;
{$ENDIF N ICS_NewMessaging} { V10.0 }
  G_WH_MAX_MSG : Word = 200;    { V8.71 was 100, but ran out }
...
{ TIcsAsyncWndControlThread }

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
destructor TIcsWndControlThread.Destroy;
begin
    // For debugging purpose, to put a breakpoint here
    inherited Destroy;
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsWndControlThread.Execute;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsWndControlThread.MySynchronize(AMethod: TThreadProcedure);
begin
    Synchronize(AMethod);
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsWndControlThread.Terminate;
begin
    inherited Terminate;
    if Assigned(FParentCtrl) then
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsWndControlThread.TerminateMe;
begin
    inherited Terminate;
    if Assigned(FParentCtrl) then
        FParentCtrl.ThreadTerminateProc;
...
begin
    ...
    if FInstances = 1 then begin
        FSubscription := TIcsMessageManager.IcsDefaultManager.SubscribeIcsMessage(IcsSocketMessageListener);
    end;
end;
...
begin
    TInterlocked.Decrement(FInstances);
    if FInstances = 0 then begin
        TIcsMessageManager.IcsDefaultManager.UnsubscribeIcsMessage(FSubscription);
    end;
    if Assigned(FThreadInternal) then begin
        if not FTerminated then begin
            FThreadInternal.Terminate;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsWndControl.ThreadExecuteProc;
begin
  // no code here, but overriden in descendants
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsWndControl.ThreadTerminateProc;
begin
  // no code here, but overriden in descendants
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsWndControl.DebugLog(LogOption: TLogOption; const Msg: String);  { V5.21 }
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
class procedure TIcsWndControl.IcsSocketMessageListener({$IFDEF RTL_MESSAGING}const Sender: TObject;{$ENDIF} const M: TIcsSocMsg);
var
{$IFDEF RTL_MESSAGING}
    Msg     : TIcsSocMsg;
{$ENDIF}
...
{$IFNDEF NO_DEBUG_LOG}
 ...
 //       DebugLog(loWsockInfo, 'MessageListener: Msg=' + IntToStr(Value.Msg));    // not allowed in class method
{$ENDIF}
    TIcsWndControl.DispatchMessage(Value);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
class procedure TIcsWndControl.DispatchMessage(const Value : TIcsSocketMsgRec);
begin
{$IFNDEF NO_DEBUG_LOG}
//    if CheckLogOptions(loWsockDump) then
//        DebugLog(loWsockInfo, 'TIcsWndConrol:DispatchMessage: Msg=' + IntToStr(Value.Msg)); // not allowed in class method
{$ENDIF}

    IcsOutputDebugStr('TIcsWndConrol:DispatchMessage: Msg=' + IntToStr(Value.Msg));   // TEMP !!!!!
  // no messages in this unit, but overriden in descendants
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
class function TIcsWndControl.PostMessage(const Sender: TObject; const Msg: Cardinal; const WParam: NativeUint; const LParam: NativeUint): Boolean;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TIcsWndControl.HasThread: Boolean;
begin
    Result := Assigned(FThreadInternal);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TIcsWndControl.GetThread: TIcsWndControlThread;
begin
    if not Assigned(FThreadInternal) then begin
        FThreadInternal := TIcsWndControlThread.Create(TRUE);
        FThreadInternal.ParentCtrl := Self;
        // Do some initialization
        FThreadInternal.Start;
{$IFDEF LINUX}
        // Wait for epoll_create to be executed
        while FEPollFd = 0 do
            Sleep(0);
{$ENDIF}
    end;
    Result := FThreadInternal;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsWndControl.AbortComponent(E:Exception);   { V8.68 added E to allow reporting }
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{ All exceptions *MUST* be handled. If an exception is not handled, the application will be shut down !  }
procedure TIcsWndControl.HandleBackGroundException(E: Exception; const Source: String = 'Unknown');  { V8.62 added source }
var
    CanAbort : Boolean;
...
{$IFNDEF NO_DEBUG_LOG}
    if Assigned(FIcsLogger) then
        FIcsLogger.DoDebugLog(Self, loWsockErr, 'Handle Background Exception, source: ' + Source + ' - ' + E.Message);   { V8.62 }
{$ENDIF}
        ...
        try
            try                          { V1.14 }
                if Assigned(FExceptAbortProc) then
                    FExceptAbortProc(E);  { V8.68 added E to allow reporting }
            finally
                AbortComponent(E);        { V8.68 added E to allow reporting }
            end;
        except
            // Ignore any exception here
        end;
...
procedure TIcsWndControl.TriggerBgException(E: Exception;  var CanClose : Boolean);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
constructor TIcsTimerThread.Create(AInterval: Cardinal; CreateSuspended: Boolean; AParentCtrl: TComponent);
begin
    FIntervalLock     := TMultiReadExclusiveWriteSynchronizer.Create;
    FEnabledLock      := TMultiReadExclusiveWriteSynchronizer.Create;
    FOnTimerLock      := TMultiReadExclusiveWriteSynchronizer.Create;
    FEvent            := TEvent.Create(nil, TRUE, FALSE, '');
    FSynchronizeEvent := TRUE;
    FParentCtrl       := AParentCtrl;
    Interval          := AInterval;
    Enabled           := TRUE;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
destructor TIcsTimerThread.Destroy;
begin
    FreeAndNil(FEvent);
    inherited Destroy;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsTimerThread.Execute;
var
     AOnTimer   : TNotifyEvent;
...
begin
     try
         ...
         while not Terminated do begin
             if Enabled then
                 WaitResult := FEvent.WaitFor(Interval)
             else
                 WaitResult := FEvent.WaitFor(10000);
             if WaitResult = wrTimeOut then begin
                 // Interval elapsed
                 if Enabled then begin
                      AOnTimer := OnTimer;
                      if Assigned(AOnTimer) then begin
                          if not FSynchronizeEvent then
                              AOnTimer(FParentCtrl)
                          else begin
                              Synchronize(Self, procedure begin AOnTimer(FParentCtrl) end);
                          end;
                      end;
                 end;
             end;
         ...
         // We can't let exception go outside of the thread
     end;
     FThreadDone := TRUE;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsTimerThread.SetEnabled(const Value: Boolean);
begin
    FEnabledLock.BeginWrite;
    try
        ...
        FEnabledLock.EndWrite;
    end;
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TIcsTimerThread.GetEnabled: Boolean;
begin
    FEnabledLock.BeginRead;
    try
        Result := FEnabled;
    finally
        FEnabledLock.EndRead;
    end;
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsTimerThread.SetOnTimer(const Value: TNotifyEvent);
begin
    FOnTimerLock.BeginWrite;
    try
        FOnTimer := Value;
    finally
        FOnTimerLock.EndWrite;
    end;
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsTimerThread.TerminateMe;
begin
    inherited Terminate;
    FEvent.SetEvent;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TIcsTimerThread.GetOnTimer : TNotifyEvent;
begin
    FOnTimerLock.BeginRead;
    try
        Result := FOnTimer;
    finally
        FOnTimerLock.EndRead;
    end;
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TIcsTimerThread.GetInterval: Cardinal;
begin
    FIntervalLock.BeginRead;
    try
        Result := FInterval;
    finally
        FIntervalLock.EndRead;
    end;
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsTimerThread.SetInterval(const AInterval: Cardinal);
begin
    if AInterval = 0 then
        raise ERangeError.Create('Timer interval can''t be zero');
    FIntervalLock.BeginWrite;
    try
        if FInterval <> AInterval then begin
            FInterval := AInterval;
            FEvent.SetEvent;
        end;
    finally
        FIntervalLock.EndWrite;
    end;
end;
...
begin
    FTimerThread := TIcsTimerThread.Create(10000, FALSE, Self);
    // A timeout should be added in case the thread doesn't start
    while not FTimerThread.ThreadStarted do
        Sleep(0);
    inherited Create(AOwner);
end;
...
begin
    if Assigned(FTimerThread) then begin
        if not FTimerThread.ThreadDone then begin
            FTimerThread.TerminateMe;
            while not FTimerThread.ThreadDone do
                Sleep(0);
        end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TIcsTimer.GetEnabled: Boolean;
begin
...
begin
    if Assigned(FTimerThread) then
        FTimerThread.Enabled := Value;
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TIcsTimer.GetInterval: Cardinal;
begin
    if Assigned(FTimerThread) then
        Result := FTimerThread.Interval
    else
        Result := 0;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TIcsTimer.GetOnTimer: TNotifyEvent;
begin
    if Assigned(FTimerThread) then
        Result := FTimerThread.OnTimer
    else
        Result := nil;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsTimer.SetOnTimer(const Value: TNotifyEvent);
begin
    if Assigned(FTimerThread) then
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TIcsTimer.GetSynchronizeEvent: Boolean;
begin
    if Assigned(FTimerThread) then
        Result := FTimerThread.SynchronizeEvent
    else
        Result := TRUE;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TIcsTimer.SetSynchronizeEvent(const Value: Boolean);
begin
    if Assigned(FTimerThread) then
        FTimerThread.SynchronizeEvent := Value;
end;
{$ENDIF ICS_NewMessaging}

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}

{ V9.4 old Windows only messaging }
{$IFNDEF ICS_NewMessaging}
...
function WndControlWindowsProc( ahWnd: HWND; auMsg: UINT; awParam: WPARAM; alParam: LPARAM): LRESULT; stdcall;
var
    Obj    : TObject;
    MsgRec : TMessage;
begin
...
begin
    ...
    try
        FillChar(TempClass, SizeOf(TempClass), 0 );   { V8.71 JK }
        // Check if the window class is already registered
        IcsWndControlWindowClass.hInstance     := HInstance;
        IcsWndControlWindowClass.lpszClassName := IcsWndControlWindowClassName;
        ClassRegistered := GetClassInfo(HInstance, IcsWndControlWindowClass.lpszClassName, TempClass);
        if not ClassRegistered then begin
           ...
           if {$IFDEF RTL_NAMESPACES}Winapi.{$ENDIF}Windows.RegisterClass(IcsWndControlWindowClass) = 0 then
                raise EIcsException.Create(
                     'Unable to register TIcsWndControl hidden window class.' + ' Error: ' + SysErrorMessage(GetLastError));    { V8.62 tell user real error }
        end;

        // Now we are sure the class is registered, we can create a window using it
        FHandle := CreateWindowEx(WS_EX_TOOLWINDOW, IcsWndControlWindowClass.lpszClassName,
                                  '',        // Window name
        ...
        if FHandle = 0 then
            raise EIcsException.Create('Unable to create TIcsWndControl hidden window. ' +' Error: ' + SysErrorMessage(GetLastError));    { V8.62 tell user real error. probably no memory }
...
begin
    try
        ...
        // All exceptions must be handled otherwise the application will terminate as soon as an exception is raised.
...
begin
    ...
    if FMsgCnt >= G_WH_MAX_MSG then
        raise EIcsException.Create('No more free global messages, total ' + IntToStr(FMsgCnt));    { V8.71 better error }
    I := 0;
    ...
    raise EIcsException.Create('Global message map full, messnr ' + IntToStr(I));    { V8.71 better error }
end;
...
begin
    ...
    if (FInterval <> 0) and FEnabled and Assigned(FOnTimer) then
        if SetTimer(FIcsWndControl.Handle, UINT_PTR(Self), FInterval, nil) = 0 then begin   { V8.04 }
            FEnabled := FALSE;
            raise EIcsTimerException.Create('No more timers');
        end
        else
            FLastEvent := IcsGetTickCount64;    { V8.71 }
end;
...
begin
    ...
    try
        if FList.Count > 0 then
            raise EIcsException.Create('Maximum number of message IDs per handler can only be set when the pool is empty');
        G_WH_MAX_MSG := Value;
    finally
        UnLock;
    end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TIcsWndHandlerPool.GetWndHandler(HandlerCount: UINT; ThreadID: TThreadID): TIcsWndHandler;
var
    I : Integer;
...
begin
    ...
    I := FList.Count - 1;
    while (I >= 0) and (TIcsWndHandlerList(FList.Items[I]).ThreadID <> ThreadID) do
        Dec(I);
    if I >= 0 then
    ...
    if (GWndHandlerMsgLow <= WM_USER) or (GWndHandlerMsgLow >= (65535 - G_WH_MAX_MSG)) then

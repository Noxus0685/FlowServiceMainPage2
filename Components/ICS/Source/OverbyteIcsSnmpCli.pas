...
const
  SnmpClientVersion         = 1000;
  CopyRight    : String     = ' TSnmpClient (c) 2024 Francois Piette V10.0 ';
...
{$ENDIF N ICS_NewMessaging} { V10 }
        procedure WMListening;      { V10 no params }
        procedure WMGetNext;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
class procedure TSnmpCli.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    SnmpCli: TSnmpCli;
begin
    if Assigned(Value.Instance) and (Value.Instance is TSnmpCli) then begin
        SnmpCli := TSnmpCli(Value.Instance);
        try
            with Value do begin
                if Msg = WM_ICS_SNMP_LISTENING then
                    SnmpCli.WMListening
                else if Msg = WM_ICS_SNMP_GETNEXT then
                   SnmpCli. WMGetNext
            end;
        except
            on E: Exception do
                SnmpCli.HandleBackGroundException(E, 'TSnmpCli:DispatchMessage: ' + IntToStr(Value.Msg));
        end;
    end
    else
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TSnmpCli.WMListening;
var
    Buffer   : AnsiString;
begin
...
{$ENDIF}
    FWSocket.SendTo(FDestAddr, SizeOf(FDestAddr), PAnsiChar(Buffer), Length(Buffer));
end;
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
        TSnmpCli.PostMessage(Self, WM_ICS_SNMP_LISTENING, 0, 0);
{$ELSE}
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
                        TSnmpCli.PostMessage(Self, WM_ICS_SNMP_GETNEXT, 0, 0);
{$ELSE}

...
Legal issues: Copyright (C) 2024 by Magenta Systems Ltd, England
...
Aug 26, 2020 V8.65 - Support Posix/MacOS, currently only 32-bit ticks.
Mar 18, 2021 V8.66 - Added OverbyteIcsDefs.inc.
Jan 26, 2023 V8.71 - Added IcsDiffSecs64.
                     Supporting TicksAPI64 on MacOS and POSIX.
...
{$ENDIF MSWINDOWS}
    ...
    function IcsDiffSecs64 (const StartTick, EndTick: int64): integer;     { V8.71 }
   ...
   // conceivable that performance counter not available in hardware
    ...
    begin
       ...
       // warning - Virtual Machines may have poor accuracy, even 2% slow
         ...
         if Tick64Correction > 900000 then
            Tick64Correction := 0;
    end ;
end;
{$ENDIF MSWINDOWS}
...
begin
{$IFDEF MSWINDOWS}
    if Ticks64Mode = TicksNone then
        IcsInitTick64 (TicksNone);
    case Ticks64Mode of
...
{$ELSE}
    if Ticks64Mode = TicksNone then
        Ticks64Mode := TicksAPI64;
    if Ticks64Mode = TicksAPI64 then
        Result := System.Classes.TThread.GetTickCount64   { V8.71 }
    else
...
function IcsDiffSecs64 (const StartTick, EndTick: int64): integer;     { V8.71 }
begin
    Result := IcsDiffTicks64 (StartTick, EndTick) div Ticks64PerSecond;
end;

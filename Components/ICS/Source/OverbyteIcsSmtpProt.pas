...
Dec 27, 2021 V8.68 - BgException now passes exception to AbortComponent virtual handler.
                     DELPHI24_UP to COMPILER24_UP for C++.
Jul 19, 2023 V8.71 - If getting an OAuth2 token fails with an error, raise exception
                       and report it using new properties OALastErrCode and OALastError.
                     Ensure inherited destroy called.
                     When setting HTML images using EMailImages, allow TStream object
                       to be added as well as the file name which will be used instead
                       of opening the file (the file need not exist).  EMailImages is
                       TStrings, use AddObject (filename, TStream).
                     Removed ImageStream and StreamArray, never properly tested, use
                       SetEMailImages instead.
                     Simplified by internally using MailMessage instead of PlainText.
...
const
  SmtpCliVersion     = 1000;
  CopyRight : String = ' SMTP component (c) 1997-2024 Francois Piette V10.0 ';
...
//        FMsg_WM_SMTP_QUIT_DELAYED : UINT; // V10.0 not used
{$ENDIF N ICS_NewMessaging} { V10 }
        ...
        FOALastErrCode       : Integer;                                 { V8.71 for OAuth2 }
        FOALastError         : String;                                  { V8.71 for OAuth2 }
        FSuccCounter         : Longint;                                 { V8.71 JK, for checking recipients }
...
{$IFDEF ICS_NewMessaging}  { V10 }
        procedure   WMSmtpRequestDone(const Value : TIcsSocketMsgRec); virtual;
{$ELSE}
...
{$ENDIF N ICS_NewMessaging} { V10 }
    ...
    protected
        ...
        property OALastErrCode     : Integer         read  FOALastErrCode
                                                     write FOALastErrCode;     { V8.71 for OAuth2 }
        property OALastError       : String          read  FOALastError
                                                     write FOALastError;       { V8.71 for OAuth2 }
...
{$ENDIF}
    ...
    protected
        ...
        FTimeStop      : Int64;                   { Milli-seconds    }      { V8.71 }
...
(* V10.0 never used
{$IFDEF ICS_NewMessaging}   { V10 }
        procedure   WMSmtpQuitDelayed(const Value : TIcsSocketMsgRec);
{$ELSE}
        procedure   WndProc(var MsgRec: TMessage); override;
...
{$IFDEF USE_SSL}
    THtmlSmtpCli = class(TSslSmtpCli)  { V8.65 was custom }
{$ELSE}
    THtmlSmtpCli = class(TSyncSmtpCli) { V8.02 }
{$ENDIF USE_SSL}
    protected                                { V8.71 was private }
    //    FPlainText              : TStrings;  { V8.71 JK use FMailMessage instead }
        FEmailImages            : TStrings;
        FHtmlText               : TStrings;
    //    FStreamArray            : TList;     { V8.71 JK image streams should be stored as objects in FEmailImages instead }
   ...
   //     procedure ClearImageStreamArray;
        { ImageStream is not completely implemented. Do not use !  V8.71 JK, so remove it }
   ...
   //     property ImageStreamCount : Integer   read  GetImageStreamCount;
    ...
    published
        ...
        property PlainText : TStrings         read  FMailMessage      { V8.71 JK, was FPlainText unnecessary }
                                              write SetMailMessage;
...
//const
//    CRLF = AnsiString(#13#10);    { V10.0 use IcsCRLF }
...
begin
    ...
    begin
        S := IcsWrapTextEx(HdrBody, IcsCRLF, [',', ';'], 1, ['"', ''''], rPos
          ...
          {$ENDIF}
            ...
            begin
               ...
               {$IFNDEF UNICODE} and FConvertToCharset {$ENDIF} then
                    Alias := String(EncodeMbcsInline(FCodePage, FCharset, FEncType, PChar(Alias), Length(Alias),
                                                                       FFoldLines, SmtpDefaultLineLength - Length(HdrName)))
                else
                    Alias := String(HdrEncodeInLine(AStr, SpecialsRFC822, AnsiChar(FEncType), AnsiString(FCharset),
                                           SmtpDefaultLineLength - Length(HdrName), FFoldLines, FCodePage, FIsMultiByteCP));
            end
            else begin
                Alias := String(AStr);
...
{$ENDIF}
    ...
    if FAllow8Bit and (not FFoldLines) then
        ...
        begin
            ...
            if (IcsIsMBCSCodePage(FCodePage) or (FCodePage = CP_UTF7))
                {$IFNDEF UNICODE} and FConvertToCharset {$ENDIF} then
                AddFoldedHeader(HdrName + String(EncodeMbcsInline(FCodePage, FCharset, FEncType, PChar(HdrBody), Length(HdrBody),
                                                                           FFoldLines, SmtpDefaultLineLength - Length(HdrName))))
            else
                AddFoldedHeader(HdrName + String(HdrEncodeInLine(ABody, SpecialsRFC822,  AnsiChar(FEncType), AnsiString(FCharset),
                                                  SmtpDefaultLineLength - Length(HdrName), FFoldLines, FCodePage, IsMultiByteCP)))
        end
...
begin
    case FTransferEncoding of
        ...
        smtpEnc8bit :
            ...
            if FWrapText then
                Result := IcsTrim(IcsWrapTextEx(FText, IcsCRLF, [#09, #32, '.', ',', '-'], FMaxLineLength, [],
                                                                          FCurrentIdx, FALSE, FCodePage, FIsMultiByteCP))
            else  { Force line break before max. line buffer = 1022 }
                Result := IcsTrim(IcsWrapTextEx(FText, IcsCRLF,  [#09, #32, '.', ',', '-'], 1022, [],
        ...
        smtpEncQuotedPrintable :
            ...
            Result := StrEncodeQPEx(FText, SmtpDefaultLineLength -1, [], False, FCurrentIdx, True, FCodepage, FAlSE);

        smtpEncBase64 :
            { Encode Base64 }
            { Multi-byte codepoints do not have to be preserved }
            { so IsMultiByteCP is always FALSE here.            }
            Result := Base64EncodeEx(FText, SmtpDefaultLineLength, FCurrentIdx, FCodepage, FALSE);
    end; {case}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
procedure TCustomSmtpClient.WMSmtpRequestDone(const Value : TIcsSocketMsgRec);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
class procedure TCustomSmtpClient.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    SmtpCli: TCustomSmtpClient;
begin
    if Assigned(Value.Instance) and (Value.Instance is TCustomSmtpClient) then begin
        SmtpCli := TCustomSmtpClient(Value.Instance);
        try
            with Value do begin
                if Msg = WM_ICS_SMTP_REQUEST_DONE then
                    SmtpCli.WMSmtpRequestDone(Value)
                else if Msg = WM_ICS_SMTP_DATA_NEXT then
                    SmtpCli.DataNext
            end;
        except
            on E: Exception do
                SmtpCli.HandleBackGroundException(E, 'TCustomSmtpClient:DispatchMessage: ' + IntToStr(Value.Msg));
        end;
    end
    else
...
begin
    ...
    FHdrLines  := nil;              { V8.71 JK }
    FOutStream := nil;              { V8.71 JK }
{$IFNDEF ICS_NewMessaging}  { V10 }
    AllocateHWnd;
{$ENDIF N ICS_NewMessaging} { V10 }
    CreateSocket;                                                     {AG/SSL}
...
begin
    try                           { V8.71 JK ensure inherited destroy called }
        if Assigned(FWSocket) then
            FreeAndNil(FWSocket);
        if Assigned(FHdrLines) then
            FreeAndNil(FHdrLines);
        if Assigned(FAuthTypesSupported) then
            FreeAndNil(FAuthTypesSupported);
        if Assigned(FMailMessage) then
            FreeAndNil(FMailMessage);
        if Assigned(FRcptName) then
            FreeAndNil(FRcptName);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomSmtpClient.AbortComponent(E:Exception);   { V8.68 added E to allow reporting }
begin
    try
        Abort;
        FLastResponse := '500 Abort on Exception: ' + E.Message;    { V8.68 }
    except
    end;
    inherited;
end;
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
            TCustomSmtpClient.PostMessage(Self, WM_ICS_SMTP_REQUEST_DONE, 0, ErrorCode);
{$ELSE}
...
begin
    ...
    FItemCount := -1;
  { V8.71 JK, -1, because on the first pass in RcptToNext it increases
      to 0 although no recipient has been  processed yet }
    FSuccCounter := -1;
    RcptToNext;
end;
...
begin
    if (FItemCount >= 0) and (FRequestResult <> 0) then  {A.G. 1/3/05 start} begin
        srteAction := srteIgnore;  { V8.71 JK was srteAbort }
        ...
        Inc(FItemCount);
        Inc(FSuccCounter);   { V8.71 JK }
    end;

 { V8.71 JK all Rcpt Names processed }
    if FItemCount >= FRcptName.Count then begin
        FDoneAsync := nil;

     // at least one successful recipient exists, so we can continue
        if FSuccCounter > 0 then begin
            FRequestResult := 0;
            ClearErrorMessage;
        end;

      // now we should finalize last operation
      // as code in the procedure .NextExecAsync, as if DoneAsync = nil

        if (FRequestType <> smtpQuit) or (FConnected = FALSE) then
            TriggerRequestDone(FRequestResult);
        Exit;
    end;

  { V8.71 JK  ctpToDone should execute also for last recipient for the possibility of ignoring it in case of its error }
  ...
  { no point in continuing without it }
    if FAuthType in [smtpAuthXOAuth2, smtpAuthOAuthBearer] then begin
        if (FOAuthToken = '') or (FTokenExpireDT < Now) then begin
            ...
            if FOALastErrCode > 0 then
                raise SmtpException.Create(FOALastError);                       { V8.71 }
            if (FOAuthToken = '') then
    ...
    try
        ...
        on E : Exception do begin
          ...
          {$ENDIF}
            ...
            FRequestResult   := 500;
            FErrorMessage    := '500 Internal client error ' + E.ClassName + ': "' + E.Message + '"';
        end;
    end;
...
begin
    FRcptName.Clear;
    if Assigned(newValue) then    { V8.71 JK }
...
{$IFDEF LINUX}
begin
  {$MESSAGE 'TODO Linux PrepareGlobalSmtpFormatSettings'}           { V8.65 pending Linux }
end;
{$ENDIF}
{$IFDEF ANDROID}
begin
...
function Rfc822DateTime(t : TDateTime) : String;
begin
    Result := FormatDateTime('ddd, d mmm yyyy hh":"mm":"ss', t, GL_En_US_FormatSettings) + ' ' + TimeZoneBias;
end;
...
function GenerateMessageID : String;                                     {AG}
begin
    Result := FormatDateTime('yyyymmddhhnnsszzz', Now + TimeZoneBiasDT) + '.' + IntToHex(IcsRandomInt(MaxWord), 4) +
        IntToHex(IcsRandomInt(MaxWord), 4) +  IntToHex(IcsRandomInt(MaxWord), 4) + IntToHex(IcsRandomInt(MaxWord), 4) +
...
begin
    ...
    try
        ...
        FOutStream.Write(PAnsiChar(IcsCRLF)^, 2);
    except
        on E: Exception do begin
            FLastResponse   := '500 ' + E.ClassName + ' ' + E.Message;
            FStatusCode     := 500;
    ...
    if FSendMode = smtpToStream then begin
        ...
        if FMsgSizeFlag then
            ...
            TriggerMessageDataSent;
{$IFDEF ICS_NewMessaging}   { V10.0 }
            TCustomSmtpClient.PostMessage(Self, WM_ICS_SMTP_DATA_NEXT, 0 , 0);
{$ELSE}
...
begin
    try    { V8.71 JK ensure inherited destroy called }
        if Assigned(FEmailBody) then
            FreeAndNil(FEMailBody);
        if Assigned(FEmailFiles) then
            FreeAndNil(FEmailFiles);
...
begin
    ...
    if FMimeBoundary <> '' then begin
        ...
        if FMailMsgText.TransferEncoding <> smtpEnc7bit then begin         {AG}
            StrCat(PAnsiChar(Line), PAnsiChar(IcsCRLF + 'Content-Transfer-Encoding: '));
...
begin
    ...
    if not (Self is THtmlSmtpCli) then // Don't set the wrong props !!
        FMailMsgText.SetText(FMailMessage.Text, FCodePage, FIsMultiByteCP, ConvertToCharset, FDefaultEncoding, FAllow8BitChars,
                                                                                           FWrapMessageText, FWrapMsgMaxLineLen);
    FEmailBody.Clear;
    if Assigned(FEMailFiles) and (FEmailFiles.Count > FCurrentFile) then begin
        FMimeBoundary := '= Multipart Boundary ' + AnsiString(FormatDateTime('mmddyyhhnn', Now));
...
{$IFNDEF WIN64}                  { V7.37 }
  ...
  {$ENDIF}
{$ENDIF}
    FTimeStop := IcsGetTrgSecs64(FTimeout);   { V8.71 }
  {$IFNDEF POSIX}
  ...
  {$ENDIF}
    while TRUE do begin
        ...
        if Terminated or ((FTimeout > 0) and IcsTestTrgTick64(FTimeStop)) then begin  { V8.71 }
...
begin
    inherited Create(AOwner);
//    FPlainText    := TStringList.Create;  { V8.71 JK }
...
begin
 {   ClearImageStreamArray;
    ...
    try
        ...
        if Assigned(FEmailImages) then
            FreeAndNil(FEmailImages);
    finally
        inherited Destroy;
    end;
end;
...
{$IFDEF UNICODE}
    ...
    if (Value = FALSE) and (csDesigning in ComponentState) and
       (not (csLoading in ComponentState)) then
        raise SmtpException.Create('ConvertToHtmlCharset can only be disabled with old ANSI compilers for backwards compatibility.');
{$ELSE}
...
begin
  ...
  { V8.71 JK, streams for image content (images may be stored in the memory, resources...)
    may be supplied as objects in newValue as TStream, still need Filename as description }
        FileObj := newValue.Objects[I];

      { Ignore any empty file name (a very common error !) }
        if FilePath > '' then begin
            { Check if file exists and raise an exception if not }
            if (FileObj <> nil) or FileExists(FilePath) then
                FEMailImages.AddObject(FilePath, FileObj)       { V8.71 JK, add object }
            else
                raise SmtpException.Create('File not found ''' + FilePath + '''');
        end;
    end;
...
begin
        ...
        if FEmailFiles.Count > 0 then begin
            ...
            16: begin
                 ...
                 { V8.71 JK, FPlainText now stored in FMailMessage }
                    FMailMsgText.SetText(FMailMessage.Text, FCodePage, FIsMultiByteCP, ConvertToCharSet, smtpEncQuotedPrintable, FALSE);
                    FMsgLineCount := 0;
                end;
            end;
        end
    ...
    if FMimeState = smtpMimePlainText then begin
        if FMailMsgText.CurrentIdx = 0 then begin
            ...
            3: StrPCopy(PAnsiChar(MsgLine), 'Content-Type: text/html; charset="' + AnsiString(FHtmlCharSet) + '"');
            ...
            5: StrPCopy(PAnsiChar(MsgLine), '');
                ...
                FLineOffset := LineNum - 1;
                FMailMsgText.SetText(FHtmlText.Text, FHtmlCodePage, FHtmlIsMultiByteCP, FHtmlConvertToCharset, smtpEncQuotedPrintable, FALSE);
                FMsgLineCount := 0;
            end;
        end
        else begin
           ...
           (FImageNumber > FEmailImages.Count) then begin
            ...
            2:  begin
                    if FImageNumber <= FEmailImages.Count then
                        ...
                        FsFileName := ExtractFileName(FEmailImages[FImageNumber - 1])
                    else
                        { Then we send attached files }
                        FsFileName := ExtractFileName(FEmailFiles[FImageNumber - FEmailImages.Count - 1]);
                ...
                {$ELSE}
                    if FHdrLines.ConvertToCharset and
                       (FHdrLines.CodePage <> IcsSystemCodePage) then
                        FsAnsiFileName := ConvertCodePage(FsFileName, IcsSystemCodePage, FHdrLines.CodePage)
                    else
                ...
                {$ENDIF}
                    ...
                    begin
                        ...
                        if (IcsIsMBCSCodePage(FHdrLines.CodePage) or (FHdrLines.CodePage = CP_UTF7))
                            ...
                            FsAnsiFileName := EncodeMbcsInline(FHdrLines.CodePage, FHdrLines.CharSet,
                                           FHdrLines.EncType, PChar(FsFileName), Length(FsFileName), FALSE, MaxInt)
                        else
                            FsAnsiFileName := HdrEncodeInline(FsAnsiFileName, SpecialsRFC822,
                                    AnsiChar(FHdrLines.EncType), AnsiString(FHdrLines.CharSet), MaxInt,
                                                         FALSE, FHdrLines.CodePage, FHdrLines.IsMultiByteCP);
                    end;
                    StrPCopy(PAnsiChar(MsgLine), 'Content-Type: ' + AnsiString(FsContentType) + ';' + #13#10#9 + 'name="' + FsAnsiFileName + '"');
                end;
            3:  begin
                    if FAttachmentEncoding = smtpEncodeBase64 then          {AG}
            ...
            4:  begin
                    if FImageNumber <= FEmailImages.Count then
                        StrPCopy(PAnsiChar(MsgLine), 'Content-Disposition: inline;' + #13#10#9 + 'filename="' + FsAnsiFileName + '"')
                    else
                        StrPCopy(PAnsiChar(MsgLine), 'Content-Disposition: attachment;' + #13#10#9 + 'filename="' + FsAnsiFileName + '"');
                end;
            5:  begin
                    if FImageNumber <= FEmailImages.Count then
                        StrPCopy(PAnsiChar(MsgLine), 'Content-ID: <IMAGE' + IcsIntToStrA(FImageNumber) + AnsiString(FHtmlImageCidSuffix) + '>')
                    else
                        { This line is just a place holder to avoid }
                        StrPCopy(PAnsiChar(MsgLine), 'X-File-ID: <FILE' + IcsIntToStrA(FImageNumber - FEmailImages.Count) + '>');
            ...
            6:  begin
                    ...
                    AAction := smtpAfterOpenFileNone;
                    try
                     { V8.71 JK, check for image stream stored as object instead of file name }
                        FStream := nil;
                        if (FImageNumber > 0) and (FImageNumber <= FEmailImages.Count) then begin
                            Obj := FEmailImages.Objects[FImageNumber - 1];
                            if (Obj <> nil) and (Obj is TStream) then begin
                                FStream := InitStreamEncBase64(Obj as TStream);
                                FStream.Position := 0;
                            end;
                        end;
                        ...
                        if FStream = nil then begin
                            if FImageNumber <= FEmailImages.Count then
                                FStream := InitFileEncBase64(FEmailImages[FImageNumber - 1], FShareMode)
                            else
                                FStream := InitFileEncBase64(FEmailFiles[FImageNumber - FEmailImages.Count - 1], FShareMode);
                       end;
                        if Assigned(FOnAfterFileOpen) then begin
    ...
    for I := 0 to HdrLines.Count - 1 do begin
        if CompareText(Copy(HdrLines[I], 1, 13), 'Content-Type:') = 0 then begin
            if FEmailFiles.Count = 0 then
                HdrLines[I] := 'Content-Type: multipart/related; ' + 'type="multipart/alternative";'
            else
...
var
    ...
    RandPart : AnsiString;
begin
    TickPart := '----=_NextPart_000_' + IcsIntToHexA(LongInt(IcsGetTickCount), 8);    { V8.71 }
...
(*  V10.0  WM_SMTP_QUIT_DELAYED never sent
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
procedure TCustomSslSmtpCli.WMSmtpQuitDelayed(const Value : TIcsSocketMsgRec);
begin
    if Assigned(FOnRequestDone) then
        FOnRequestDone(Self, FRequestType, Value.LParam);
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
class procedure TCustomSslSmtpCli.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    SmtpCli: TCustomSslSmtpCli;
begin
    if Assigned(Value.Instance) and (Value.Instance is TCustomSslSmtpCli) then begin
        try
            SmtpCli := TCustomSslSmtpCli(Value.Instance);
            with Value do begin
                if Msg = WM_ICS_SMTP_QUIT_DELAYED then
                    SmtpCli.WMSmtpQuitDelayed(Value)
            end;
        except
            on E: Exception do
                SmtpCli.HandleBackGroundException(E, 'TCustomSslSmtpCli:DispatchMessage: ' + IntToStr(Value.Msg));
        end;
    end
    else
        inherited DispatchMessage(Value);

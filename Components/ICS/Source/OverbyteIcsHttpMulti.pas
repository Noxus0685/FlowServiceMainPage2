...
const
    HttpMultiCopyRight : String = ' TIcsHttpMulti (c) 2024 V10.0 ';
...
{$ENDIF ICS_NewMessaging}   { V10 }
    ...
    // show user the list we found
        if fLogRDir then
            doCopyEvent (LogLevelInfo, 'Source HTTP Files:' + IcsCRLF + IcsFmtFileDirList (SrcFileList, false)) ;
{$IFDEF ICS_NewMessaging}   { V10 }
        Sleep(0);
{$ELSE}
        ProcessMessages;
{$ENDIF ICS_NewMessaging}   { V10 }
    ...
    // build list of target files, so we don't copy unnecessary stuff
        ...
        if fLogLDir then
            doCopyEvent (LogLevelInfo, 'Target Files on PC:' + IcsCRLF + IcsFmtFileDirList (TarFileList, false)) ;
{$IFDEF ICS_NewMessaging}   { V10 }
        Sleep(0);
{$ELSE}
        ProcessMessages;

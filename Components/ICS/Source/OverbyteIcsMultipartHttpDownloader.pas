...
const
    MultipartHttpDownloaderVersion = 1000;
    CopyRight : String             = ' TMultipartHttpDownloader (c) 2024 F. Piette V10.0 ';
...
{$ENDIF N ICS_NewMessaging} { V10 }
        ...
        procedure WMStartMulti;   { V10.0 no params }
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
        TMultipartHttpDownloader.PostMessage(Self, WM_ICS_HTTPD_START_MULTI, 0, 0);
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
class procedure TMultipartHttpDownloader.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    HttpCli:TMultipartHttpDownloader;
begin
    if Assigned(Value.Instance) and (Value.Instance is TMultipartHttpDownloader) then begin
        HttpCli := TMultipartHttpDownloader(Value.Instance);
        try
            with Value do begin
                if Msg = WM_ICS_HTTPD_START_MULTI  then
                    HttpCli.WMStartMulti()
                end;
        except
            on E: Exception do
                HttpCli.HandleBackGroundException(E, 'TMultipartHttpDownloader:DispatchMessage: ' + IntToStr(Value.Msg));
        end;
    end
    else
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TMultipartHttpDownloader.WMStartMulti;
var
    Chunk  : THttpBigInt;
    Offset : THttpBigInt;
    I      : Integer;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TMultipartHttpDownloader.AbortComponent(E:Exception);    { V8.68 added E to allow reporting }
begin
    try
        Abort;
    except

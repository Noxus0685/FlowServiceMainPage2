...
Dec 09, 2024 V9.4  Replace expiring ICS created certificate at same time warnings start
                   ...
                   Added Error to Disconnect() client method, and send message instead
                     of calling WMClientLosed directly.
...
{$IFDEF ICS_NewMessaging}        { V9.4 }
//      Ics.Fmx.OverbyteIcsMessMan,  { V9.4 }
...
const
    WSocketServerVersion     = 1000;
    CopyRight : String       = ' TWSocketServer (c) 1999-2024 F. Piette V10.0 ';
...
type
    ...
    { Altough you may use it directly, you'll probably wants to use your own derived component to add data and methods suited to your application. }
    { If you use a derived component, then assign it's class to TWSocketServer ClientClass property.  }
...
{$ENDIF} // USE_SSL
    ...
    { For each connection, it instanciate a new TWSocketClient (or derived) to handle connection. Use ClientClass to specify your derived. }
...
{$IFDEF ICS_NewMessaging}   { V10 }
        procedure WMClientClosed(const Value: TIcsSocketMsgRec);
{$ELSE}
...
{$ENDIF ICS_NewMessaging}   { V10 }
    ...
    protected
        ...
        property  ClientClass: TWSocketClientClass    read  FClientClass
                                                      write FClientClass;
    ...
    TWSocketMultiListenItem = class(TCollectionItem {$IFDEF xxPOSIX}, IIcsEventSource{$ENDIF})
    private
      FAddr: string;
      FHSocket: TSocket;
...
{$ENDIF} // USE_SSL
    ...
    protected
      function GetItem(Index: Integer): TWSocketMultiListenItem; {$IFDEF USE_INLINE} inline; {$ENDIF}
      procedure SetItem(Index: Integer; Value: TWSocketMultiListenItem); {$IFDEF USE_INLINE} inline; {$ENDIF}
    public
      constructor Create(AOwner: TPersistent; AItemClass: TWSocketMultiListenItemClass);
      function Add: TWSocketMultiListenItem;{$IFDEF USE_INLINE} inline; {$ENDIF}
      function FindItemIndex(const AHSocket: TSocket): Integer; {$IFDEF USE_INLINE} inline; {$ENDIF}
      function FindItemHandle(const AHSocket: TSocket): TWSocketMultiListenItem;
      function FindItemID(ID: Integer): TWSocketMultiListenItem; {$IFDEF USE_INLINE} inline; {$ENDIF}
      function Insert(Index: Integer): TWSocketMultiListenItem; {$IFDEF USE_INLINE} inline; {$ENDIF}
    ...
    protected
        ...
        procedure MlSocketError(AItem: TWSocketMultiListenItem;  const ASockFunc: String; ALastError: Integer = 0;
        ...
        procedure MlSetAddr(var FldAddr: string; var FldSocketFamily: TSocketfamily;
                                                       const FldOldSocketFamily: TSocketfamily; const NewValue: string); virtual;
        procedure MlSetSocketFamily(var FldSocketFamily: TSocketfamily; var FldOldSocketFamily: TSocketfamily;
...
{$IFNDEF ICS_NewMessaging}   { V10 }
        procedure Ml_Do_FD_CLOSE(AItem: TWSocketMultiListenItem; AMsg: TMessage); virtual;
...
{$ENDIF} // AUTO_X509_CERTS
    ...
    public
        ...
        function  FindBinding(const MAddr: String; MPort: Integer; var MIndex: Integer): boolean;      { V8.45 }
        function  ValidateHosts(Stop1stErr: Boolean=True; NoExceptions: Boolean=False; AllowSelfSign: Boolean=False): String; virtual; { V8.48, V8.63 }
        function  RecheckSslCerts(var CertsInfo: String; Stop1stErr: Boolean=True; NoExceptions: Boolean=False;
                                                                                            AllowSelfSign: Boolean=False): Boolean; { V8.48, V8.63 }
        function  LoadOneCert(HostNr: Integer; ForceLoad: Boolean; var LoadNew: Boolean; AllowSelfSign: Boolean=False): Boolean; { V8.57, V8.63 }
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
class procedure TCustomWSocketServer.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    WSocket : TCustomWSocketServer;
begin
    if Assigned(Value.Instance) and (Value.Instance is TCustomWSocketServer) then begin
        WSocket := Value.Instance as TCustomWSocketServer;
        try
            if Value.Msg = WM_ICS_SRV_CLIENT_CLOSED then
                WSocket.WMClientClosed(Value);
            except
                on E:Exception do begin
                    if Assigned(WSocket) then
                        WSocket.HandleBackGroundException(E, 'TWSocketServer:DispatchMessage: WMClientClosed');
            end;
        end
    end
    else
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocketServer.WMClientClosed(const Value: TIcsSocketMsgRec);
var
    Client : TWSocketClient;
    PIdRec : PClientIdRec;
begin
    PIdRec := PClientIdRec(Value.LParam);  { angus V7.00 }
    try
        Client := TWSocketClient(PIdRec^.PClient);
        { ensure client not freed already }
        if IsClient(Client) and (Client.CliId = PIdRec^.CliId) then begin
            try
                TriggerClientDisconnect(Client, Value.WParam);
            finally
                { Calling Free will automatically remove client from list because we installed a notification handler. }
...
{ destroyed.                                                                }
procedure TCustomWSocketServer.Notification(AComponent: TComponent; Operation: TOperation);
begin
...
{$ENDIF}
        ...
        try
            ...
            // safely ignore any exception here. Component user may already have accepted and closed the connection.
        end;
        raise;
    end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocketServer.TriggerClientConnect(Client: TWSocketClient; Error: Word);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocketServer.TriggerClientDisconnect(Client: TWSocketClient; Error: Word);
begin
...
var
    ...
    PIdRec : PClientIdRec;
begin
{ pass CliId to WMClientClosed so correct client is closed  }
    New(PIdRec);
    PIdRec^.PClient := Client;
    PIdRec^.CliId   := Client.CliId;
{.$IFDEF ICS_NewMessaging}   { V10 }
//     if TCustomWSocketServer.PostMessage(Self, WM_ICS_SRV_CLIENT_CLOSED, WPARAM(ErrCode), LPARAM(PIdRec)) then
//        System.Dispose(PIdRec);
{.$ELSE}
    ...
    WMClientClosed(Msg);
//    if NOT PostMessage(Handle, FMsg_WM_CLIENT_CLOSED, WPARAM(ErrCode), LPARAM(PIdRec)) then  { V9.4 send message }
//       System.Dispose(PIdRec);
{.$ENDIF ICS_NewMessaging}   { V10 }
end;
...
begin
    inherited;
    FMultiListenIndex := -1;
    FMultiListenSockets := TWSocketMultiListenCollection.Create(Self, MultiListenItemClass);
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomMultiListenWSocketServer.MlClose(AItem: TWSocketMultiListenItem);
var
    iStatus : Integer;
begin
...
{$ENDIF N ICS_NewMessaging} { V9.4 }
    ...
    if (not (csDestroying in ComponentState)) and (not AItem.CloseInvoked) then begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomMultiListenWSocketServer.MlSocketError(AItem: TWSocketMultiListenItem; const ASockFunc: String;
...
begin
    ...
    try
        if ALastError = 0 then
            ...
            ErrCode := ALastError;
        Line  := 'Listening socket index #' + IntToStr(FMultiListenIndex) + ' ' + WSocketErrorDesc(ErrCode) +
                                                                             ' (#' + IntToStr(ErrCode) +' in ' + ASockFunc + ')' ;
        if (ErrCode = WSAECONNRESET) or (ErrCode = WSAENOTCONN) then begin
        ...
        LastError := ErrCode;
        RaiseException(Line, ErrCode, WSocketErrorDesc(ErrCode), FriendlyMsg, ASockfunc, AItem.Addr, AItem.Port, FProtoStr);  { V8.36 }
    finally
        FMultiListenIndex := -1;
    end;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFNDEF ICS_NewMessaging}  { V10 }
procedure TCustomMultiListenWSocketServer.Ml_Do_FD_CLOSE(AItem : TWSocketMultiListenItem; AMsg  : TMessage);
begin
    if (AItem.HSocket <> INVALID_SOCKET) then begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomMultiListenWSocketServer.MlListen(AItem: TWSocketMultiListenItem);
var
    iStatus : Integer;
    ErrorCode : Integer;
    FriendlyMsg : string;
    ...
    try
      ...
      {$IFDEF MSWINDOWS}  { V8.49 }
        ...
        { V8.36 Prevent other applications accessing this address and port }
            optval  := -1;
            iStatus := WSocket_SetSockOpt(AItem.HSocket, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, @optval, SizeOf(optval));
            if iStatus <> 0 then begin
      ...
      {$ENDIF}
        ...
        iStatus := WSocket_bind(AItem.HSocket, PSockAddr(@AItem.Fsin)^, SizeOfAddr(AItem.Fsin));
        if iStatus = 0 then
            AItem.State := wsBound
        else begin
            ...
            if (ErrorCode = WSAEADDRINUSE) or (ErrorCode = WSAEACCES)  then   { V8.36 more friendly message for common error }
                FriendlyMsg := 'Another server is already listening on ' + AItem.Addr + ':' + AItem.Port;
        ...
        if iStatus = 0 then begin
         ...
         { V8.64 keep actual port }
            ...
            if WSocket_getsockname(AItem.HSocket, PSockAddr(@saddr)^, saddrlen) = 0 then
...
{$IFDEF ICS_NewMessaging}   { V10 }
    // pending !!!!
{$ELSE}
        iStatus := WSocket_WSAASyncSelect(AItem.HSocket, Handle, FMsg_WM_ASYNCSELECT, AItem.FSelectEvent);
        if iStatus <> 0 then begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomMultiListenWSocketServer.MlPause(AItem: TWSocketMultiListenItem);
begin
{$IFDEF ICS_NewMessaging}   { V10 }
    // pending !!!!
{$ELSE}
    if not AItem.Paused then
        AItem.FPaused := WSocket_WSAASyncSelect(AItem.HSocket, Handle, 0, 0) = 0;
{$ENDIF ICS_NewMessaging}   { V10 }
end;
...
{$ELSE}
    if AItem.Paused then
        AItem.FPaused := not (WSocket_WSAASyncSelect(AItem.HSocket, Handle, FMsg_WM_ASYNCSELECT, AItem.SelectEvent) = 0);
{$ENDIF ICS_NewMessaging}   { V10 }
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomMultiListenWSocketServer.MlSetAddr(var FldAddr: string; var FldSocketFamily: TSocketfamily;
                                                                const FldOldSocketFamily: TSocketfamily; const NewValue: string);
var
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomMultiListenWSocketServer.MlSetSocketFamily(var FldSocketFamily: TSocketfamily;
                                                            var FldOldSocketFamily: TSocketfamily; const NewValue: TSocketFamily);
begin
...
begin
    ...
    try
        ...
        on E:Exception do
            Result := 'Socket 1 Listen Failed: ' + Self.Addr + ' port ' + Self.Port + ' - ' + E.Message;
    end;
    if Assigned(FMultiListenSockets) then begin
        for I := 0 to FMultiListenSockets.Count - 1 do begin
            if FMultiListenSockets[I].State <> wsListening then
            try
                ...
                on E:Exception do begin
                    ...
                    Result := Result + 'Socket ' + IntToStr (I + 1) + ' Listen Failed: ' + Self.Addr + ' port ' + Self.Port + ' - ' + E.Message;
                end;
            end;
        end;
    end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomMultiListenWSocketServer.SetMultiListenIndex(const Value: Integer);
begin
...
begin
    ...
    for I := 0 to FMultiListenSockets.Count -1 do begin
        ...
        if (LItem.HSocket <> INVALID_SOCKET) then
            WSocket_WSAASyncSelect(LItem.HSocket, Handle, FMsg_WM_ASYNCSELECT, LItem.SelectEvent);
    end;
end;
...
begin
    ...
    if ThreadID = IcsGetCurrentThreadID then begin // not thread-safe
        for I := 0 to FMultiListenSockets.Count -1 do begin
            LItem := FMultiListenSockets[I];
            if (LItem.HSocket <> INVALID_SOCKET) then
                WSocket_WSAASyncSelect(LItem.HSocket, Handle, 0, 0);
        end;
    end;
...
var
    ...
    AItem   : TWSocketMultiListenItem;
begin
    if msg.wParam = WPARAM(GetHSocket) then begin
        FMultiListenIndex := -1;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomMultiListenWSocketServer.TriggerClientConnect(Client: TWSocketClient; Error: Word);
begin
...
begin
    RealPort := Self.Port;
    if Self.GetState = wsListening then RealPort := Self.BindPort;  { V8.64 for dynamic ports }
    Result := 'Socket 1 State: ' + SocketStateNames[Self.GetState] + ' ' +  SocketFamilyNames [Self.SocketFamily] + ' on ' +
...
begin
    Result := False;
    if GetState <> wsListening then Exit;
    if MultiListenSockets.Count > 0 then begin
        for K := 0 to MultiListenSockets.Count - 1 do begin
...
begin
    if FHSocket <> INVALID_SOCKET then begin
        if FSelectEvent and FD_ADDRESS_LIST_CHANGE = 0 then begin
            ...
            Result := WSocket_WSAASyncSelect(FHSocket, OwnerServer.Handle, OwnerServer.FMsg_WM_ASYNCSELECT, FSelectEvent) <> SOCKET_ERROR;
        end
        else
            Result := True;
        if Result then
            Result := (WSocket_WSAIoctl(FHSocket, SIO_ADDRESS_LIST_CHANGE, nil, 0, nil, 0, LBytesRcvd, nil, nil) <> SOCKET_ERROR) or
...
begin
    if FHSocket <> INVALID_SOCKET then begin
        if FSelectEvent and FD_ROUTING_INTERFACE_CHANGE = 0 then begin
            ...
            Result := True;
        if Result then
            Result := (WSocket_WSAIoctl(FHSocket, SIO_ROUTING_INTERFACE_CHANGE,  @Fsin, SizeOfAddr(Fsin), nil, 0, LBytesRcvd,
                                                           nil, nil) <> SOCKET_ERROR) or (WSocket_WSAGetLastError = WSAEWOULDBLOCK);
    end
    else
        Result := False;
{$ENDIF}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TWSocketMultiListenCollection.FindItemHandle(const AHSocket: TSocket): TWSocketMultiListenItem;
var
    I: Integer;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TWSocketMultiListenCollection.FindItemIndex(const AHSocket: TSocket): Integer;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TWSocketMultiListenCollection.SetItem(Index: Integer; Value: TWSocketMultiListenItem);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{ Triggered when socket is closed. Need to inform server socket to update client list and trigger client disconnect event. }
...
begin
    if not FSessionClosedFlag then begin
        ...
        if Assigned(FServer) then begin
            ...
            PIdRec^.CliId   := FCliId;
{$IFDEF ICS_NewMessaging}   { V10 }
            if TWSocket.PostMessage(Server, WM_ICS_SRV_CLIENT_CLOSED, WPARAM(ErrCode), LPARAM(PIdRec)) then
                System.Dispose(PIdRec);
{$ELSE}
            if NOT PostMessage(Server.Handle, Server.FMsg_WM_CLIENT_CLOSED, WPARAM(ErrCode), LPARAM(PIdRec)) then
                System.Dispose(PIdRec);
{$ENDIF ICS_NewMessaging}   { V10 }
        end;
...
procedure TWSocketClient.TriggerSessionClosed(ErrCode : Word);
begin
    Server.Disconnect(Self, ErrCode);   { V9.4 let server class send message }
end;
...
{$IFDEF AUTO_X509_CERTS}  { V8.59 }
     ...
     { V8.63 something sensible in log before all the ordering stuff }
        X509Log('Starting SSL/TLS Certificate Order for Domain: ' + HostNames[0] + ', From: ' + CertProduct +
                    ' Using Challenge: ' + ChallengeTypeLits[CertChallenge] + ', With: ' + SupplierProtoLits[CertSupplierProto]);
...
{$ENDIF} // AUTO_X509_CERTS
             ...
             { V8.57 do we want a client SSL certificate from the browser, NOTE you should check it the OnSslHandshakeDone event and
                  close the connection if invalid, beware this usually causes the browser to request a certificate which can be obtrusive. }

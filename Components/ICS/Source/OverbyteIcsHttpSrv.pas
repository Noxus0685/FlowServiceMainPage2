...
Mar 07, 2025 V9.5 Allow built in HTTP error respones to be customised using new event
                    OnHttpCustomError which is called by the error handlers with the error
                    error, path, and existing Body, that may be replaced or modified as required.
...
const
    ...
    CopyRight : String = ' THttpServer (c) 1999-2025 F. Piette V10.0 ';
    DefServerHeader : string = 'Server: ICS-HttpServer-V10.0';   { V8.09 }
...
type
    ...
    THttpGetFlag         = (hgSendDoc, hgSendStream, hgWillSendMySelf,hg404, hg403, hg401, hg400, hgAcceptData, hgSendDirList,  hg501);                           { V8.08 added hg501 }
    THttpSendType        = (httpSendHead, httpSendDoc);
    THttpMethod          = (httpMethodNone, httpMethodGet, httpMethodPost, httpMethodHead, httpMethodOptions, httpMethodPut,
                                httpMethodDelete, httpMethodTrace, httpMethodPatch, httpMethodConnect);              { V8.08 more methods }
...
{$IFDEF ICS_NewMessaging}   { V10 }
        procedure WMHttpDone(const Value : TIcsSocketMsgRec); virtual;
{$ELSE}
...
{$ENDIF}  *)
        ...
        procedure TriggerCustomError(Client: TObject; ErrNr: Integer; const Info: String; var Body: String);    { V9.5 }
    public
        constructor Create(AOwner: TComponent); override;
        destructor  Destroy; override;
...
{$ENDIF}
function StreamWriteStrA(AStrm : TStream; const AStr: String {$IFDEF COMPILER12_UP}; DstCodePage: LongWord = CP_ACP {$ENDIF}): Integer;
function StreamWriteLnA(AStrm : TStream; const AStr: String {$IFDEF COMPILER12_UP}; DstCodePage: LongWord = CP_ACP {$ENDIF}): Integer;
function StreamReadStrA(AStrm : TStream; ByteCnt: Integer {$IFDEF COMPILER12_UP}; SrcCodePage: LongWord = CP_ACP {$ENDIF}): String;
function StreamWriteA(AStream : TStream; Buf: PChar; CharCnt: Integer {$IFDEF COMPILER12_UP}; DstCodePage: LongWord = CP_ACP {$ENDIF}): Integer;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
class procedure THttpConnection.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    HttpConn: THttpConnection;
begin
    if Assigned(Value.Instance) and (Value.Instance is THttpConnection) then begin
        HttpConn := THttpConnection(Value.Instance);
        try
            with Value do begin
                if Msg = WM_ICS_HTTPSRV_DONE then
                    HttpConn.WMHttpDone(Value)
            end;
        except
            on E: Exception do
                HttpConn.HandleBackGroundException(E, 'THttpConnection:DispatchMessage: ' + IntToStr(Value.Msg));
        end;
    end
    else
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure THttpConnection.WMHttpDone(const Value: TIcsSocketMsgRec);
begin
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
            THttpConnection.PostMessage(Self, WM_ICS_HTTPSRV_DONE, 0, 0);
{$ELSE}

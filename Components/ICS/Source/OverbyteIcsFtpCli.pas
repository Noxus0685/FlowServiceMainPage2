...
Methods:
  ...
  Noop       - No operation command, the server should acknowledge the command with 421 or 500 if
               not recognised but does nothing else, used to try and keep the control connection
               open on long file transfer, typically an hour or more.  Beware some servers
...
Feb 14, 2023 V8.71 Added UTF8_ON and UTF8_ONAsync commands as an alternate to using
                     Opts command with an argument, thanks to Jaroslav Kulísek.
                   Ensure inherited destroy called.
                   Generally free objects instead of destroying them.
                   Using Int64 ticks.
                   Increased sync Timeout to 30 seconds and make sure reset
                     with Progress more often so requests don't timeout.  Note this
                     may need substantial increase for MD5/CRC of very large files.
                   Increased CloseEndSecs timeout from 5 to 30 seconds to allow time
                     for large buffers to be sent without transfer failing.
                   Fixed unicode compiler bug parsing Cmlsd/XCmlsd command response.
                   Removed very old UseBandwidthControl conditional code, only
                     supporting BUILTIN_THROTTLE now.
                   Added NoopAsync method sends NOOP no operation command to try and
                     keep the control connection alive during long transfers, note there
                     is no sync version and the response is ignored (but logged). Beware
                     some older FTP servers may treat NOOP as illegal and fail the
                     transfer (including ICS FTP server V8.70 and earlier).  Not
...
Dec xx, 2024 V10.0 Updated for DEFINE ICS_NewMessaging that implements new cross platform
                     message handler for Windows and Linux, planned for mobile platforms.
...
const
  FtpCliVersion      = 1000;
  CopyRight : String = ' TFtpCli (c) 1996-2025 F. Piette V10.0 ';
  FtpClientId : String = 'ICS FTP Client V10.0 ';   { V2.113 sent with CLNT command  }
...
type
  ...
  TZlibProgress   = procedure(Sender: TObject; Count: Int64; var Cancel: Boolean) of object;  { V2.113 }
  ...
  protected
    ...
    FStartTime          : Int64;    { V8.65, V8.71 now Int64 }
    FStopTime           : Int64;    { V8.65 }
    FState              : TFtpState;
...
{$ENDIF N ICS_NewMessaging} { V10 }
  ...
  { FZStreamRec         : TZStreamRec;    V2.102 Zlib stream control record, used for Immediate }
    ...
    FExpectingNoopResponse: Boolean;     { V8.71 next response is for NOOP command for kee-alive, ignore it }
...
(* {$IFNDEF NO_DEBUG_LOG} { V10.0 now in desceendent }
...
{$IFDEF ICS_NewMessaging}   { V10 }
    procedure   WMFtpRequestDone(const Value: TIcsSocketMsgRec); virtual;
    procedure   WMFtpSendData(const Value: TIcsSocketMsgRec); virtual;
    procedure   WMFtpCloseDown(const Value: TIcsSocketMsgRec); virtual;
{$ELSE}
...
{$ENDIF ICS_NewMessaging}   { V10 }
  ...
  {$ENDIF}
    ...
    function    OpenFileStream (const FileName: string; Mode: Word): TStream;  { V2.113 }  {$IFDEF USE_INILE} inline; {$ENDIF}
    ...
    procedure   HandleHttpTunnelError(Sender: TObject; ErrCode: Word; TunnelServerAuthTypes: THttpTunnelServerAuthTypes; const Msg: String);
...
{$IFDEF ICS_NewMessaging}   { V10 }
   class procedure DispatchMessage(const Value : TIcsSocketMsgRec); override;   { V10 }
{$ENDIF ICS_NewMessaging}  { V10 }
    ...
    procedure   UTF8_ONAsync;    virtual;    { V8.71 JK UTF-8 ON }
    procedure   NoopAsync;       virtual;    { V8.71   no operation (keep-alive) }
    ...
    property    StartTick         : Int64                read  FStartTime;      { V2.113 when last transfer started, in case it failed, V8.71 Int64 }
{$IF DEFINED(BUILTIN_THROTTLE)}
...
{$ENDIF}
    ...
    property ProxyPort            : String               read  FProxyPort
                                                         write FProxyPort;
    property HttpTunnelAuthType   : THttpTunnelAuthType  read  FHttpTunnelAuthType  { V7.18 }
  ...
  protected
    ...
    FTimeStop      : Int64;                   { V8.71 Int64 }    { Milli-seconds    }
  ...
  public
    ...
    function    UTF8_ON    : Boolean;    { V8.71 JK UTF-8 ON }
...
type
    ...
    protected
        ...
        procedure ControlSocketSslShutDownComplete(Sender: TObject; Bidirectional : Boolean; ErrCode: Integer); virtual;
        procedure TransferSslHandshakeDone(Sender: TObject; ErrCode: Word;  PeerCert: TX509Base; var Disconnect : Boolean); virtual;
        procedure TransferSslVerifyPeer(Sender: TObject;  var Ok: Integer; Cert: TX509Base); virtual;
        procedure TransferSslCliGetSession(Sender: TObject; var SslSession: Pointer; var FreeSession: Boolean); virtual;
        procedure TransferSslCliNewSession(Sender: TObject; SslSession: Pointer; WasReused: Boolean; var IncRefCount: Boolean); virtual;
        procedure TransferSslCliCertRequest(Sender: TObject; var Cert: TX509Base); virtual;
    public
        constructor Create(AOwner : TComponent); override;
...
begin
   case RqType of
      ...
      ftpUTF8_ONAsync: result:='UTF8_ONAsync';      { V8.71 }
{$IFDEF USE_SSL}
...
{$ENDIF N ICS_NewMessaging} { V10 }
    ...
    FCloseEndSecs       := 30;       { V2.100, V8.71 was 5 secs but buffers large now }
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF ICS_NewMessaging}   { V10 }
class procedure TCustomFtpCli.DispatchMessage(const Value: TIcsSocketMsgRec);
var
    FtpCli: TCustomFtpCli;
begin
    if Assigned(Value.Instance) and (Value.Instance is TCustomFtpCli) then begin
        FtpCli := TCustomFtpCli(Value.Instance);
        try
            with Value do begin
                if Msg = WM_ICS_FTP_REQUEST_DONE then
                    FtpCli.WMFtpRequestDone(Value)
                else if Msg = WM_ICS_FTP_SENDDATA then
                    FtpCli.WMFtpSendData(Value)
                else if Msg = WM_ICS_FTP_CLOSEDOWN then
                    FtpCli.WMFtpCloseDown(Value)
            end;
        except
            on E: Exception do
                FtpCli.HandleBackGroundException(E, 'TFtpCli:DispatchMessage: ' + IntToStr(Value.Msg));
        end;
    end
    else
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.WMFtpRequestDone(const Value: TIcsSocketMsgRec);
begin
    if Assigned(FOnRequestDone) then
        FOnRequestDone(Self, FRequestType, Value.LParam);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.WMFtpSendData(const Value: TIcsSocketMsgRec);
begin
    Progress;   { V8.71 reset sync timer }

    { Record the starting time }
    FStartTime := IcsGetTickCount64;
    FDurationMsecs := 0;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure  TCustomFtpCli.WMFtpCloseDown(const Value: TIcsSocketMsgRec);
begin
    Progress;    { V8.71 reset sync timer }
    { see if last buffer has been fully sent, or given up waiting }
    if (FDataSocket.BufferedByteCount = 0) or IcsTestTrgTick64(FCloseEndTick) then begin   { V8.71 Int64 }
        FDataSocket.ShutDown(1); { 18/05/2005 was CloseDelayed; }
        FEofFlag := TRUE;
    end
    else if ((FControlSocket.State = wsConnected) and (FDataSocket.State = wsConnected)) then begin
        TCustomFtpCli.PostMessage(Self, WM_ICS_FTP_CLOSEDOWN, 0, 0);  // not finished, call ourself again
    end;
end;
...
begin
    ...
    { Record the starting time }
    FStartTime := IcsGetTickCount64;     { V8.65, V8.71 Int64 }
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.AbortComponent(E:Exception);    { V8.68 added E to allow reporting }
begin
    try
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure ZlibOnProgress(Sender: TObject; Count: Int64; var Cancel: Boolean);
var
            ...
            if FZStreamState > ftpZStateNone then begin
                if (FZStreamState = ftpZStateSaveDecom) and (FModeZStream.Size > 0) and
                    ...
                    try
                        ...
                        ZlibDecompressStreamEx (FModeZStream, FLocalStream, Self, ZlibOnProgress);  { V1.113 added callback }
                        NewSize := FLocalStream.Size - NewSize ;
                        TriggerDisplay('! Download size, compressed ' + IntToKByte(FModeZStream.Size) +
                                                                    ' bytes, Uncompressed ' + IntToKByte (NewSize) + 'bytes') ;
                    except
                ...
                FZStreamState := ftpZStateNone;
                if Assigned (FModeZStream) then
                    FModeZStream.Free;    { V8.71 }
                FModeZStream := nil;
                try
                    if FileExists(FZCompFileName) then
                        DeleteFile (FZCompFileName);  { V1.113 }
                except
                end;
            end;
        end;
    finally
        if Assigned(FLocalStream) and (FStreamFlag = FALSE) then
...
begin
    ...
    if Abort then begin
        TriggerDisplay('! Abort requested by application');   { V8.71 restore logging }
     //   FDataSocket.Close;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.ExecAsync(RqType: TFtpRequest; Cmd: String; OkResponses: array of Word;  DoneAsync: TFtpNextProc);
                                       { Command to execute, List of responses like '200 221 342' ,  What to do when done  }
var
    I : Integer;
begin
{$IFNDEF NO_DEBUG_LOG}
...
{$ENDIF}
    ...
    { V2.90 some FTP responses are multiline, welcome banner, FEAT command, keep them all  }
    FLastMultiResponse := '';
    FRequestDoneFlag   := FALSE;
    FExpectingNoopResponse := False;     { V8.71 }
    FNext              := NextExecAsync;
...
{$IFDEF USE_SSL}
    ...
    if (FFctPrv in [ftpFctAuth]) and
       ((FStatusCode = 234) or (FStatusCode = 334)) then begin
        ...
        FControlSocket.OnSslShutDownComplete := TSslFtpClient(Self).ControlSocketSslShutDownComplete;
...
begin
    ...
    if FConnectionType = ftpProxy then begin
        if (IcsCompareText(FPort, 'ftp') = 0) or (IcsCompareText(FPort, '21') = 0) then
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.UTF8_ONAsync;                                           { V8.71 JK }
begin
    FFctPrv := ftpFctUTF8_ON;
    ExecAsync(ftpUTF8_ONAsync, 'OPTS UTF8 ON', [200], nil);
end;

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.NoopAsync;                                              { V8.71 keep-alive }
begin
 { handled specially since may be sent while another file transfer command is running, don't want to kill it }
    FExpectingNoopResponse := True;
    SendCommand('NOOP Keep-Alive');
end;
...
{$ENDIF}
    TriggerDisplay('! Aborting');       // !!!!  TEMP
    StateChange(ftpAbort);
{$IFDEF USE_SSL}
    if Assigned(FControlSocket.OnSslShutDownComplete) then
        TSslFtpClient(Self).ControlSocketSslShutDownComplete(FControlSocket, TRUE, 426);
{$ENDIF}
    DestroyLocalStream;
    FResumeAt := 0;        { V2.111 clear starting position }
...
(*    if ftpFctNoop in FFctSet then begin         { V8.71 }
        FFctPrv := ftpFctNoop;
        FFctSet := FFctSet - [FFctPrv];
        NoopAsync;
        Exit;
...
begin
    HighLevelAsync(ftpConnectAsync, [ftpFctOpen, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct]);
end;

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.ConnectHostAsync;       { V7.01 }
begin
    HighLevelAsync(ftpConnectHostAsync, [ftpFctOpen, ftpFctHost, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct]);
end;

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.ConnectFeatAsync;           { V7.09 }
begin
    HighLevelAsync(ftpConnectFeatAsync, [ftpFctOpen, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct, ftpFctFeat]);
end;

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.ConnectFeatHostAsync;       { V7.09 }
begin
    HighLevelAsync(ftpConnectFeatHostAsync, [ftpFctOpen, ftpFctHost, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct, ftpFctFeat]);
end;

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.ReceiveAsync;
begin
    HighLevelAsync(ftpReceiveAsync, [ftpFctOpen, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct, ftpFctCwd,
                                                                    ftpFctTypeSet, ftpFctPort, ftpFctGet,  ftpFctQuit]);
end;
...
begin
    ...
    HighLevelAsync(ftpPutAsync, [ftpFctPort, ftpFctPut]);
    if DataSocket.LastError <> 0 then    { V2.100 }
       raise FtpException.Create('Socket Error - ' +  GetWinsockErr(DataSocket.LastError)); { V2.100 }
end;
...
begin
    HighLevelAsync(ftpRestPutAsync, [ftpFctRest, ftpFctPort, ftpFctPut]);
end;
...
begin
    HighLevelAsync(ftpRestartPutAsync, [ftpFctOpen, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct, ftpFctCwd,
                                                            ftpFctTypeSet, ftpFctRest, ftpFctPort, ftpFctPut, ftpFctQuit]);
end;
...
begin
    HighLevelAsync(ftpTransmitAsync, [ftpFctOpen, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct, ftpFctCwd,
                                                                        ftpFctTypeSet, ftpFctPort, ftpFctPut, ftpFctQuit]);
end;
...
begin
    HighLevelAsync(ftpAppendAsync, [ftpFctPort, ftpFctAppend]);
end;
...
begin
    HighLevelAsync(ftpTransmitAsync, [ftpFctOpen, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct, ftpFctCwd,
                                                                    ftpFctTypeSet, ftpFctPort, ftpFctAppend, ftpFctQuit]);
end;
...
begin
    HighLevelAsync(ftpDirAsync, [ftpFctPort, ftpFctDir]);
end;
...
begin
    HighLevelAsync(ftpDirectoryAsync, [ftpFctOpen, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct, ftpFctCwd,
                                                                                            ftpFctPort, ftpFctDir, ftpFctQuit]);
end;
...
begin
    HighLevelAsync(ftpLsAsync, [ftpFctPort, ftpFctLs]);
end;
...
begin
    HighLevelAsync(ftpListAsync,  [ftpFctOpen, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct, ftpFctCwd,
                                                                                            ftpFctPort, ftpFctLs, ftpFctQuit]);
end;
...
begin
    HighLevelAsync(ftpSystemAsync, [ftpFctOpen, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct, ftpFctSyst, ftpFctQuit]);
end;
...
begin
    HighLevelAsync(ftpAuthAsync, [ftpFctOpen, ftpFctAuth, ftpFctQuit]);
end;
...
begin
    HighLevelAsync(ftpRestartGetAsync, [ftpFctOpen, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct, ftpFctCwd,
                                                           ftpFctTypeSet, ftpFctRest, ftpFctPort, ftpFctGet, ftpFctQuit]);
end;
...
begin
    HighLevelAsync(ftpRestGetAsync, [ftpFctRest, ftpFctPort, ftpFctGet]);
end;
...
begin
    HighLevelAsync(ftpMkdirAsync, [ftpFctOpen, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct, ftpFctCwd, ftpFctMkd, ftpFctQuit]);
end;
...
begin
    HighLevelAsync(ftpRmdirAsync, [ftpFctOpen, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct, ftpFctCwd, ftpFctRmd, ftpFctQuit]);
end;
...
begin
    HighLevelAsync(ftpDeleteAsync, [ftpFctOpen, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct, ftpFctCwd, ftpFctDele, ftpFctQuit]);
end;
...
begin
    HighLevelAsync(ftpDoQuoteAsync, [ftpFctOpen, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct, ftpFctCwd, ftpFctQuote, ftpFctQuit]);
end;
...
begin
    HighLevelAsync(ftpRenameAsync, [ftpFctOpen, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct, ftpFctCwd,
...
begin
    HighLevelAsync(ftpSizeAsync, [ftpFctOpen, ftpFctAuth, ftpFctUser, ftpFctPass, ftpFctAcct, ftpFctCwd, ftpFctSize, ftpFctQuit]);
end;

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.MlsdAsync;    { V2.90 machine list directory        }
begin
    HighLevelAsync(ftpMlsdAsync, [ftpFctPort, ftpFctMlsd]);
end;
...
begin
    HighLevelAsync(ftpSiteDmlsdAsync, [ftpFctPort, ftpFctSiteDmlsd]);
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.DataSocketGetDataAvailable(Sender: TObject; ErrCode: word);
var
    Len     : Integer;
    ...
    ACodePage : LongWord;
begin
    if not Progress then    { did user abort }
        Exit;
    ...
    if Len = 0 then
        ...
        if (aSocket.State = wsConnected) and
           (aSocket.LastError <> WSAEWOULDBLOCK) then begin
            TriggerDisplay('! Data: Receive Error - ' + GetWinsockErr(aSocket.LastError));
            aSocket.Shutdown(2);
            Exit;
        end;
    end
...
{ abandon data if aborting xfer, it's probably corrupted - Angus }
        ...
        if FDisplayFileFlag then begin
            ...
            if (FCodePage <> CP_UTF8) and (ftpAutoDetectCodePage in FOptions) and (CharsetDetect(@Buffer, Len) = cdrUtf8) then          { AG V7.02 }
                ACodePage := CP_UTF8
            else
                ACodePage := FCodePage;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.DataSocketGetSessionConnected(Sender: TObject; ErrCode: word);
begin
{$IFNDEF NO_DEBUG_LOG}                                             { 2.104 }
    if CheckLogOptions(loProtSpecInfo) then
        DebugLog(loProtSpecInfo, '! Data Session Connected (Get)');
{$ENDIF}
    if not Progress then    { V8.71 did user abort }
        Exit;
    ...
    if ErrCode <> 0 then begin
        FLastResponse := 'Unable to establish data connection - ' + WSocketGetErrorMsgFromErrorCode(ErrCode);
        FStatusCode   := 550;
        SetErrorMessage;
        FDataSocket.Close;
        FRequestResult := FStatusCode;
...
procedure TCustomFtpCli.DataSocketPutSessionConnected(Sender: TObject; ErrCode: word);
begin
{$IFNDEF NO_DEBUG_LOG}                                             { 2.105 }
    if CheckLogOptions(loProtSpecInfo) then
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.DataSocketGetSessionAvailable(Sender: TObject; ErrCode: word);
var
    aSocket : TSocket;
begin
{$IFNDEF NO_DEBUG_LOG}                                             { 2.105 }
...
{$ENDIF}
    Progress;    { V8.71 reset sync timeout }
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.DataSocketGetSessionClosed(Sender: TObject; ErrCode: word);
begin
    TriggerDisplay('! Data Session closed');      { V8.71 }
    DestroyLocalStream;
    TriggerDisplay('! Local File Stream Closed');      { V8.71 }
    FFileReceived := TRUE;
    FError        := ErrCode;
    Next3GetAsync;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.DataSocketPutSessionAvailable(Sender: TObject; ErrCode : word);
var
    aSocket : TSocket;
begin
{$IFNDEF NO_DEBUG_LOG}                                             { 2.105 }
...
{$IFDEF ICS_NewMessaging}   { V10.0 }
        TCustomFtpCli.PostMessage(Self, WM_ICS_FTP_SENDDATA, 0, 0);
{$ELSE}
        PostMessage(Handle, FMsg_WM_FTP_SENDDATA, 0, 0);
{$ENDIF ICS_NewMessaging}   { V10.0 }
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.DataSocketPutDataSent(Sender: TObject; ErrCode: word);
var
    Count : Integer;
begin
...
{$ENDIF}
        if Count > 0 then begin
            ...
            FCloseEndTick := IcsGetTrgSecs64(FCloseEndSecs);  { but not forever V8.71 Int64 }
            Progress;    { V8.71 reset sync timer }
{$IFDEF ICS_NewMessaging}   { V10.0 }
            TCustomFtpCli.PostMessage(Self, WM_ICS_FTP_CLOSEDOWN, 0, 0);
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.DataSocketPutSessionClosed(Sender: TObject; ErrCode: word);
begin
{$IFNDEF NO_DEBUG_LOG}                                             { 2.105 }
    if CheckLogOptions(loProtSpecInfo) then
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.DataSocketPutDataAvailable(Sender: TObject; ErrCode: word);
var
...
begin
    FStopTime := IcsGetTickCount64;     { V8.65, V8.71 Int64 }
    Buffer := IntToKByte(FByteCount) + 'bytes received/sent in ';
    FDurationMsecs := IcsElapsedMsecs64(FStartTime);  { V8.71 Int64 }
 (*{   if LongWord (FStopTime) >= LongWord (FStartTime) then   { V2.102 fix zero duration downloads }
    ...
    if FDurationMsecs < 5000 then
        Buffer := Buffer + IntToStr(FDurationMsecs) + ' milliseconds'
    else begin
        Buffer := Buffer + IntToStr(FDurationMsecs div 1000) + ' seconds';
        if FStopTime > FStartTime then begin
            if FByteCount > 32767 then
                BytesSec := 1000 * (FByteCount div FDurationMsecs)
            else
                BytesSec := (1000 * FByteCount) div FDurationMsecs;
...
begin
    ...
    if Length(Result) = 0 then
        Result := 'temp'; { might be saving to stream only }
    ...
    I := Length(Ticks);
    if I < 6 then
        Ticks := '123' + Ticks; { if windows running short }
  ...
  { V2.111 never resume directory listings }
    ...
    try
        ...
        on E:Exception do begin
            ...
            FLastResponse := 'Unable to open local file ' + FLocalFileName + ': ' + E.Message; { V2.101}
            FStatusCode   := 550;
            SetErrorMessage;
            FDataSocket.Close;
            FRequestResult := FStatusCode;
...
begin
    ...
    if not (((FStatusCode div 10) = 15) or  (FStatusCode = 125)) then begin  { Accept range 150-159, 125  }
        SetErrorMessage;
        FNext := nil;
        FDataSocket.Close;
        DestroyLocalStream;
...
begin
    DisplayLastResponse;
    GetInteger(@FLastResponse[1], FStatusCode);
    if not ((FStatusCode = 125) or (FStatusCode = 226) or (FStatusCode = 250)) then begin
        SetErrorMessage;
        DestroyLocalStream;
        FResumeAt := 0;        { V2.111 clear starting position }
        FDataSocket.Close;
...
{$ENDIF}
    ...
    //  NewPos       := FResumeAt;
        if not Assigned(FLocalStream) and not FStreamFlag then begin
            FLocalStream := OpenFileStream(FLocalFileName, fmOpenRead + FShareMode); { V2.113 }
        end;
        if FResumeAt > 0 then
   ...
   //   FModeZStream := TMemoryStream.Create;   { V1.113 memorystream very slow, use filestream }
     ...
     { option 1 - compress data into stream, send stream  }
        ...
        try
            ZlibCompressStreamEx (FLocalStream, FModeZStream, Z_BEST_SPEED, zsZLib, false, Self, ZlibOnProgress);  { V1.113 added callback }
            FModeZStream.Position := 0 ;
            TriggerDisplay('! Upload Size, compressed ' + IntToKByte (FModeZStream.Size) + ' bytes, Uncompressed ' +
...
{$ENDIF}
    ...
    if FPassive then
        ...
        if (FDataPortRangeStart = 0) and (FDataPortRangeEnd = 0) then begin
            ...
            if (FLastDataPort < FDataPortRangeStart) or (FLastDataPort > FDataPortRangeEnd) then
            ...
            while TRUE do begin
                ...
                try
                    ...
                    if FDataSocket.LastError = WSAEADDRINUSE then begin
                        ...
                        HandleError('Data connection winsock bind failed - ' + GetWinsockErr(FDataSocket.LastError));
                        Exit;
                    end;
                end;
            end;
    ...
    if FPassive then begin
        ...
        if saddr.sin6_family = AF_INET6 then
         //   Msg := 'EPRT |2|' + WSocketIPv6ToStr(PIcsIPv6Address(@saddr.sin6_addr)^) + '|' + IntToStr(DataPort) + '|'
            Msg := 'EPRT |2|' + WSocketSockAddrToStr(saddr) + '|' + IntToStr(DataPort) + '|'
          ...
          //      Msg := 'EPRT |1|' + WSocketIPv4ToStr(TIcsIPv4Address(IPAddr)) + '|' + IntToStr(DataPort) + '|';    { V9.4 }
                Msg := 'EPRT |1|' + WSocketSockAddrToStr(saddr) + '|' + IntToStr(DataPort) + '|';    { V9.4 }
        end
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.ControlSocketDnsLookupDone(Sender: TObject; ErrCode: Word);
var
    I: Integer;
begin
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loProtSpecInfo) then
        DebugLog(loProtSpecInfo, 'Control DNS Lookup Done - '  + FControlSocket.DnsResultList.CommaText);  { V8.60 }
{$ENDIF}
    if ErrCode <> 0 then begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.HandleHttpTunnelError(Sender: TObject; ErrCode: Word;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.HandleSocksError(Sender: TObject; ErrCode: Integer; Msg: String);
begin
    FLastResponse := Msg;
end;
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loProtSpecInfo) then
        DebugLog(loProtSpecInfo, 'Control Socket Connect, error='  +  IntToStr (ErrCode) + ' to ' + IcsFmtIpv6AddrPort(AddrResolvedStr, FPort));  { V8.60 }
{$ENDIF}
    ...
    if ErrCode <> 0 then begin
      ...
      {$ENDIF}
        ...
        FLastResponse := FLastResponse + ' to ' + IcsFmtIpv6AddrPort(AddrResolvedStr, FPort);  { V8.60 }
        FLastAddrOK := '';  { V8.60 }
        FStatusCode    := 500;
...
begin
    Progress;    { V8.71 reset sync timer }
    Len := FControlSocket.Receive(@FReceiveBuffer[FReceiveLen], SizeOf(FReceiveBuffer) - FReceiveLen - 1);
    ...
    while FReceiveLen > 0 do begin
        ...
        while (Length(RawResponse) > 0) and IsCRLF(RawResponse[Length(RawResponse)]) do
             SetLength(RawResponse, Length(RawResponse) - 1);
        { Auto-detect UTF-8 if Option is set }
        if (FCodePage <> CP_UTF8) and (ftpAutoDetectCodePage in FOptions) and (CharsetDetect(RawResponse) = cdrUtf8) then          { AG V7.02 }
            ACodePage := CP_UTF8
        else
            ACodePage := FCodePage;
...
{$ENDIF}
        ...
        if FState = ftpWaitingBanner then begin
            DisplayLastResponse;
            if (FLastResponse = '') or (not IsDigit(FLastResponse[1])) then
         ...
         { V8.71 awaiting response for NOOP command for keep-alive, ignore it }
            if FExpectingNoopResponse then begin
                FExpectingNoopResponse := False;
                DisplayLastResponse;
                Exit;
            end;
        ...
        { V2.113 save response to SITE CMLSD, XCMLSD and SITE INDEX commands to stream }
            if (FFctPrv in [ftpFctSiteCmlsd, ftpFctXCmlsd, ftpFctSiteIndex]) then begin
                ...
                if (FStatusCode in [200, 250]) and (p^ = '-') then begin
                    if FLocalStream <> nil then begin
                        try
                            { V7.01 write raw UTF8 or ANSI response to stream, less 200- or 250-  }
                            if Length(RawResponse) > 4 then
                                LocalStreamWrite(RawResponse[5], Length(RawResponse) - 4);
                            LocalStreamWrite(AnsiString(IcsCRLF), 2);     { V8.71 unicode fix }
                        except
                            TriggerDisplay('! Error writing local file');
                            Exit;
                        end;
            ...
            if (FLastResponse = '') or (not IsDigit(FLastResponse[1])) then begin
        ...
        else begin { Unexpected data received }
            FExpectingNoopResponse := False;    { V8.71 }
            DisplayLastResponse;
        end;
    end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomFtpCli.ControlSocketSessionClosed(Sender: TObject; ErrCode: Word);
var
...
{$ENDIF}
    ...
    if (ErrCode <> 0) and (FState = ftpInternalReady) and ((FRequestType = ftpQuitAsync) or (FFctPrv = ftpFctQuit)) then
        ErrCode := 0;
    ...
    if FRequestType <> ftpRqAbort then begin
        if (ErrCode <> 0) or ((FRequestType <> ftpQuitAsync) and (LClosedState in [ftpWaitingBanner, ftpWaitingResponse])) then begin
            FLastResponse  := '500 Control connection closed - ' + WSocketGetErrorMsgFromErrorCode(ErrCode);
            FStatusCode    := 500;
...
begin
    ...
    if not FRequestDoneFlag then begin
        ...
        if (ErrCode = 0) and Assigned(FNextRequest) then begin
            ...
            FNextRequest   := nil;
{$IFDEF ICS_NewMessaging}   { V10.0 }
            TCustomFtpCli.PostMessage(Self, WM_ICS_FTP_REQUEST_DONE, 0, ErrCode);
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function  TFtpClient.UTF8_ON    : Boolean;      { V8.71 JK }
begin
    Result := Synchronize(UTF8_ONASync);
end;
...
begin
    Result := inherited Progress;    { did user cancel transfer }
...
{$ENDIF}
  ...
  {$ENDIF}
    while TRUE do begin
        ...
        if Terminated or ((FTimeout > 0) and IcsTestTrgTick64(FTimeStop)) then begin    { V8.71 Int64 }
            { Timeout occured }
{$IFNDEF NO_DEBUG_LOG}
            if CheckLogOptions(loProtSpecInfo) then
...
{$ENDIF}
      ...
      {$IFDEF MSWINDOWS}
        ...
        if ftpWaitUsingSleep in FOptions then
            ...
            MsgWaitForMultipleObjects(0, DummyHandle, FALSE, 100, QS_ALLINPUT);    { V8.71 JK was 1000 }
      {$ENDIF}
{$IFDEF ICS_NewMessaging}   { V10 }
       Sleep(0);
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TSslFtpClient.TransferSslHandshakeDone(Sender: TObject; ErrCode: Word; PeerCert: TX509Base; var Disconnect: Boolean);
begin
    if (ErrCode <> 0) then begin
    ...
    if ((Sender = FDataSocket) or (FSslType = sslTypeImplicit)) then
        Exit;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TSslFtpClient.TransferSslCliCertRequest(Sender: TObject; var Cert: TX509Base);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TSslFtpClient.TransferSslVerifyPeer(Sender: TObject; var Ok: Integer; Cert: TX509Base);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TSslFtpClient.TransferSslCliGetSession(Sender: TObject; var SslSession: Pointer; var FreeSession: Boolean);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TSslFtpClient.TransferSslCliNewSession(Sender: TObject; SslSession: Pointer; WasReused  : Boolean; var IncRefCount: Boolean);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TSslFtpClient.ControlSocketSslShutDownComplete(Sender: TObject; Bidirectional: Boolean; ErrCode: Integer);
begin
...
procedure TSslFtpClient.SetAcceptableHostsList(const SemiColonSeparatedList : String);
var
    Host : String;
    Buf  : String;
    I    : Integer;

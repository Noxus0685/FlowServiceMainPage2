...
type
  ...
  FDiameterCheckColumn: TCheckColumn;
  FPointCheckColumn: TCheckColumn;
  FCheckedDiameters: TDictionary<NativeInt, Boolean>;
  FCheckedPoints: TDictionary<NativeInt, Boolean>;


  procedure SetModified;
  procedure EnsureSelectionColumns;
...
procedure TFormTypeEditor.EnsureSelectionColumns;
begin
  if FDiameterCheckColumn = nil then
  begin
    FDiameterCheckColumn := TCheckColumn.Create(GridDiameters);
    FDiameterCheckColumn.Header := ''✓'';
    FDiameterCheckColumn.Width := 36;
    FDiameterCheckColumn.ReadOnly := False;
    FDiameterCheckColumn.Parent := GridDiameters;
    FDiameterCheckColumn.Index := 0;
  end;

  if FPointCheckColumn = nil then
  begin
    FPointCheckColumn := TCheckColumn.Create(GridPoints);
    FPointCheckColumn.Header := ''✓'';
    FPointCheckColumn.Width := 36;
    FPointCheckColumn.ReadOnly := False;
    FPointCheckColumn.Parent := GridPoints;
    FPointCheckColumn.Index := 0;
  end;
end;
...
begin
  ...
  // == Наименование
  // =====================================================
  if (FDiameterCheckColumn <> nil) and (ACol = FDiameterCheckColumn.Index) then
...
begin
  ...
  if (FPointCheckColumn <> nil) and (ACol = FPointCheckColumn.Index) then
  begin
    if Value.AsBoolean then

...
type
  ...
  private
    ...
    FPreparedForShutdown: Boolean;
    FShuttingDown: Boolean;
    FSubscribedToProtocol: Boolean;
    ...
    { Saves settings through the captured, live protocol manager. }
    procedure SaveProtocolSettingsInternal;
    { Saves settings only while the frame is active. }
  ...
  public
    ...
    { Saves protocol settings and detaches listeners while the manager is still alive. }
...
begin
  inherited;
  FProtocolManager := ProtocolManager;
  FPreparedForShutdown := False;
  FShuttingDown := False;
  FSubscribedToProtocol := False;
  ...
  if (FProtocolManager <> nil) and FProtocolManager.LogEnabled then
    InitializeFullLog;
  ...
  begin
    FProtocolManager.SubscribeBatch(FBatchListener);
    FSubscribedToProtocol := True;
  end;
  ActiveWorkTable := nil;
  ...
  if FProtocolManager <> nil then
    FProtocolManager.AddMessage(pcInfo, psEngine, 'ApplicationVersion',
      'Версия программы',
...
begin
  ...
  try
    FProtocolManager.LoadSettings(FileName);
    { The manager is the source of truth.  Keep the guard raised while FMX
      change notifications are generated by assigning the persisted values. }
...
begin
  if FShuttingDown or FPreparedForShutdown then
    Exit;

  SaveProtocolSettingsInternal;
end;

procedure TFrameProtocol.SaveProtocolSettingsInternal;
var
  Ini: TCustomIniFile;
  I: Integer;
  FileName: string;
begin
  ...
  if Trim(FileName) = '' then
    Exit;
  FProtocolManager.SaveSettings(FileName);
...
var
  LogDirectory: string;
begin
  if (FProtocolManager = nil) or not FProtocolManager.LogEnabled then Exit;
...
var
  ...
  LineSize: Int64;
begin
  if (Msg = nil) or (FProtocolManager = nil) or not FProtocolManager.LogEnabled then
    Exit;
...
begin
  { The destructor does not call external managers because they may already be destroyed. }
  FShuttingDown := True;

  if OptimizationTimer <> nil then
  begin
    OptimizationTimer.Enabled := False;
    OptimizationTimer.OnTimer := nil;
  end;
...
procedure TFrameProtocol.PrepareForShutdown;
begin
  if FPreparedForShutdown then
    Exit;

  FPreparedForShutdown := True;
  ...
  begin
    ...
    if FSubscribedToProtocol then
      FProtocolManager.UnsubscribeBatch(FBatchListener);
  end;
...
begin
  if FShuttingDown or (csDestroying in ComponentState) or
     (FProtocolManager = nil) then
    Exit;
  if not FProtocolManager.ProtocolEnabled then
    Exit;
  LastItem := nil;
...
begin
  if FUpdatingOptimizationControls or (csDestroying in ComponentState) or
     (FProtocolManager = nil) then Exit;
  FUpdatingOptimizationControls := True;
  try
    SwitchProtocol.IsChecked := FProtocolManager.ProtocolEnabled;
    SwitchLog.IsChecked := FProtocolManager.LogEnabled;
    CheckBoxSampleChart.IsChecked := FProtocolManager.SampleChartEnabled;
    CheckBoxStat.IsChecked := FProtocolManager.StatisticsEnabled;
  finally FUpdatingOptimizationControls := False; end;
end;

procedure TFrameProtocol.OptimizationControlChanged(Sender: TObject);
begin
  if FLoadingSettings or FUpdatingOptimizationControls or
     FShuttingDown or FPreparedForShutdown or
     (FProtocolManager = nil) then Exit;
  FProtocolManager.ProtocolEnabled := SwitchProtocol.IsChecked;
  FProtocolManager.LogEnabled := SwitchLog.IsChecked;
  FProtocolManager.SampleChartEnabled := CheckBoxSampleChart.IsChecked;
  FProtocolManager.StatisticsEnabled := CheckBoxStat.IsChecked;
  if FProtocolManager.LogEnabled and (FFullLogWriter = nil) then InitializeFullLog
  else if not FProtocolManager.LogEnabled then FreeAndNil(FFullLogWriter);

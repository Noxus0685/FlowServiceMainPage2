...
type
  ...
  EMeasurementScenario = (
    mrsSuccessfulPass,
    mrsPartialDeviceError,
    mrsWrongKfRange,
    mrsFlowPassesPoint,
    mrsHighVariation,
    mrsStabilityLostDuringHold,
    mrsSecondAttemptStabilization,
    mrsFlowOvershoot
  );
  ...
  private
    ...
    FScenarioCurrentTimeMs: Int64;
    FScenarioStartTimeMs: Int64;
  ...
  class function IsPointEquivalent(AP1: TDevicePoint; AP2: TPointSpillage): Boolean; overload;
    ...
    function RunScenario(AScenario: EMeasurementScenario; out AResultText: string): Boolean;
    ...
    property IsScenarioRun: Boolean read FIsScenarioRun;
...
function TMeasurementRun.GetScenarioDeviceValue(AScenario: EMeasurementScenario; APoint: TDevicePoint;
  ADeviceIndex: Integer; const AReferenceValue: Double): Double;
var
  AllowedError: Double;
  ErrorPercent: Double;
begin
  AllowedError := GetScenarioAllowedError(APoint, ADeviceIndex);
  ErrorPercent := AllowedError * 0.5;
  case AScenario of
    mrsPartialDeviceError:
      if (ADeviceIndex mod 2 = 1) or ((FWorkTable <> nil) and (FWorkTable.DeviceChannels <> nil) and
        (FWorkTable.DeviceChannels.Count = 1)) then
        ErrorPercent := AllowedError * 1.5;
    mrsWrongKfRange:
      ErrorPercent := AllowedError * 3.0;
  end;
  Result := AReferenceValue * (1.0 + ErrorPercent / 100.0);
end;

function TMeasurementRun.GetScenarioVirtualPointTimeoutSec(APoint: TDevicePoint): Double;
begin
  Result := 120.0;
  if APoint <> nil then
    Result := Max(Result, Max(APoint.RequiredStabilizationSec, APoint.Pause) * 3.0);
end;

procedure TMeasurementRun.ResetRuntimeForScenario;
var
  I: Integer;
begin
  ...
  if FPoints <> nil then
    for I := 0 to FPoints.Count - 1 do
      ...
      begin
        FPoints[I].Status := mptsNone;
        FPoints[I].RepeatsCompleted := 0;
      end;
...
procedure TMeasurementRun.GenerateScenarioValues(AScenario: EMeasurementScenario; const ATimeMs: Int64);
var
  Point: TDevicePoint;
  Elapsed: Double;
...
begin
  ...
  Flow := Target;
  case AScenario of
    mrsFlowPassesPoint: Flow := Target * (0.70 + Elapsed * 0.08);
    mrsHighVariation: if Trunc(Elapsed) mod 2 = 0 then Flow := Target * 1.10 else Flow := Target * 0.90;
    mrsStabilityLostDuringHold: if (Elapsed > 30) and (Elapsed < 40) then Flow := Target * (1.0 + (Elapsed - 30) * 0.03);
...
begin
  ...
  try
    ...
    try
      ...
      if (AResultText = '') and (Point <> nil) and (FCurrentStage = msWaitStable) then
      begin
        if AScenario = mrsSuccessfulPass then
        begin
          MaxStableWaitSec := 30.0;
          if (FWorkTable <> nil) and (FWorkTable.ValueFlowRate <> nil) then
            MaxStableWaitSec := Max(MaxStableWaitSec,
              FWorkTable.ValueFlowRate.StabilitySettings.WindowDurationSec +
              FWorkTable.ValueFlowRate.StabilitySettings.ConfirmationTimeSec +
              FRequiredStabilizationSec + 10.0);
          if (((FScenarioCurrentTimeMs - FScenarioPointStartTimeMs) / 1000.0) > MaxStableWaitSec) then
          begin
        ...
        else if (((FScenarioCurrentTimeMs - FScenarioPointStartTimeMs) / 1000.0) > GetScenarioVirtualPointTimeoutSec(Point)) then
        begin
...
var
  ...
  procedure LogValue(const AIndex: Integer; const AChannelName, ADeviceUUID: string;
    AFlowMeter: TObject; AValue: TMeterValue);
  var
    LocalSettings: TMeterValueStabilitySettings;
    SampleCount: Integer;
  begin
    if AValue <> nil then
    begin
      LocalSettings := AValue.StabilitySettings;
      SampleCount := Length(AValue.GetSamples);
      AddScenarioLog(Format('WaitStableValue: ChannelIndex=%d; ChannelName=%s; DeviceUUID=%s; FlowMeter=%x; ValueFlow.Hash=%s; ValueFlow.Name=%s; Source=RuntimeObject; StabilityEnabled=%s; MinSampleCount=%d; WindowDurationSec=%.3f; MaxSampleAgeSec=%.3f; ConfirmationTimeSec=%.3f; SampleCount=%d',
        [AIndex, AChannelName, ADeviceUUID, NativeUInt(AFlowMeter), AValue.Hash, AValue.Name,
         BoolToStr(LocalSettings.Enabled, True), LocalSettings.MinSampleCount,
         LocalSettings.WindowDurationSec, LocalSettings.MaxSampleAgeSec,
         LocalSettings.ConfirmationTimeSec, SampleCount]));
    end
    else
      AddScenarioLog(Format('WaitStableValue: ChannelIndex=%d; ChannelName=%s; DeviceUUID=%s; FlowMeter=%x; ValueFlow=nil',
        [AIndex, AChannelName, ADeviceUUID, NativeUInt(AFlowMeter)]));
  end;
...
begin
  ...
  FLastStableProtocolTick := UInt64(GetStabilityAnalysisTimeMs);
  ...
  if FWorkTable <> nil then
  begin
    LogValue(-1, 'Etalon', '', FWorkTable, FWorkTable.ValueFlowRate);
    if FWorkTable.DeviceChannels <> nil then
      for I := 0 to FWorkTable.DeviceChannels.Count - 1 do
      begin
        Channel := FWorkTable.DeviceChannels[I];
        ...
        if Channel.FlowMeter <> nil then
          LogValue(I, Channel.Name, Channel.FlowMeter.UUID, Channel.FlowMeter, StableValue)
        else
          LogValue(I, Channel.Name, '', nil, StableValue);
      end;
  end;
...
begin
  ...
  try
    ...
    begin
      Lines.Add('DeviceStableDurationSec=' + SFloat((NowTickMs - FDeviceStableSinceMs) / 1000));
      Lines.Add('DeviceStableRemainingSec=' + SFloat(Max(0.0, FRequiredStabilizationSec - ((NowTickMs - FDeviceStableSinceMs) / 1000))));
...
begin
  ...
  try
    try
      ...
      if FIsScenarioRun then
      begin
        FLastSaveMeasurementResultsResult := 'scenario-runtime-only';
        AddDiagnosticEvent('ScenarioRun: persistent database save skipped');
      end
      else

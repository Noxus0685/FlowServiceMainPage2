    ...
    case Cur.Tag of
      ...
      Ord(tnManufacturer):
        begin
          ...
          if not (
            SameText(Trim(AType.Manufacturer), Trim(Cur.TagString))
            or SameText(NormalizeTreeKey(AType.Manufacturer), Trim(Cur.TagString))
          ) then
            Exit(False);
        end;
      ...
      Ord(tnModification):
        begin
          ...
          if not (
            SameText(Trim(AType.Modification), Trim(Cur.TagString))
            or SameText(NormalizeTreeKey(AType.Modification), Trim(Cur.TagString))
          ) then
            Exit(False);
        end;
    end;
...
begin
  ...
  // копия выбранной строки (1 в 1 по текстовым полям)
  ...
  // Если создаём НЕ из выбранной строки, подставляем поля из выбранной ветки дерева.
  // Для копирования 1 в 1 из грида не переопределяем изготовителя/модификацию деревом.
  if (not HasGridSelection) and

...
void TFlowMeter::Init()
{
    ...
    Pnt.Qrate = 0.1; //×àñòü îò FlowMax
    Pnt.Mrate = 0.1; //×àñòü îò FlowMax
    Pnt.Q = 0.06;

    Pnt.Name = "Q1";
    Pnt.Q = 60;
	Pnt.Num = 3; //Êîë-âî èçìåðåíèé
    Pnt.State = 0; //<Ñîñòîíèå
	Pnt.Time = 120; //Âðåìÿ èçìåðåíèÿ
    Pnt.Volume = 0;
    Pnt.Imp = 0;
    Pnt.Accuracy = 10; //Òî÷íîñòü âûõîäà íà ðàñõîä
	Pnt.Error = 0.1; //Ïîãðåøíîñòü äîïóñòèìàÿ èçìåðåíèû
    Pnt.RagePlus = 10;
    Pnt.RageMinus = 10;
    Pnt.IsRageFree = 0;
    ...
    Pnt.Qrate = 0.5; //×àñòü îò FlowMax
    Pnt.Mrate = 0.5; //×àñòü îò FlowMax
    Pnt.Q = 150;
    Pnt.Name = "Q2";
    Pnt.Num = 1; //Êîë-âî èçìåðåíèé
    Pnt.State = 0; //<Ñîñòîíèå
	Pnt.Time = 120; //Âðåìÿ èçìåðåíèÿ
    Pnt.Volume = 0;
    Pnt.Imp = 0;
    ...
    Pnt.Qrate = 1; //×àñòü îò FlowMax
    Pnt.Mrate = 1; //×àñòü îò FlowMax
    Pnt.Q = 1500;
	Pnt.Name = "Q3";
    Pnt.Num = 1; //Êîë-âî èçìåðåíèé
    Pnt.State = 0; //<Ñîñòîíèå
    ...
    miOwner = "Ôèç. ëèöî";
    //
    // Ñåðèéíûé íîìåð ïîâåðÿåìîãî óñòðîéñòâà
    SerialNum = "";
    //
    // Íîìåð ÃÐÑÈ ïîâåðÿåìîãî óñòðîéñòâà
    CertificateNum = "";

    DN = "15";
    docTitle = "1592-2015";

    means = "60661.15.3Ð.00540801";
...
void TFlowMeter::SetSendStatus(UnicodeString text)
{
    if (text == "-") {
        SendStatus = 0;
    } else if (text == "Îòïðàâëÿåòñÿ") {
        SendStatus = 1;
    }

    else if (text == "Îòïðàâëåí")
    {
        SendStatus = 2;
    }
...
void TFlowMeter::SetStatus(UnicodeString status)
{
	...
	{
		Status = 0;
	} else if (status == "Íå Ãîäåí")
	{
        Status = 1;
	}
	else if (status == "Ãîäåí")
    {
        Status = 2;
	}
    else
...
bool TFlowMeter::SaveDataPoint()
{
    ...
    try {
        ...
        Qt = ValueVolumeFlow->GetStrNum(Point.Q, "ì3/÷");
        ...
        DataPoint.EtlPres =
            EtalonMeter->ValuePressure->GetStringValue(); //Äàâëåíèå
       ...
       if (MeterFlowCategory==WEIGHTS_VOLUME_FLOWMETER_TYPE)
        ...
        DataPoint.Pres = ValuePressure->GetStringValue(); //Äàâëåíèå
        DataPoint.TempAir = ValueAirTemperture->GetStringValue();
        DataPoint.PresAir = ValueAirPressure->GetStringValue(); //Äàâëåíèå
...
bool TFlowMeter::GetPointUseFlow(float Q, tPoint &point)
{
    ...
    for (int i = 0; i < Points.size(); i++) {
        // Âû÷èñëÿåì ðàñõîä äëÿ äàííîé êàëèáðîâî÷íîé òî÷êè
        if (Points[i].Time > 0) {
            Qp = Points[i].Q;
            pnt = Points[i];
...
int TFlowMeter::CheckFullStatus()
{
    ...
    if ((!Points.empty()) && (!DataPoints.empty())) {
        for (int i = 0; i < Points.size(); i++) {
            ...
            if (Points[i].Time > 0) {
                Q = Points[i].Q;
                Points[i].State = 0; //Îáíóëÿåì ñîñòîÿíèå
            } else {
                Status = 0;
                Result = "-";
                return 0;
...
void TFlowMeter::SaveTestMetersToFile(TFlowMeter* AFlowMeter, int IsBackUp)
{
    ...
    // < OMNI XML êðîññïëàòôîðìåííûé âåíäîð
    XmlDoc->XML->Clear();
    XmlDoc->FileName = "";
    XmlDoc->Active = true;
    //
    // Ñîçäàäèì ãëàâíóþ âåòêó è äîáàâèì óçåë îá óñòðîéñòâå
    ...
    if (!Etalons.empty()) {
        ...
        for (int j = 0; j < Etalons.size(); j++) {
...

            {
               ...
               {
                    ShowMessage("Îøèáêà ñîõðàíåíèÿ äàííûõ");
               }
...
TFlowMeter* TFlowMeter::LoadEtalonsFromFile()
{
	...
	if (!DirectoryExists(TSettingsClass::Dir)) {
		Result = "Ôàéë óñòðîéñòâ íå ñóùåñòâóåò";
		throw Exception("Ôàéë óñòðîéñòâ íå ñóùåñòâóåò");
	}
    ...
    if (!FileExists(fname)) {
        Result = "Ôàéë óñòðîéñòâ íå ñóùåñòâóåò";
        //	throw Exception("Ôàéë òèïîâ íå ñóùåñòâóåò");
        ShowMessage(Format("Ôàéë '%s' íå ñóùåñòâóåò. Ñîçäàí íîâûé ôàéë.",
            ARRAYOFCONST((fname))));
    ...
    if (document == NULL) {
        Result = "Ôàéë ïóñòîé èëè ïîâðåæäåí";
    ...
    /* }	else
	{
				...
				Result = "Âåðñèÿ íå ìîæåò áûòü îáðàáîòàíà";
				throw Exception(Result);
 *///	}
...
{
    // Ñîðòèðîâêà âåêòîðà ïî ñâîéñòâó DN
...
{
    // Ñîðòèðîâêà âåêòîðà ïî ñâîéñòâó ðàñõîä
...
UnicodeString TFlowMeter::GetMeterCategory()
	...
	{
				FlowTypeName =   "Âåñîâîå óñòðîéñòâî + ÎÐ";
		...
		FlowTypeName =   "Âåñîâîå óñòðîéñòâî + ÌÐ";
		...
		FlowTypeName =   "Ìàññîâûé ðàñõîäîìåð";
	...
	{
				FlowTypeName =   "Îáúåìíûé ðàñõîäîìåð";
...
void TFlowMeter::SetMeterFlowType(UnicodeString meterFlowType)
{
  ...
  //  	WEIGHTS_TYPE = 0,    //Âåñîâîå óñòðîéñòâî
  //	WEIGHTS_VOLUME_FLOWMETER_TYPE = 1, //Âåñîâîå óñòðîéñòâî + ÎÐ
  //	WEIGHTS_MASS_FLOWMETER_TYPE = 2, // Âåñîâîå óñòðîéñòâî + ÌÐ
  //	MASS_FLOWMETER_TYPE = 3, //Ìàññîâûé ðàñõîäîìåð
 //	VOLUME_FLOWMETER_TYPE = 4, //Îáúåìíûé ðàñõîäîìåð
  //	TANK_TYPE = 5, // Ìåðíèê


	if (meterFlowType == "Âåñîâîå óñòðîéñòâî")
	{
        SetMeterFlowType(WEIGHTS_TYPE);
	}

		else if (meterFlowType == "Âåñîâîå óñòðîéñòâî + ÎÐ")
	{
        SetMeterFlowType(WEIGHTS_VOLUME_FLOWMETER_TYPE);
	}

	else if (meterFlowType == "Âåñîâîå óñòðîéñòâî + ÌÐ") {

        SetMeterFlowType(WEIGHTS_MASS_FLOWMETER_TYPE);
	}
		else if (meterFlowType == "Ìàññîâûé ðàñõîäîìåð") {

        SetMeterFlowType(MASS_FLOWMETER_TYPE);
	}

	else if (meterFlowType == "Îáúåìíûé ðàñõîäîìåð")
	{
        SetMeterFlowType(VOLUME_FLOWMETER_TYPE);

	}

		else if (meterFlowType == 	"Ìåðíèê")
	{
...
// Ðàñ÷¸ò âðåìåíè äî äîñòèæåíèÿ ïðåäåëà ïî îáúåìó â ìñ
    ...
    float pointVolume, float DelayCoef, float TimeCoef)
{
    ...
    flow = ValueFlow->GetFloatValue("ë/ñ"); //Ïîëó÷àåì ë/ñ
    volume = ValueVolume->GetFloatValue();
    //Ïðîãíîç ïî îáúåìó íà ñëåäóþùóþ ñåêóíäó â ë
...
void TFlowMeter::ApiSent()
{
    ...
    if (!FlowMeters.empty()) {
        k = FlowMeters.size();
        while (l < FlowMeters.size()) {
            if (FlowMeters[l]->SendStatus == 1) //"Îòïðàâëÿåòñÿ")
            {
                FlowMeters[l]->SendStatus = 2; //"Îòïðàâëåí";
            }
            l = l + 1;
        }
    }
...
int TFlowMeter::ApiCheckResult()
{
    ...
    if (!FlowMeters.empty()) {
        ...
        for (int j = 0; j < FlowMeters.size(); j++) {
            ...
            if ((res == "Ãîäåí") && (st != 2)) {
                success = true;
            } else if ((FlowMeters[j]->Result == "Íå ãîäåí") &&
...
UnicodeString TFlowMeter::ApiConvert()
{
    ...
    if (!FlowMeters.empty()) {
        ...
        for (int j = 0; j < FlowMeters.size(); j++) {
            ...
            if (((FlowMeters[j]->Result == "Ãîäåí") ||
                    (FlowMeters[j]->Result == "Íå ãîäåí")) &&
                (FlowMeters[j]->SendStatus != 2))
            {
                ...
                //!!!Âåðíîå	jDevice->AddPair("ProtocolNum", TSettingsClass::ProtocolNumTitle+IntToStr(TSettingsClass::ProtocolNum++));
...
void TFlowMeter::AutoFill(TFlowMeter* Meter)
{
    ...
    tPoint     Point;
      ...
      for (int j = 0; j <  Meter->Point.Series; j++) {
        ...
        DataPoint.Qt =  Meter->ValueVolumeFlow->GetStrNum(Point.Q, "ì3/÷");
        ...
        DataPoint.EtlPres =
            RandStr(EtalonMeter->ValuePressure->GetDoubleValue(), 0.2); //Äàâëåíèå
        DataPoint.EtlTempAir =
            RandStr(EtalonMeter->ValueAirTemperture->GetDoubleValue(), 0.2);
        DataPoint.EtlPresAir =
            RandStr(EtalonMeter->ValueAirPressure->GetDoubleValue(), 0.2); //Äàâëåíèå
        ...
        DataPoint.PresAir = RandStr(EtalonMeter->ValueAirPressure->GetDoubleValue(), 0.2); //Äàâëåíèå
...
{
    // Ñîðòèðîâêà ïî Qt îò áîëüøåãî ê ìåíüøåìó
    if (MinMax == 0) {
...
{
if (MinMax == 1) {
    // Ñîðòèðîâêà ïî Date îò ìåíüøåãî ê áîëüøåìó
    std::sort(DataPoints.begin(), DataPoints.end(),
        [](const tDataPoint& a, const tDataPoint& b) {
...
{
    // Ñîðòèðîâêà ïî Date îò áîëüøåìó ê ìåíüøåãî
    std::sort(DataPoints.begin(), DataPoints.end(),
        [](const tDataPoint& a, const tDataPoint& b) {
            return StrToDateTimeDef(a.Date, TDateTime(0)) > StrToDateTimeDef(b.Date, TDateTime(0));
        });
		...
		if (TryStrToDouble_(dp.ErrorMass, errmass))
		   {
			...
			relatedPointsM.push_back(&dp); // Ñîõðàíÿåì óêàçàòåëü íà îðèãèíàëüíûé îáúåêò
		   }
		...
		if (TryStrToDouble_(dp.ErrorVolume, errvol))
		   {
			...
			relatedPointsV.push_back(&dp); // Ñîõðàíÿåì óêàçàòåëü íà îðèãèíàëüíûé îáúåêò
		   }

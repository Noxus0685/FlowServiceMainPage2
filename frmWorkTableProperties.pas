...
begin
  ...
  AddMeterValueTab('Давление', 'Давление', 0, EditPressure, ButtonSelectPressure,
    EditPressureMin, EditPressureMax, ButtonSelectPressureClick);
  AddMeterValueTab('Температура', 'Температура', 1, EditTemperture, ButtonSelectTemperture,
    EditTempertureMin, EditTempertureMax, ButtonSelectTempertureClick);
  AddMeterValueTab('Расход', 'Расход', 2, EditFlowRate, ButtonSelectFlowRate,
    EditFlowRateMin, EditFlowRateMax, ButtonSelectFlowRateClick);
  AddMeterValueTab('Жидкость', 'Количество жидкости', 3, EditQuantity, ButtonSelectQuantity,
...
procedure TFrameWorkTableProperties.AddLimitRow(AParent: TFmxObject; const AKind: Integer;
  out AMinEdit: TEdit; out AMaxEdit: TEdit);
var
  Item: TLayout;
  RowGrid: TGridPanelLayout;
  MinLabel: TLabel;
...
begin
  ...
  AMinEdit.Margins.Rect := TRectF.Create(0, 3, 8, 3);

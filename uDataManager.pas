...
begin
  ...
  for D in ActiveDeviceRepo.Devices do
    if (D <> nil)
      and (D <> ADevice)
      and (D.State <> osDeleted)
      and SameText(Trim(D.Manufacturer), Result.OldManufacturer) then
      Exit;
...
begin
  ...
  for T in ActiveTypeRepo.Types do
    if (T <> nil)
      and (T <> AType)
      and (T.State <> osDeleted)
      and SameText(Trim(T.Manufacturer), Result.OldManufacturer) then
      Exit;
  Result.NeedRemoveOldBranch := True;
end;

...
begin
  ...
  UpdaterPath := TPath.Combine(ProgramFolder, UPDATER_EXE_NAME);

  if not TFile.Exists(UpdaterPath) then
    raise Exception.Create('Updater.exe не найден в папке программы.');

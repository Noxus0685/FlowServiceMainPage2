...
type
  ...
  private
    ...
    OutputSettingsItem: TTreeViewItem;
    ...
    LayoutImpulse: TLayout;
    LayoutVoltage: TLayout;
    LayoutCurrent: TLayout;
    LayoutInterface: TLayout;
    LayoutVisual: TLayout;
    ...
    procedure AddTabRow(AParent: TControl; const ACaption: string; AControl: TControl);
    ...
    procedure ApplyOutputHeight(ARoot: TControl);
...
begin
  ...
  OutputSettingsItem := TTreeViewItem.Create(Self);
  OutputSettingsItem.Parent := CategoryOutput;
  OutputSettingsItem.Text := '';
  OutputSettingsItem.Height := 0;
  OutputSettingsItem.Stored := False;

  TabControlOutputType := TTabControl.Create(Self);
  TabControlOutputType.Parent := OutputSettingsItem;
  ...
  AddTabRow(LayoutVoltage, 'Диапазон', cbVoltageRange);
  EditVoltageQminRate := TEdit.Create(Self);
  EditVoltageQminRate.OnExit := EditVoltageExit;
  AddTabRow(LayoutVoltage, 'Qmin', EditVoltageQminRate);
  EditVoltageQmaxRate := TEdit.Create(Self);
  EditVoltageQmaxRate.OnExit := EditVoltageExit;
  AddTabRow(LayoutVoltage, 'Qmax', EditVoltageQmaxRate);

  TabCurrent := TTabItem.Create(Self);
  TabCurrent.Parent := TabControlOutputType;
  TabCurrent.Text := 'Ток';
  LayoutCurrent := TLayout.Create(Self);
  LayoutCurrent.Parent := TabCurrent;
  LayoutCurrent.Align := TAlignLayout.Top;
  LayoutCurrent.Height := 0;
  LayoutCurrent.Stored := False;
  ...
  AddTabRow(LayoutCurrent, 'Диапазон', cbCurrentRange);
  EditCurrentQminRate := TEdit.Create(Self);
  EditCurrentQminRate.OnExit := EditCurrentExit;
  AddTabRow(LayoutCurrent, 'Qmin', EditCurrentQminRate);
  EditCurrentQmaxRate := TEdit.Create(Self);
  EditCurrentQmaxRate.OnExit := EditCurrentExit;
  AddTabRow(LayoutCurrent, 'Qmax', EditCurrentQmaxRate);

  TabInterface := TTabItem.Create(Self);
  TabInterface.Parent := TabControlOutputType;
  TabInterface.Text := 'Интерфейс';
  LayoutInterface := TLayout.Create(Self);
  LayoutInterface.Parent := TabInterface;
  LayoutInterface.Align := TAlignLayout.Top;
  LayoutInterface.Height := 0;
  LayoutInterface.Stored := False;
  EditProtocolName := TEdit.Create(Self);
  EditProtocolName.OnExit := InterfaceEditExit;
  AddTabRow(LayoutInterface, 'Библиотека', EditProtocolName);
  ...
  AddTabRow(LayoutInterface, 'Скорость', cbBaudRate);
  ...
  AddTabRow(LayoutInterface, 'Четность', cbParity);
  EditDeviceAddress := TEdit.Create(Self);
  EditDeviceAddress.OnExit := InterfaceEditExit;
  AddTabRow(LayoutInterface, 'Адрес', EditDeviceAddress);

  TabVisual := TTabItem.Create(Self);
  TabVisual.Parent := TabControlOutputType;
  TabVisual.Text := 'Визуальный';
  LayoutVisual := TLayout.Create(Self);
  LayoutVisual.Parent := TabVisual;
  LayoutVisual.Align := TAlignLayout.Top;
  LayoutVisual.Height := 0;
  LayoutVisual.Stored := False;
  ...
  AddTabRow(LayoutVisual, 'Тип ввода', cbInputType);
end;
...
procedure TFrameFlowMeterProperties.AddTabRow(AParent: TControl;
...
begin
  ...
  Row.Position.X := 0;
  Row.Position.Y := AParent.Height;
  Row.Width := 600;
  Row.Height := 32;
  Row.Padding.Rect := TRectF.Create(40, 3, 10, 3);
  Row.Stored := False;
  AParent.Height := AParent.Height + Row.Height;
...
begin
  ...
  TabControlOutputType.ActiveTab := TabFrequency;
        ApplyOutputHeight(LayoutFrequency);
...
procedure TFrameFlowMeterProperties.ApplyOutputHeight(ARoot: TControl);
begin
  if ARoot = nil then
    Exit;
  TabControlOutputType.Height := ARoot.Height;
  if OutputSettingsItem <> nil then
    OutputSettingsItem.Height := ARoot.Height;
  Realign;
